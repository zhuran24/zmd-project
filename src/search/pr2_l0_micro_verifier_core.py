...
def _load_canonical_dependency_floor_manifest(source_root: Path) -> dict[str, Any]:
    ...
    if not manifest_path.exists():
        _generate_default_dependency_floor_manifest(source_root, manifest_path)
    return _load_dependency_floor_manifest(manifest_path)
...
def _load_dependency_floor_manifest(manifest_path: Path) -> dict[str, Any]:
    ...
    """
    raw = _read_regular_file_bytes(manifest_path.resolve())
    ...
    root = Path(str(manifest.get("floor_root"))).resolve()
...
def _generate_default_dependency_floor_manifest(source_root: Path, manifest_path: Path) -> None:
    generator = source_root / "scripts" / "generate_pr2_dependency_floor_manifest.py"
    if not generator.is_file():
        raise RuntimeError(
            "default PR2 dependency floor manifest is missing and the host-specific "
            f"generator was not found: {generator}"
        )
    completed = subprocess.run(
        [
            str(Path(os.path.abspath(sys.executable))),
    ...
    if completed.returncode != 0:
        ...
        stdout = completed.stdout[-1000:].strip()
        detail = stderr or stdout or f"exit={completed.returncode}"
        raise RuntimeError(
            "default PR2 dependency floor manifest is host-specific and missing; "
            f"automatic generation failed: {detail}"
        )
    if not manifest_path.is_file():
        raise RuntimeError(
            "default PR2 dependency floor manifest generation completed but did not "

...
def _load_canonical_dependency_floor_manifest(source_root: Path) -> dict[str, Any]:
    ...
    Trust boundary: the canonical floor manifest is reviewed data, not a runtime
    first-use artifact.  The source-pinned constants below bind its exact bytes
    before any floor file is trusted; missing or regenerated bytes fail closed and
    must be resealed by the close-kernel review path.
    """

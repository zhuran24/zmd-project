...
from src.search.phase3b.forced_anchor.master import _build_exact_overlay, _check, _display_path, _mapping
from src.search.phase3b.forced_anchor.proto_reduction import _proto_profile

JOINED_XY_PROFILE_AUDIT_SOURCE = "phase3b_joined_xy_profile_audit_v1"
DEFAULT_JOINED_XY_CANDIDATE = "67x13"


def build_phase3b_joined_xy_profile_audit(
    project_root: Path,
    *,
    candidate: str = DEFAULT_JOINED_XY_CANDIDATE,
    block_size: int = 64,
    block_templates: str = "",
    ...
    try:
        cases = [
            ...
            _build_case(
                project_root,
                case_id="selected_block_active_guard_joined_xy",
                block_geometry=EXACT_POWER_COVERAGE_WITNESS_BLOCK_GEOMETRY_SELECTED_BLOCK_ACTIVE_GUARD_JOINED_XY,
                ghost_rect=(ghost_w, ghost_h),
                block_size=block_size,
                block_templates=block_templates,
                master_search_profile=master_search_profile,
    ...
    ready = bool(comparison.get("joined_xy_profile_valid", False))
    report: dict[str, Any] = {
        "metadata": {
            "source": JOINED_XY_PROFILE_AUDIT_SOURCE,
            "generated_at": now_iso(),
            "diagnostic_semantics": "no_solve_joined_xy_profile_audit",
        ...
        "status": {
            ...
            "outcome": (
                "joined_xy_profile_audit_passed"
                if ready
                else (
                    "joined_xy_profile_audit_failed"
                    if cases and comparison
                    else "joined_xy_profile_audit_incomplete"
                )
            ),
            "recommendation": (
                "run_bounded_joined_xy_presolve_probe"
                if ready
                else "inspect_joined_xy_profile_audit_failure"
            ),
        },
        "cases": cases,
...
def render_phase3b_joined_xy_profile_audit_markdown(report: Mapping[str, Any]) -> str:
    status = _mapping(report.get("status"))
    comparison = _mapping(report.get("comparison"))
    lines = [
        "# Phase 3B Joined-XY Profile Audit",
        "",
        "- Diagnostic semantics: no_solve_joined_xy_profile_audit",
        ...
        "## Cases",
        "",
        "| Case | Vars | Constraints | Block x/y Targets | Elements | Active Guards | Block Selected | Local Selected | Joined x/y Targets | Padded Idx Vars |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for case in list(report.get("cases", [])):
        ...
        prefixes = _mapping(case.get("variable_prefix_counts"))
        lines.append(
            "| "
            + " | ".join(
                [
                    ...
                    _cell(witness.get("joined_xy_target_channel_count")),
                    _cell(prefixes.get("cover_choice_padded_idx__")),
                ]
            )
            + " |"
        )
    lines.extend(
        [
            ...
            "## Comparison",
            "",
            f"- Joined profile valid: {comparison.get('joined_xy_profile_valid')}",
            f"- Block x/y retained: {comparison.get('per_block_xy_variables_retained')}",
            f"- Padded selector removed: {comparison.get('padded_selector_removed')}",
            f"- Joined x/y targets present: {comparison.get('joined_xy_targets_present')}",
...
def render_phase3b_joined_xy_profile_audit_text(report: Mapping[str, Any]) -> str:
    status = _mapping(report.get("status"))
    comparison = _mapping(report.get("comparison"))
    return "\n".join(
        [
            "phase3b joined-xy profile audit",
            "diagnostic_semantics=no_solve_joined_xy_profile_audit",
            f"solver_invoked={bool(_mapping(report.get('metadata')).get('solver_invoked', True))}",
            f"proof_source={bool(_mapping(report.get('metadata')).get('proof_source', True))}",
            f"outcome={status.get('outcome')}",
            f"joined_xy_profile_valid={comparison.get('joined_xy_profile_valid')}",
            f"padded_selector_removed={comparison.get('padded_selector_removed')}",
            f"joined_xy_targets_present={comparison.get('joined_xy_targets_present')}",
    ...
    *,
    ...
    return {
        ...
        "variable_prefix_counts": {
            ...
            "cover_choice_joined_x__": _prefix_count(variables, "cover_choice_joined_x__"),
            "cover_choice_joined_y__": _prefix_count(variables, "cover_choice_joined_y__"),
...
def _comparison(cases: list[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    new = _mapping(by_id.get("selected_block_active_guard_joined_xy"))
    ...
    selected_geometry_delta = _int(
        new_w.get("joined_xy_selected_geometry_constraint_count")
    ...
    selectors_unchanged = (
        ...
        and _int(new_w.get("block_selector_count")) == _int(old_w.get("block_selector_count"))
        and _int(new_w.get("local_selector_count")) == _int(old_w.get("local_selector_count"))
    )
    per_block_retained = (
        _int(new_prefix.get("cover_choice_block_x__"))
        == _int(old_prefix.get("cover_choice_block_x__"))
        and _int(new_prefix.get("cover_choice_block_y__"))
        == _int(old_prefix.get("cover_choice_block_y__"))
        and _int(new_prefix.get("cover_choice_block_x__")) > 0
        and _int(new_prefix.get("cover_choice_block_y__")) > 0
    )
    joined_targets_present = (
        _int(new_prefix.get("cover_choice_joined_x__")) > 0
        and _int(new_prefix.get("cover_choice_joined_y__")) > 0
        and _int(new_w.get("joined_xy_target_channel_count")) > 0
        and _int(new_w.get("joined_xy_element_constraint_count")) > 0
    )
    padded_removed = _int(new_prefix.get("cover_choice_padded_idx__")) == 0
    no_grouped_targets = (
    ...
    selected_geometry_reduced = selected_geometry_delta < 0
    valid = (
        bool(new)
        and bool(old)
        ...
        and _int(new_w.get("block_selected_geometry_constraint_count")) == 0
        and _int(new_w.get("block_final_join_element_constraint_count"))
        == _int(new_w.get("joined_xy_element_constraint_count"))
        and active_guard_unchanged
        and selectors_unchanged
        and per_block_retained
        and joined_targets_present
        and padded_removed
        and no_grouped_targets
        and no_pairwise_literals
        and selected_geometry_reduced
    )
    return {
        "joined_xy_profile_valid": bool(valid),
        ...
        "per_block_xy_variables_retained": bool(per_block_retained),
        "joined_xy_targets_present": bool(joined_targets_present),
        "padded_selector_removed": bool(padded_removed),
        "no_grouped_xy_targets": bool(no_grouped_targets),
        "no_pairwise_cover_literals": bool(no_pairwise_literals),
        "selected_geometry_reduced": bool(selected_geometry_reduced),
    }
...
def _checks(report: Mapping[str, Any]) -> list[dict[str, str]]:
    ...
    return [
        ...
        _check("joined_xy_profile_valid", "pass" if bool(comparison.get("joined_xy_profile_valid", False)) else "fail", str(comparison)),
        ...
        _check("per_block_xy_retained", "pass" if bool(comparison.get("per_block_xy_variables_retained", False)) else "fail", str(comparison.get("per_block_xy_variables_retained"))),
        _check("joined_xy_targets_present", "pass" if bool(comparison.get("joined_xy_targets_present", False)) else "fail", str(comparison.get("joined_xy_targets_present"))),
        _check("padded_selector_removed", "pass" if bool(comparison.get("padded_selector_removed", False)) else "fail", str(comparison.get("padded_selector_removed"))),
        _check("no_grouped_xy_targets", "pass" if bool(comparison.get("no_grouped_xy_targets", False)) else "fail", str(comparison.get("no_grouped_xy_targets"))),
        ...
        _check("selected_geometry_reduced", "pass" if bool(comparison.get("selected_geometry_reduced", False)) else "fail", str(comparison.get("selected_geometry_constraint_delta"))),
    ]

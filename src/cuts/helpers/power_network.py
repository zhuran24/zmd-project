...
poles do not need any pole-to-pole network; the protocol core links to every
placed pole wirelessly and automatically. The pole-jump graph this module
builds models a game mechanic that does not exist, and its
``pole_jump_radius`` never had canonical_rules backing. Kept only as an
inert tombstone until the F8 family is physically deleted (rides the M3
step_8 reseal batch). Do not wire this into any certified or production
path. See memory card ``p1-3-m2-coverage-stencil-ruling``.

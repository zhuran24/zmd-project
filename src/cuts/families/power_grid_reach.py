...
def _validate_protocol_core_template_sot(state: BState, t0: float) -> Optional[ValidationResult]:
    """Cross-check hard-coded 9x9 protocol_core footprint against canonical_rules (shared SoT validator)."""
    return canonical_sot.validate_template_dims_sot(
        state, "protocol_core", (_PROTOCOL_CORE_SIZE, _PROTOCOL_CORE_SIZE), t0
    )

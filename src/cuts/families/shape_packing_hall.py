...
def _parse_cert_payload(cert_payload: bytes) -> Dict[str, Any]:
    return validate_cert_payload("shape_packing_hall", cert_payload)
...
def evaluate_geometric_shape_packing_hall(cut: Cut, state: BState) -> bool:
    ...
    Performance + soundness contract (Gemini F6 round 1 HIGH #2 + round 2
    CRITICAL #1):
    - Round 1 stripped the partition recompute on the assumption that an
      active cut had already cleared ``step_6_attach_scope_check``.
    - Round 2 caught that the caller chain does not guarantee this: a

...
from scripts.build_phase3b_checkpoint_free_signature_bucket_powered_support_compact_item_batched_counter_probe_readiness import (
    ...
    build_signature_bucket_powered_support_compact_item_batched_counter_probe_readiness,
    validate_future_command_template,
)


def test_powered_support_compact_item_batched_counter_probe_readiness_builds_artifact(tmp_path: Path) -> None:
    ...
    assert readiness["readiness"]["classification"] == "ready_for_powered_support_compact_item_batched_counter_probe_review"
    ...
    assert (output_dir / "signature_bucket_powered_support_compact_item_batched_counter_probe_readiness.json").exists()
    assert (output_dir / "future_command_template.json").exists()


def test_powered_support_compact_item_batched_counter_probe_readiness_missing_s138_blocks(tmp_path: Path) -> None:
    inputs = _write_inputs(tmp_path)
    inputs["s138_implementation"].unlink()
    ...
    assert checks["s138_patch_implemented_and_verified"] == "failed"
...
def test_powered_support_coverer_command_template_is_exact_and_bounded(tmp_path: Path) -> None:
    ...
    assert template["environment"][COMPACT_ITEM_BATCHED_COUNTER_OPTIMIZATION_ENV_VAR] == "1"
        ...
        (
            {
                **base,
                "environment": {
                    ...
                    if key != COMPACT_ITEM_BATCHED_COUNTER_OPTIMIZATION_ENV_VAR
                },
            },
            f"{COMPACT_ITEM_BATCHED_COUNTER_OPTIMIZATION_ENV_VAR}_must_be_1",
        ),
    ]
...
def test_powered_support_coverer_readiness_rejects_traversal_namespace_before_write(tmp_path: Path) -> None:
    bad_output_dir = (
        tmp_path
        / ".artifacts"
...
def _write_inputs(tmp_path: Path) -> dict[str, Path]:
    ...
    paths = {
        "s133_probe_review": evidence / "s133_probe_review.json",
        "s137_review_summary": evidence / "s137_review_summary.json",
        "s138_implementation": evidence / "s138_implementation.json",
        "agents": tmp_path / "AGENTS.md",
    }
    paths["s133_probe_review"].write_text(
        json.dumps(
            {
                "status": "completed",
                ...
                "interpretation": {
                    ...
                    "baseline_compact_item_seconds": 1.22,
                    "current_compact_item_seconds": 1.17,
                    "phase_seconds": {"compact_item_accumulation": 1.17},
                },
                "subphase_summary": {
                    "totals": {"compact_item_optimization_attempts": 10},
                },
            }
        )
        + "\n",
    ...
    paths["s137_review_summary"].write_text(
        json.dumps(
            {
                "review_verdict": "pass",
    ...
    paths["s138_implementation"].write_text(
        json.dumps(
            {
                "status": "implemented_and_verified",
                "env_var": COMPACT_ITEM_BATCHED_COUNTER_OPTIMIZATION_ENV_VAR,
            }
        )
        + "\n",
        encoding="utf-8",
    )
    paths["agents"].write_text("S138 implemented\n", encoding="utf-8")
    return paths

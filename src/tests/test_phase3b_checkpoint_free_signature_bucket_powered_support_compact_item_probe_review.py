...
from scripts.build_phase3b_checkpoint_free_signature_bucket_powered_support_compact_item_probe_review import (
    EXPECTED_RUN_ID,
    build_signature_bucket_powered_support_compact_item_probe_review,
)


def test_powered_support_compact_item_probe_review_classifies_union_hotspot(
    tmp_path: Path,
) -> None:
...
def test_powered_support_compact_item_probe_review_classifies_filter_hotspot(
    tmp_path: Path,
) -> None:
    review = _build_review(
        tmp_path,
...
def test_powered_support_compact_item_probe_review_classifies_compact_hotspot(
    tmp_path: Path,
) -> None:
    review = _build_review(
        tmp_path,
...
def test_powered_support_compact_item_probe_review_classifies_effective(
    tmp_path: Path,
) -> None:
    review = _build_review(
        tmp_path,
...
def test_powered_support_compact_item_probe_review_reports_optimization_not_used(
    tmp_path: Path,
) -> None:
    ...
    totals["compact_item_optimization_used"] = 0
    totals["compact_item_optimized_update_count"] = 0

    review = _build_review(tmp_path, probe)

    assert review["status"] == "compact_item_accumulation_optimization_not_used"
    assert (
        review["interpretation"]["classification"]
...
def test_powered_support_compact_item_probe_review_reports_missing_instrumentation(
    tmp_path: Path,
) -> None:
    probe = _probe_payload()
...
def test_powered_support_compact_item_probe_review_missing_phase_is_inconclusive(
    tmp_path: Path,
) -> None:
    probe = _probe_payload()
    ...
    assert review["status"] == "powered_support_compact_item_probe_inconclusive"
    assert (
        review["interpretation"]["classification"]
...
def test_powered_support_compact_item_probe_review_strict_safety_disqualifies(
    tmp_path: Path,
) -> None:
    probe = _probe_payload()
...
def test_powered_support_compact_item_probe_review_rejects_hard_boundary_truthy_flag(
    tmp_path: Path,
) -> None:
    probe = _probe_payload()
...
def test_powered_support_compact_item_probe_review_rejects_wrong_run_id(
    tmp_path: Path,
) -> None:
    probe = _probe_payload()
    probe["run_id"] = "wrong"
...
def test_powered_support_compact_item_probe_review_namespace_guard(tmp_path: Path) -> None:
    readiness = tmp_path / "readiness.json"
    probe = tmp_path / "probe.json"
    readiness.write_text(json.dumps(_readiness_payload()) + "\n", encoding="utf-8")
    probe.write_text(json.dumps(_probe_payload()) + "\n", encoding="utf-8")
    with pytest.raises(ValueError, match="S133 probe review namespace"):
        build_signature_bucket_powered_support_compact_item_probe_review(
...
def _build_review(tmp_path: Path, probe_payload: dict[str, object]) -> dict[str, object]:
    ...
    return build_signature_bucket_powered_support_compact_item_probe_review(
        project_root=tmp_path,
        readiness_path=readiness,
        probe_path=probe,
...
def _readiness_payload() -> dict[str, object]:
    return {
        "status": "completed",
        "readiness": {
            "classification": "ready_for_powered_support_compact_item_probe_review",
            "s128_baseline_compact_item_seconds": 1.2,
            "s128_baseline_phase_seconds": {

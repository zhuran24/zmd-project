from __future__ import annotations

import json
import zipfile
from pathlib import Path

import pytest

from scripts.build_phase3b_checkpoint_free_signature_bucket_fallback_reason_probe_external_review_package import (
    DEFAULT_RUN_ID,
    REVIEW_WORDING_REQUIREMENTS,
    build_signature_bucket_fallback_reason_probe_external_review_package,
)


def test_fallback_reason_probe_external_review_package_builds_clean_zip(
    tmp_path: Path,
) -> None:
    output_dir = _output_dir(tmp_path)

    package = build_signature_bucket_fallback_reason_probe_external_review_package(
        project_root=tmp_path,
        output_dir=output_dir,
        inputs=_write_inputs(tmp_path),
    )

    run_dir = output_dir / DEFAULT_RUN_ID
    zip_path = Path(package["zip_path"])
    assert package["status"] == "completed"
    assert zip_path.is_file()
    assert package["clean_extraction_validation"]["validated"] is True
    assert package["sensitive_path_comparison"]["changed"] is False
    request_text = (run_dir / "review_request.md").read_text(encoding="utf-8")
    assert zip_path.name in request_text
    assert package["zip_sha256"] in request_text
    for phrase in REVIEW_WORDING_REQUIREMENTS:
        assert phrase in request_text


def test_fallback_reason_probe_external_review_package_manifest_contains_context(
    tmp_path: Path,
) -> None:
    package = build_signature_bucket_fallback_reason_probe_external_review_package(
        project_root=tmp_path,
        output_dir=_output_dir(tmp_path),
        run_id="custom_s65_review_package",
        inputs=_write_inputs(tmp_path),
    )

    with zipfile.ZipFile(Path(package["zip_path"]), "r") as archive:
        names = set(archive.namelist())
        manifest = json.loads(archive.read("manifest.json").decode("utf-8"))
        s63 = json.loads(
            archive.read("evidence/s63_signature_bucket_fallback_reason_probe_readiness.json").decode(
                "utf-8"
            )
        )
        source_snippet = archive.read(
            "source_context/src/models/exact_coordinate_master_fallback_reason_snippets.py"
        ).decode("utf-8")

    assert "review_request.md" in names
    assert "evidence/s62_signature_bucket_mandatory_region_fallback_instrumentation_implementation.json" in names
    assert "evidence/s63_signature_bucket_fallback_reason_probe_readiness.json" in names
    assert "code_context/scripts/build_phase3b_checkpoint_free_signature_bucket_fallback_reason_probe_review.py" in names
    assert "test_context/src/tests/test_phase3b_checkpoint_free_signature_bucket_fallback_reason_probe_review.py" in names
    assert manifest["review_required_before_authorization"] is True
    assert manifest["external_review_is_authorization"] is False
    assert manifest["probe_executed"] is False
    assert s63["readiness"]["classification"] == "ready_for_fallback_reason_probe_review"
    assert s63["probe_execution_enabled"] is False
    assert "EXACT_GHOST_SIGNATURE_BUCKET_MANDATORY_REGION_FALLBACK_INSTRUMENTATION_ENV" in source_snippet
    assert "def _record_fallback_entry" in source_snippet


def test_fallback_reason_probe_external_review_package_no_write(
    tmp_path: Path,
) -> None:
    output_dir = _output_dir(tmp_path)

    package = build_signature_bucket_fallback_reason_probe_external_review_package(
        project_root=tmp_path,
        output_dir=output_dir,
        inputs=_write_inputs(tmp_path),
        no_write=True,
    )

    assert package["status"] == "completed"
    assert package["clean_extraction_validation"]["reason"] == "no_write"
    assert not (output_dir / DEFAULT_RUN_ID).exists()


def test_fallback_reason_probe_external_review_package_rejects_bad_namespace(
    tmp_path: Path,
) -> None:
    with pytest.raises(ValueError, match="S65 external review package namespace"):
        build_signature_bucket_fallback_reason_probe_external_review_package(
            project_root=tmp_path,
            output_dir=tmp_path / "bad",
            no_write=True,
        )


def _write_inputs(root: Path) -> dict[str, Path]:
    inputs_dir = root / "inputs"
    inputs_dir.mkdir(parents=True, exist_ok=True)
    paths = {
        "s59_probe": inputs_dir / "s59_probe.json",
        "s60_strategy": inputs_dir / "s60_strategy.json",
        "s61_review_summary": inputs_dir / "s61_review_summary.json",
        "s62_implementation": inputs_dir / "s62_implementation.json",
        "s63_readiness": inputs_dir / "s63_readiness.json",
        "s63_future_command": inputs_dir / "s63_future_command.json",
        "agents": inputs_dir / "AGENTS.md",
        "s63_builder": inputs_dir / "s63_builder.py",
        "s64_builder": inputs_dir / "s64_builder.py",
        "s63_tests": inputs_dir / "test_s63.py",
        "s64_tests": inputs_dir / "test_s64.py",
        "exact_coordinate_master_source": inputs_dir / "exact_coordinate_master.py",
        "s62_focused_tests": inputs_dir / "test_master.py",
        "exact_contract_tests": inputs_dir / "test_exact_contract.py",
    }
    paths["s59_probe"].write_text(
        json.dumps(
            {
                "status": "completed",
                "execute_no_solve": True,
                "cp_solver_solve_called": False,
                "checkpoint_written": False,
                "proof_source": False,
            }
        )
        + "\n",
        encoding="utf-8",
    )
    paths["s60_strategy"].write_text(
        json.dumps(
            {
                "status": "completed",
                "source_mutation_performed": False,
                "interpretation": {
                    "classification": "mandatory_region_counting_effective_but_fallback_residual_strategy_required"
                },
            }
        )
        + "\n",
        encoding="utf-8",
    )
    paths["s61_review_summary"].write_text(
        json.dumps(
            {
                "review_verdict": "pass_safe_to_request_authorization",
                "review_is_authorization": False,
                "authorization_required_next": True,
            }
        )
        + "\n",
        encoding="utf-8",
    )
    paths["s62_implementation"].write_text(
        json.dumps(
            {
                "status": "implemented_and_verified",
                "env_var": "EXACT_GHOST_SIGNATURE_BUCKET_MANDATORY_REGION_FALLBACK_INSTRUMENTATION",
            }
        )
        + "\n",
        encoding="utf-8",
    )
    paths["s63_readiness"].write_text(
        json.dumps(
            {
                "status": "completed",
                "readiness": {"classification": "ready_for_fallback_reason_probe_review"},
                "probe_execution_enabled": False,
            }
        )
        + "\n",
        encoding="utf-8",
    )
    paths["s63_future_command"].write_text(
        json.dumps(
            {
                "command": [
                    "python",
                    "scripts/run_phase3b_checkpoint_free_overlay_timing_probe.py",
                    "--execute-no-solve",
                    "--candidate-key",
                    "42x32",
                    "--run-id",
                    "local_hotspot_42x32_signature_bucket_fallback_reason_inst_no_solve_001",
                ],
                "environment": {
                    "EXACT_GHOST_SIGNATURE_BUCKET_TIGHTENING_INSTRUMENTATION": "1",
                    "EXACT_GHOST_SIGNATURE_BUCKET_MANDATORY_REGION_COUNTING": "1",
                    "EXACT_GHOST_SIGNATURE_BUCKET_MANDATORY_REGION_FALLBACK_INSTRUMENTATION": "1",
                },
            }
        )
        + "\n",
        encoding="utf-8",
    )
    paths["agents"].write_text(
        "\n".join(
            [
                "## GPT Project Review Standing Authorization",
                "- Current S59 enabled no-solve probe result: mandatory_region_counting_effective.",
                "- Current S60/S61 residual-fallback review state: passed.",
                "- Current S62 fallback-reason instrumentation state: implemented.",
                "- Current S63/S64 fallback-reason probe readiness state: ready.",
            ]
        )
        + "\n",
        encoding="utf-8",
    )
    paths["exact_coordinate_master_source"].write_text(
        "\n".join(
            [
                "EXACT_GHOST_SIGNATURE_BUCKET_MANDATORY_REGION_FALLBACK_INSTRUMENTATION_ENV = (",
                "    \"EXACT_GHOST_SIGNATURE_BUCKET_MANDATORY_REGION_FALLBACK_INSTRUMENTATION\"",
                ")",
                "",
                "def resolve_ghost_signature_bucket_mandatory_region_fallback_instrumentation_enabled():",
                "    return False",
                "",
                "class CoordinateExactMasterDelegate:",
                "    def _record_fallback_entry(self):",
                "        top_fallback_entries = []",
                "        fallback_reasons = {}",
            ]
        )
        + "\n",
        encoding="utf-8",
    )
    paths["s62_focused_tests"].write_text(
        "\n".join(
            [
                "def test_ghost_signature_bucket_mandatory_region_fallback_instrumentation_default_off_is_absent():",
                "    assert True",
                "",
                "def test_ghost_signature_bucket_mandatory_region_fallback_instrumentation_supported_fixture_has_empty_output():",
                "    assert True",
                "",
                "def test_ghost_signature_bucket_mandatory_region_fallback_instrumentation_rejects_unknown_value():",
                "    assert True",
                "",
                "def test_exact_core_overlay_signature_bucket_mandatory_region_fallback_instrumentation_matches_legacy():",
                "    assert True",
            ]
        )
        + "\n",
        encoding="utf-8",
    )
    for key in (
        "s63_builder",
        "s64_builder",
        "s63_tests",
        "s64_tests",
        "exact_contract_tests",
    ):
        paths[key].write_text(f"# fixture for {key}\n", encoding="utf-8")
    return paths


def _output_dir(root: Path) -> Path:
    return (
        root
        / ".artifacts"
        / "phase3b_local_13900ks_tuning_20260430"
        / "65_signature_bucket_fallback_reason_probe_external_review_package"
    )

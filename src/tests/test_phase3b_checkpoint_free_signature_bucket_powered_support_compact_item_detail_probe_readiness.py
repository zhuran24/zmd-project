...
from scripts.build_phase3b_checkpoint_free_signature_bucket_powered_support_compact_item_detail_probe_readiness import (
    ...
    build_signature_bucket_powered_support_compact_item_detail_probe_readiness,
    validate_future_command_template,
)


def test_powered_support_compact_item_detail_probe_readiness_builds_artifact(tmp_path: Path) -> None:
    ...
    assert readiness["readiness"]["classification"] == "ready_for_powered_support_compact_item_detail_probe_review"
    ...
    assert (output_dir / "signature_bucket_powered_support_compact_item_detail_probe_readiness.json").exists()
    assert (output_dir / "future_command_template.json").exists()


def test_powered_support_compact_item_detail_probe_readiness_missing_s144_blocks(tmp_path: Path) -> None:
    inputs = _write_inputs(tmp_path)
    inputs["s144_implementation"].unlink()
    ...
    assert checks["s144_patch_implemented_and_verified"] == "failed"
...
def test_powered_support_coverer_command_template_is_exact_and_bounded(tmp_path: Path) -> None:
    ...
    assert template["environment"][COMPACT_ITEM_DETAIL_INSTRUMENTATION_ENV_VAR] == "1"
        ...
        (
            {
                **base,
                "environment": {
                    ...
                    if key != COMPACT_ITEM_DETAIL_INSTRUMENTATION_ENV_VAR
                },
            },
            f"{COMPACT_ITEM_DETAIL_INSTRUMENTATION_ENV_VAR}_must_be_1",
        ),
    ]
...
def test_powered_support_coverer_readiness_no_write_and_namespace_guard(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="S145 readiness namespace"):
        build_signature_bucket_powered_support_compact_item_detail_probe_readiness(
            project_root=tmp_path,
...
def _write_inputs(tmp_path: Path) -> dict[str, Path]:
    ...
    paths = {
        "s140_probe_review": evidence / "s140_probe_review.json",
        "s143_review_summary": evidence / "s143_review_summary.json",
        "s144_implementation": evidence / "s144_implementation.json",
        "agents": tmp_path / "AGENTS.md",
    }
    paths["s140_probe_review"].write_text(
        json.dumps(
            {
                "status": "completed",
    ...
    paths["s143_review_summary"].write_text(
        json.dumps(
            {
                "review_verdict": "pass",
    ...
    paths["s144_implementation"].write_text(
        json.dumps(
            {
                "status": "implemented_and_verified",
                "env_var": COMPACT_ITEM_DETAIL_INSTRUMENTATION_ENV_VAR,
            }
        )
        + "\n",
        encoding="utf-8",
    )
    paths["agents"].write_text("S144 implemented\n", encoding="utf-8")
    return paths

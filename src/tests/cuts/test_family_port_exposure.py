...
def test_multiset_eval_single_literal_match():
    ...
    state = _make_state(
        crusher_poses=[("crusher", 7)],
        refinery_poses=[("refinery", 3)],
    )
...
def test_multiset_eval_missing_one_pose():
    ...
    cut = _make_port_exposure_cut(_make_port_exposure_cert())
    state = _make_state(
        crusher_poses=[("crusher", 7)],  # has crusher pose 7
        refinery_poses=[("refinery", 99)],  # but refinery pose 99 not 3
    )
...
def test_multiset_eval_slot_anonymity():
    ...
    cut = _make_port_exposure_cut(_make_port_exposure_cert())
    state = _make_state(
        crusher_poses=[("crusher", 1), ("crusher", 7)],  # pose 7 at second slot
        refinery_poses=[("refinery", 5), ("refinery", 3)],  # pose 3 at second slot
    )
    assert evaluate_literal_multiset(cut, state) is True
...
def test_multiset_eval_2_same_pose_required():
    ...
    cut = Cut(
        ...
        family="port_exposure",
        literals=(
            CutLiteral(AnonymousSlotRef("crusher", 0), 7),
            CutLiteral(AnonymousSlotRef("crusher", 1), 7),  # same pose, different slot
        ),
        geometric_payload=None,
        scope=CutScope(
    ...
    state_short = _make_state(crusher_poses=[("crusher", 7), ("crusher", 8)])
    assert evaluate_literal_multiset(cut, state_short) is False
    # state has 2 pose=7
    state_ok = _make_state(crusher_poses=[("crusher", 7), ("crusher", 7)])
...
def test_multiset_eval_empty_literals_false():
    cut = Cut(
        cut_id="empty-lit",
        family="port_exposure",
        literals=(CutLiteral(AnonymousSlotRef("crusher", 0), 1),),  # need ≥ 1 per __post_init__
        geometric_payload=None,
        scope=CutScope(
            ghost_rect_id=GHOST_AGNOSTIC, blocked_cells_hash="h",
            exterior_blocks_hash="h", source_digest="poc_source_digest",
...
def test_multiset_eval_unknown_group_false():
    cut = Cut(
        cut_id="unknown-grp",
        family="port_exposure",
        literals=(CutLiteral(AnonymousSlotRef("never_exists", 0), 1),),
        geometric_payload=None,
        scope=CutScope(
            ghost_rect_id=GHOST_AGNOSTIC, blocked_cells_hash="h",
            exterior_blocks_hash="h", source_digest="poc_source_digest",
...
def test_validate_port_exposure_ok():
    cert_payload = _make_port_exposure_cert(
        port_cell=(10, 10), port_direction="right", front_cell=(10, 11),
    )
...
def test_validate_port_exposure_unsound_front_cell_math():
    ...
    cert_payload = _make_port_exposure_cert(
        port_cell=(10, 10), port_direction="right", front_cell=(99, 99),  # 错
    )
    cut = _make_port_exposure_cut(cert_payload)
...
def test_validate_port_exposure_schema_err_unknown_direction():
    cert_payload = _make_port_exposure_cert(port_direction="diagonal")
    cut = _make_port_exposure_cut(cert_payload)
    state = _make_state(cell_owner={(10, 11): ("refinery", 0)})
    vr = validate_port_exposure(cut, state, CANONICAL_RULES)

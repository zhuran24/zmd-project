...
def test_anchor119_row_domain_guard_patch_review_bundle_ready(tmp_path: Path) -> None:
    ...
    (project_root / "src/search/phase3b/anchor119").mkdir(parents=True, exist_ok=True)
    (project_root / "src/search/phase3b/anchor119/guarded_precheck_spec.py").write_text(
        "# spec\n", encoding="utf-8"
    )
    (project_root / "src/search/phase3b/anchor119/guarded_precheck_runtime.py").write_text(
        "# runtime\n", encoding="utf-8"
    )
...
def test_anchor119_row_domain_guard_patch_review_bundle_cli_writes_and_no_write_skips(
    ...
    script = (
        repo_root
        / "scripts" / "phase3b" / "coordinate_validation" / "anchor119_row_domain" / "build_guard_patch_review_bundle.py"
    )

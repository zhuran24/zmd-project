...
from scripts.phase3b.checkpoint_free.signature_bucket.mandatory_region.build_mandatory_region_counting_probe_external_review_package import (
    DEFAULT_RUN_ID,
    REVIEW_WORDING_REQUIREMENTS,
    build_signature_bucket_mandatory_region_counting_probe_external_review_package,
)


def test_mandatory_region_counting_probe_external_review_package_builds_clean_zip(
    tmp_path: Path,
) -> None:
...
def test_mandatory_region_counting_probe_external_review_package_manifest_contains_context(
    tmp_path: Path,
) -> None:
    inputs = _write_package_inputs(tmp_path)
    output_dir = _output_dir(tmp_path)
    ...
    package = build_signature_bucket_mandatory_region_counting_probe_external_review_package(
        project_root=tmp_path,
        output_dir=output_dir,
        run_id="custom_s54_review_package",
        inputs=inputs,
    )
...
def test_mandatory_region_counting_probe_external_review_package_no_write(
    tmp_path: Path,
) -> None:
    inputs = _write_package_inputs(tmp_path)
    output_dir = _output_dir(tmp_path)
...
def test_mandatory_region_counting_probe_external_review_package_rejects_bad_namespace(
    tmp_path: Path,
) -> None:
    with pytest.raises(ValueError, match="S54 external review package namespace"):
        build_signature_bucket_mandatory_region_counting_probe_external_review_package(
            project_root=tmp_path,

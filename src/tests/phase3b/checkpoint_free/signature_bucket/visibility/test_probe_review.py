...
from scripts.phase3b.checkpoint_free.signature_bucket.visibility.build_probe_review import (
    EXPECTED_RUN_ID,
    build_signature_bucket_visibility_probe_review,
)


def test_signature_bucket_visibility_probe_review_classifies_visible_no_hotspot(
    tmp_path: Path,
) -> None:
    readiness_path, probe_path = _write_inputs(tmp_path, dominant_phase=None)
    ...
    assert review["interpretation"]["classification"] == "instrumentation_visible"
    assert review["signature_instrumentation"]["visibility_status"] == "instrumentation_visible"
...
def test_signature_bucket_visibility_probe_review_classifies_missing_as_visibility_failed(
    tmp_path: Path,
) -> None:
    ...
    assert review["status"] == "completed"
    assert review["interpretation"]["classification"] == "visibility_patch_failed"
    assert review["signature_instrumentation"]["present"] is False


def test_signature_bucket_visibility_probe_review_classifies_phase_hotspots(
    tmp_path: Path,
) -> None:
    cases = {
    ...
    for phase, expected in cases.items():
        case_root = tmp_path / phase
        readiness_path, probe_path = _write_inputs(case_root, dominant_phase=phase)
        review = build_signature_bucket_visibility_probe_review(
            project_root=case_root,
            readiness_path=readiness_path,
            probe_path=probe_path,
            output_dir=_output_dir(case_root),
            no_write=True,
        )
        assert review["interpretation"]["classification"] == expected
        assert review["signature_instrumentation"]["dominant_phase"] == phase


def test_signature_bucket_visibility_probe_review_safety_failure_requires_manual_review(
    tmp_path: Path,
) -> None:
...
def test_signature_bucket_visibility_probe_review_sensitive_change_disqualifies(
    tmp_path: Path,
) -> None:
    readiness_path, probe_path = _write_inputs(tmp_path)
    probe = json.loads(probe_path.read_text(encoding="utf-8"))
...
def test_signature_bucket_visibility_probe_review_rejects_bad_namespace(tmp_path: Path) -> None:
    readiness_path, probe_path = _write_inputs(tmp_path)

    with pytest.raises(ValueError, match="S48 probe review namespace"):
        build_signature_bucket_visibility_probe_review(
            project_root=tmp_path,
            readiness_path=readiness_path,
            probe_path=probe_path,
...
def _write_inputs(
    root: Path,
    *,
    dominant_phase: str | None = "per_anchor_mandatory_scan",
    include_instrumentation: bool = True,
) -> tuple[Path, Path]:
    root.mkdir(parents=True, exist_ok=True)
    readiness_path = root / "s47_readiness.json"
    probe_path = root / "probe.json"
    readiness_path.write_text(
        json.dumps(
            {
                "status": "completed",
                "readiness": {"classification": "ready_for_visibility_probe_review"},

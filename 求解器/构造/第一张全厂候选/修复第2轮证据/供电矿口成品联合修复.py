...
fixedpoles=[p for p in l['power_poles'] if p['id']!='P000']
for uid,opts in options.items():
 if uid=='P000':continue
 for z,u in opts:
  if not any(powered(p,u) for p in fixedpoles):m.add(z<=sum(pv for pv,pu in options['P000'] if powered(pu,u)))
for uid in miner:
 terminals=[]
...
res={'status':s.status_name(status),'wall':time.monotonic()-start,'workers':5,'source_count':52,'sink_count':52,'movable':sorted(movable),'scope':'219台机器允许平移2格、任意合法朝向，角区5台与1桩允许平移5格；核心及其他桩固定。允许六个成品角色在大型机位间重指派，固定各配方台数；所有制造机位必须被真实供电桩覆盖；兑现52矿口路径和6条成品入核心路径，其余路径和连续流须另验。'}
...
(BASE/'供电矿口成品联合修复.json').write_text(json.dumps(res,ensure_ascii=False,indent=2)+'\n')

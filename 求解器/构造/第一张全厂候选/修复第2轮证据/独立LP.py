...
(HERE/'独立LP-诊断见证-总流.json').write_text(json.dumps({'nonzero_values':[{'key':keys[j],'value':str(v)} for j,v in enumerate(x) if v],'solver':all_flow},ensure_ascii=False,indent=2)+'\n')

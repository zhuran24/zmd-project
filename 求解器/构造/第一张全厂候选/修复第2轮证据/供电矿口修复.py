...
for u in units:
 ...
 radius=5 if uid in {'M005','M070','M081','M100','M199','P000'} else 1
...
res={'status':s.status_name(status),'wall':time.monotonic()-start,'workers':1,'source_count':52,'sink_count':52,'movable':sorted(movable),'scope':'219台机器允许平移1格、任意合法朝向，角区5台与1桩允许平移5格；核心及其他桩固定。只核52矿口路径、真实供电及端口空格计数，不核其余生产链和成品流。'}
...
(BASE/'供电矿口修复.json').write_text(json.dumps(res,ensure_ascii=False,indent=2)+'\n')

...
l['power_poles'].append(dict(id='P026',x0=6,y0=6,x1=7,y1=7,orientation=0))
movable={u['id'] for u in l['machines']}|{'P000','P026'}
...
# Every manufacturing pose must be covered by a real, selected power pole.
def powered(p,u):return u['x0']<=p['x0']+6 and u['x1']>=p['x0']-5 and u['y0']<=p['y0']+6 and u['y1']>=p['y0']-5
fixedpoles=[p for p in l['power_poles'] if p['id'] not in movable]
for uid,opts in options.items():
 if uid.startswith('P'):continue
 for z,u in opts:
  if not any(powered(p,u) for p in fixedpoles):m.add(z<=sum(pv for pid in ['P000','P026'] for pv,pu in options[pid] if powered(pu,u)))
for uid in miner:
 terminals=[]
...
pk={uid:m.new_bool_var('packrole') for uid,opts in options.items() if opts[0][1].get('kind')=='大'}
fl={uid:m.new_bool_var('fillrole') for uid in pk}
for uid in pk:m.add(pk[uid]+fl[uid]<=1)
m.add(sum(pk.values())==3);m.add(sum(fl.values())==3)
emits=defaultdict(list);productstarts={};products={'高容谷地电池','精选荞愈胶囊'}
for uid,opts in options.items():
 if uid not in pk:continue
 terminals=[]
 for z,u in opts:
  fronts=[nb(c,u['Din']) for c in side(u,u['Din']) if nb(c,u['Din']) in free]
  m.add(sum(free[c] for c in fronts)>=5*pk[uid]+4*fl[uid]).only_enforce_if(z)
  dr=(u['Din']+2)%4
 ...
 m.add(sum(terminals)==pk[uid]+fl[uid])
...
hint=json.loads((BASE/'增桩矿口修复.json').read_text())['placements']
for uid,opts in options.items():
 for z,u in opts:
  if not isinstance(z,int):m.add_hint(z,int(u==hint.get(uid)))
...
s.parameters.stop_after_first_solution=True;s.parameters.random_seed=224;s.parameters.log_search_progress=True
start=time.monotonic();status=s.solve(m)
res={'status':s.status_name(status),'wall':time.monotonic()-start,'workers':5,'source_count':52,'sink_count':52,'movable':sorted(movable),'scope':'219台机器允许平移2格、任意合法朝向，角区5台与1桩允许平移5格；核心及其他桩固定，另加P026一座2×2桩，允许其在初始(6,6)附近平移8格。允许六个成品角色在大型机位间重指派，固定各配方台数；所有制造机位必须被真实供电桩覆盖；兑现52矿口路径和6条成品入核心路径，其余路径和连续流须另验。'}
if status in [cp_model.OPTIMAL,cp_model.FEASIBLE]:
 ...
 for (c,di),source in sorted(sources.items()):
  ...
  paths.append({'source':list(source),'target':list(target),'item':('高容谷地电池' if s.value(pk[source[0]]) else '精选荞愈胶囊') if target[0]=='CORE' else '源矿' if miner[target[0]]['recipe_ids'][0]=='粉碎-源矿' else '蓝铁矿','path':path})
 # Reassign six final roles to chosen large-machine poses; preserve all recipe counts.
 original={u['id']:u for u in l['machines']}
 remap={}
 for mark,model in [(pk,'封装机'),(fl,'灌装机')]:
  olds=sorted(uid for uid in mark if s.value(mark[uid]));news=sorted(uid for uid,u in original.items() if u['model']==model)
  assert len(olds)==len(news)==3;remap.update(zip(olds,news))
 oldleft=set(pk)-set(remap);newleft=set(pk)-set(remap.values())
 for uid in sorted(oldleft&newleft):remap[uid]=uid
 oldleft=set(pk)-set(remap);newleft=set(pk)-set(remap.values());remap.update(zip(sorted(oldleft),sorted(newleft)))
 finalchosen={}
 for uid,u in chosen.items():
  new=remap.get(uid,uid);v=dict(u,id=new)
  if new in original:
   for field in ['model','recipe_ids','settings']:v[field]=original[new][field]
  finalchosen[new]=v
 for p in paths:
  p['source'][0]=remap.get(p['source'][0],p['source'][0]);p['target'][0]=remap.get(p['target'][0],p['target'][0])
 res['placements']={uid:finalchosen[uid] for uid in movable};res['large_role_permutation']=remap
 res['paths']=paths;res['transport_cells']=sum(s.value(v) for v in used.values());res['bridges']=sum(s.value(v) for v in bridge.values())
(BASE/'增桩矿口成品修复.json').write_text(json.dumps(res,ensure_ascii=False,indent=2)+'\n')

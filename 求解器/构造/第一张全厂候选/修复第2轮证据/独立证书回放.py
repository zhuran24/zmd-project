...
tw=read('独立LP-诊断见证-总流.json');tx=[Q() for _ in keys]
for v in tw['nonzero_values']:tx[idx[tuple(v['key'])]]=Q(v['value'])
total=verify(r['source_relaxed_max_total_channel_flow']['certificate'],all_objective,tx)
zero_total=Q(total['objective'])==0
...
out={'candidate_sha256':r['candidate_sha256'],'strict_infeasible_exact':bool(bad) or (zero_total and any(n.startswith('source:') and Q(row['rhs'])>0 for n,row in rows.items())),'zero_row_contradictions':bad,
     'relaxed_total_flow':total,'all_batches_forced_zero':zero_total,'batch_columns_checked':len(batch_columns),
     'per_item':item_verifications,'solver_called':False,
     'scope':'固定快照、配方和物品支持；严格模型由矛盾行或全部通道流上界零与矿口等式下限一的冲突否证；最优值证书仅用于矿源<=1且去目标的放宽模型。'}

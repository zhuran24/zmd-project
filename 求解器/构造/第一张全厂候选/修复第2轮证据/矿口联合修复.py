...
movable={'M005','M070','M081','M100','M199','P000'}
...
for uid,opts in options.items():
 ...
 for z,u in opts:
  for s,need in [(u['Din'],1),((u['Din']+2)%4,2 if uid=='M199' else 1)]:
   qs=[nb(c,s) for c in side(u,s) if nb(c,s) in free]
   m.add(sum(free[c] for c in qs)>=need*z)
m.minimize(100*sum(used.values())+sum(obj))
...
res={'status':s.status_name(status),'wall':time.monotonic()-start,'workers':5,'source_count':52,'sink_count':52,'movable':sorted(movable),'scope':'仅指定6个实体在局部[2,10]^2重摆，其余非运输实体固定；只核矿口路径，不核其余生产链、供电和成品流。'}
...
(BASE/'矿口联合修复.json').write_text(json.dumps(res,ensure_ascii=False,indent=2)+'\n')

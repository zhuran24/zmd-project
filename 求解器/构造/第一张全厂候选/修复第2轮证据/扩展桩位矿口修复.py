...
for u in units:
 ...
 radius=8 if uid=='P000' else 5 if uid in {'M005','M070','M081','M100','M199','P000'} else 1
...
s=cp_model.CpSolver();s.parameters.num_workers=1;s.parameters.max_time_in_seconds=float(sys.argv[1]) if len(sys.argv)>1 else 180
s.parameters.random_seed=222;s.parameters.log_search_progress=True
start=time.monotonic();status=s.solve(m)
res={'status':s.status_name(status),'wall':time.monotonic()-start,'workers':1,'source_count':52,'sink_count':52,'movable':sorted(movable),'scope':'219台机器允许平移1格、任意合法朝向，角区5台允许平移5格，P000允许平移8格；核心及其他桩固定。只核52矿口路径、真实供电及端口空格计数，不核其余生产链和成品流。'}
...
(BASE/'扩展桩位矿口修复.json').write_text(json.dumps(res,ensure_ascii=False,indent=2)+'\n')

...
hint=json.loads((BASE/'全机局部矿口修复.json').read_text())['placements']
for uid,opts in options.items():
 for z,u in opts:
...
res={'status':s.status_name(status),'wall':time.monotonic()-start,'workers':5,'source_count':52,'sink_count':52,'movable':sorted(movable),'scope':'219台机器允许平移2格、任意合法朝向，角区5台与1桩允许平移5格；核心及其他桩固定，另加P026一座2×2桩，允许其在初始(6,6)附近平移8格。只核52矿口路径、真实供电及端口空格计数，不核其余生产链和成品流。'}
...
(BASE/'增桩矿口修复.json').write_text(json.dumps(res,ensure_ascii=False,indent=2)+'\n')

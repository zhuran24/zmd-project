...
arcs={(c,s):m.new_bool_var(f'a_{c}_{s}') for c in grid for s in range(4) if nb(c,s) in grid}
ins={};outs={};bridge={};used={};rows=[]
rows.append([0]*10)
for a in range(4):
 for b in range(4):
  if a!=b:rows.append([int(s==a) for s in range(4)]+[int(s==b) for s in range(4)]+[0,1])
for h in [0,2]:
 for v in [1,3]:rows.append([int(s in [h,v]) for s in range(4)]+[int(s in [(h+2)%4,(v+2)%4]) for s in range(4)]+[1,1])
for c in sorted(grid):
 iv=[m.new_bool_var('') for s in range(4)];ov=[m.new_bool_var('') for s in range(4)]
 ...
 for s in range(4):
  ...
  m.add(iv[s]==incoming+int((c,s) in sources))
  m.add(ov[s]==arcs.get((c,s),0)+sum(sinks[c,s]))
for c in grid:
 for s in [0,1]:
  if nb(c,s) in grid:m.add(bridge[c]+bridge[nb(c,s)]<=1)
# Shape restriction: enough input/output front cells of every machine belong to
# one common empty-cell road component. It is a necessary routing proxy only
# inside this chosen construction class, not a proof of physical bandwidth.
reached={c:m.new_bool_var('reached') for c in grid}
for c in grid:m.add(reached[c]<=free[c])
root=(1,2)
road={(c,s):m.new_int_var(0,4900,'road') for c in grid for s in range(4) if nb(c,s) in grid}
for (c,s),v in road.items():
 m.add(v<=4900*free[c]);m.add(v<=4900*free[nb(c,s)])
total=sum(reached.values())
for c in grid:
 incoming=sum(road.get((nb(c,s),(s+2)%4),0) for s in range(4))
 outgoing=sum(road.get((c,s),0) for s in range(4))
 m.add(incoming-outgoing==reached[c]-(total if c==root else 0))
for (c,s) in sources:m.add(reached[c]==1)
corefront=[nb(c,dr) for dr in [l['core']['Din'],(l['core']['Din']+2)%4] for c in side(l['core'],dr)[1:8] if nb(c,dr) in reached]
m.add(sum(reached[c] for c in corefront)>=6)
...
for uid,opts in options.items():
 ...
 for z,u in opts:
  for s,need in [(u['Din'],1),((u['Din']+2)%4,2 if u.get('model')=='采种机' else 3 if s==u['Din'] and u.get('model')=='研磨机' else 5 if s==u['Din'] and u.get('model')=='封装机' else 4 if s==u['Din'] and u.get('model')=='灌装机' else 2 if s==u['Din'] and u.get('model')=='塑形机' else 3 if s!=u['Din'] and u.get('recipe_ids')==['粉碎-砂叶'] else 2 if s!=u['Din'] and u.get('recipe_ids')==['粉碎-荞花'] else 1)]:
   ...
   m.add(sum(reached[c] for c in qs)>=need*z)
...
s.parameters.random_seed=222;s.parameters.log_search_progress=True
start=time.monotonic();status=s.solve(m)
res={'status':s.status_name(status),'wall':time.monotonic()-start,'workers':5,'source_count':52,'sink_count':52,'movable':sorted(movable),'scope':'219台机器允许平移2格、任意合法朝向，角区5台与1桩允许平移5格；核心及其他桩固定。要求所需数量的机器端口邻格与52矿口和至少6个核心存货邻格处于同一无容量道路分量；只兑现52矿口实体路径，其余路径和连续流须另验。'}
if status in [cp_model.OPTIMAL,cp_model.FEASIBLE]:
 ...
 for (c,di),source in sorted(sources.items()):
  ...
  paths.append({'source':list(source),'target':list(target),'item':'源矿' if miner[target[0]]['recipe_ids'][0]=='粉碎-源矿' else '蓝铁矿','path':path})
 res['paths']=paths;res['transport_cells']=sum(s.value(v) for v in used.values());res['bridges']=sum(s.value(v) for v in bridge.values())
(BASE/'道路连通矿口修复.json').write_text(json.dumps(res,ensure_ascii=False,indent=2)+'\n')

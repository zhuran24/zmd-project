...
# Break exchange symmetry before assigning recipe identities.
for typ in ['小','中','大']:
 group=[r for r in rows if r[1]==typ]
 for a,b in zip(group,group[1:]):model.Add(a[2]+70*a[3]<b[2]+70*b[3])
s=cp_model.CpSolver();s.parameters.num_workers=6;s.parameters.max_time_in_seconds=float(sys.argv[1]) if len(sys.argv)>1 else 300;s.parameters.random_seed=42
# Save every improving incumbent durably, even if the process is later interrupted.
class CB(cp_model.CpSolverSolutionCallback):
 def on_solution_callback(self):
  out={'objective':0,'wall':self.WallTime(),'core':core,'poles':poles,'reserved_rectangle':rect,'machines':[]}
  ...
  (BASE/'实验/匿名摆放.json').write_text(json.dumps(out,ensure_ascii=False,indent=1))
  print('INCUMBENT',round(self.WallTime(),2),0,flush=True)
st=s.Solve(model,CB());print('FINAL',s.StatusName(st),s.WallTime(),s.ObjectiveValue(),s.BestObjectiveBound(),flush=True)
(BASE/'实验/匿名摆放状态.json').write_text(json.dumps(dict(status=s.StatusName(st),wall=s.WallTime(),objective=s.ObjectiveValue(),bound=s.BestObjectiveBound()),indent=2))

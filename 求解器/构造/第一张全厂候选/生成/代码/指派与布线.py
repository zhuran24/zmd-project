...
d=load(BASE/'实验/离散摆放.json');assert 'units' in d,d['status']
...
for i,p in enumerate(poses):models[p['model']].append(i)
...
for model,ix in models.items():
 ids=[m for m in mids if raw[m]['kind']==model]
...
for f in feed:
 if f['source'] in raw and f['target'] in raw:neighbors[f['source']].append(f['target']);neighbors[f['target']].append(f['source'])
mineral={f['target'] for f in feed if f['source'].startswith('ORE')};finals={f['source'] for f in feed if f['target']=='CORE'}
def unary(m,p):
 x,y=cent[p];return 10000*(max(0,inneed[m]-len(ps[p]))+max(0,outneed[m]-len(po[p])))+(3*min(x,y) if m in mineral else 0)+(2*(abs(x-cx-4)+abs(y-cy-4)) if m in finals else 0)
def local(m):
 x,y=cent[assign[m]]
 return unary(m,assign[m])+sum(abs(x-cent[assign[n]][0])+abs(y-cent[assign[n]][1]) for n in neighbors[m])
groups=[[m for m in mids if raw[m]['kind']==model] for model in models];groups=[g for g in groups if len(g)>1]
for step in range(160000):
 ...
 temp=max(.05,4*(1-step/160000)**2)
 if after>before and rng.random()>math.exp(min(0,(before-after)/temp)):assign[a],assign[b]=assign[b],assign[a]
machines=[]
for m in mids:
 u=dict(poses[assign[m]],id=m,recipe_ids=[raw[m]['recipes'][0]['recipe']],settings={'manufacture_on':True});machines.append(u)
...
cost=[]
for f in orefeeds:
 u=mu[f['target']];qs=ports(u,u['Din']);cost.append([min((abs(p[3][0]-q[0][0])+abs(p[3][1]-q[0][1]) for q in qs),default=10000) for p in physical_sources])
...
for (item,rate),fs in group.items():
 ...
 for a in fs:
  ...
  for b in fs:
   ...
   row.append(min((abs(x[0][0]-y[0][0])+abs(x[0][1]-y[0][1]) for x in pa for y in pb),default=1e5)+(10000 if a['source']==b['target'] else 0))
  costs.append(row)

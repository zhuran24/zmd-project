#!/usr/bin/env python3
"""会议 3 seat-opus-2 实测：70×70、217 台、按机型建位置变量（不编号），
运输格占位 + 不分物品的聚合容量流（机器端口边上按配方收支）。

用法: python3 aggflow.py --flow 1 --rect none|x,y,w,h --time 600 --threads 6 --power 0|P

模型（压 U 方向的放松，但见文末「刻度」说明）：
- 位置变量 x[k,o,ax,ay]：机型 k、朝向 o、锚点；不给机器编号。
- 仓库取货口固定为一种合法排布：左边 x=0 占 y=1..69，下边 y=0 占 x=1..69，角格空。
- 协议核心位置变量（2 种端口朝向）。
- 运输格 t[c]、桥接器 b[c]<=t[c]；容量 CAP*(t+b)，桥两轴放松成可转弯（真放松）。
- 流量刻度 CAP=20 表示 1 件/tick；即流量取 1/20 的整数倍。
"""
...
TYPES = {
    # name: (class, count)
    'crush': ('S', 69), 'refine': ('S', 51), 'parts': ('S', 6), 'mold': ('S', 6),
    'plant': ('M', 32), 'seed': ('M', 17),
    'grind': ('L', 32), 'pack': ('L', 3), 'fill': ('L', 3),
}
...
spine={(35,y) for y in range(1,70)}|{(x,35) for x in range(1,70)}
m = cp_model.CpModel()
t0 = time.time()
...
for k,lst in placements.items():
    for v,ina,outa,fp in lst:
        for cell,dr in ina+outa:m.AddImplication(v,t[cell])
if args.tmax > 0:
...
if st in (cp_model.OPTIMAL, cp_model.FEASIBLE):
    ...
    for c in fixed_blocked:
        grid[c[1]][c[0]] = 'o'
    letter = {'crush': 'c', 'refine': 'r', 'parts': 'p', 'mold': 'm', 'plant': 'P', 'seed': 'S',
              'grind': 'G', 'pack': 'K', 'fill': 'F'}
    for k, lst in placements.items():
        for (v, ina, outa, fp) in lst:
            if solver.Value(v):
                name = v.Name().split('_')

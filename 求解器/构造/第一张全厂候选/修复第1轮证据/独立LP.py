...
result={
    ...
    'pass':False,'strict':strict,'strict_without_presolve':strict_no_presolve,'source_relaxed_with_targets':relaxed_target,
...
print(json.dumps({'pass':False,'strict':strict,'relaxed_target':relaxed_target,'disconnected_sources':len(disconnected),'channels':len(edges),'paths':len(paths),'total_relaxed_flow':all_flow['objective'],'per_item_max':{it:v['source_relaxed_max_production'] for it,v in per_item.items()}},ensure_ascii=False))

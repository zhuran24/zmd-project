...
for cv, ina, outa in core:
    for cell, side in outa:
        opts=[v for dr in range(4) if dr!=side for v in inport_k['crush'].get((cell,dr),[])]
        m.Add(sum(opts)>=1).OnlyEnforceIf(cv)
# A small unit has batch capacity one; it cannot absorb two full ore mouths.
for v, ina, outa, fp in placements['crush']:
    fixed_touch=sum(any(cell==q and dr!=sd for q,sd in ore_cells) for cell,dr in ina)
    if fixed_touch>1:m.Add(v==0)
# Full long input sides are deliberately reserved to avoid original pack/fill input deficits.
for v, ina, outa, fp in placements['grind']:
    if len(ina)<6 or len(outa)<1:m.Add(v==0)
if args.tmax > 0:

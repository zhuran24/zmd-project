"""绑定 catalog.HASHES 中71条旧版本的投影；尚未迁移到现行72条及其序号。"""
...
def geometry_checks(g,r):
 ...
 for coord,group,size,origin,far in [('x',left,H,a,rect['y1']),('y',bottom,W,b,rect['x1'])]:
  ...
  if origin in(2,3):ck(67,m<=(1 if far==69 else 2)*(origin-1),{'axis':coord,'m':m,'origin':origin})
  if origin==4:ck(69,m<=6 and size<=21,{'axis':coord,'m':m,'span':size})
 ck(67,A<=1005 or(a>=4 and b>=4 and W<=66 and H<=66),{'area':A})
 J=0
 ...
 if dict(n)==expected and B==0:ck(65,A<=1089,{'area':A})
 else:r.formal_na(65,'台数/箱体前件不成立')
 ...
 if four:
  ck(71,4*(T+F)+2*P>=(921 if B==0 else 918)and(B!=0 or 4*(T+F)+2*J>=921+X+Y),{'T':T,'F':F,'P':P,'J':J,'X':X,'Y':Y})
 else:r.formal_na(71,'四机型恰下限的前件不成立')
 ck(64,A+4*P<=1182 and((4*A+14*P<=4639 and 4*A+16*P-2*J+X+Y<=4639)if four and B==0 else 4*A+16*P-2*J<=4608)and(A!=1113 or B==0 and P<=12 and all(n[k]==v for k,v in MIN_COUNTS.items())),{'A':A,'P':P,'J':J,'X':X,'Y':Y})
 gapsides={}
 for p in l['power_poles']:
  vals=[]
 ...
 ck(66,loss<=23*P-217 and(P!=10 or sum(v<=2 for v in gapsides.values())<=1),{'gaps':gapsides,'loss':loss})
...
def flow_checks(g,f,x,r):
 ...
 for p in g.lay['power_poles']:
  ...
  if p['id']in st['side_gaps']:ck(66,cnt<=[13,14,14,17,18,19,22][st['side_gaps'][p['id']]],{'pole':p['id'],'manufacturing_covered':cnt})
 ...
 for u in g.lay['transport']:
  ...
  if len(neighbors)==4:
   four_nodes.append(uid);ck(70,g.types[uid]in('splitter','merger','bridge'),{'unit':uid})
   if all(g.types[v]in('machine','box')for v in neighbors):ck(70,Counter(g.units[v]['Din']%2 for v in neighbors)==Counter({0:2,1:2}),{'unit':uid,'neighbors':neighbors})
 ck(70,len(four_nodes)>=664-3*T,{'four_neighbor_nodes':four_nodes})
...
def plant_checks(g,f,x,r,total,bval):
 ...
 for plant in ['荞花','砂叶']:
  ...
  r.formal(68,bool(good),{'plant':plant,'projection':'分物品且桥分轴的正流图含采种/种植再生环'})
  ...
  r.formal(68,len(turns)>=4,{'plant':plant,'turning_transport_units':sorted(turns)})
 ...
 r.formal(68,tt-bb>=4 and tt+bb>=64 and tt>=34 and area>=1378,{'plant_transport':tt,'bridges':bb,'occupied_area':area})

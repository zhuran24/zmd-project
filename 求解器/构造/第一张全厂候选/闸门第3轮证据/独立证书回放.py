...
# 将放宽模型的总流零上界转为严格模型的完整Farkas证书。
# 原对偶给 A^T y <= -1_flow，b^T y = 0；给52条源等式各加乘子1。
# 源流列是全部flow列的子集，于是 A^T y' <= 0，b^T y' = 52 > 0。
ys={v['row']:Q(v['value']) for v in r['source_relaxed_max_total_channel_flow']['certificate']['multipliers']}
source_rows=[n for n in rows if n.startswith('source:')]
assert len(source_rows)==52
for n in source_rows:ys[n]=ys.get(n,Q())+1
col=[Q() for _ in keys];bound=Q()
for n,y in ys.items():
    row=rows[n]
    assert row['relation']=='eq' or y<=0
    bound+=y*Q(row['rhs'])
    for j,v in row['terms']:col[j]+=y*Q(v)
assert all(v<=0 for v in col) and bound==52
farkas={'bound':str(bound),'all_column_coefficients_nonpositive':True,'columns_checked':len(col),
        'inequality_multipliers_nonpositive':True,'multipliers':[{'row':n,'value':str(y)} for n,y in ys.items() if y]}
(HERE/'严格LP-Farkas证书.json').write_text(json.dumps(farkas,ensure_ascii=False,indent=2)+'\n')
...
out={'candidate_sha256':r['candidate_sha256'],'strict_infeasible_exact':bool(bad) or bound>0,'zero_row_contradictions':bad,
     'strict_farkas':{k:v for k,v in farkas.items() if k!='multipliers'},

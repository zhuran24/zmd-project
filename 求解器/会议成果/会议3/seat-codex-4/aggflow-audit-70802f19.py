...
TYPES = {
    # name: (class, count)
    'crush': ('S', 68), 'refine': ('S', 51), 'parts': ('S', 6), 'mold': ('S', 6),
    'plant': ('M', 32), 'seed': ('M', 16),
...
if args.rect != 'none':
    ...
    for x in range(rx, rx + rw):
        for y in range(ry, ry + rh):
            rect_cells.add((x, y))
blocked = fixed_blocked | rect_cells
...
print(json.dumps({k: v for k, v in res.items() if k != 'grid'}), flush=True)
if args.out:

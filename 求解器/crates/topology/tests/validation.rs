...
fn every_formal_constraint_has_coverage_row() {
    ...
    assert_eq!(cat["constraints"].as_array().unwrap().len(), 77);
...
fn check_catalog_against_sources(document: &[u8]) -> std::process::Output {
    use std::io::Write;
...
fn catalog_is_checked_against_live_formal_sources() {
    let result = check_catalog_against_sources(include_bytes!("../../../数据/正式静态目录.json"));
    assert!(
...
#[test]
fn catalog_mutations_are_rejected_by_live_source_gate() {
    // 四种已复现漏检加输入、输出与高度反例；候选未使用的配方也必须拒绝。
    for (section, id, fields, replacement) in [
        ("recipes", "精炼-蓝铁粉末", vec!["duration", "value"], "2"),
        (
            "recipes",
            "研磨-致密蓝铁",
            vec!["inputs", "蓝铁粉末", "value"],
            "3",
        ),
        (
            "recipes",
            "研磨-致密蓝铁",
            vec!["outputs", "致密蓝铁粉末", "value"],
            "2",
        ),
        (
            "recipes",
            "精炼-蓝铁粉末",
            vec!["duration", "category"],
            "候选",
        ),
        ("units", "供电桩", vec!["coverage", "width", "value"], "13"),
        ("units", "供电桩", vec!["coverage", "height", "value"], "13"),
        (
            "units",
            "协议核心",
            vec!["ports", "input_count", "category"],
            "候选",
        ),
    ] {
        let mut cat: serde_json::Value =
            serde_json::from_str(include_str!("../../../数据/正式静态目录.json")).unwrap();
        let mut field = cat[section]
            .as_array_mut()
            .unwrap()
            .iter_mut()
            .find(|row| row["id"] == id)
        ...
        *field = replacement.into();
        let result = check_catalog_against_sources(&serde_json::to_vec(&cat).unwrap());
        let diagnostic = String::from_utf8_lossy(&result.stderr);
        assert!(!result.status.success(), "漏验 {section}/{id}/{fields:?}");
        assert!(
            diagnostic.contains(&format!("{section}.{id}.{}", fields.join("."))),
            "{diagnostic}"
        );
    }
}
...
#[test]
fn catalog_full_field_mutation_regressions_pass() {
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .unwrap()
        .parent()
    ...
    let result = std::process::Command::new("python")
        .env("PYTHONDONTWRITEBYTECODE", "1")
        .arg(root.join("数据/工具/test_formal_catalog.py"))
        .output()
        .unwrap();
    assert!(
        result.status.success(),

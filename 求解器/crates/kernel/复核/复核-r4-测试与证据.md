...
|---|---:|---|---:|---|
| benchmark_brick_60 | 57/97 | 0.586346, 0.850765, 0.846336 | 0.846336 | 三次均达标 |
| benchmark_candidate_b | 535/630 | 17.245435, 18.218400, 17.337984 | 17.337984 | 三次均达标 |
| benchmark_brick | 81/100 | 2.228398, 2.658316, 2.649718 | 2.649718 | 附加对照，无本轮达标主张 |
...
| 8 | a→b | b | 2 | 冷却未到 | 0 |
| 9 | b→c | c | 1 | 冷却未到 | 0 |
| 10 | c→d | d | 5 | 尝试成功，空箱 | 0 |
| 11 | d→a | a | 4 | 冷却未到 | 0 |
| 12 | a→b | b | 3 | 冷却未到 | 0 |
| 13 | b→c | c | 2 | 冷却未到 | 0 |
| 14 | c→d | d | 1 | 冷却未到 | 0 |
| 15 | d→a | a | 5 | 尝试成功，空箱 | 0 |
| 16 | a→b | b | 4 | 冷却未到 | 0 |
| 17 | b→c | c | 3 | 冷却未到 | 0 |
| 18 | c→d | d | 2 | 冷却未到 | 0 |
| 19 | d→a | a | 1 | 冷却未到 | 0 |
| 20 | a→b | b | 5 | 尝试成功，空箱 | 0 |
...
|---|---|---|
| `transfer.judgment` | 桥接器双通路，`J\|0\|0\|5` | north/east箱各按全箱尝试；无逐格判定冒名 |
| `manufacturing.port_slot_relation` | 研磨混做核验，`J\|1\|0\|3` | 矿粉/砂叶粉末从存货口进入input格 |
| `manufacturing.output_blocked` | 研磨混做核验，`J\|3\|0\|6` | C\|3\|grinder完成后J\|3\|0\|6执行整批出缓存守卫；阻塞保批负例另有原生测试 |
| `bridge.inventory_scope` | 桥接器双通路，`J\|0\|0\|0` | 两轴各8件进出，物种与轴对应无互换 |
| `bridge.scheduling_scope` | 桥接器双通路，`J\|0\|0\|0` | 同一桥存取侧共享环和授权，实际跨两轴服务 |
| `transfer.cooldown_scope` | 桥接器双通路，`J\|0\|0\|5` | 单箱单冷却；实际尝试后5→4→3→2→1→5 |
| `transfer.pause` | 传输拒收与暂停核验，`J\|0\|0\|7` | east_box停用且正冷却保持；没有恢复事件 |
| `transfer.failure_cooldown` | 传输拒收与暂停核验，`J\|0\|0\|4` | south_box容量拒收后冷却=5，空箱尝试也起冷却 |
| `transfer.partial_acceptance` | 传输拒收与暂停核验，`J\|0\|0\|4` | t=0整箱容量拒收，电池与砂叶均不入库 |
| `transfer.resume_event` | 未执行，已登记原因 | 未执行；固定开关/供电域的已披露缺口 |
| `manufacturing.input_mixing` | 研磨混做核验，`J\|1\|0\|3` | 同一研磨机先后两配方，单格不并存异种 |
| `manufacturing.input_capacity_scope` | 研磨混做核验，`J\|1\|0\|3` | 实际入料核普通存货格容量；满容量边界另有单元测试 |
| `manufacturing.recipe_match_scope` | 研磨混做核验，`J\|2\|0\|6` | 两个输入格合并匹配2矿粉+1砂叶粉末 |
| `manufacturing.recipe_completeness` | 研磨混做核验，`J\|2\|0\|6` | t=2实际凑足整批后归集 |
| `manufacturing.recipe_lock_time` | 研磨混做核验，`J\|2\|0\|6` | J\|2\|0\|6锁配方并开始工作 |
| `manufacturing.empty_slot_identity` | 研磨混做核验，`J\|1\|0\|3` | 第一批归集清空输入格后第二物种可进入 |
| `manufacturing.input_collection` | 研磨混做核验，`J\|2\|0\|6` | 同一事件两条BC入缓存，2+1原料完整入批 |
| `manufacturing.buffer_power_gate` | 研磨混做核验，`J\|2\|0\|6` | BC搬运与制造子动作实际执行；停电分支另有单元回归 |
| `bridge.capacity` | 桥接器双通路，`J\|0\|0\|0` | 双轴独立容量1；同种双轴拒收另有原生测试 |
| `transfer.phase` | 桥接器双通路，`J\|0\|0\|5` | 初始0实际触发；正冷却场景另核，非法历史相位有负例 |
| `manufacturing.recipe_selection` | 研磨混做核验，`J\|2\|0\|6` | 按显式配方序选择；本例单次只有一个可匹配配方 |
| `manufacturing.input_slot_selection` | 研磨混做核验，`J\|1\|0\|3` | 已有物种回现格，新物种选择明确输入格 |
| `manufacturing.recipe_quantity_match` | 研磨混做核验，`J\|2\|0\|6` | 按每配方所需数量归集，剩余库存不凭空抹除 |
| `manufacturing.recipe_extra_items` | 研磨混做核验，`J\|2\|0\|6` | 匹配器检查该输入并集；不声称已区分全部额外物种读法 |
| `connection.bridge_first_contact` | 未执行，已登记原因 | 仅历史输入检查；未执行建造或定向 |
| `warehouse.delivery_count` | 桥接器双通路，`J\|5\|0\|5` | 两成品实际无线入库各8；空箱不计产量 |
| `warehouse.empty_slot_identity` | 桥接器双通路，`J\|5\|0\|5` | 首次入库新建成品格、绑定身份；匿名/历史格另有单元正反例 |
| `warehouse.external_supply` | 轮询均分核验，`J\|0\|0\|27` | 成功出矿事务同事件补回；30刻独立差分各30件 |
| `damping.branch` | 阻尼切支恢复核验，`J\|0\|0\|0` | 源取货侧有多个级；实际查可用出支集，门切支/恢复后重查 |
| `damping.belt_adjacency` | 阻尼连续带核验，`J\|0\|0\|0` | 不接受exercised：路径带串不是几何邻接判据，见发现1 |
| `damping.belt_component_rule` | 阻尼连续带核验，`J\|0\|0\|0` | 多级排序实际沿两条相继带计单个path_runs元件 |

...
def compare_bytes(a,b,timed=False):
    ...
    if a.lstrip().startswith((b'{',b'[')):tokens(a)
    if b.lstrip().startswith((b'{',b'[')):tokens(b)
    mapping=[]
    if timed:
...
def prepare():
    ...
    assert len(rows)==44
    capture=REPO/'target/health-capture/continuation-20260923/cases';capture.mkdir(parents=True,exist_ok=True)
    cases=[]
    for row in rows:
...
def capture_case(case,label,f,bins):
    ...
    if case['id']=='topology-contract':
        contract=json.loads((S/'数据/候选B/contract.json').read_text());contract['candidate']='第0批冻结来源A/A';write(root/'artifacts/contract.json',contract)
    results=[]
    for step in [*case['prerequisites'],dict(id='main',argv=case['argv'],expected=case['expected'])]:
        ...
        result=command(ident,step['argv'],S,step['expected'],case['environment'])
...
def main():
    ...
    for case in cases:
        a=capture_case(case,'a',f,bins);b=capture_case(case,'b',f,bins);results.append(compare_capture(a,b,fields));write(RUN/'continuation-aa-results.json',results);print('A/A passed',case['id'],flush=True)

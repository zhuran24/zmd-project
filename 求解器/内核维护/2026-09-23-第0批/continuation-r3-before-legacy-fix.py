...
def fixture_source(path):
    """Relocate two historical fixture references without changing their hashes."""
    data = read(path)
    mappings = []
    historical_root = Path('/home/zhuran24/zmd-research-fresh/求解器')
    for ref, relative in [
        (data['catalog'], '数据/正式静态目录.json'),
        (data['parameters']['axis_registry'], '规格/选择点参数轴.md'),
    ]:
        before = ref['path']
        assert before == str(historical_root / relative), ('unexpected fixture reference', before)
        target = ROOT / relative
        assert target.is_file(), ('missing relocated fixture dependency', str(target))
        ref['path'] = str(target.resolve())
        mappings.append(dict(before=before, after=ref['path'],
                             preserved_reference_sha256=ref['sha256'],
                             target_sha256=hashlib.sha256(target.read_bytes()).hexdigest()))
    save('fixture-relocation', dict(fixture=str(path),

...
HASH_CACHE={}
def cached_scan():
    # Every path is lstat'ed each time. Cache only bytes whose inode, ctime,
    # mtime, size, mode and link count are identical to our last actual hash.
    # Start/end inventories use audit_files.scan for unconditional full hashing.
    entries={};errors=[];rehashed=0;reused=0
    def visit(p):
        nonlocal rehashed,reused
        rel=str(p.relative_to(ROOT))
        if any(rel==e or rel.startswith(e+'/') for e in BASE['excludes']):return
        try:
            s=p.lstat();row={'mode':stat.S_IMODE(s.st_mode)}
            if stat.S_ISDIR(s.st_mode):
                row['kind']='directory'
                for name in sorted(os.listdir(p)):visit(p/name)
            elif stat.S_ISLNK(s.st_mode):row.update(kind='symlink',target=os.readlink(p),size=s.st_size)
            elif stat.S_ISREG(s.st_mode):
                key=(s.st_dev,s.st_ino,s.st_mode,s.st_size,s.st_mtime_ns,s.st_ctime_ns,s.st_nlink)
                cached=HASH_CACHE.get(rel)
                if cached and cached[0]==key:digest=cached[1];reused+=1
                else:
                    digest=sha(p);rehashed+=1;s2=p.lstat()
                    assert key==(s2.st_dev,s2.st_ino,s2.st_mode,s2.st_size,s2.st_mtime_ns,s2.st_ctime_ns,s2.st_nlink),('changed during hash',rel)
                    HASH_CACHE[rel]=(key,digest)
                row.update(kind='file',size=s.st_size,sha256=digest)
            else:row['kind']='special'
            entries[rel]=row
        except (OSError,AssertionError) as e:errors.append({'path':rel,'error':str(e)})
    visit(ROOT)
    return dict(entries=entries,errors=errors,rehashed=rehashed,reused=reused)
def protect(label):
    now=cached_scan();changes=diff(BASE['entries'],now['entries'])
    concurrent=[d for d in changes if d['path'].startswith('求解器/候选约束轮次/')]
    unexpected=[d for d in changes if d not in concurrent]
    save(label+'-protection.json',dict(errors=now['errors'],concurrent=concurrent,unexpected=unexpected,rehashed_files=now['rehashed'],unchanged_metadata_cached_files=now['reused']))

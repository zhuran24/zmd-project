...
assert compare_checker(closures,record,S);save('own-closure-positive.json',dict(passed=True,artifacts=closed,checker_files=sum(x['role']=='checker' and x['path'].endswith('.rs') for x in record['fingerprints'])))

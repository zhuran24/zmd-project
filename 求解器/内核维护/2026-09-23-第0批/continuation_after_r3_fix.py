...
command('continue-production-b2',['cargo','build','--workspace','--bins',*cargo_args(f['continuation_profile']),'--message-format=json'],S)
art=artifacts('continue-production-b2');old=json.loads((RUN/'continuation-production-a-artifacts.json').read_text());rows=[]
for a in art:
 if a.get('executable'):
...
bounded_probes(snapshot);safe(snapshot);profile,tests,art=harnesses(snapshot);isolation(snapshot,profile,tests,art);verify_build()

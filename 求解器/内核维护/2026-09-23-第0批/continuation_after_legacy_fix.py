...
command('continue-production-b3',['cargo','build','--workspace','--bins',*cargo_args(f['continuation_profile']),'--message-format=json'],S)
art=artifacts('continue-production-b3');old=json.loads((RUN/'continuation-production-a-artifacts.json').read_text());rows=[]
for a in art:
 if a.get('executable'):

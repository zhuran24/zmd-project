"""2026-09-26 第 84—85 轮修正版三审同步：求解约束 75 条→77 条后的目录重建、引用重锁与审计；由 guard.py 调用。

改动来源：主会话三审第 84—85 轮修正版，新增「存货首件死锁」「成品停收的停批范围」两条，
「存货误料停机与可用台数」正文与据改写。做法沿用 2026-09-26-第81轮三审同步（r27）：所有内容与授权检查先于第一次业务写入；
写入前备份原字节到 before/。
"""
import copy
import hashlib
...
OLD_SHA = 'f0e18dc1bc89b8c7d28eba92c1e7753a50ba1fb03072e6f55f0582b91707c352'
OLD_SOURCE_SHA = '4813a423fca1d155c22bd99ca2601ef5f0ece0e0e8d52d3369173bbe7adc0304'
OLD_VERSION = '2026-09-26-r27-constraints-75'
VERSION = '2026-09-26-r28-constraints-77'
OLD_COUNT, NEW_COUNT = 75, 77
ADDED = ['存货首件死锁', '成品停收的停批范围']
CHANGED = ['存货误料停机与可用台数']
NECESSARY = '§6：达标必要条件，保持原文前件；未作运行认证'
NEW_DESTINATION = {'存货首件死锁': NECESSARY, '成品停收的停批范围': NECESSARY}
THIS_RUN = '求解器/内核维护/2026-09-26-第84-85轮三审同步/'
...
def formal_module(text):
    ...
    spec = importlib.util.spec_from_loader('formal_catalog_r28', loader=None)
...
def sync():
    ...
    new_tool = replace_once(tool_text, [
        ("    assert len(rows) == 75 and all(r.get('basis') for r in rows)", "    assert len(rows) == 77 and all(r.get('basis') for r in rows)"),
        ("三份正式来源指纹与全文、75 条约束上下文", "三份正式来源指纹与全文、77 条约束上下文")], 'formal_catalog.py')
    plan[FORMAL_TOOL] = new_tool.encode()
    p = ROOT / '数据/工具/test_formal_catalog.py'
    plan[p] = replace_once(p.read_text(), [("self.assertEqual(len(projection['constraints']), 75)", "self.assertEqual(len(projection['constraints']), 77)")], p.name).encode()
    p = ROOT / '数据/工具/check_revision.py'
    plan[p] = replace_once(p.read_text(), [("assert len(catalog['constraints']) == 75", "assert len(catalog['constraints']) == 77")], p.name).encode()
    p = ROOT / 'crates/topology/tests/validation.rs'
    plan[p] = replace_once(p.read_text(), [('assert_eq!(cat["constraints"].as_array().unwrap().len(), 75);', 'assert_eq!(cat["constraints"].as_array().unwrap().len(), 77);')], p.name).encode()
    p = ROOT / '规格/check_revision.py'
    plan[p] = replace_once(p.read_text(), [
        ("check(len(expected) == len(rows) == 75, '正式约束 75 条覆盖')", "check(len(expected) == len(rows) == 77, '正式约束 77 条覆盖')"),
        ('内核维护/2026-09-26-第81轮三审同步/只读文件指纹.json', '内核维护/2026-09-26-第84-85轮三审同步/只读文件指纹.json')], p.name).encode()

    # 2. 目录：按 77 条版本的 formal_catalog 回源重建
    ...
    for ref in new_sources:
        if Path(ref['path']) == CATALOG:
            ...
            # r27 之后流水线入档第 84—85 轮、本轮三审改状态行与进度段，旧指纹已过期，按现行字节同步
    ...
    plan[p] = replace_once(p.read_text(), [(OLD_SOURCE_SHA, source_sha),
        ('现行三份正式源与 75 条约束覆盖已同步', '现行三份正式源与 77 条约束覆盖已同步')], p.name).encode()
    p = ROOT / '规格/规则覆盖表.md'
    t = replace_once(p.read_text(), [('登记正式约束全部 75 条', '登记正式约束全部 77 条')], p.name)
    plan[p] = rebuild_semantics_table(t, current).encode()
    p = ROOT / '数据/规则覆盖表.md'
    t = replace_once(p.read_text(), [('任务 16 行与 75 条正式约束逐行/逐项登记', '任务 16 行与 77 条正式约束逐行/逐项登记')], p.name)
    plan[p] = rebuild_contract_table(t, current).encode()
    p = ROOT / '数据/候选B/转换说明.md'
    plan[p] = replace_once(p.read_text(), [('75 条正式约束目录与来源引用已同步', '77 条正式约束目录与来源引用已同步'),
                                           ('（75 条，算术计数）', '（77 条，算术计数）')], p.name).encode()

    # 6. 修订记录追加 r28
    revisions = ROOT / '规格/修订记录.md'
    addition = f'''\n## 2026-09-26 r28：三审采纳第 84—85 轮修正版，正式约束 75 条改为 77 条\n\n依据主会话三审后的现行三份正式文件，使用 `数据/工具/formal_catalog.py` 的回源投影重建目录。目录版本 `{VERSION}`，SHA-256 `{new_sha}`；约束 77 条。\n\n| 文件 | SHA-256 |\n|---|---|\n'''
    addition += ''.join(f"| {s['path']} | `{s['sha256']}` |\n" for s in current['sources'])
    addition += '''\n求解约束新增「存货首件死锁」「成品停收的停批范围」两条；「存货误料停机与可用台数」原句后接首件死锁与取货堵死两类、四类按并集扣除后的台数下限推广到调试期后每个可到达状态，据行随之改写；正式空矩形上界仍为 1110。目录的约束投影与正式约束来源全文同步；任务、18 类单位、18 条配方、32 项常量和 19 项物料流量均不变。formal_catalog.py、test_formal_catalog.py、数据/工具/check_revision.py、规格/check_revision.py、topology 的 validation.rs 中写死的条数改为 77；两张规则覆盖表的正式约束节按新目录重排，新增两条的去向按同类必要条件登记。54 份内核输入（45 个样例、9 个 fixture）只重锁目录 SHA；候选 B 来源清单、样例来源校验、运行语义来源 SHA、规格自查的只读快照入口同步；候选 B 报告用本轮构建的 topology 重新生成。求解充分条件.txt 同时由 4 条变 9 条，它不是内核来源，内核不读。\n\n本轮执行记录及安全测试结果见[2026-09-26 第 84—85 轮三审同步记录](../内核维护/2026-09-26-第84-85轮三审同步/记录.md)。本节不修改旧轮记录与历史证据。\n'''
    assert '\n## 2026-09-26 r28' not in revisions.read_text()
...
def candidate_report():
    ...
    text = rec.read_text()
    old_line = text.split('\n')[2]
    assert old_line.startswith('现行目录为 `2026-09-26-r27-constraints-75`'), old_line
    new_line = (f"现行目录为 `{VERSION}`；候选 B 当前报告为 {counts['能检且通过']} 项通过、{counts['能检且不通过']} 项失败、"
                ...
                "目录重锁、报告重新生成与本轮安全验证见[2026-09-26 第 84—85 轮三审同步记录](../../内核维护/2026-09-26-第84-85轮三审同步/记录.md)；下文是旧轮史料。")

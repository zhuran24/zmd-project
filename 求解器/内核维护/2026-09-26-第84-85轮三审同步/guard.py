"""本轮（2026-09-26 第 84—85 轮修正版三审同步）命令记录与历史全量 SHA-256 守卫；沿用 2026-09-26-三审同步的 guard.py，未改逻辑。"""
...
def run(name, argv):
    ...
    if argv[0] == 'cargo':
        ...
        # 只允许候选 B 静态校验：topology 读契约、报告写到标准输出（落本轮日志）
        assert argv == [str(ROOT / 'target/debug/topology'), '数据/候选B/contract.json'], argv
    guard(name + '-before')
...
if __name__ == '__main__':
    ...
    elif mode in ['sync', 'candidate_report', 'audit']:
        import work
        getattr(work, mode)()
    else:

...
assert run('formal-catalog-verify', ['python3', '-B', '数据/工具/formal_catalog.py']) == 0
...
code = run('semantics-new-topology', [str(ROOT / 'target/release/topology'), str(OUT / 'semantics-33-growers-buckflower.json')])
report = (OUT / 'semantics-new-topology.stdout.log').read_text()
row = next(s for s in report.splitlines() if '矿系不入库/计划' in s)
assert '| Fail |' in row and code != 0, row
unknown_rows = [next(s for s in report.splitlines() if '正式条目/' + name + ' |' in s)
                for name in ['矿系不入库', '非成品零入库', '传输按仓库余量判定']]
assert all('| Unknown |' in s for s in unknown_rows), unknown_rows

...
def main():
    ...
    ids={'K3-r4-L1-01','K3-r4-L3-01','K3-r4-L3-02','K3-r4-L3-03'}
    logged=re.findall(r'^\| (K3-r4-L\d-\d+) \|',(SOLVER/'规格/内核输入-修订记录.md').read_text(),re.M)
    assert len(logged)==len(ids)==4 and set(logged)==ids
    for phrase in ('当前版本只支持逐时刻有限历史','无限或符号函数需另增函数编码',
                   'phase=before_boundary/after_closure','更一般中途恢复还需已审阶段去重状态'):
        assert phrase not in body,phrase
    for phrase in ('event-order-v3','order-expr-v1','is_power_of_two','单位槽位保留语法','运行扫描与制造完成','event-order-v2','slot_width','in_boundary','in_closure','seen_boundaries','无身份空格集合从当前 warehouse'):
    ...
    results.append({'axis_count':99,'findings':4,'status':'全集、修订记录及已定位旧句检查通过'})
    ...
    for path in ('选择点参数轴.md','选择点清单.md','规则覆盖表.md'):
        assert 'event-order-v3' in (SOLVER/'规格'/path).read_text()
    assert '跨tick身份' in (SOLVER/'规格/规则覆盖表.md').read_text()
    results.append({'status':'通过','shared_request':'§7三表v3及全局身份引用已回核'})
    result={'status':'通过','checks':results,'scope':'逐行覆盖/引用及第四轮辖域；新§7请求已回核规格线答复及共享表v3/身份入口。本检查不是语义穷尽证明'}

"""重跑第6轮规格与共享依赖检查；日志限本目录；黄金重算按既有程序更新C线运行记录，其余依赖只读。"""
import hashlib
import json
import os
...
def main():
    ...
    commands = [
        ...
        ('cargo-test', ['cargo', 'test', '--offline', '--manifest-path', str(ROOT/'求解器/Cargo.toml')]),
        ('候选B校验', ['cargo', 'run', '--offline', '--quiet', '--manifest-path', str(ROOT/'求解器/Cargo.toml'), '--', str(ROOT/'求解器/数据/候选B/contract.json')]),
    ]
    commands += [
    ...
    filenames = {'规格自查': '规格自查结果.json', '原转移回归': '原转移回归结果.json',
                 '修订回归': '修订回归结果.json', '正式目录': '正式目录.log',
    ...
    for name, command in commands:
        result = subprocess.run(command, cwd=ROOT, text=True, capture_output=True,
            env={**os.environ, 'PYTHONDONTWRITEBYTECODE': '1', 'CARGO_TARGET_DIR': str(ROOT/'求解器/target')})

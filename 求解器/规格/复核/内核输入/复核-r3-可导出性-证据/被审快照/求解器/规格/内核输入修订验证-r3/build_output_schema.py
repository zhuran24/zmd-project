...
schema=obj({'schema':{'const':'kernel-output-v1'},'run_id':string,'profile_id':string,'producer':obj({'kind':{'enum':['kernel','bounded_reference_checker','manual_expected']},'path':string,'claim':string}),'status':{'enum':['completed','invalid_input','unsupported','unresolved','inconclusive']},'fingerprints':array(obj({'role':{'enum':['input','catalog','axis_registry','profile','formal_source','golden','checker','semantics','schema']},'path':string,'sha256':ref('Hash')})),
'parameter_assignment':ref('Parameters'),'input_history':obj({'timeline':{'type':'object'},'construction':{'type':'object'},'debug_operations':array({'type':'object'}),'environment':{'type':'object'},'reachability':ref('Decision')}),'uncovered_axes':array(obj({'axis':string,'reason':string,'disposition':string})),
...
schema.update({'$schema':'https://json-schema.org/draft/2020-12/schema','$id':'urn:zmd:kernel-output-v1','$defs':{'Quantity':q,'Decision':dec,'Time':time,'Hash':{'type':'string','pattern':'^[0-9a-f]{64}$'},'Warehouse':warehouse,'Content':content,'Progress':progress,'StateSeed':state,'Parameters':parameters,'Event':event,'Tick':tick}})

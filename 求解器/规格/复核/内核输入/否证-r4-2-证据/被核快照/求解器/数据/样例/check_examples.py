...
KNOWN_VALUES = {
    ...
    "connection.port_meeting": "shared_edge_opposite",
...
def check(data, path):
    ...
    for uid, moment in zip(order, moments):
        ...
        require(symbol not in symbols, "建成时刻须互异")
    ...
    for before, after in zip(build_events, build_events[1:]):
        merged["relations"].append({"before": before, "after": after, "relation": "strict", "basis": ["蓝图见证"]})

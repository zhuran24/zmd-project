...
fingerprints = json.loads((spec_dir / '第6轮修订验证/开工只读指纹.json').read_text())
check(all(hashlib.sha256(Path(path).read_bytes()).hexdigest() == digest for path, digest in fingerprints.items()), f'只读文件 {len(fingerprints)} 份指纹未变')
...
check(all(term in transfer for term in ['同一快照', '一次重算', '保持completed', '全部存货格', 'competing_new_species', 'len(U)>=2 and len(E)>=1', 'recipe_quantity_match=at_least', 'recipe_extra_items=allow']), '第6轮边界、库存、仓库停止及配方两轴在可执行条款定位')
...
check('event-order-v2' in axes and 'event-order-v2' in choices and 'continuation' in semantics, '内核输入请求新增第6节在共享正文并入')
...
interface_status = {
    ...
    'schemas_present': {name: name in input_body for name in ['event-order-v1', 'event-order-v2', 'damping-branch-v1', 'poll-memory-v1']},
    'revision6_description_mismatches': [field for field in ['time.instant_order', 'polling.membership_change', 'manufacturing.recipe_completeness', 'manufacturing.output_blocked', 'warehouse.empty_slot_identity'] if field not in input_axis_map or next(row[3] for row in axis_rows if row[0] == field) not in input_axis_map[field][3]],
...
result = {'status': 'PASS', 'scope': '规格线文档覆盖、99轴、当前目录、定位回归、条件算术与本轮只读指纹；非全规则或运行认证', 'axis_count': len(axis_ids), 'independent_item_count': len(independent_ids), 'finding_count': len(new_ids), 'revision6_finding_count': 5, 'catalog_sha256': hashlib.sha256((root / '求解器/数据/正式静态目录.json').read_bytes()).hexdigest(), 'axis_registry_sha256': hashlib.sha256((spec_dir / '选择点参数轴.md').read_bytes()).hexdigest(), 'request_sha256': request_hash, 'shared_interface': interface_status, 'checks': results}

...
def main():
    ...
    source_paths += [SPEC/name for name in ['选择点参数轴.md', '受限模型声明.md', '内核配置-v1.json', '受限转移定义.md', '内核输入.md', '内核输出.md']]
    ...
    report = {'status': 'PASS', 'scope': '当前C线原产物只读检查，未临时刷新哈希或改赋值；非全称认证',
              ...
              'golden_match': True, 'full_record_matches_recomputation': True, 'ticks': len(ticks),

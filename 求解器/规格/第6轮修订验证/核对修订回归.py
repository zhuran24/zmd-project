"""第6轮局部转移回归（空格序后效按第7轮规范维护）；按规则不变量另核后态，不是完整执行器。"""
import copy
import itertools
import json
...
def transfer(box, warehouse, empty_order):
    ...
    original = {'box': copy.deepcopy(box), 'warehouse': copy.deepcopy(warehouse), 'cooldown': 0, 'delivered': {}, 'warehouse_empty_slot_order': copy.deepcopy(empty_order)}
    targets = {}
    for item in box:
    ...
    # 库存和当前候选序一并提交；不再继承已占格。
    result['warehouse_empty_slot_order'] = [key for key in empty_order
        if not result['warehouse'][key]['quantity'] and result['warehouse'][key]['identity'] is None]
...
def run():
    ...
    for order in itertools.permutations(incoming):
        ...
        assert state == {'box': box, 'warehouse': warehouse, 'cooldown': 0, 'delivered': {}, 'warehouse_empty_slot_order': ['W0', 'W1']}

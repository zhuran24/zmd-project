...
def run(seed, m, L, k, phi_extra, adv_ticks, wrong=None, fixed_order_seed=None, init=None):
    ...
    def do(j, t):
        ...
        if typ == 'enter':
            ...
            if c and c[0] == 'done':
                _, outk, q = recipe[n]
                tk = M[n]['take']
                if tk is None or (tk[0] == outk and tk[1] + q <= m):
                    M[n]['take'] = (outk, cnt(tk) + q); M[n]['cache'] = None
                    return True
            return False
        if typ == 'start':
            n = j[1]
        ...
        if typ == 'shift':
            pn, i = j[1], j[2]
            p = paths[pn]
            if p[i] and p[i][1] <= t - 1 and p[i + 1] is None:
    ...
    while True:
        ...
        for n in 'CABK':
            c = M[n]['cache']
            if c and c[0] == 'prog' and c[1] <= t - 1:
                M[n]['cache'] = ('done', None)
        # 下游收货（对手）
        for i in range(k):
            if down[i] and down[i][1] <= t - 1:
                acc = (rnd.random() < 0.5) if adv else pattern[(i * period_k) + (t % period_k)]
                if acc:
                    down[i] = None
        took = []
        ...
        if not adv:
            key = (tuple((n, M[n]['store'], M[n]['cache'] and M[n]['cache'][0], M[n]['take']) for n in 'CABK'),
                   tuple(tuple(None if c is None else (c[0], min(1, t - c[1])) for c in paths[pn]) for pn in ('CA', 'AC', 'CB', 'BK')),
                   tuple(None if c is None else min(1, t - c[1]) for c in down), t % period_k)
...
if __name__ == '__main__' and sys.argv[1] == 'wrong_full':
    m = int(sys.argv[2])
...
if __name__ == '__main__':
    ...
    for r in range(runs):
        ...
        k = rnd.choice([2, 3])
        res = run(seed0 * 100000 + r, m, L, k, rnd.choice([0, 0, 1, 3]), rnd.choice([30, 100, 300]), wrong=(mode == 'wrong'))
        if res is None:
            continue
        agg['runs'] += 1
        if res['viol']:

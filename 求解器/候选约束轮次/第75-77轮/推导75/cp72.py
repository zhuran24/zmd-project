#!/usr/bin/env python3
"""Round 72 CP-SAT boundary relaxation; all ten physical pole positions.
...
def caps():
    ...
    for (x,y),c in group.items():
        n=int(x in (1,68))+int(y in (1,68))
        loss=max(23-c,23-local.get((x,y),23),15 if n==2 else 10 if n else 0)
        if y-5>=17 and y+7<=70 and 0<=47-x<=6: loss=max(loss,[10,9,9,6,5,4,1][47-x])
        if x-5>=49 and x+7<=70 and 0<=15-y<=6: loss=max(loss,[10,9,9,6,5,4,1][15-y])
        out[x,y]=loss
    return out
...
def build(strip=False, fixed_j=None, cap=None, hint=None,project=False,groups=False):
    ...
    # R69 exact one-dimensional projections are independently rechecked here.
    tables=json.loads((ROUNDS/'第69-71轮/推导69/new_edge_local_certificate.json').read_text())['cases'][0]['lines']
    for no,(line,tab) in enumerate(zip(LINES,tables)):
        meet=[i for i,b in enumerate(bs) if cells(b['x'],b['y'],b['w'],b['h'])&line]
        c=model.new_int_var(0,1,'line_core'+str(no));p=model.new_int_var(0,2,'line_poles'+str(no));l=model.new_int_var(0,13,'line_loss_floor'+str(no));g=model.new_int_var(-4,len(line),'line_cost_floor'+str(no))
        model.add(c==sum(vs[i] for i in meet if bs[i]['kind']=='c'));model.add(p==sum(vs[i] for i in meet if bs[i]['kind']=='p'))
        model.add(l<=sum(vs[i]*bs[i]['loss'] for i in meet if bs[i]['kind']=='p'))
        model.add_allowed_assignments([c,p,l,g],[tuple(state)+(val,) for state,val in tab['frontier'] if state[2]<=13])
        actual=len(line)-sum(vs[i]*len(cells(bs[i]['x'],bs[i]['y'],bs[i]['w'],bs[i]['h'])&line) for i in meet)-2*sum(vs[i]*bs[i]['j'] for i in meet)
        model.add(actual>=g)
    if groups:
        assert not project
    ...
    score=model.new_int_var(180,187 if cap is None else cap,'S')

...
def build(R,edge0=True,minimize=False):
    ...
    occ={c:m.new_bool_var(f'occ_{c}') for c in cov}
    for c,vv in cov.items(): m.add(sum(vv)==occ[c])
    for i,d in enumerate(bd):
        for k,cs in d['need']:
            m.add(sum(1-occ[c] if c in occ else 1 for c in cs)>=k).only_enforce_if(vs[i])
    ...
    m.add(9*J<=23*P-217)
    xval=m.new_int_var(0,400,'X'); yval=m.new_int_var(0,400,'Y')
    m.add(xval==len(X)+2*corner-sum(v*d['xc'] for v,d in zip(vs,bd)))
    m.add(yval==len(Y)-sum(v*d['yc'] for v,d in zip(vs,bd)))

...
if SNAPSHOT.exists():
    assert before == json.loads((HERE / '原件开工指纹.json').read_text())
else:
...
for case in cases:
    ...
    if case != 'cooldown_control':
        ...
        # 每个变异使用全新构建目录，禁止跨副本复用内置目录和旧测试二进制。
        case_target = HERE / 'fresh-targets' / case
        assert not case_target.exists(), '重跑须先将本席旧构建目录另行存档'
        env['CARGO_TARGET_DIR'] = str(case_target)
        ...
        assert row['topology_recompiled'], '缺少重新编译证据，不能采用测试结果'
        ...
        row['cli_counts'] = dict(re.findall(r'## (能检且通过|能检且不通过|不能静态检)（(\d+) 项', output))

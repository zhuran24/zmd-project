...
def validate_schema(value,schema,root,path='$'):
    ...
    supported={'$ref','$schema','$id','$defs','type','properties','required','additionalProperties','items','minItems','minLength','pattern','minimum','enum','const','anyOf','oneOf','allOf','maxItems','if','then','else'}
    checker.require(not (set(schema)-supported),'schema 出现未实现关键词')
    if 'allOf' in schema:
        for branch in schema['allOf']:validate_schema(value,branch,root,path)
    if 'oneOf' in schema:
        count=0
        for branch in schema['oneOf']:
            try:validate_schema(value,branch,root,path)
            except checker.CheckError:pass
            else:count+=1
        checker.require(count==1,path+': oneOf 匹配数须为1，实为'+str(count))
    if 'if' in schema:
    ...
    if isinstance(value,dict):
        ...
        checker.require(len(value)<=schema.get('maxItems',len(value)),path+': 数组过长')
...
def main():
    ...
    (BASE.parents[1]/'规格/内核输入修订验证-r4/中途种子.json').write_text(json.dumps(captured,ensure_ascii=False,indent=2)+'\n')
    (BASE.parents[1]/'规格/内核输入修订验证-r4/周期排序.json').write_text(json.dumps(periodic,ensure_ascii=False,indent=2)+'\n')
    ...
    target=BASE.parents[1]/'规格/内核输入修订验证-r4/运行回归结果.json';target.write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')

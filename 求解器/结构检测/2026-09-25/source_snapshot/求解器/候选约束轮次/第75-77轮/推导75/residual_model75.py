...
def main():
    ...
    p.add_argument('--shift',type=int,nargs=2,default=[0,1]);p.add_argument('--seconds',type=float,default=600);p.add_argument('--workers',type=int,default=2);p.add_argument('--name',required=True);p.add_argument('--subsolver');p.add_argument('--fixed')
    ...
    if a.fixed:
        keys={tuple(b[k] for k in ('kind','x','y','w','h','axis')) for b in json.loads(Path(a.fixed).read_text())['chosen']}
        for b,v in zip(bs,vs):m.add(v==int(tuple(b[k] for k in ('kind','x','y','w','h','axis')) in keys))
    out=dict(encoding=a.encoding,scope='M0 plus residual all-pole union; no forced warehouse cells, no residual single-pole capacities',cap=a.cap,shift=a.shift,workers=a.workers,time_limit=a.seconds,fixed=a.fixed,

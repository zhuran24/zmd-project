...
def main():
    ...
    for W,H in [(21,53),(53,21)]:
        for a in range(4,71-W):
            if a==4 and H>21: continue
            for b in range(4,71-H):
                if b==4 and W>21: continue
                r=(a,b,W,H); pats=legal_patterns(r)
                if not pats:
                    results.append(dict(rect=r,patterns=[],branches=[],reason='矿石走廊与共同边带组合'))
                    continue
                tables=[line_table(r,s,False,False) for s in 'WESN' if {'W':a>0,'E':a+W<70,'S':b>0,'N':b+H<70}[s]]
                ...
                for P in (10,11,12):
                    ...
                    X=combine(outer0,P,budget)
                    if budget>=15 and outer1:
                        X=min(X,combine(outer1,P-1,budget-15))
                    Y=combine(tables,P,budget)
                    allowed=187-16*P+2*J
                    branches.append(dict(P=P,X_lower=X,Y_lower=Y,allowed=allowed,keep=X+Y<=allowed))
                results.append(dict(rect=r,patterns=pats,branches=branches))
                if len(results)%30==0:
                    print(json.dumps(dict(done=len(results),elapsed=time.monotonic()-t0,caches=solve_line.cache_info()._asdict()),ensure_ascii=False),flush=True)
    candidates=[dict(id=f"W{z['rect'][2]}H{z['rect'][3]}_x{z['rect'][0]}y{z['rect'][1]}",**z,allowed_P=[b['P'] for b in z['branches'] if b['keep']]) for z in results if any(b['keep'] for b in z['branches'])]
    out=dict(raw_positions=len(results),positions_total=len(candidates),elapsed=time.monotonic()-t0,positions=results,candidates=candidates)
    (ROOT/'positions.json').write_text(json.dumps(out,ensure_ascii=False,indent=2))
    print(json.dumps({k:v for k,v in out.items() if k not in ('positions','candidates')},ensure_ascii=False),flush=True)

...
- **`src/cuts/` (active F1-F7+F9 lifecycle; F8 retired):** `CutScope` binds source digest / ghost+exterior hash / artifact hashes / oracle versions / assumptions. `step_8_apply_to_master()` now translates F1/F5/F6/F7; `benders_loop._maybe_attach_framework_cuts()` provides the direct bridge under `EXACT_CUT_FRAMEWORK_ATTACH`, which is still unsafe/default-off and forbidden in certified runs. F2/F3/F4/F9 remain fail-closed at translation/production eligibility, and Stage B B4-B5 plus PIC C/D/E remain before owner promotion (B0-B3 landed). Exactness rule is **FP=0**: a cut must never delete a legal solution; false-negatives are acceptable, false-positives fatal — prefer HOLD/QUARANTINE/UNKNOWN over an incompletely verified cut.
...
6. **Cut integration is partial, not promoted.** F1/F5/F6/F7 have Step-8 translations and a direct env-gated bridge; F2/F3/F4/F9 remain fail-closed, F8 is retired, and certified promotion is blocked on Stage B B4-B5, PIC C/D/E and B6 owner action (B0-B3 landed). Seeing the bridge does not imply certified pruning is enabled.

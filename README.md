...
- **PR2 剩余项(真实 backlog;2026-07-06 owner 拍板:整条 backlog 编码项=P1.2 收口前提、按四阶段排期推进——round-20 画 TCB 线只停外审循环,不取消这些待办):** #1(最小 TCB 闭包,含 #5-F import-time 专门线)、#2/#3(loader / read-once 精化)、#5 独立枚举(候选域独立重推导)、#8(argv0/contract digest)、#9b/#9c(OS 写隔离 / 原生 TOCTOU)。#7(certify 生产入口 = go-live 最后通电)已于 2026-07-04 由 `scripts/run_supervisor_seal.py` 落地,但只补"supervisor 可执行入口"这一条机器条件。
...
8. **PR2 / L0 / L1 / loader / read-once residual-risk remains** — the close-kernel rounds have since converged (merged at `6e06922`; owner drew the TCB line and stopped the review loop, 2026-07-03), while #1/#2/#3/#5-independent-enumeration/#8/#9 deepening stay real backlog (since 2026-07-06 scheduled as the P1.2 closeout prerequisite by owner ruling); do not treat the structural checker as a substitute for human review.
...
6. **PR2 仍开放（真实 backlog；2026-07-06 owner 拍板=P1.2 收口前提、按四阶段排期）：** #2/#3（loader min-snapshot+fd / read-once）→ #1（最小 TCB 闭包，含 #5-F）→ #5 独立枚举 → #8 深化 → #9b/#9c（OS write 隔离 / native `.pyd/.so` TOCTOU）。#5 close-kernel 结构门已不在此列（round-19/20 后 `6e06922` 合入 main,外审画线停止）。#7（production certify 入口）已于 2026-07-04 由 `scripts/run_supervisor_seal.py` 落地，但不关闭这些剩余项。另有押后的 **resume-envelope finding**（`pr2-resume-envelope-deferred-finding`，commit `05a2a85`）：非证明字段如 `created_at` 克隆进 durable state 能过终态门但 resume 时自拒——判真但越界、押后到 #2/#3。

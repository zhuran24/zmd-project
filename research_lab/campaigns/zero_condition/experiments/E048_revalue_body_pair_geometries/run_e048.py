#!/usr/bin/env python3
"""E048: revalue three E047 body-pair geometries with the faithful joint middle."""
...
OUT = ROOT / "research_lab/local/zero_condition/E048_revalue_body_pair_geometries/run-001"
...
SEED_ASSIGNMENTS = [
    E047_RESULT.with_name(f"SEED_{index:02d}_ASSIGNMENT.json")
    for index in range(1, 4)
]
SEED_ENDPOINTS = [
    E047_RESULT.with_name(f"SEED_{index:02d}_ENDPOINT.json")
    for index in range(1, 4)
]
...
EXPECTED_ENV = {
    ...
    "EXACT_MASTER_RANDOM_SEED": "278000",
...
EXPECTED_HASHES: dict[Path, str] = {
    ...
    SEED_ASSIGNMENTS[0]: "9d9a79a5f9d5113a6b56b61a9da9b37f2cf50aa0f70175725ee96e8784db6eb1",
    SEED_ASSIGNMENTS[1]: "0ce0d29624ba1f34bfec37b920c4dfe98cd49df29e36fa07d35ae3e1d2b815a8",
    SEED_ASSIGNMENTS[2]: "6451e8fd40c93ea0644ebbb07f2bad3cc350ba8894987e0d24c852c553017dbc",
    SEED_ENDPOINTS[0]: "260065baf3790111f9ffcad8563f2071ec62c2b6cd3e311fb67b01127500d236",
    SEED_ENDPOINTS[1]: "92ab91774683fd127abc111e4fbf574b989bff0938872193a1b0c4ed12029099",
    SEED_ENDPOINTS[2]: "4659077182863125a50c1b77d8ceaefc89628b88a0bc48de6bea7ff389595fa7",
...
EXPECTED_FIXED_OBJECTIVES = [146, 151, 153]
...
def verify_identity() -> dict[str, Any]:
    if git_output("branch", "--show-current") != "research/main":
        raise RuntimeError("E048 must run on research/main")
    mismatches = {
    ...
    result_46 = load_json(E046_RESULT)
    result_47 = load_json(E047_RESULT)
    if result_46.get("verdict") != "OBJECTIVE145_SINGLETON_GEOMETRY_SATURATION_SIGNAL":
        raise RuntimeError("E046 trigger verdict drift")
    if result_47.get("verdict") != "BODY_PAIR_GEOMETRY_SEEDS_PROPOSED":
        raise RuntimeError("E047 trigger verdict drift")
    seeds = result_47.get("materialized_seeds", [])
    if len(seeds) != 3:
        raise RuntimeError("E047 materialized seed count drift")
    if [int(seed["objective"]) for seed in seeds] != EXPECTED_FIXED_OBJECTIVES:
        raise RuntimeError("E047 fixed objective drift")
    return {
...
def reconstruct_objective144_context(
    *,
    e041: Any,
    e043: Any,
    e045: Any,
    ...
    context_145 = e046.reconstruct_current_context(
        e041=e041,
        e043=e043,
        e045=e045,
        inputs=inputs,
    )
    solution_145 = e041.solution_from_assignment(e046.E045_ASSIGNMENT)
    ordinary_geometry_144 = e041.solution_from_assignment(E046_ORDINARY_ASSIGNMENT)
    original_prepare = e043.prepare_blocks_for_seed
    blocks, selected_ids, mode_summary, move = original_prepare(
        base_solution=solution_145,
        seed_solution=ordinary_geometry_144,
        result_41=context_145,
        pools=inputs["pools"],
        e041=e041,
    )
    checkpoint = load_json(E046_SEED_A_CHECKPOINT)
    if json_safe(mode_summary) != json_safe(checkpoint["mode_summary"]):
        raise RuntimeError("E048 reconstructed E046 mode context drift")
    if json_safe(move) != json_safe(checkpoint["move"]):
        raise RuntimeError("E048 reconstructed E046 move context drift")
    return {
        "final_blocks": blocks,
        ...
        "source": "reconstructed E046 objective-144 geometry-conditioned context",
    }


def prepare_blocks_for_pair(
    *,
    ...
    if len(moved) != 2:
        raise RuntimeError(f"E048 pair move count drift: {moved}")
    moved_set = set(moved)
    moved_locations: dict[str, tuple[str, int]] = {}
    ...
    for block in blocks:
        ...
        for destination, state in enumerate(base_states):
            ...
            if instance_id in moved_set:
                moved_locations[instance_id] = (block_id, destination)
    ...
    for instance_id in moved:
        if instance_id in moved_locations:
            continue
        row = seed_solution[instance_id]
        block_id = f"geometry_singleton::{instance_id}"
        if block_id in selected_ids_by_block:
            raise RuntimeError(f"E048 duplicate singleton block: {block_id}")
        ...
        singleton = {
            ...
            "owner_refresh": "geometry_singleton_pair",
        }
        blocks.append(singleton)
        selected_ids_by_block[block_id] = {instance_id}
        moved_locations[instance_id] = (block_id, 0)

    if set(moved_locations) != moved_set:
        raise RuntimeError("E048 moved-location coverage drift")
    enabled = set(base_enabled)
    enabled.update(moved_locations.values())
    ...
    for block in blocks:
        ...
        for destination, payload in enumerate(block["selected_literal_payloads"]):
            ...
            if current_pose_idx not in modes:
                raise RuntimeError(
                    f"E048 current pose missing from modes: {block_id}/{destination}"
                )
    ...
    if len(set().union(*all_sets)) != sum(len(values) for values in all_sets):
        raise RuntimeError("E048 selected instance overlap")
    ...
    return (
        ...
        mode_summary,
        {
            ...
            "moved_locations": {
                instance_id: {
                    "block_id": moved_locations[instance_id][0],
                    "destination": moved_locations[instance_id][1],
                }
                for instance_id in moved
            },
...
def run() -> dict[str, Any]:
    ...
    e046 = import_module("zmd_e048_e046", E046_RUNNER)
    e045 = import_module("zmd_e048_e045", E045_RUNNER)
    e043 = import_module("zmd_e048_e043", E043_RUNNER)
    e041 = import_module("zmd_e048_e041", E041_RUNNER)
    e001 = import_module("zmd_e048_e001", E001_RUNNER)
    e002 = import_module("zmd_e048_e002", E002_RUNNER)
    e004 = import_module("zmd_e048_e004", E004_RUNNER)
    e014 = import_module("zmd_e048_e014", E014_RUNNER)
    e015 = import_module("zmd_e048_e015", E015_RUNNER)
    e027 = import_module("zmd_e048_e027", E027_RUNNER)
    e031 = import_module("zmd_e048_e031", E031_RUNNER)
    conditional_mode_module = import_module(
        "zmd_e048_conditional_mode",
        E041_HELPER,
    )
    ...
    parent_endpoint = load_json(PARENT_ENDPOINT)
    if parent_endpoint.get("status") != "OPTIMAL" or int(parent_endpoint["objective"]) != PARENT_OBJECTIVE:
        raise RuntimeError("E048 parent endpoint drift")
    context_144 = reconstruct_objective144_context(
        e041=e041,
        e043=e043,
        e045=e045,
        e046=e046,
    ...
    if not isinstance(mandatory, list) or not isinstance(generic, Mapping):
        raise RuntimeError("E048 frozen instance/generic payload drift")
    ...
    for index, (assignment_path, endpoint_path, expected) in enumerate(
        ...
        if endpoint.get("status") != "OPTIMAL" or int(endpoint["objective"]) != expected:
            raise RuntimeError(f"E048 seed {index} endpoint drift")
        moved = [
            instance_id
            for instance_id in sorted(solution)
            if e041.body_cells(
                pools=inputs["pools"],
                facility_type=str(solution[instance_id]["facility_type"]),
                pose_idx=int(solution[instance_id]["pose_idx"]),
            )
            != e041.body_cells(
                pools=inputs["pools"],
                facility_type=str(parent_solution[instance_id]["facility_type"]),
                pose_idx=int(parent_solution[instance_id]["pose_idx"]),
            )
        ]
        if len(moved) != 2:
            raise RuntimeError(f"E048 seed {index} move count drift: {moved}")
        seed_inputs.append((chr(64 + index), solution, endpoint, expected))

    original_prepare = e043.prepare_blocks_for_seed
    old_out = e043.OUT
    e043.prepare_blocks_for_seed = lambda **kwargs: prepare_blocks_for_pair(
        **kwargs,
        e043=e043,
    )
    e043.OUT = OUT
    ...
    if any(
        ...
        return {
            "schema": "zmd_zero_condition_e048_revalue_body_pair_geometries_v1",
            "created_at_utc": utc_now(),
            "authority": "research_only_noncertified",
            "verdict": "BODY_PAIR_GEOMETRY_CALIBRATION_REJECTED",
            "identity": identity,
            "seed_results": seed_results,
            "best_seed": None,
            "routing": {"status": "NOT_REACHED_CALIBRATION_REJECTED"},
            "decision": "REFINE_BODY_PAIR_CONTEXT",
    ...
    if not feasible:
        return {
            ...
            "created_at_utc": utc_now(),
            "authority": "research_only_noncertified",
            "verdict": "BODY_PAIR_GEOMETRY_JOINT_NONTERMINAL",
            "identity": identity,
            "seed_results": seed_results,
            "best_seed": None,
            "routing": {"status": "NOT_REACHED_NO_FEASIBLE_JOINT_STATE"},
            "decision": "CONTINUE_OR_REFORMULATE_BODY_PAIR_JOINT_SOLVES",
            "truth_boundary": "Three frozen E047 pair geometries only.",
            "ledger_effect": "none",
        }
    ...
    if best_objective == 0:
        verdict = "BODY_PAIR_GEOMETRY_COMPONENT_CANDIDATE"
        decision = "ENTER_EXACT_ROUTING"
        best_assignment = Path(best[0]["best_child"]["assignment_path"])
        best_solution = e041.solution_from_assignment(ROOT / best_assignment)
        ...
        verdict = "BODY_PAIR_GEOMETRY_JOINT_IMPROVEMENT"
        decision = "RECOMPUTE_RESIDUAL_FROM_SELECTED_PAIR_GEOMETRY"
    else:
        verdict = "BOUNDED_BODY_PAIR_LANGUAGE_SATURATED"
        decision = "BROADEN_OR_BUILD_NATIVE_SIMULTANEOUS_GEOMETRY_MODEL"
    ...
    return {
        ...
        "decision": decision,
        "truth_boundary": (
            "Three E047 simultaneous body-pair geometries under the reconstructed "
            "objective-144 context; each receives one exact fixed-state calibration "
            "and one free bounded port-mode/assignment/binding solve."
        ),
        "ledger_effect": "none",
    }
...
def main() -> int:
    if RESULT_PATH.exists():
        raise FileExistsError("refusing to overwrite E048 terminal result")
    try:
        result = run()
        ...
        failure = {
            "schema": "zmd_zero_condition_e048_revalue_body_pair_geometries_failure_v1",
            "created_at_utc": utc_now(),

#!/usr/bin/env python3
"""Freeze the E024 best branch-specific shared-optimum face."""
...
OUT = (
    ROOT
    / "research_lab/local/zero_condition/E024_branch_specific_leader/run-001/OPTIMUM_FACE_AUDIT.json"
)
...
E024_RUNNER = (
    ROOT
    / "research_lab/campaigns/zero_condition/experiments/E024_branch_specific_leader/run_e024.py"
)
EXPECTED_HASHES = {
    BEST_ASSIGNMENT: "4f49e6dc8aaaf8e677596cd631f0eb34fc735612a4ff5a3e09dbb50836633018",
    RESULT: "a0a69a8c0f9c7f59d8924f9f13e0e277fe5f254a35aeaeb34c6c721becd4d17f",
    ...
    E024_RUNNER: "5b6c04077a62c590c8c863290d01f807bdf7b012cca0a142c814d632374a0899",
}
...
def run() -> dict[str, Any]:
    ...
    if result.get("verdict") != "BRANCH_SPECIFIC_LEADER_IMPROVES":
        raise RuntimeError("E024 result verdict drift")
    if int(result["best_child"]["objective"]) != 168:
        raise RuntimeError("E024 best objective drift")

    e001 = import_module("zmd_e024face_e001", E001_RUNNER)
    e004 = import_module("zmd_e024face_e004", E004_RUNNER)
    e015 = import_module("zmd_e024face_e015", E015_RUNNER)
    audit = import_module("zmd_e024face_audit", FACE_AUDIT)
    ...
    return {
        "schema": "zmd_zero_condition_e024_optimum_face_audit_v1",
        ...
        "face": face,
        "verdict": "BRANCH_SPECIFIC_OPTIMUM_FACE_EXACT",
        "truth_boundary": (
            "Exact per-commodity ranges over the fixed E024 best branch-specific "
...
def main() -> int:
    if OUT.exists() or FAILURE.exists():
        raise FileExistsError("refusing to overwrite E024 optimum-face audit")
    try:
        result = run()
        ...
        failure = {
            "schema": "zmd_zero_condition_e024_optimum_face_audit_failure_v1",
            "created_at_utc": utc_now(),

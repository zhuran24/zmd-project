#!/usr/bin/env python3
"""Finish E055 by minimizing total mismatch on the exact positive-count face."""
...
EXPECTED_RUNNER_SHA = "5722d921d66f4bd4b7126c42a9be50d47ee4cb2c7c0873f2a3f161d402cabad6"
EXPECTED_POSITIVE_SHA = "e39b89f95fdd995eef582e672c97c2ca70ab2a742877bfd3fa0448dae3995b6e"
TOTAL_SECONDS = 150.0
...
def run() -> dict[str, Any]:
    if sha256_file(RUNNER) != EXPECTED_RUNNER_SHA:
        raise RuntimeError("E055 computational runner drift")
    if sha256_file(POSITIVE_PATH) != EXPECTED_POSITIVE_SHA:
        raise RuntimeError("E055 positive checkpoint drift")
    runner = import_module("zmd_e055_resume_runner", RUNNER)
    ...
    if positive.get("status") != "OPTIMAL":
        raise RuntimeError("E055 positive checkpoint is not terminal")
    optimum_positive = int(positive["positive_commodity_count"])
    if optimum_positive != 18:
        raise RuntimeError("E055 resume expects the exact one-zero face")
    ...
    if total["status"] == "OPTIMAL":
        ...
        if total_value < runner.EXPECTED_TOTAL:
            verdict = "ASSIGNMENT_FRONTIER_SECONDARY_TOTAL_IMPROVEMENT"
            decision = "RECOMPUTE_RESIDUAL_WITH_ONE_ZERO"
        else:
            verdict = "ASSIGNMENT_FRONTIER_LEX_SATURATED_AT_18_139"
            decision = "RELEASE_CAUSAL_CONTEXT_PORT_MODES"
    elif total["status"] == "FEASIBLE":
        verdict = "ASSIGNMENT_FRONTIER_TOTAL_FEASIBLE_NONTERMINAL"
        decision = "RETAIN_WITNESS_AND_CONTINUE_TOTAL_SOLVE"
    else:
        verdict = "ASSIGNMENT_FRONTIER_TOTAL_NONTERMINAL"
...
def main() -> int:
    if RESULT_PATH.exists() or FAILURE_PATH.exists():
        raise FileExistsError("refusing to overwrite E055 resume outputs")
    try:
        result = run()
        print(
            json.dumps(
        ...
        failure = {
            "schema": "zmd_zero_condition_e055_assignment_frontier_resume_failure_v1",
            "created_at_utc": utc_now(),

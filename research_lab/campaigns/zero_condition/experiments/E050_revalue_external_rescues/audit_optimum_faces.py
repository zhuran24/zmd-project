#!/usr/bin/env python3
"""Freeze the three E050 external-rescue joint optimum faces."""
...
ASSIGNMENTS = [OUT.with_name(f"SEED_{label}_BEST_ASSIGNMENT.json") for label in "ABC"]
...
EXPECTED_HASHES = {
    ASSIGNMENTS[0]: "192417a4a9bd0892225554da0d19fb7c42535fcb69c11c9b588b5759fc38899b",
    ASSIGNMENTS[1]: "ba27dbdb2196bf4e5de3c5fa710195c245fe8ab5b1092c007be98aac5058937e",
    ASSIGNMENTS[2]: "8964829329cc98d4ea58d691854d6d81a9723248a6467d9a159d010bbcdabe55",
    RESULT: "ece91ce0ebc74caa5a1e7791c64ebdf6a692926aa8f74e780384d6898a41944d",
...
def run() -> dict[str, Any]:
    ...
    e001 = import_module("zmd_e050face_e001", E001_RUNNER)
    e004 = import_module("zmd_e050face_e004", E004_RUNNER)
    e015 = import_module("zmd_e050face_e015", E015_RUNNER)
    audit = import_module("zmd_e050face_audit", FACE_AUDIT)
    stack = e001.import_stack()
    inputs = e001.load_model_inputs(stack)
    result = load_json(RESULT)
    faces: dict[str, Any] = {}
    for index, label in enumerate("ABC"):
        faces[label] = audit.profile_optimum_face(
            label=f"E050_EXTERNAL_RESCUE_{label}",
            solution=load_json(ASSIGNMENTS[index])["solution"],
            optimum=int(result["seed_results"][index]["best_child"]["objective"]),
            inputs=inputs,
            e004=e004,
            e015=e015,
        )
    return {
        "schema": "zmd_zero_condition_e050_optimum_face_audits_v1",
        ...
        "faces": faces,
        "verdict": "EXTERNAL_RESCUE_JOINT_OPTIMUM_FACES_EXACT",
        "truth_boundary": (
            "Exact per-commodity ranges over the three fixed E050 best-child "
            "shared-objective optimum faces only."
        ),
        "ledger_effect": "none",
    }
...
def main() -> int:
    if OUT.exists() or FAILURE.exists():
        raise FileExistsError("refusing to overwrite E050 optimum-face audits")
    try:
        result = run()
            ...
            json.dumps(
                {
                    ...
                    "faces": {
                        label: {
                            "pair_sum": face["blue_source_ore_sum"],
                            "varying": face["varying_commodities"],
                        }
                        for label, face in result["faces"].items()
                    },
                    "result_path": str(OUT),
        ...
        failure = {
            "schema": "zmd_zero_condition_e050_optimum_face_audits_failure_v1",
            "created_at_utc": utc_now(),

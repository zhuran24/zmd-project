#!/usr/bin/env python3
"""Freeze the two E046 geometry-conditioned joint optimum faces."""
...
EXPECTED_HASHES = {
    SEED_A_ASSIGNMENT: "cb67a16cc022bed9cd332aebf65962cda1fdf819ecac4b8d768f7ae6738198f4",
    SEED_B_ASSIGNMENT: "a37ca68b8b2c7730bd8cbd31cf081e771a807cc1007aa1590856a0759359fc83",
    RESULT: "cd9c45fd0c57af15306329758ad80ecf14962ba27c680cda91b0c4e8cebf59c0",
...
def run() -> dict[str, Any]:
    ...
    e001 = import_module("zmd_e046face_e001", E001_RUNNER)
    e004 = import_module("zmd_e046face_e004", E004_RUNNER)
    e015 = import_module("zmd_e046face_e015", E015_RUNNER)
    audit = import_module("zmd_e046face_audit", FACE_AUDIT)
    stack = e001.import_stack()
    inputs = e001.load_model_inputs(stack)
    result = load_json(RESULT)
    seed_a = audit.profile_optimum_face(
        label="E046_GEOMETRY_SEED_A",
        solution=load_json(SEED_A_ASSIGNMENT)["solution"],
        optimum=int(result["joint_seed_results"][0]["best_child"]["objective"]),
        inputs=inputs,
        e004=e004,
        e015=e015,
    )
    seed_b = audit.profile_optimum_face(
        label="E046_GEOMETRY_SEED_B",
        solution=load_json(SEED_B_ASSIGNMENT)["solution"],
        optimum=int(result["joint_seed_results"][1]["best_child"]["objective"]),
        inputs=inputs,
        e004=e004,
        e015=e015,
    )
    return {
        "schema": "zmd_zero_condition_e046_optimum_face_audits_v1",
        ...
        "seed_a_face": seed_a,
        "seed_b_face": seed_b,
        "verdict": "OBJECTIVE145_GEOMETRY_OPTIMUM_FACES_EXACT",
        "truth_boundary": (
            "Exact per-commodity ranges over the two fixed E046 best-child shared-"
...
def main() -> int:
    if OUT.exists() or FAILURE.exists():
        raise FileExistsError("refusing to overwrite E046 optimum-face audits")
    try:
        result = run()
        ...
        failure = {
            "schema": "zmd_zero_condition_e046_optimum_face_audits_failure_v1",
            "created_at_utc": utc_now(),

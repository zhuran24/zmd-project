#!/usr/bin/env python3
"""Freeze the E021 best fifth-step shared-optimum face."""
...
OUT = (
    ROOT
    / "research_lab/local/zero_condition/E021_fifth_step_class_probe/run-001/OPTIMUM_FACE_AUDIT.json"
)
FAILURE = OUT.with_name("OPTIMUM_FACE_AUDIT_FAILURE.json")
BEST_ASSIGNMENT = (
    ROOT
    / "research_lab/local/zero_condition/E021_fifth_step_class_probe/run-001/BEST_FIFTH_ASSIGNMENT.json"
)
RESULT = (
    ROOT
    / "research_lab/local/zero_condition/E021_fifth_step_class_probe/run-001/RESULT.json"
)
E001_RUNNER = (
    ROOT
...
EXPECTED_HASHES = {
    BEST_ASSIGNMENT: "76b622bf8968275e68a3420cf6afb9a57bab1326e708c273a60b98e2a06037d9",
    RESULT: "bf582d3a86b8308a8816b447b4b484f7d64044988003236a41fa384457e2bdb9",
...
def run() -> dict[str, Any]:
    ...
    if result.get("verdict") != "FIFTH_STEP_CLASS_INVARIANT_IMPROVEMENT":
        raise RuntimeError("E021 result verdict drift")
    if int(result["branch_response"]["best_child_objective"]) != 173:
        raise RuntimeError("E021 best objective drift")

    e001 = import_module("zmd_e021face_e001", E001_RUNNER)
    e004 = import_module("zmd_e021face_e004", E004_RUNNER)
    e015 = import_module("zmd_e021face_e015", E015_RUNNER)
    audit = import_module("zmd_e021face_audit", FACE_AUDIT)
    ...
    return {
        "schema": "zmd_zero_condition_e021_optimum_face_audit_v1",
        ...
        "face": face,
        "verdict": "FIFTH_OPTIMUM_FACE_EXACT",
        "truth_boundary": (
            "Exact per-commodity ranges over the fixed E021 best fifth-step "
...
def main() -> int:
    if OUT.exists() or FAILURE.exists():
        raise FileExistsError("refusing to overwrite E021 optimum-face audit")
    try:
        result = run()
        ...
        failure = {
            "schema": "zmd_zero_condition_e021_optimum_face_audit_failure_v1",
            "created_at_utc": utc_now(),

...
<!-- DOC-SUBJECT:current_project_state FIELD:claude_phase_contract START sha256:2b95b7a0603189d9ceb80d2476942629b8c5dc3cc4ceff49b4c2bb5338b08b99 -->
...
```powershell
# 前置 (Edge 每次重启后跑一次): 默认连用户日常 Edge 主实例 (已登录零配置);
# ⚠️ Edge 在跑但没带调试端口时, 脚本会温和重启 Edge — 执行前注意用户是否正用着
...
- **包递交通道 (2026-06-12 owner 裁决)**: 项目快照包**上传到「终末地」Project 文件页 (来源区), 不随消息发附件** (Codex 时代文件页模式从未触发风控; 会话内反复传大附件疑似风控诱因)。新包上传后**删老快照包**; 依赖包 `zmd_py313_linux_x86_64.zip` (wheels) **永远不删**。prompt 指认文件区包文件名 + sha256 让 GPT 开工前校验。**上传已脚本化 (2026-06-12 跑通, 18MB 大文件实测)**: `upload_project_file.py --file <包> --replace` (另有 --list / --delete-name 运维模式; ⚠️ --replace 白名单语义会删期间新出现的文件, owner 正在手动操作文件区时别跑, 详见 gpt_dispatch/README.md)。**dispatch 已并入文件区通道 (2026-06-12)**: `--package-channel sources` (默认) = 子进程调 upload_project_file 传文件区 (默认白名单清旧快照, `--keep-old-snapshots` 关闭清理) + 消息只发纯文字 prompt (prompt 须自带文件区包名+sha 指认, 脚本对漏指认仅 WARN); `--package-channel attachment` = 旧附件模式备选。sources 下 `--pack` 自动把产出复制成 sha 唯一名 (`zmd_snapshot_<sha8>.zip`) 防并发覆盖。
...
- **本 checkout 无 `.venv`** — 主环境 = `C:\Program Files\Python313\` 的 python.org 3.13.14 (`python`, 2026-06-12 起, 无自动更新), 依赖已从商店版 `pip freeze` + `--no-deps` 全量克隆 (249 包, ortools 9.15.6755; 已知良性不一致: litellm 钉 jsonschema==4.23.0 但实装 4.25.1, 与源环境一致, pip 解析器会因此拒装 → 克隆/补装用 `--no-deps`)。商店版 Store Python (`python3.13`) 保留作备份, 此后不再保证与主环境同步; 装新依赖默认进主环境。CLAUDE.md/记忆里的 `.venv\Scripts\python.exe` / `.venv/bin/python` 写法是旧环境遗留, 在这里会假成功或报错。
...
- **子代理模型按任务重量派 (2026-06-11 用户裁决, 取代旧"默认 opus")**: 派遣 sub-agent (Agent 工具 / Workflow 内 `agent()`) 按任务**具体难度/重量**选模型, 不按任务类别套公式 —— 轻活 (小资料搜索、轻量代码查询、轻量/小功能编写这一档) 用 **sonnet**; 重活 (调研、大范围多角度代码查询、重度/大模块编写这一档) 用 **opus**; 特别重要/做错代价大的任务用 **fable**。例子是示意刻度不是类别白名单: 同是"搜索", 轻的 sonnet 重的 opus; 拿不准往上取一档。主会话当前是 fable, 轻/重活需**显式传 `model`** (继承不再默认正确)。**Agent 工具无独立 effort/thinking-budget 旋钮** —— 控制力度唯一硬杠杆是 `model` 参数, 软杠杆是 prompt 措辞; harness 底层 reasoning-effort 没暴露在 Agent 参数里。细则见记忆 `[[subagent-model-by-weight]]`。

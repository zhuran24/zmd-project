...
- **非必要不用 Workflow, 审查任务外发 GPT (2026-06-10 用户裁决)**: 默认自己干或单个 Agent 子代理; 审查/外审类任务用 Claude in Chrome 浏览器插件把任务发到 chatgpt.com 让 GPT 做 (沿用项目外审规范: 材料完整 / 不准 priming / 新窗口零历史), **不开本地多代理审查 workflow**。发送设置: **模型选 GPT Pro 并开扩展模式; 请求发在 ChatGPT 的「终末地」Project 里面** (不是裸新会话)。"必要" = 用户明确点名, 或离不开本地多路编排且无法外发。(实测教训: 审查 workflow 38min + API 超时挂 critic + 审查 agent 并发跑 pytest 互删 `.pytest_tmp` 污染全量。)

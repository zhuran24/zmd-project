...
| `data/examples/industrial_planner/current_delivery/*` | POSTPROCESS_ONLY | stable current landing/download bundle | fixed consumer entry built from the checked-in current-viewer pointer, with one landing page plus a copied viewer bundle under `current_delivery/viewer/` and a one-file ZIP alias plus pointer sidecars under `current_delivery/downloads/` so downstream users do not need to guess the active release id |
...
| `data/examples/industrial_planner/industrial_planner_latest_single_base_delivery_bundle.zip` | POSTPROCESS_ONLY | repo-front latest bundle ZIP alias | shorter checked-in alias that mirrors the stable current-delivery bundle ZIP for the active `valley4_protocol_core` 70×70 delivery line without widening scope |
...
| `third_party_snapshots/*` | POSTPROCESS_ONLY | vendored fixture/snapshot inputs | build-time adapter fixtures only; now includes a vendored endfield-calc raw repository fixture (`0.5.2`, commit unavailable from uploaded archive) |

...
| Path | Status | 说明 |
|---|---|---|
| `PROJECT_LOCK.md` | CURRENT_CODE_ALIGNED | 认证边界权威；2026-07-30 窄增 AB16 selected-loader/pathname transport、fixed no-overwrite retirement，以及完整 absolute-directory-chain/retired-inode watch 与 kernel `EAGAIN` 线性化合同，并登记 path-preregistration v4、formal context v3/admission v2 successors；绝对 artifact identity 与 research-only authority 保持不变，W0 D6 v2/v3、F8 retirement、partial attach 与 Stage B boundary 不变 |

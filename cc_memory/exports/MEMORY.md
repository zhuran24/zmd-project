...
- `codex-claude` — 2026-06-23 owner 指定进入「省额度模式」:**尽量把全部工作交给 Codex**——不只实现,连原本要交给 Claude/Opus 子代理的【审查、读代码、调研】也都交 Codex…
...
- `pr1-supervisor-mint-preflight` — PR1 supervisor 地基两块 2026-06-23 落地(未提交,在工作树): == 第一块 mint 收敛 == exact_campaign.supervisor_seal 成唯一 durable CERTIFIED mint(模块私有 token…

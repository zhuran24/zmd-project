...
- `codex-desktop-call-count-is-start-plus-wait` — codex_desktop 桥(`codex_desktop_mcp.py` 的 app-server 客户端)跑**一个** codex 任务 = **两次 MCP 桥调用**…
...
- `owner-meeting-record-includes-process-20260630` — owner 偏好(2026-06-30):完整会议记录默认含讨论过程+分歧+收敛路径+最终结论,不只终稿摘要。本会话多次开 Agents Team(2 claude+2 codex),owner 要会议文档既记终稿也记 R1/R2 全过程(谁主张啥/谁让步/在哪收敛)…
- `owner-no-vnext-status-card-20260630` — owner 裁决(2026-06-30):暂不做 vnext 项目现状卡,项目推进现状继续由 harness resume 锚+cc_memory 承载…
...
- `precompact-flow-current-20260630` — Pre-compact(2026-06-30):三阶段 记忆更新->查漏->/compact;级联只由调本skill驱动;查漏已脚本自动化=precompact_judge_run.py(app-server,不碰CLI,由-RunAfterTurn worker自动起),我只triage;先审后用…
...
- `precompact-judge-singleflight-guard-needed-20260630` — precompact_judge_run.py(查漏脚本,查漏回合后我手起 --launch-detached)没有 single-flight 并发保护…

...
- entries: 39
- hard edges: 16
- pending relation suggestions: 30

## Start Here

- `cc-memory-meta-index` — cc_memory 自身的操作手册=元记忆常驻层(pinned,每会话boot必读),优先级高于项目内容记忆。分组索引读写纪律/关系边/检索脾气(rerank分场景、三条调参死路)/git-db危险/hooks,各条指向详情节点;改系统行为先读它、新增系统级元知识就更新它…
...
- `cc-memory-hook-backstop-landed-20260620` — 2026-06-20 落地 cc-memory-hook-4-a-i 的三层方案(原标未实现)。finalize 收口/WAL/lease/退出码/正则/两窟窿修复 + 11 项测试…
...
- `setter-barrier-p1-3b-getframe` — owner 2026-06-19 决策 setter barrier 整体留 P1.3B 窗口(详见 task + [[p1-2-c2-ast-allowlist]])…

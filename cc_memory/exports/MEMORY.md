...
- `codex-agenttype-schema-structuredoutput-empty-loop` — 症状: workflow 里用 agent({agentType:'codex', schema}) 派 codex 子代理 + 严格 schema, 会让它疯狂空转…
...
- `codex-read-only-mcp-sandbox-approval-policy` — 2026-06-19 根因:codex 子代理在 read-only sandbox 调写命令(mem.py boot/codegraph)挂死。修复:stdio MCP 代理 codex_mcp_sandbox_strip.py 从 tools/list schema 剥掉 sandbox/approval…
...
- `p1-2-resume-2026-06-21-origin-main-67139a5-p-landed-b-stale-gate-c0-c3-c4-c5-codex-claude` — **RESUME SNAPSHOT — 读这条即可跟上 P1.2 现状(2026-06-21)。** 它 supersede 旧条目里已过时的「下一步」行…

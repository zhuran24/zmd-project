...
- `close-kernel-ast-checker-design-lessons` — 写 AST/source-sha checker(如 close-kernel 第二道门)约束「不可信维护者能 reseal 后把代码改成啥」时,几条反复踩出来的可复用设计教训(2026-07-01 PR2 #5 round-10/11/12 逐轮外审坐实…
...
- `pr2-5-round10-11-12-fspinout` — PR2 #5 close-kernel「第二道门」round-10/11/12 + F 单列(2026-07-01;分支 pr2-5-domain-frontier-gate…
...
- `worktree-artifact-and-codex-report-lost` — 两个本会话实撞的操作坑(2026-07-01,承 [[pr2-5-round10-11-12-fspinout]]): **① `git worktree add` 不拷 git-ignored 文件 → 外部化工件在新 worktree 缺失**…

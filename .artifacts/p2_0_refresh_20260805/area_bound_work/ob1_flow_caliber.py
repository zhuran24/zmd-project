...
理由（写进 receipt）：
  ...
  - (c) 是钉死目标下唯一的聚合平衡解：17 未知数 × 17 平衡方程的 Fraction 高斯
    消元非奇异 ⇒ 各操作的聚合活动（机器当量）与各商品的聚合吞吐唯一。注意
    配方图含两个种子 SCC（buckwheat/sandleaf 循环）、不是 DAG——唯一性说的是
    聚合量，不是逐条路径流。扣无线终品使上界论证不依赖任何终品交付语义
    （协议箱无线段 / hub 口贴放）的争议——保守方向，代价见 receipt。

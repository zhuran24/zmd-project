...
> **(2026-06-04 currency)** 前两项已不再 deferred —— IndustrialPlanner 单向 exporter 与 compatibility sidecar 均已交付 (见 spec 16 export contract / spec 22 precision export + validator / spec 23 outer-base planning; 对应 `src/adapters/industrial_planner/export_blueprint.py` / `export_industrial_planner_bundle.py` / `blueprint_validator.py` 等已落地)。下列保留作历史 deferment 记录。

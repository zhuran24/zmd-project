...
（实现注记，F-BIND-R1-01：上述"满额、不容空置"是本基地 R=S=52 完美匹配下由真实商品**精确计数约束**推出的数值必然结果，不是 binding domain 的结构性假设。binding 的 generic output 槽 domain 仍包含 `__unused__` 哨兵选项——在 52=52 配置下精确计数自动逼满、哨兵恒为 0，行为与满占等价；若未来需求小于槽数，模型不会将合法空置误判为 INFEASIBLE。规则层面端口空置合法性见 specs/03 §3.4.1/§3.5.1 等。）
...
> **[DEPRECATED 指针]** 下游章节（08 §8.3.1 / 09 §9.4.1）所需的"机器度数 / 端口拓扑"**当前权威源** = `rules/canonical_rules.json` 的 `operation_type` + `port_topology`（见 §4.7）。本节变体度数字典仅作历史参考，**不得**引为活真源。

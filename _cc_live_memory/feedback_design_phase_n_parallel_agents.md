...
**Why**: main Claude 同种 RLHF bias 在设计当下容易"看上去都对", 实际 Gemini cross-check 出洞 (per gemini-review-algorithm-math(已归档) Day 15/16a/16b round 14 catch 3 致命 bug + 2 schema 漏之教训). audit 链 (Gemini per-commit + GPT pro 大节点 + ultrareview) 是**事后**催熟, parallel 子代理是**事前**预防. 异种 ensemble 已是项目既有 (Gemini + GPT pro 异源), 但 implementation 设计当下还是 main 一个人埋头写完才发出去 — 那个时间窗口 audit 链够不到, N 路并行子代理才覆盖.
...
- 实施前必经 Gemini cross-check (per gemini-review-algorithm-math(已归档))
...
**⚠️ team 是重武器 (见 [[effort-matches-stakes]])**: 编排成本高、消息往返多。2026-06-14 审记忆树那次 team 是 **owner 主动让开的**(不是 CC 自己冒的; 别记成"CC 用力过猛"=归因错误)。它适合"维度本身不确定 + 真需要相互挑战涌现"的任务。**owner 授权或高 stakes 时该用就用**; 只有没授权、自己给低 stakes 简单活无脑上 team 才是该防的。

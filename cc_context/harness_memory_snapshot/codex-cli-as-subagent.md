...
注意：codex-desktop 应用本体也叫 `codex.exe`（重度定制，`~/.codex` 下有 ultracode/skills/workflow/pets 等）；杀 exec 残留进程时按 commandline 含 `exec` 精确匹配，**绝不误杀桌面应用本体**（同 [[self-protection]] 的按 PID/特征单杀原则）。

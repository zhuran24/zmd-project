...
在每个「告一段落」时刻 (一轮验收完成、双发出去进入等待、长任务收尾) 跑 `-Context` 自查 `context_used_tokens`; **≥ ~400k** → 把该记的记全再发 /compact 压缩。**这 400k 是给 1M context 会话定的压缩点** —— 200k 上限物理上根本到不了 400k, 所以这数从来就是 1M 情况下的值; 曾误判过 558k 还说「空间充裕」、又瞎推「该放 750k」, 都被 owner 戳穿, **别再擅自放大**。

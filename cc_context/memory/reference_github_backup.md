...
- **living-status auto-stamp** (2026-06-02, `.git/hooks/pre-commit` 在 memory-sync **之前**调 `cc_context/tools/stamp_living_status.py`, fail-soft `|| true` 绝不阻断 commit): 每 commit 自动把单一 living 现状源 (handoff) 里 `<!-- AUTO-STATUS:BEGIN/END -->` 块的**可推导字段** (最新 review 包 version+sha 读自 `cc_context/review/LATEST_PACKAGE.json` / spike HEAD / CLAUDE.md Current Phase) 重生成 → 可推导现状**不可能 stale**; 并在 handoff 判断散文没提到最新包版本时**stderr 大声 warn** (judgment 散文 lag 的提示, 自动推不出只能靠它)。**这是治"现状记忆一直被忘记更新"根因的强制函数** —— 那个根因 = handoff 更新是项目里唯一没钩子/不挂产物完成定义的关键动作, 光记规则 (被动文本) 治不住 (记完 rule#7 又犯)。`LATEST_PACKAGE.json` 由 `build_v*_win.py` 在 build 时写 (single anchor)。stamp 只改 AUTO-STATUS 块, 绝不碰人写散文。**hook 机器本地不入库, 换机要重建** (内容: PYV 探 .venv → 调 stamp `|| true`, 在 SRC sync 前)。

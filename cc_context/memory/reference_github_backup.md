...
2026-06-01 起: 仓库 + CC 上下文实时备份到 GitHub `github.com/zhuran24/endfield-exact-solver`。动机: 老电脑坏过、得找人从硬盘提数据, 不能只靠本地。**2026-06-06 supersede**: 当前树不再允许明文 API key / token 作为“私库可接受风险”存在；Gemini 脚本只读 `GEMINI_API_KEY`, preflight 有 secret scan。**硬约束**: ① 仍不要随意翻 public; ② 正常开发不 rebase / 不重写历史, 除非用户明确决定做 secret-history purge; ③ 免费私库未必有 push-protection, 所以当前树 secret=0 必须由本地 gate 保证。

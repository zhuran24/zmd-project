...
- ~~**binding RAB-SEP `routing_context!=None` 路径**~~ ✅ 2026-06-14 (`routing_aware_binding_diff.py`: 独立重导 front-status, 查 filter 过度剪枝=false-INFEASIBLE + clear-deficit cut-cert pinned soundness; 600 实例 0 不一致, 341 cert 全 sound)。子缺口待补: routing-free 输出端口 front 被挡**不应**剪枝 (req_in 当前留空使 routing_free={}, 该 nuance 未 fuzz)。
- **cuts soundness 其余类**: 已覆盖 = routing-aware binding cut cert (上一条)。仍未覆盖 = **lazy connectivity cut (specs/09 §9.8) 的 W/X cutset 证书独立复验** (需 SUT 暴露 cut 的 X-集才能独立重验, 当前 telemetry 只给 size); 主问题 Benders nogood / F1-F9 cut family 的"不删真解"独立复验 (设计待定, 可能需 benders-loop 集成或 per-cut pinned 重判)。

...
项目快照包在本 Project **文件区 (来源/Sources)**: `zmd_snapshot_09da11dd.zip`, sha256 `09da11ddf5ba42a152da3eaae5ca9460e3fafae0eb4f08bb59057f0a7c8100d3`, 对应干净 git 树 HEAD `25910fd`。**只认这个文件名, 文件区其它快照包一律无视; 开工前先校验 sha256, 对不上停下来报告, 不要在错包上工作**。zip 内 `project/` 为仓库根 (ZIP_LZMA, `python -m zipfile -e <zip> .` 解包)。依赖 wheels 同在文件区 (`zmd_py313_linux_x86_64.zip`), 沙盒 Python 3.13, 离线安装。

...
OUT_DIR = Path(tempfile.gettempdir()) / "_pkg_build_v25"
ZIP_ROOT_NAME = "_phase1_2_pkg_v25"
PROJECT_DIR = OUT_DIR / "project"
OUT_ZIP = REPO / "cc_context" / "review" / "phase1_2_spike_review_v25.zip"
...
V22_TO_V24_SECTION = """## v22 → v25 状态变化 (两轮外审 B → 全修)
...
**纠正后 sizing 结论 (LSB + bytes/term-by-kind)**: (a) 全 9 族 compact lowering → 100K 便宜 (~1–4 MB, 任何
...
compact 全族安全。详 `project/docs/research/p1_2_spike_sizing_gate_20260601/` (v3) + verdict.md 修正段。

build: spike 分支 HEAD `2cf96d6` (overlay 经 git show 自动取); master sizing gate `a7eff5d`+`cce55a3` (v3 LSB+bytes-by-kind)。
...
def build_readme() -> str:
    ...
    text = _replace_once(text, "# 终末地工业规划器 — 项目快照 (v22)",
                         "# 终末地工业规划器 — 项目快照 (v25)", "title")
    text = _replace_once(text, "`66cf16e`; data-producing",
                         "`2cf96d6` (v25: 三轮外审 B 全修, 详下方 v22→v25 节); data-producing",
    ...
    new_extract = (
        ...
        "\n"
        "```bash\n"
        "unzip -q phase1_2_spike_review_v25.zip\n"
        "cd _phase1_2_pkg_v25\n"
        "cd project\n"
        "```\n"
    )
...
def build_tree() -> tuple[int, int]:
    ...
    print("Overlaying spike data (HEAD 2cf96d6)...")

# 终末地 IndustrialPlanner 精确求解器 — campaign 持久化 + resume 面 · 独立全面 soundness 审查 (零先验白板)
...
**本面 = campaign 持久化、断点续跑、checkpoint 权威** (`src/search/exact_campaign.py` 为核)。
...
- **只审本面** (`src/search/exact_campaign.py`)。怀疑跨面缺陷时, **交叉引述 `PROJECT_LOCK.md` 契约条款**而非在本轮重证其它面内部正确性。求解算法本身不审; 只审 campaign 状态持久化/恢复是否会污染或伪造 certified 结果 (stale checkpoint / 伪 best_certified / 非权威路径写入)。

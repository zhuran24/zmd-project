# 终末地 IndustrialPlanner 精确求解器 — 预处理 / canonical 投影面 · 独立全面 soundness 审查 (零先验白板)
...
**本面 = 预处理与 canonical 规则投影 (recipe/commodity/mandatory 实例派生)** (`src/interchange/preprocess_context.py (+ src/preprocess/)` 为核)。
...
- **只审本面** (`src/interchange/preprocess_context.py (+ src/preprocess/)`)。怀疑跨面缺陷时, **交叉引述 `PROJECT_LOCK.md` 契约条款**而非在本轮重证其它面内部正确性。下游求解面不审; 只审预处理派生出的 certified 输入 (canonical_rules / mandatory_exact_instances / generic_io 等) 是否忠实、缺键/畸形是否 fail-closed。

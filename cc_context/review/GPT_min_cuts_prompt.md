**终末地 IndustrialPlanner 精确求解器 — cut 家族 (F1–F9) 生命周期面 soundness 审查**
...
本面 = cut 家族(F1–F9)的生成、replay、apply 生命周期与 cut_manager(`src/cuts/` + `src/models/cut_manager.py` 为核)。 它有没有 soundness 缺陷,能让求解器在 canonical 数据 + 默认 env 下输出 **false-CERTIFIED**(把预算耗尽/异常/非三态 status 误读成不可行证明、铸出删合法解的 master nogood、或输出 CERTIFIED 而证明前提不成立)?把判读过程和结论写清楚——有问题给可复现触发条件,没问题说明从哪些角度排除。

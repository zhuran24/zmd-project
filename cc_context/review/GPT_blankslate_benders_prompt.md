# 终末地 IndustrialPlanner 精确求解器 — Benders/LBBD 主循环面 · 独立全面 soundness 审查 (零先验白板)
...
**本面 = Benders/LBBD 主循环 (master→binding→routing→flow 的外层决策与 cut 铸造)** (`src/search/benders_loop.py` 为核)。
...
- **只审本面** (`src/search/benders_loop.py`)。怀疑跨面缺陷时, **交叉引述 `PROJECT_LOCK.md` 契约条款**而非在本轮重证其它面内部正确性。本面只审主循环对各子问题返回值的消费 (status 解读 / build_stats 消费 / cut 铸造前提 / 预算终态); 子问题自身的几何/编码正确性属 F-RT-*/F-BIND-*/F-GM-*/F-CUT-* 跨面, 怀疑时交叉引述 PROJECT_LOCK 不重证。

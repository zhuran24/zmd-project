# 终末地 IndustrialPlanner 精确求解器 — 并行调度 / wave 面 · 独立全面 soundness 审查 (零先验白板)
...
**本面 = 多进程并行 wave 调度与结果汇聚** (`src/search/exact_parallel_scheduler.py` 为核)。
...
- **只审本面** (`src/search/exact_parallel_scheduler.py`)。怀疑跨面缺陷时, **交叉引述 `PROJECT_LOCK.md` 契约条款**而非在本轮重证其它面内部正确性。单 worker 求解正确性不审; 只审并行调度/汇聚是否会让畸形或非穷尽 wave 结果被当成 certified 进 frontier。

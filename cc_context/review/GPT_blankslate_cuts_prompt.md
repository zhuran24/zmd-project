# 终末地 IndustrialPlanner 精确求解器 — cut 家族 / cut_manager 面 · 独立全面 soundness 审查 (零先验白板)
...
**本面 = cut 家族 (F1–F9) 的生成、replay、apply 生命周期与 cut_manager** (`src/cuts/ (+ src/models/cut_manager.py)` 为核)。
...
- **只审本面** (`src/cuts/ (+ src/models/cut_manager.py)`)。怀疑跨面缺陷时, **交叉引述 `PROJECT_LOCK.md` 契约条款**而非在本轮重证其它面内部正确性。主循环对 cut 的消费属 benders 面; 只审 cut 自身的几何/proof 忠实性与跨 iteration·跨 candidate 生命周期。power-conditioned cut 通道是 env-gated, 不在本次 (A) 范围。

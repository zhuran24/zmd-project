...
- README v22→v24 changelog 段 (含 LSB 纠正数字 + 7 finding) + 标题 v24 + Build line spike HEAD 2cf96d6
  + Finding5#2 表行 YES→PARTIAL (与 verdict 一致)
- 排除 cc_context + scripts/gemini_cross_check* (同 v23)
spike overlay 用 `git show {SPIKE_BRANCH}:`, 分支 HEAD 现 = 2cf96d6, verdict/spike 代码/remap_audit 自动取修复版。
"""
...
OUT_DIR = Path(tempfile.gettempdir()) / "_pkg_build_v26"
ZIP_ROOT_NAME = "_phase1_2_pkg_v26"
PROJECT_DIR = OUT_DIR / "project"
OUT_ZIP = REPO / "cc_context" / "review" / "phase1_2_spike_review_v26.zip"
...
V22_TO_V24_SECTION = """## v22 → v26 状态变化 (四轮外审 B → 全修)
...
**v24 → v25 (v24 外审第三次 B → 7 patch)**: v24 送外审 (两份独立) 又判 B, finding 都是证据精度/工件/锁门
(非 soundness, 不改方向)。v25 全修: (1) proto bytes/term **按约束类型分** (实测 linear ~3–4 / BoolOr no-good
~10–11, 旧 4–6 对 BoolOr 低估 ~2.5x); (2) F9 sizing 补测从只跑前 2 条改**全 6 条** (scoped max **784** 非 524,
all-type UB **3341**); (3) scale_ramp remap 字段**精确化** (代码已 emit + `remap_audit.json` 承载, 历史 ramp
jsonl 未重跑不带, 文案对齐); (4) G10 `_read_a3_fixture_stats` **硬化** (坏 JSON/缺字段不再静默放行, 都计
schema_err); (5) A3 emitter `run_emit` pass 加 **family>=9** (源头一致); (6) verdict writer 加 Layer-2 **risk #6**
(补 dangling) + verdict 顶 **writer 边界 banner** (手写 addenda 不由 writer 生成); (7) sizing 结论按约束类型
条件化 + cap 按 **max/p99**。

**v25 → v26 (v25 外审第四次 B → 两份独立并集全修)**: v25 送外审 (两份独立都 substantive), 都判 **B/PATCH, 无 soundness 洞**, 核心 framing 都 sound。并集 4 条 sizing 证据 + 1 条 guard 硬化 (主代理逐条对真代码核实):
- **[A-F1, 最重] type-pool 数 ≠ 真 master concrete literal 数**: sizing_gate 之前按 facility **type** pose pool overlap (total 81,795); 但真 pose-bool master 按 `(facility_type, operation_type)` **group×pose** 建变量 (266 inst → 19 group; mfg_3x3=8/mfg_5x5=4/mfg_6x4=5) → concrete ≈4× (**325,747**); group 展开后 F9 784→**11,644**, F4 5429→**20,157**。→ **all-type UB 数 (F9 3341 / F4 5429 / ~16–18K) 是 type-pool cheap proxy, 不是真-master literal 上界**; 单 group F9 **784** + region LSB **~264** 仍是真实单 group/region 尺度。P1.3A expanded cap 输入 = `len(final_concrete_literals)` (group/template/optional 展开后), 不是 type-pool 数。sizing_gate v5 加 `exp_group_all` 列。
...
**纠正后 sizing 结论 (LSB + bytes/term-by-kind + concrete-literal)**: (a) 全 9 族 compact lowering → 100K 便宜 (~1–4 MB, 任何
约束类型); (b) expanded lowering 预算 = (region/window × pool) × (per-term 字节: linear ~4 / BoolOr ~11):
fixture F1/F9 scoped max 784 term/cut 走 linear ~0.3 GB / 走 BoolOr ~0.86 GB; routing/all-type UB (F4 5429 /
F9 3341) 走 BoolOr ~3.7–6 GB; 大 region/window 趋近全 pool (~16–18K term) 任何类型都数 GB。P1.3A lowering 硬
约束 = **按约束类型**设 per-cut term cap + cumulative proto budget (cap 按 max/p99 非 family-avg), 跨**所有**族;
compact 全族安全。**v26 关键收紧 (A-F1)**: expanded cap 的输入必须是真 translator group/template/optional 展开后的 **concrete literal vector 长度**, 不是 type-pool 数 (后者是 ~4× 偏小的 proxy); 别把 3341/5429/16–18K 当真-master 上界。详 `project/docs/research/p1_2_spike_sizing_gate_20260601/` (v5: concrete-literal + OR-Tools 实测) + verdict.md v25 外审修正段。

build: spike 分支 HEAD `dc3516a` (overlay 经 git show 自动取); master sizing gate v5 `0a7f37f` (concrete-literal + summary 锁门 + OR-Tools ExportToFile 实测) + F7/F8 唯一性守卫 (cuts 414→416)。
...
def build_readme() -> str:
    ...
    text = _replace_once(
        ...
        "| 2 | 真 cut body 分布 (replacing toy 1-3-5 literal) | A3 jsonl 50 cert × 9 family 真 oracle emit ✅; **但** B2 translator 把 body lower 成合成/remap 小约束 (remap_audit 36/50 unknown), 非真 registry-bound body sizing | **PARTIAL** — 见 v22→v25 节 + verdict.md: compact lowering 全族安全, expanded lowering 跨所有族需 term cap, 已移交 P1.3A |",
    ...
    new_extract = (
        ...
        "\n"
        "```bash\n"
        "unzip -q phase1_2_spike_review_v26.zip\n"
        "cd _phase1_2_pkg_v26\n"
        "cd project\n"
        "```\n"
    )
...
def build_tree() -> tuple[int, int]:
    ...
    print("Overlaying spike data (HEAD dc3516a)...")
...
def main() -> int:
    ...
    (REPO / "cc_context" / "review" / "LATEST_PACKAGE.json").write_text(
        _json.dumps({
            "version": ZIP_ROOT_NAME.rsplit("_", 1)[-1],  # _phase1_2_pkg_v26 -> v26
            "sha256": sha,

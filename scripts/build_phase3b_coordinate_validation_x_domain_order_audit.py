...
from src.search.phase3b_coordinate_validation_x_domain_order_audit import (
    DEFAULT_X_DOMAIN_ORDER_AUDIT_GROUP_ID,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a Phase 3B x-domain/order standalone audit diagnostic."
    )
    parser.add_argument("--project-root", type=Path, default=PROJECT_ROOT)
    parser.add_argument("--candidate", default="67x13")
    parser.add_argument("--anchor-index", type=int, default=119)
    parser.add_argument("--group-id", default=DEFAULT_X_DOMAIN_ORDER_AUDIT_GROUP_ID)
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(".artifacts/phase3b_coordinate_validation_x_domain_order_audit"),
    )
...
def main() -> int:
    ...
    if not args.no_write:
        ...
        prefix = args.output_prefix or "coordinate_validation_x_domain_order_audit"
        ...
        _atomic_write_text(
            md_path,
            render_phase3b_coordinate_validation_x_domain_order_audit_markdown(report),
        )
        _atomic_write_text(
            txt_path,
            render_phase3b_coordinate_validation_x_domain_order_audit_text(report),
        )
        print(f"coordinate_validation_x_domain_order_audit_json={_display_path(project_root, json_path)}")
        print(f"coordinate_validation_x_domain_order_audit_md={_display_path(project_root, md_path)}")
        print(f"coordinate_validation_x_domain_order_audit_txt={_display_path(project_root, txt_path)}")
    return 0
...
def _print_summary(report: Mapping[str, Any]) -> None:
    ...
    print("phase3b coordinate validation x-domain/order audit")
    print(f"- outcome: {status.get('outcome')}")
    print(f"- subset status: {subset.get('status')}")
    print(f"- core labels: {_mapping(report.get('core_input')).get('label_count')}")
    print(f"- audit entries: {audit.get('entry_count')}")
    print(f"- all tuples allowed: {audit.get('all_pose_tuples_allowed')}")
    print(f"- all values match tuple: {audit.get('all_forced_values_match_pose_tuple')}")
    if variants:
        print(
            "- standalone statuses: "
            + ", ".join(
                f"{variant.get('variant')}={variant.get('status')}"

...
from src.search.phase3b_family_lookup_assumption_probe import (
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Probe forced family lookup literal assumptions in rebuilt component slices."
    )
    ...
    parser.add_argument(
        "--variants",
        default=",".join(DEFAULT_ASSUMPTION_VARIANTS),
        help="Comma-separated rebuilt family lookup variants to probe.",
    )
    parser.add_argument("--slot-limit", type=int, default=2)
    parser.add_argument("--family-limit-per-slot", type=int, default=2)
    parser.add_argument(
        "--solver-profile-json",
        default=None,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(".artifacts/phase3b_family_lookup_assumption_probe"),
    )
...
def main() -> int:
    ...
    if not args.no_write:
        ...
        json_path = output_dir / "family_lookup_assumption_probe.json"
        md_path = output_dir / "family_lookup_assumption_probe.md"
        txt_path = output_dir / "family_lookup_assumption_probe.txt"
        atomic_write_json(json_path, report)
        _atomic_write_text(
            md_path,
            render_phase3b_family_lookup_assumption_probe_markdown(report),
        )
        _atomic_write_text(
            txt_path,
            render_phase3b_family_lookup_assumption_probe_text(report),
        )
        print(
            f"family_lookup_assumption_probe_json={_display_path(project_root, json_path)}"
        )
        print(f"family_lookup_assumption_probe_md={_display_path(project_root, md_path)}")
        print(f"family_lookup_assumption_probe_txt={_display_path(project_root, txt_path)}")
    return 0
...
def _print_summary(report: Mapping[str, Any]) -> None:
    ...
    print("phase3b family lookup assumption probe")

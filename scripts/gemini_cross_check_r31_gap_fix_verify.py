#!/usr/bin/env python3
"""Round 31 Gemini AUDIT mode — verify Gap 6-10 fix landed (round 30 NOT GO → GO).

Same audit-mode prompt 模式 (round 30) — 真数据 inline + armor + 反 GO 章.
任务: 验 round 30 列的 5 Gap (6+7+8+9+10) 是否真修, 跟 真数据 + spec 一致.

Output: /home/zhuran24/linwin_share/gemini_round_31_gap_fix_verify_response.md
"""
...
DOC_PATHS = [
    ...
    # round 30 verdict (上次 finding 来源)
...
## Round 30 5 Gap fix 情况

我按 round 30 verdict 修:
...
逐个 Gap verify, **cite 修后的 file:line**:
- Gap 6: union region 数学是否跟 round 30 推荐一致? 5 ghost case (单边 ≥ 2 /
  union 1 cell / 跨边 1+1 / bottom 3 cells / left 3 cells) 是否都 sound 处理?
- Gap 7: state.groups 遍历是否真接 真数据 (operation_type 19 种), helper
  path 是否完整 cover?
- Gap 8: helper chain (operation_type → facility_type → template.dimensions /
  placement_rule / port_rule) 跟真 canonical_rules.json schema 严格一致吗?
  有没有漏 Field 还是 NULL safety hole?
- Gap 9: candidate_placements lookup 跟 viewer 真数据 schema 一致吗?
  direction encoding N/S/E/W 假设是否对? (我假设 N=(-1,0), S=(+1,0), E=(0,+1),
  W=(0,-1) — grid coord (x=row, y=col); 这是 spec implicit, 没显式 doc)
- Gap 10: PoseId=str 全 cascade 对吗 (CutLiteral / GroupState / Counter key /
...
def main() -> int:
    ...
    out_path = SHARE / "gemini_round_31_gap_fix_verify_response.md"

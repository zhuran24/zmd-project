#!/usr/bin/env python3
"""Build phase1_2_spike_review_v16.zip — Phase 1.2 spike close gate snapshot (post-三审 fix).
...
v16 does NOT include spike-only implementation code (per MERGER §5.1 rollback-safety
...
OUT_DIR = Path("/tmp/_phase1_2_pkg_v16")
PROJECT_DIR = OUT_DIR / "project"
SEVENZ_PATH = OUT_DIR / "project.7z"
OUT_ZIP = Path("/home/zhuran24/linwin_share/phase1_2_spike_review_v16.zip")
...
Build: master commit `f17f13e` (详 `COMMIT_LOG.md`) + spike overlay (6 file + 1
SPIKE_COMMIT_LOG, 详 `SPIKE_COMMIT_LOG.md`, spike branch HEAD `f54f4f8`).
...
```bash
unzip -q phase1_2_spike_review_v16.zip
cd _phase1_2_pkg_v16
...
- `data/cuts/spike/` — spike 实测数据 4 file (v15 加 telemetry_21050.jsonl, v16 同)
...
| (v16 build) | review-pkg v16 build script — GPT pro v15 三审 fix (F8 streaming + repro walk-up + RSS evidence split) |
"""
...
12 commit Phase A (A1/A2/A3) + Phase B (B1-B6) + verdict-fix + verdict-fix-2
(v15: GPT pro v14 二审 G10 downgrade; v16: GPT pro v15 三审 RSS evidence
boundary split). Per MERGER §5.1 rollback-safety, spike-only implementation
...
- `data/cuts/spike/scale_ramp_results.jsonl`
- `data/cuts/spike/phase_b_results.json`
- `data/cuts/spike/telemetry_21050.jsonl` (v15 新增: 204 rss + 14 proto + 1 dark_matter; v16 同)
...
def main() -> int:
    ...
    (PROJECT_DIR / "README.md").write_text(README_V16, encoding="utf-8")
    ...
    (OUT_DIR / "README.md").write_text(README_V16, encoding="utf-8")

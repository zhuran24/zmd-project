...
from src.search.phase3b_coordinate_validation_direct_equality_core import (
    DEFAULT_DIRECT_EQUALITY_CORE_FIELD_VARIANT,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a Phase 3B direct equality deletion/bisection core diagnostic."
    )
    ...
    parser.add_argument(
        "--group-id",
        default=DEFAULT_DIRECT_EQUALITY_CORE_GROUP_ID,
    )
    parser.add_argument(
        "--field-variant",
        default=DEFAULT_DIRECT_EQUALITY_CORE_FIELD_VARIANT,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(".artifacts/phase3b_coordinate_validation_direct_equality_core"),
    )
...
def main() -> int:
    ...
    if not args.no_write:
        ...
        prefix = args.output_prefix or "coordinate_validation_direct_equality_core"
        ...
        _atomic_write_text(
            md_path,
            render_phase3b_coordinate_validation_direct_equality_core_markdown(report),
        )
        _atomic_write_text(
            txt_path,
            render_phase3b_coordinate_validation_direct_equality_core_text(report),
        )
        print(f"coordinate_validation_direct_equality_core_json={_display_path(project_root, json_path)}")
        print(f"coordinate_validation_direct_equality_core_md={_display_path(project_root, md_path)}")
        print(f"coordinate_validation_direct_equality_core_txt={_display_path(project_root, txt_path)}")
    return 0
...
def _print_summary(report: Mapping[str, Any]) -> None:
    ...
    print("phase3b coordinate validation direct equality core")
    print(f"- outcome: {status.get('outcome')}")
    print(f"- full status: {full.get('status')}")
    print(f"- equality labels: {core.get('equality_label_count')}")
    print(f"- deletion tests: {core.get('deletion_test_count')}")
    print(f"- final key count: {core.get('final_key_count')}")

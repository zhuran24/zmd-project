...
from src.search.phase3b_coordinate_validation_actual_path_assumption_core import (
    DEFAULT_ACTUAL_PATH_FIELD_VARIANT,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a Phase 3B actual-path coordinate validation assumption-core diagnostic."
    )
    parser.add_argument("--project-root", type=Path, default=PROJECT_ROOT)
    parser.add_argument("--candidate", default="67x13")
    parser.add_argument("--anchor-index", type=int, default=133)
    parser.add_argument("--field-variant", default=DEFAULT_ACTUAL_PATH_FIELD_VARIANT)
    parser.add_argument(
    ...
    parser.add_argument("--collect-force-equality-labels", action="store_true")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(".artifacts/phase3b_coordinate_validation_actual_path_assumption_core"),
    )
...
def main() -> int:
    ...
    if not args.no_write:
        ...
        prefix = args.output_prefix or "coordinate_validation_actual_path_assumption_core"
        ...
        print(f"actual_path_assumption_core_json={_display_path(project_root, json_path)}")
        print(f"actual_path_assumption_core_md={_display_path(project_root, md_path)}")
        print(f"actual_path_assumption_core_txt={_display_path(project_root, txt_path)}")
    return 0
...
def _print_summary(report: Mapping[str, Any]) -> None:
    ...
    print("phase3b actual-path coordinate validation assumption core")
    print(f"- outcome: {status.get('outcome')}")
    print(f"- validation status: {validation.get('status')}")
    print(f"- core status: {validation.get('infeasible_assumption_core_status')}")
    print(f"- core size: {actual.get('core_size')}")
    print(f"- forced slots: {validation.get('forced_slot_field_count')}")

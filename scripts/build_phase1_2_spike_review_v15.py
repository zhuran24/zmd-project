#!/usr/bin/env python3
"""Build phase1_2_spike_review_v15.zip — Phase 1.2 spike close gate snapshot (post-二审 fix).

v13 → v14 change: include Finding 5 close gate spike verdict.
v14 → v15 change: downgrade G10 → SOFT-FAIL per GPT pro v14 二审 (verdict.md +
phase_a_report.md spike branch fix + README factual update + telemetry_21050.jsonl 入包).
...
v15 reflects GPT pro v14 二审 finding fix:
- spike branch verdict.md + phase_a_report.md 改 G10 PASS → SOFT-FAIL (commit
  `0691175` on spike branch); A3 fixture is 44 cert across 8 families, F3
  `port_exposure` missing because generator is still Phase 1.1 stub.
- master repro entrypoint fix (commit `832f157`): 3 Python repro file 加 sys.path
  bootstrap, shell repro 用 BASH_SOURCE 算 project root, 不再 hard-code v12 路径.
- data/cuts/spike/telemetry_21050.jsonl 入包 (per finding 4: raw telemetry artifact
  needed for third-party 204 rss / 14 proto / 1 dark_matter sample 复核).
- B2 ramp RSS 数字口径修正: README 跟 verdict.md 内嵌表 0.61–0.83 → 0.84–1.03 GB
  per raw jsonl + phase_b_results.json (per finding 3).

v15 does NOT include spike-only implementation code (per MERGER §5.1 rollback-safety
...
OUT_DIR = Path("/tmp/_phase1_2_pkg_v15")
PROJECT_DIR = OUT_DIR / "project"
SEVENZ_PATH = OUT_DIR / "project.7z"
OUT_ZIP = Path("/home/zhuran24/linwin_share/phase1_2_spike_review_v15.zip")
...
SPIKE_OVERLAY_FILES = [
    ...
    # NEW v15 per GPT pro v14 二审 finding 4: raw telemetry needed for 三方
    # 复核 204 rss_sample + 14 proto_sample + 1 dark_matter_emit 原始分布
    # (verdict.md §"Raw artifacts" 已 ref 此文件路径; v14 漏入包).
    "data/cuts/spike/telemetry_21050.jsonl",
]
...
Build: master commit `832f157` (详 `COMMIT_LOG.md`) + spike overlay (6 file + 1
SPIKE_COMMIT_LOG, 详 `SPIKE_COMMIT_LOG.md`, spike branch HEAD `0691175`).
...
- `data/cuts/spike/oracle_emit_fixture_45cert.jsonl` — A3 44 cert × 8 family
  oracle 实测 (real `cert_payload_b64` real `pose_count` / `cell_count` /
  `literal_count` per cert; F3 缺)
...
- `data/cuts/spike/telemetry_21050.jsonl` — Phase B raw telemetry (204
  rss_sample + 14 proto_sample + 1 dark_matter_emit). 跟 phase_b_results.json
  (aggregate) 对应.

(per MERGER §5.4 G/N criteria, MERGER §5.2 Layer 1 sizing close vs Layer 2
convergence/adversarial defer P1.3A)

### Spike verdict — SOFT-FAIL (G10 fixture coverage incomplete)
...
| G10 oracle real-emit cert fixture (A3) | ≥45 + 9 families + 0 unsound | 44 cert / 8 families / 0 unsound | SOFT-FAIL (missing F3) |
...
12 hard G PASS / 2 G SOFT-FAIL (G6a wall hit 180s cap; G10 misses F3/≥45 target) / 0 hard N trigger.
...
G10 SOFT-FAIL detail: A3 oracle_emit_fixture_45cert.jsonl 实测 44 cert 跨 8
families, F3 `port_exposure` 0 cert 因为 `generate_port_exposure_cuts()` 仍是
Phase 1.1 stub `return []`. 44 cert 全 validator `kind="ok"` (N6 仍 PASS, 0
unsound), 但 evidence coverage 不完整 — 不能宣称 ≥45 + 9 families G10 阈值过.
...
| 2 | 真 cut body 分布 (replacing toy 1-3-5 literal) | A3 jsonl has 44 cert across 8 families; F3 `port_exposure` absent | PARTIAL |
...
6 项, 含 v15 新增 #0 G10 fixture coverage).
...
```bash
unzip -q phase1_2_spike_review_v15.zip
cd _phase1_2_pkg_v15
...
# Audit reproducer (v15 入口 fix 后, 直接 cd project 跑即可, 无需手设 PYTHONPATH)
...
cat data/cuts/spike/telemetry_21050.jsonl | head -20
```
...
- `oracle_emit_fixture_45cert.jsonl` — 44 cert × 8 family A3 fixture (real oracle
  emit, 65KB; F3 missing per 二审)
- `scale_ramp_results.jsonl` — B2 5-tier ramp 0/1K/10K/50K/100K
- `phase_b_results.json` — Phase B aggregate (B3 + B2 + B4 + G/N pass map)
- `telemetry_21050.jsonl` — Phase B raw telemetry (204 rss_sample + 14
  proto_sample + 1 dark_matter_emit). 跟 phase_b_results.json (aggregate) 对应.
...
- `repro/repro_f7_facility_cells_mismatch.py` (v15 sys.path bootstrap)
- `repro/repro_f8_facility_cells_mismatch.py` (v15 sys.path bootstrap)
- `repro/repro_source_digest_quarantine.py` (v15 sys.path bootstrap)
- `repro/repro_f8_slow_generator.sh` (v15 BASH_SOURCE-based path)
...
│   ├── cutset.py                   (F2)
│   ├── port_exposure.py            (F3, stub return [])
...
- `data/cuts/spike/` — spike 实测数据 4 file (v15 加 telemetry_21050.jsonl)
...
| (v15 build) | review-pkg v15 build script — GPT pro v14 二审 fix (G10 SOFT-FAIL doc + telemetry overlay) |
"""
...
11 commit Phase A (A1/A2/A3) + Phase B (B1-B6) + verdict-fix (v15: GPT pro v14
二审 G10 downgrade). Per MERGER §5.1 rollback-safety, spike-only implementation
code 不入本包 (PR #1 verdict-only style):
...
- `data/cuts/spike/scale_ramp_results.jsonl`
- `data/cuts/spike/phase_b_results.json`
- `data/cuts/spike/telemetry_21050.jsonl` (v15 新增: 204 rss + 14 proto + 1 dark_matter)
...
def main() -> int:
    ...
    (PROJECT_DIR / "README.md").write_text(README_V15, encoding="utf-8")
    ...
    (OUT_DIR / "README.md").write_text(README_V15, encoding="utf-8")

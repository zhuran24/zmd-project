...
EXPECTED_RUN_ID = "local_hotspot_42x32_signature_bucket_compact_item_detail_inst_no_solve_001"
DEFAULT_PROBE = (
    ARTIFACT_ROOT
...
DEFAULT_OUTPUT_DIR = ARTIFACT_ROOT / "146_signature_bucket_powered_support_compact_item_detail_probe_review"
...
HOTSPOT_LABELS = {
    "compact_item_key_build": "compact_item_key_build_hotspot",
    "local_counter_update": "local_counter_update_hotspot",
    "merge_fanout": "merge_fanout_hotspot",
    "compact_signature_storage": "compact_signature_storage_hotspot",
    "stats_finalize": "compact_item_detail_stats_finalize_hotspot",
}


def main(argv: Sequence[str] | None = None) -> int:
    args = _parse_args(argv)
    review = build_signature_bucket_powered_support_compact_item_detail_probe_review(
    ...
    print("phase3b signature bucket powered support compact-item detail probe review")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Review the future S145-enabled 42x32 no-solve powered support compact-item detail "
...
def build_signature_bucket_powered_support_compact_item_detail_probe_review(
    *,
    project_root: Path,
    readiness_path: Path,
    probe_path: Path,
    ...
    payload = {
        "schema": "phase3b-signature-bucket-powered-support-compact-item-detail-probe-review/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "review_kind": "enabled_signature_bucket_powered_support_compact_item_detail_no_solve_probe_review",
...
def render_probe_review_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Phase3B S146 Powered Support Compact-Item detail Probe Review",
        "",
        ...
        "- Proof source: `false`",
        "",
        "## Compact-Item Detail Phase Seconds",
        "",
    ]
...
def _subphase_summary(probe: Mapping[str, Any]) -> dict[str, Any]:
    ...
    detail = _mapping(coverer.get("compact_item_detail_instrumentation"))
    raw_phase_seconds = _mapping(detail.get("phase_seconds"))
    phase_seconds = {
        str(key): float(value)
    ...
    parent_phase_seconds = {
        str(key): float(value)
        for key, value in _mapping(coverer.get("phase_seconds")).items()
        if isinstance(value, (int, float))
    }
    ...
    return {
        ...
        "compact_item_detail_instrumentation_present": bool(detail),
        "compact_item_detail_instrumentation_enabled": detail.get("enabled") is True,
        ...
        "all_required_sections_present": bool(
            ...
            and detail
            and detail.get("enabled") is True
        ...
        "parent_phase_seconds": parent_phase_seconds,
        "parent_totals": _mapping(coverer.get("totals")),
        "totals": _mapping(detail.get("totals")),
        "per_template": detail.get("per_template"),
        "top_slow_groups": detail.get("top_slow_groups"),
        "duplicate_compression": _mapping(detail.get("duplicate_compression")),
        "raw_port_profile_cache_instrumentation": port_profile,
        "raw_powered_support_coverer_instrumentation": coverer,
        "raw_compact_item_detail_instrumentation": detail,
    }
...
def _interpretation(
    *,
    ...
    if (
        ...
        return {
            "classification": "powered_support_compact_item_detail_probe_inconclusive",
            "reason": "readiness_not_ready_for_powered_support_compact_item_detail_probe_review",
        }
    if not subphase_summary.get("all_required_sections_present"):
        return {
            "classification": "compact_item_detail_instrumentation_missing",
            "reason": "compact_item_detail_instrumentation_missing",
        }
    ...
    baseline_compact_item_seconds = _float(
        _mapping(readiness.get("readiness")).get("s140_current_compact_item_seconds")
    )
    current_compact_item_seconds = _float(
        _mapping(subphase_summary.get("parent_phase_seconds")).get(
    ...
    key_build_count = _int(totals.get("key_build_count"))
    local_update_count = _int(totals.get("local_counter_update_count"))
    merge_update_count = _int(totals.get("merge_update_count"))
    unique_item_count = _int(totals.get("unique_item_count"))
    signature_storage_item_count = _int(totals.get("signature_storage_item_count"))
    if (
        key_build_count is None
        or local_update_count is None
        or merge_update_count is None
        or unique_item_count is None
        or signature_storage_item_count is None
    ):
        return {
            "classification": "compact_item_detail_instrumentation_missing",
            "reason": "compact_item_detail_totals_missing",
            "baseline_compact_item_seconds": baseline_compact_item_seconds,
            "current_compact_item_seconds": current_compact_item_seconds,
        }
    if (
        key_build_count <= 0
        or local_update_count <= 0
        or merge_update_count <= 0
        or unique_item_count <= 0
        or signature_storage_item_count <= 0
    ):
        return {
            "classification": "powered_support_compact_item_detail_probe_inconclusive",
            "reason": "compact_item_detail_totals_zero",
            "baseline_compact_item_seconds": baseline_compact_item_seconds,
            "current_compact_item_seconds": current_compact_item_seconds,
            "key_build_count": key_build_count,
    ...
    duplicate_compression = _mapping(subphase_summary.get("duplicate_compression"))
    unique_to_local_ratio = _float(duplicate_compression.get("unique_to_local_ratio"))
    merge_to_local_ratio = _float(duplicate_compression.get("merge_to_local_ratio"))
    if (
        unique_to_local_ratio is not None
        and merge_to_local_ratio is not None
        and unique_to_local_ratio >= 0.95
        and merge_to_local_ratio >= 0.95
    ):
        classification = "duplicate_compression_absent"
    else:
    ...
    return {
        ...
        "merge_update_count": merge_update_count,
        "unique_item_count": unique_item_count,
        "signature_storage_item_count": signature_storage_item_count,
        "unique_to_local_ratio": unique_to_local_ratio,
        "merge_to_local_ratio": merge_to_local_ratio,
        "phase_seconds": phase_seconds,
    }
...
def _next_gate(interpretation: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "status": "prepare_powered_support_compact_item_detail_result_strategy_and_external_review",
        "classification": classification,
    }
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        "review_json": output_dir / "signature_bucket_powered_support_compact_item_detail_probe_review.json",
        "review_md": output_dir / "signature_bucket_powered_support_compact_item_detail_probe_review.md",
...
def _assert_review_namespace(output_dir: Path) -> None:
    ...
    if "146_signature_bucket_powered_support_compact_item_detail_probe_review" not in normalized:
        raise ValueError("S146 probe review namespace required")

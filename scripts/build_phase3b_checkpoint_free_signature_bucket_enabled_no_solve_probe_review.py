...
DEFAULT_OUTPUT_DIR = ARTIFACT_ROOT / "43_signature_bucket_enabled_no_solve_probe_review"
ENV_VAR = "EXACT_GHOST_SIGNATURE_BUCKET_TIGHTENING_INSTRUMENTATION"
EXPECTED_RUN_ID = "local_hotspot_42x32_signature_bucket_inst_no_solve_001"
...
PHASE_CLASSIFICATIONS = {
    ...
    "stats_finalize": "instrumentation_finalize_hotspot",
}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("phase3b signature bucket enabled no-solve probe review")
    print(f"status={review['status']}")
    print(f"classification={review['interpretation']['classification']}")
    print(f"dominant_phase={review['signature_instrumentation'].get('dominant_phase')}")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Review the single enabled 42x32 no-solve signature-bucket "
            "instrumentation probe and classify the internal hotspot."
        )
    )
...
def build_signature_bucket_enabled_no_solve_probe_review(
    *,
    project_root: Path,
    readiness_path: Path,
    probe_path: Path,
    ...
    status = (
        "completed"
        if interpretation["classification"] not in {"manual_review_required", "disqualified_sensitive_path_mutation"}
        else interpretation["classification"]
    )
    paths = _paths(output_dir)
    payload = {
        "schema": "phase3b-signature-bucket-enabled-no-solve-probe-review/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "review_kind": "enabled_signature_bucket_no_solve_probe_review",
...
def render_probe_review_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Phase3B S43 Signature Bucket Enabled No-Solve Probe Review",
        "",
...
def _signature_instrumentation_summary(probe: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "required_optional_constraints_added": totals.get("required_optional_constraints_added"),
    }
    ...
    *,
    ...
    if not readiness_ok or not safety_ok:
        return {
            ...
            "reason": "Readiness or probe safety flags are not in the expected S43 state.",
    ...
    if not signature_instrumentation.get("present"):
        return {
            ...
            "reason": (
                "The enabled no-solve probe completed, but signature_tightening_instrumentation "
                "was missing. Inspect the S41 env gate and the exact-core overlay build_stats "
                "finalization path before running another probe."
            ),
            "next_engineering_step": "inspect_S41_env_gate_and_exact_core_overlay_finalization_path",
        }
    ...
    return {
        ...
        "reason": f"Dominant S41 phase is {dominant_phase!r}.",
        "dominant_phase": dominant_phase,
        "dominant_phase_seconds": dominant_seconds,
...
def _next_gate(interpretation: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "status": (
            ...
            if classification not in {"manual_review_required", "disqualified_sensitive_path_mutation"}
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        "review_json": output_dir / "signature_bucket_enabled_no_solve_probe_review.json",
        "review_md": output_dir / "signature_bucket_enabled_no_solve_probe_review.md",
...
def _assert_review_namespace(path: Path) -> None:
    ...
    if (
        ...
        or "43_signature_bucket_enabled_no_solve_probe_review" not in normalized
    ):
        raise ValueError(f"Refusing to write outside S43 probe review namespace: {path}")

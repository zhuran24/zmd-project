...
DEFAULT_RUN_ID = "s103_s104_outer_overlay_subphase_probe_review_001"
...
S100_STRATEGY = (
    ...
    / "signature_bucket_outer_exact_core_overlay_residual_strategy.json"
)
S101_REVIEW_SUMMARY = (
    ARTIFACT_ROOT
    / "101_signature_bucket_outer_exact_core_overlay_residual_external_review_package"
    / "s100_s101_outer_overlay_residual_review_001"
...
PACKAGE_INPUTS = {
    ...
    "s100_strategy": S100_STRATEGY,
    "s101_review_summary": S101_REVIEW_SUMMARY,
    "s102_implementation": S102_IMPLEMENTATION,
    "s103_readiness": S103_READINESS,
    "s103_future_command": S103_COMMAND,
    ...
    / "scripts"
    / "build_phase3b_checkpoint_free_signature_bucket_outer_overlay_subphase_probe_readiness.py",
    "s104_review_builder": PROJECT_ROOT
    / "scripts"
    / "build_phase3b_checkpoint_free_signature_bucket_outer_overlay_subphase_probe_review.py",
    "s103_readiness_tests": PROJECT_ROOT
    / "src"
    / "tests"
    / "test_phase3b_checkpoint_free_signature_bucket_outer_overlay_subphase_probe_readiness.py",
    "s104_review_tests": PROJECT_ROOT
    / "src"
    / "tests"
    / "test_phase3b_checkpoint_free_signature_bucket_outer_overlay_subphase_probe_review.py",
}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("phase3b signature bucket outer overlay subphase probe review package")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build S105 external review package for the S103/S104 one-shot "
            "outer-overlay subphase no-solve probe readiness and review tooling."
        )
    )
    parser.add_argument("--output-dir", type=Path, default=S105_DIR)
    parser.add_argument("--run-id", default=DEFAULT_RUN_ID)
    parser.add_argument("--no-write", action="store_true")
    return parser.parse_args(argv)


def build_outer_overlay_subphase_probe_external_review_package(
    *,
    project_root: Path,
    output_dir: Path,
    ...
    payload = {
        "schema": "phase3b-signature-bucket-outer-overlay-subphase-probe-external-review-package/v0",
        "generated_at": _now(),
        "status": status,
        "run_id": run_id,
...
def _review_request_text(package_filename: str, package_sha256: str) -> str:
    return "\n".join(
        [
            "# Review Request: Phase3B S103/S104 Outer Overlay Subphase Probe Readiness",
            "",
            ...
            "",
            "Context: S102 implemented default-off, stats-only subphase instrumentation under `EXACT_GHOST_SIGNATURE_BUCKET_RESIDUAL_OVERLAY_INSTRUMENTATION`. S103 is a plan-only readiness artifact for exactly one future enabled `42x32` no-solve probe, and S104 is fail-closed review tooling for the future probe output.",
            "",
            "Future command under review:",
            "",
            "`python scripts/run_phase3b_checkpoint_free_overlay_timing_probe.py --execute-no-solve --candidate-key 42x32 --run-id local_hotspot_42x32_signature_bucket_outer_overlay_subphase_inst_no_solve_001`",
            "",
            ...
            "",
            "1. Is S103 sufficiently narrow and fail-closed for a single future `42x32 --execute-no-solve` probe?",
            "2. Does S103 correctly keep `probe_execution_enabled=false` and require review/authorization before execution?",
            "3. Is S104 strict enough on sensitive-path schema, hard-boundary flags, run id, candidate key, no-solve status, and missing subphase fields?",
            "4. Are the S104 classifications useful for deciding the next review-first hotspot strategy?",
...
def _summary_markdown(payload: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Phase3B S105 Outer Overlay Subphase Probe External Review Package",
            "",
...
def _expanded_review_bundle_text(
    *,
    ...
    parts = [
        "# Expanded Review Bundle: Phase3B S103/S104 Outer Overlay Subphase Probe",
        "",
...
def _assert_package_namespace(output_dir: Path) -> None:
    ...
    if "105_signature_bucket_outer_overlay_subphase_probe_external_review_package" not in normalized:
        raise ValueError("S105 package namespace required")

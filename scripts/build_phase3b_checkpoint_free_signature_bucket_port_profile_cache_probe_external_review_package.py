...
S119_DIR = ARTIFACT_ROOT / "119_signature_bucket_port_profile_cache_probe_external_review_package"
DEFAULT_RUN_ID = "s117_s118_port_profile_cache_probe_review_001"

PACKAGE_INPUTS = {
    "s111_probe_review": ARTIFACT_ROOT
    / "111_signature_bucket_model_shell_subphase_probe_review"
    / "signature_bucket_model_shell_subphase_probe_review.json",
    "s113_probe_execution": ARTIFACT_ROOT
    / "113_signature_bucket_model_shell_subphase_probe_execution"
    / "signature_bucket_model_shell_subphase_probe_execution.json",
    "s114_strategy": ARTIFACT_ROOT
    / "114_signature_bucket_port_profile_cache_strategy"
    / "signature_bucket_port_profile_cache_strategy.json",
    "s115_review_summary": ARTIFACT_ROOT
    / "115_signature_bucket_port_profile_cache_external_review_package"
    / "s114_port_profile_cache_review_001"
    / "external_review_reply_summary.json",
    "s116_implementation": ARTIFACT_ROOT
    / "116_signature_bucket_port_profile_cache_instrumentation_implementation"
    / "signature_bucket_port_profile_cache_instrumentation_implementation.json",
    "s117_readiness": ARTIFACT_ROOT
    / "117_signature_bucket_port_profile_cache_probe_readiness"
    / "signature_bucket_port_profile_cache_probe_readiness.json",
    "s117_future_command": ARTIFACT_ROOT
    / "117_signature_bucket_port_profile_cache_probe_readiness"
    ...
    "s117_readiness_builder": PROJECT_ROOT
    / "scripts"
    / "build_phase3b_checkpoint_free_signature_bucket_port_profile_cache_probe_readiness.py",
    "s118_review_builder": PROJECT_ROOT
    / "scripts"
    / "build_phase3b_checkpoint_free_signature_bucket_port_profile_cache_probe_review.py",
    "s119_package_builder": PROJECT_ROOT
    / "scripts"
    / "build_phase3b_checkpoint_free_signature_bucket_port_profile_cache_probe_external_review_package.py",
    "s117_readiness_tests": PROJECT_ROOT
    / "src"
    / "tests"
    / "test_phase3b_checkpoint_free_signature_bucket_port_profile_cache_probe_readiness.py",
    "s118_review_tests": PROJECT_ROOT
    / "src"
    / "tests"
    / "test_phase3b_checkpoint_free_signature_bucket_port_profile_cache_probe_review.py",
    "s119_package_tests": PROJECT_ROOT
    / "src"
    / "tests"
    / "test_phase3b_checkpoint_free_signature_bucket_port_profile_cache_probe_external_review_package.py",
}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("phase3b signature bucket port/profile cache probe review package")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build S119 external review package for S117/S118 one-shot "
            "port/profile cache no-solve probe readiness and review tooling."
        )
    )
    parser.add_argument("--output-dir", type=Path, default=S119_DIR)
    parser.add_argument("--run-id", default=DEFAULT_RUN_ID)
    parser.add_argument("--no-write", action="store_true")
    return parser.parse_args(argv)


def build_port_profile_cache_probe_external_review_package(
    *,
    project_root: Path,
    output_dir: Path,
    ...
    payload = {
        "schema": "phase3b-signature-bucket-port-profile-cache-probe-external-review-package/v0",
        "generated_at": _now(),
        "status": status,
        "run_id": run_id,
...
def _review_request_text(package_filename: str, package_sha256: str) -> str:
    return "\n".join(
        [
            "# Review Request: Phase3B S117/S118 Port/Profile Cache Probe Readiness",
            "",
            ...
            "",
            "Context: S116 implemented default-off, stats-only port/profile cache instrumentation under `EXACT_GHOST_SIGNATURE_BUCKET_PORT_PROFILE_CACHE_INSTRUMENTATION`. S117 is a plan-only readiness artifact for exactly one future enabled `42x32` no-solve probe, and S118 is fail-closed review tooling for the future probe output.",
            "",
            "Future command under review:",
            "",
            "`python scripts/run_phase3b_checkpoint_free_overlay_timing_probe.py --execute-no-solve --candidate-key 42x32 --run-id local_hotspot_42x32_signature_bucket_port_profile_cache_inst_no_solve_001`",
            "",
            ...
            "",
            "1. Is S117 sufficiently narrow and fail-closed for a single future `42x32 --execute-no-solve` port/profile cache probe?",
            "2. Does S117 correctly keep `probe_execution_enabled=false` and require review/authorization before execution?",
            "3. Is S118 strict enough on sensitive-path schema, hard-boundary flags, run id, candidate key, no-solve status, and missing port/profile cache fields?",
            "4. Are the S118 classifications useful for deciding the next review-first `_index_pools()` hotspot strategy?",
...
def _summary_markdown(payload: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Phase3B S119 Port/Profile Cache Probe External Review Package",
            "",
...
def _expanded_review_bundle_text(
    *,
    ...
    parts = [
        "# Expanded Review Bundle: Phase3B S117/S118 Port/Profile Cache Probe",
        "",
...
def _assert_package_namespace(output_dir: Path) -> None:
    ...
    if "119_signature_bucket_port_profile_cache_probe_external_review_package" not in normalized:
        raise ValueError("S119 package namespace required")

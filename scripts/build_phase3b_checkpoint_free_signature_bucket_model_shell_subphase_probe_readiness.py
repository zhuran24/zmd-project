...
FUTURE_RUN_ID = "local_hotspot_42x32_signature_bucket_model_shell_inst_no_solve_001"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    readiness = build_signature_bucket_model_shell_subphase_probe_readiness(
        project_root=PROJECT_ROOT,
        output_dir=_resolve_path(PROJECT_ROOT, args.output_dir),
        no_write=bool(args.no_write),
    )
    print("phase3b signature bucket model-shell subphase probe readiness")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build S110 readiness artifacts for one future enabled 42x32 no-solve "
            "model-shell subphase instrumentation probe without executing it."
        )
    )
...
def build_signature_bucket_model_shell_subphase_probe_readiness(
    *,
    project_root: Path,
    output_dir: Path,
    no_write: bool = False,
    ...
    status = (
        "completed"
        if readiness["classification"] == "ready_for_model_shell_subphase_probe_review"
        else "manual_review_required"
    )
    paths = _paths(output_dir)
    payload = {
        "schema": "phase3b-signature-bucket-model-shell-subphase-probe-readiness/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "readiness_kind": "future_enabled_signature_bucket_model_shell_subphase_no_solve_probe_plan_only",
        ...
        "next_gate": {
            "status": "hold_for_readiness_review_before_model_shell_subphase_probe",
            "candidate_step": (
                "execute exactly one enabled 42x32 no-solve model-shell subphase "
                "probe only after S110 readiness review accepts this plan"
            ),
            "blocked_actions": [
                "do_not_execute_enabled_probe_in_s110",
...
def build_future_command_template() -> dict[str, Any]:
    ...
    return {
        "schema": "phase3b-signature-bucket-model-shell-subphase-future-command/v0",
        "command": command,
...
def render_readiness_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Phase3B S110 Model-Shell Subphase Probe Readiness",
        "",
...
def _evaluate_readiness(
    *,
    ...
    checks = [
        _check("s106_probe_completed", _input_value(loaded_inputs, "s106_execution", "status") == "completed"),
        _check(
            "s106_model_shell_hotspot",
            _input_value(loaded_inputs, "s106_execution", "classification")
            == "model_shell_construction_hotspot",
        ),
        _check(
            "s104_model_shell_hotspot",
            _nested_value(loaded_inputs, "s104_review", "interpretation", "classification")
            == "model_shell_construction_hotspot",
        ),
        _check(
            "s107_strategy_ready",
            _input_value(loaded_inputs, "s107_strategy", "classification")
            == "model_shell_construction_subphase_strategy_required",
        ),
        _check(
            "s108_review_passed",
            _input_value(loaded_inputs, "s108_review_summary", "review_verdict") == "pass",
        ),
        _check(
            "s109_patch_implemented_and_verified",
            _input_value(loaded_inputs, "s109_implementation", "status")
            == "implemented_and_verified",
        ),
        _check(
            "s109_model_shell_env_matches",
            _input_value(loaded_inputs, "s109_implementation", "env_var")
            == MODEL_SHELL_INSTRUMENTATION_ENV_VAR,
        ),
    ...
    return {
        ...
        "checks": checks,
        "s106_classification": _input_value(loaded_inputs, "s106_execution", "classification"),
        "s107_classification": _input_value(loaded_inputs, "s107_strategy", "classification"),
        "s109_env_var": _input_value(loaded_inputs, "s109_implementation", "env_var"),
    }
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        "readiness_json": output_dir / "signature_bucket_model_shell_subphase_probe_readiness.json",
        "readiness_md": output_dir / "signature_bucket_model_shell_subphase_probe_readiness.md",
...
def _assert_readiness_namespace(output_dir: Path) -> None:
    ...
    if "110_signature_bucket_model_shell_subphase_probe_readiness" not in normalized:
        raise ValueError("S110 readiness namespace required")

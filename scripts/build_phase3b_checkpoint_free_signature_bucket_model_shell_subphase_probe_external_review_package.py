...
DEFAULT_RUN_ID = "s110_s111_model_shell_subphase_probe_review_001"
...
PACKAGE_INPUTS = {
    ...
    "s107_strategy": S107_STRATEGY,
    "s108_review_summary": S108_REVIEW_SUMMARY,
    "s109_implementation": S109_IMPLEMENTATION,
    "s110_readiness": S110_READINESS,
    "s110_future_command": S110_COMMAND,
    ...
    / "scripts"
    / "build_phase3b_checkpoint_free_signature_bucket_model_shell_subphase_probe_readiness.py",
    "s111_review_builder": PROJECT_ROOT
    / "scripts"
    / "build_phase3b_checkpoint_free_signature_bucket_model_shell_subphase_probe_review.py",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("phase3b signature bucket model-shell subphase probe review package")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build S112 external review package for S110/S111 one-shot "
            "model-shell subphase no-solve probe readiness and review tooling."
        )
    )
    parser.add_argument("--output-dir", type=Path, default=S112_DIR)
    parser.add_argument("--run-id", default=DEFAULT_RUN_ID)
    parser.add_argument("--no-write", action="store_true")
    return parser.parse_args(argv)


def build_model_shell_subphase_probe_external_review_package(
    *,
    project_root: Path,
    output_dir: Path,
    ...
    payload = {
        "schema": "phase3b-signature-bucket-model-shell-subphase-probe-external-review-package/v0",
        "generated_at": _now(),
        "status": status,
        "run_id": run_id,
...
def _review_request_text(package_filename: str, package_sha256: str) -> str:
    return "\n".join(
        [
            "# Review Request: Phase3B S110/S111 Model-Shell Subphase Probe Readiness",
            "",
            ...
            "",
            "Context: S109 implemented default-off, stats-only model-shell constructor subphase instrumentation under `EXACT_GHOST_SIGNATURE_BUCKET_MODEL_SHELL_INSTRUMENTATION`. S110 is a plan-only readiness artifact for exactly one future enabled `42x32` no-solve probe, and S111 is fail-closed review tooling for the future probe output.",
            "",
            "Future command under review:",
            "",
            "`python scripts/run_phase3b_checkpoint_free_overlay_timing_probe.py --execute-no-solve --candidate-key 42x32 --run-id local_hotspot_42x32_signature_bucket_model_shell_inst_no_solve_001`",
            "",
            ...
            "",
            "1. Is S110 sufficiently narrow and fail-closed for a single future `42x32 --execute-no-solve` model-shell subphase probe?",
            "2. Does S110 correctly keep `probe_execution_enabled=false` and require review/authorization before execution?",
            "3. Is S111 strict enough on sensitive-path schema, hard-boundary flags, run id, candidate key, no-solve status, and missing model-shell subphase fields?",
            "4. Are the S111 classifications useful for deciding the next review-first constructor-hotspot strategy?",
...
def _summary_markdown(payload: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Phase3B S112 Model-Shell Subphase Probe External Review Package",
            "",
...
def _expanded_review_bundle_text(
    *,
    ...
    parts = [
        "# Expanded Review Bundle: Phase3B S110/S111 Model-Shell Subphase Probe",
        "",
...
def _assert_package_namespace(output_dir: Path) -> None:
    ...
    if "112_signature_bucket_model_shell_subphase_probe_external_review_package" not in normalized:
        raise ValueError("S112 package namespace required")

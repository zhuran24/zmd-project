...
EXPECTED_RUN_ID = "local_hotspot_42x32_signature_bucket_powered_support_coverer_inst_no_solve_001"
DEFAULT_PROBE = (
    ARTIFACT_ROOT
...
DEFAULT_OUTPUT_DIR = ARTIFACT_ROOT / "126_signature_bucket_powered_support_coverer_probe_review"
...
HOTSPOT_LABELS = {
    ...
    "compact_item_accumulation": "compact_item_accumulation_hotspot",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    review = build_signature_bucket_powered_support_coverer_probe_review(
    ...
    print("phase3b signature bucket powered support-coverer probe review")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Review the future S125-enabled 42x32 no-solve powered support-coverer "
...
def build_signature_bucket_powered_support_coverer_probe_review(
    *,
    project_root: Path,
    readiness_path: Path,
    probe_path: Path,
    ...
    payload = {
        "schema": "phase3b-signature-bucket-powered-support-coverer-probe-review/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "review_kind": "enabled_signature_bucket_powered_support_coverer_no_solve_probe_review",
...
def render_probe_review_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Phase3B S126 Powered Support-Coverer Probe Review",
        "",
...
def _interpretation(
    *,
    ...
    if (
        ...
        return {
            "classification": "powered_support_coverer_probe_inconclusive",
            "reason": "readiness_not_ready_for_powered_support_coverer_probe_review",
        }
    ...
    classification = HOTSPOT_LABELS.get(
        dominant_phase,
        "powered_support_coverer_detail_visible",
    )
    return {
...
def _next_gate(interpretation: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "status": "prepare_powered_support_coverer_hotspot_specific_strategy_and_external_review",
        "classification": classification,
    }
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        "review_json": output_dir / "signature_bucket_powered_support_coverer_probe_review.json",
        "review_md": output_dir / "signature_bucket_powered_support_coverer_probe_review.md",
...
def _assert_review_namespace(output_dir: Path) -> None:
    ...
    if "126_signature_bucket_powered_support_coverer_probe_review" not in normalized:
        raise ValueError("S126 probe review namespace required")

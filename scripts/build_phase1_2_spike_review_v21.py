#!/usr/bin/env python3
"""Build phase1_2_spike_review_v21.zip — GPT v20 七审 union fix (clean GO).
...
OUT_DIR = Path("/tmp/_phase1_2_pkg_v21")
PROJECT_DIR = OUT_DIR / "project"
SEVENZ_PATH = OUT_DIR / "project.7z"
OUT_ZIP = Path("/home/zhuran24/linwin_share/phase1_2_spike_review_v21.zip")
...
(v21 含 F9 generator refactor + toy_translator F3 fail-closed for malformed payload).

Build: master commit `d342784` (详 `COMMIT_LOG.md`) + spike overlay (7 data
file + 1 SPIKE_COMMIT_LOG, 详 `SPIKE_COMMIT_LOG.md`, spike branch HEAD
`a2338e4`; data-producing Phase B rerun commit `ed48a05` post-F3-real-parse fix —
v21 维持 v20 B2 数据, 不需要重跑 per Reviewer A).
...
```bash
unzip -q phase1_2_spike_review_v21.zip
cd _phase1_2_pkg_v21
...
| (v21 build) | review-pkg v21 build script — v20 七审 union fix |
"""
...
+ lint cleanup (per GPT v20 七审 Reviewer A + B union fix).
...
(per GPT v17 四审 Reviewer B patch 0002 spec; v21 含 F3 fail-closed for malformed
payload + verdict generator label/raw artifacts fix + spike lib no-placeholder
f-string lint cleanup):
...
- `scripts/spike_prod_scale_lib/toy_translator.py` (v19: + port_exposure
  ...
  → return [] (caller skip) — no synthetic 3-literal fallback for malformed F3)
...
- `scripts/spike_prod_scale_lib/feasible_smoke.py` (v20: Python 3.14
  portability tweak; v21: lint cleanup)
...
src 影响. 不作为 "已证实 stdlib regression" 主张.

Verdict data overlay 入包 (覆盖到 project 相同路径, v21 8 files — v20 数据 +
v21 verdict.md 行级 fix):
...
def main() -> int:
    ...
    (PROJECT_DIR / "README.md").write_text(README_V21, encoding="utf-8")
    ...
    (OUT_DIR / "README.md").write_text(README_V21, encoding="utf-8")

#!/usr/bin/env python3
"""Build phase1_2_entry_review_v10.zip — Phase 1.1 final hardening 落地 (3 轮 deliverable 全 land).

v9 → v10 关键差异 (反映 R3 final delivery 落地, commit `db8d9cd`):
...
- CutStore 状态机补洞 (不存在 cut 不能 reactivate; quarantined cut 不能重激活)
- watcher 查询返副本 (cuts_affected_by_*() 不暴露内部 set)
- F2 cutset free_cells 排除 exterior_blocks (不允许路线穿过静态阻挡)
- F1/F2 payload 解析加严 (base64 + bitset 长度 + 高位不置; cell 不宽松 int(...))
- Cut runtime schema 加严 (空 cut_id / 非 tuple literals / 坏 minimization_audit fail-closed)
- optional HiGHS/SCIP test skip (缺依赖时不 collection error)
- exit_criteria script 文件名修 (test_family_3_port_exposure → test_family_port_exposure; test_replay_suite → test_replay)
- 文档对齐 (cuts 测试数 178 → 181; source_digest 不写 placeholder; strict gate default ON)

v8 → v9 关键差异 (Phase 1.1 exit hardening + Gemini math review meta-audit, 见 v9 archive):
- 178 cuts pytest pass (v8 时 172)
- mypy strict 全清 (v8 时 37 errors)
- radon A 无 D (v8 时 D(27/24/23))
- docs/项目说明/ 整 dir 21 sub-doc 新加 (项目顶层重组)
- F8 mode 锁 geometric / F9 area-only / CP-SAT no AddLazyConstraint verdict

跟 v8/v9 一样的 strategy:
...
OUT_DIR = Path("/tmp/_phase1_2_pkg_v10")
PROJECT_DIR = OUT_DIR / "project"
SEVENZ_PATH = OUT_DIR / "project.7z"
OUT_ZIP = Path("/home/zhuran24/linwin_share/phase1_2_entry_review_v10.zip")
...
README_V10 = """# Phase 1.2 Entry Review (v10) — Phase 1.1 final hardening 落地
...
**Phase 1.1: GO blessed** (3 轮外部 deliverable 全 land):
- R1 exit hardening (`9e01a6e`) — 8 项 入门 fix
- R2 docs/项目说明/ v1.1 (`ecc96c7`) — merge v2 plan + Gemini math review meta-audit
- **R3 final delivery (`db8d9cd`) — 10 项 adversarial soundness 升级**:
  - `validate_cut_integrity()` 新加 (cert_payload hash + oracle_cert_hash + geo↔cert payload 一致)
  - BState.source_digest 不信外部值, replay 按当前 source 重算
  - CutStore 状态机补洞 (不存在 cut 不能 reactivate; quarantined cut 不能重激活)
  - watcher 查询返副本 (cuts_affected_by_*() 不暴露内部 set)
  - F2 cutset free_cells 排除 exterior_blocks
  - F1/F2 payload 解析加严 (base64 + bitset 长度 + 高位不置)
  - Cut runtime schema 加严 (literal slot 类型 / pose_id 类型 / minimization_audit fail-closed)
  - optional HiGHS/SCIP test skip (缺依赖时不 collection error)

验收:
- **181 cuts pytest pass** (`python` 普通 + `python -O`, v9 时 178)
...
**进入 Phase 1.2**: P1.2A entry hardening ✅ done (3 轮 deliverable). 待 P1.2B-F5/F6/F7/F8/F9 实施 (按顺序: F5 fallback 优先 → F9 主力 lift → F2/F4 generator → F6 → F7 → F8).
...
```bash
unzip -q phase1_2_entry_review_v10.zip
cd _phase1_2_pkg_v10
...
CHANGELOG = """# Commit log (Phase 1.1 → Phase 1.2 entry, v6 audit → v10 build)
...
def main() -> int:
    ...
    (PROJECT_DIR / "README.md").write_text(README_V10, encoding="utf-8")
    ...
    (OUT_DIR / "README.md").write_text(README_V10, encoding="utf-8")

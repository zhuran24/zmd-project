...
FROZEN_ARTIFACTS = {
    "rules/canonical_rules.json": "C3FC3A34E67B2321048A8861A9B178C744361698A838039B0361287C9FB542C0",
...
# 2026-08-09：两套记忆系统（cc_memory/、cc_memory_vnext/）连同全部 hook 整体移除，
# 待空白重建。这里做的是**登记表清空**，不是删掉这条 lane——正走 9cf7600 立下的那条
# 路：「a genuine retirement has to edit MEMORY_TEST_DIRS first」。缺根即 BLOCK 的机械
# 原样留着（见 check_memory_tests），重建时把新的测试根、卡校验器、scope 前缀填回这三
# 个常量，lane 即复活，不必再动逻辑。
#
# 空登记表**不等于**静默跳过：lane 每跑一次 warn 一句。「活 hook 现在没有任何机械守
# 着」是欠账状态而不是正常态，而这正是 6b2fb40 当初建这条 lane 要说的那句话——2026-08-03
# 普查发现 240 条守着 SessionStart/UserPromptSubmit/PreToolUse 的测试没有任何门收集，
# 改坏一个 hook 一直是静默的。下线期间把这句话继续说出来，比让它安静更贴原意。
MEMORY_TEST_DIRS: tuple[str, ...] = ()
MEMORY_SCOPE_PREFIXES: tuple[str, ...] = ()
MEMORY_CARD_VERIFIER = ""
...
def check_memory_cards(gate: GateResult) -> None:
    ...
    """
    # 未登记（空字符串）与登记了却找不到，是两件事：前者是 2026-08-09 记忆层整体
    # 移除后的有意留白，后者才是「校验器被删/改名而登记没跟上」的事故。不加这一
    # 支的话空登记会走进下面那条，报出一个路径为空的 BLOCK，费解且拦错了人。
    if not MEMORY_CARD_VERIFIER:
        gate.warn(
            "记忆卡校验器未登记（MEMORY_CARD_VERIFIER 为空）— 卡片本身对不对没人跑；"
            "重建记忆层时与 MEMORY_TEST_DIRS 一并填回"
        )
        return
...
def check_memory_tests(gate: GateResult, *, always: bool) -> None:
    ...
    登记表整个为空是第三种情况，2026-08-09 起就是它：记忆层已被有意移除、待空
    白重建，没有任何根可查。它与「登记了却找不到」相反——后者是事故，前者是
    走完了上面那句「真要下线先改登记」的正规手续，所以不 BLOCK。但也不静默：
    每跑一次 warn 一句，因为活 hook 眼下确实无人守护，而把这件事说出来正是这
    条 lane 存在的理由。
    """
    if not MEMORY_TEST_DIRS:
        print("\n[memory] 记忆层测试 lane（登记表为空，已下线）")
        gate.warn(
            "记忆层 lane 已下线：MEMORY_TEST_DIRS 为空 — 2026-08-09 两套记忆系统与全部 "
            "hook 整体移除待重建，期间没有任何机械守着活 hook；重建后把测试根填回 "
            "MEMORY_TEST_DIRS（连同 MEMORY_CARD_VERIFIER / MEMORY_SCOPE_PREFIXES）即复活"
        )
        return

    scope_note = "全量" if always else "staged 触及登记的记忆层路径"

...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Profile exact master status for recorded anchors forced one-at-a-time."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(".artifacts/phase3b_forced_anchor_master"),
    )
...
def main() -> int:
    ...
    report = build_phase3b_forced_anchor_master_profile(
        ...
        search_branching=str(args.search_branching),
    )
    _print_summary(report)
    if not args.no_write:
        output_dir = _resolve_output_dir(project_root, args.output_dir)
        prefix = args.output_prefix or f"forced_anchor_master_{str(args.candidate)}"
        json_path = output_dir / f"{prefix}.json"
        md_path = output_dir / f"{prefix}.md"
        txt_path = output_dir / f"{prefix}.txt"
        atomic_write_json(json_path, report)
        _atomic_write_text(md_path, render_phase3b_forced_anchor_master_markdown(report))
        _atomic_write_text(txt_path, render_phase3b_forced_anchor_master_text(report))
        print(f"forced_anchor_master_json={_display_path(project_root, json_path)}")
        print(f"forced_anchor_master_md={_display_path(project_root, md_path)}")
        print(f"forced_anchor_master_txt={_display_path(project_root, txt_path)}")
    return 0
...
def _print_summary(report: Mapping[str, Any]) -> None:
    ...
    print("phase3b forced-anchor master profile")

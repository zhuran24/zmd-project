...
from src.search.phase3b.family_lookup.semantic_repro import (
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a tiny active-coverage plus family-lookup semantic repro from exact artifacts."
    )
    ...
    parser.add_argument(
        "--variants",
        default=",".join(DEFAULT_SEMANTIC_REPRO_VARIANTS),
        help="Comma-separated semantic repro variants.",
    )
    parser.add_argument("--slot-limit", type=int, default=3)
    parser.add_argument("--family-limit-per-slot", type=int, default=3)
    parser.add_argument(
        "--solver-profile-json",
        default=None,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(".artifacts/phase3b_family_lookup_semantic_repro"),
    )
...
def main() -> int:
    ...
    if not args.no_write:
        ...
        json_path = output_dir / "family_lookup_semantic_repro.json"
        md_path = output_dir / "family_lookup_semantic_repro.md"
        txt_path = output_dir / "family_lookup_semantic_repro.txt"
        atomic_write_json(json_path, report)
        _atomic_write_text(
            md_path,
            render_phase3b_family_lookup_semantic_repro_markdown(report),
        )
        _atomic_write_text(
            txt_path,
            render_phase3b_family_lookup_semantic_repro_text(report),
        )
        print(f"family_lookup_semantic_repro_json={_display_path(project_root, json_path)}")
        print(f"family_lookup_semantic_repro_md={_display_path(project_root, md_path)}")
        print(f"family_lookup_semantic_repro_txt={_display_path(project_root, txt_path)}")
    return 0
...
def _print_summary(report: Mapping[str, Any]) -> None:
    ...
    print("phase3b family lookup semantic repro")
    ...
    print(f"- selected slots: {extraction.get('selected_slot_count', 0)}")
    print(f"- selected family ids: {extraction.get('selected_family_ids', [])}")

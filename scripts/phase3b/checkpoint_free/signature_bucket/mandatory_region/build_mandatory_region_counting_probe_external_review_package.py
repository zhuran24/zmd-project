...
DEFAULT_RUN_ID = "s52_s53_region_probe_readiness_review_001"
...
DEFAULT_INPUTS = {
    ...
    / "scripts" / "phase3b" / "checkpoint_free" / "signature_bucket" / "mandatory_region" / "build_mandatory_region_counting_probe_readiness.py",
    ...
    / "scripts" / "phase3b" / "checkpoint_free" / "signature_bucket" / "mandatory_region" / "build_mandatory_region_counting_probe_external_review_package.py",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    package = build_signature_bucket_mandatory_region_counting_probe_external_review_package(
        project_root=PROJECT_ROOT,
        output_dir=_resolve_path(PROJECT_ROOT, args.output_dir),
        run_id=str(args.run_id),
        no_write=bool(args.no_write),
    )
    print("phase3b signature bucket mandatory region-counting probe external review package")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a clean-extraction-validated S54 external review package for "
            "the S52/S53 mandatory region-counting probe readiness and review tooling."
        )
    )
...
def build_signature_bucket_mandatory_region_counting_probe_external_review_package(
    *,
    project_root: Path,
    output_dir: Path,
    ...
    payload = {
        "schema": "phase3b-signature-bucket-mandatory-region-counting-probe-external-review-package/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "package_kind": "external_review_mandatory_region_counting_probe_readiness_package",
    ...
    if not no_write:
        ...
        (run_dir / "external_review_package_summary.md").write_text(
            render_signature_bucket_mandatory_region_counting_probe_external_review_package_markdown(
                payload
            ),
            encoding="utf-8",
        )
...
def render_signature_bucket_mandatory_region_counting_probe_external_review_package_markdown(
    payload: Mapping[str, Any],
) -> str:
    validation = _mapping(payload.get("clean_extraction_validation"))
    return "\n".join(
        [
            "# Phase3B S54 Mandatory Region Counting Probe External Review Package",
            "",
            ...
            "",
            "This package asks external review whether the S52/S53 readiness and review tooling is safe to request authorization for exactly one future enabled 42x32 no-solve probe. It is not authorization.",
            "",
        ]
    )
...
def _review_request_text(
    ...
    *,
    ...
    return "\n".join(
        [
            "# Review Request: Phase3B S52/S53 Mandatory Region Counting Probe Readiness",
            "",
            ...
            "",
            "Context: S51 implemented and verified the default-off `EXACT_GHOST_SIGNATURE_BUCKET_MANDATORY_REGION_COUNTING` patch. S52 is only a readiness artifact for one future enabled `42x32` no-solve probe, and S53 is only the review tooling that will classify the probe after it exists. No probe is executed by this package.",
            "",
            "Please review these questions:",
            "",
            "1. Does S52 correctly restrict the future command to exactly one `42x32` `--execute-no-solve` probe with run id `local_hotspot_42x32_signature_bucket_region_counting_inst_no_solve_001`?",
            "2. Are the two required env gates explicit and sufficient: `EXACT_GHOST_SIGNATURE_BUCKET_TIGHTENING_INSTRUMENTATION=1` and `EXACT_GHOST_SIGNATURE_BUCKET_MANDATORY_REGION_COUNTING=1`?",
            "3. Do S52/S53 preserve the safety boundary: no `CpSolver.Solve`, no `main.py`, no `ExactCampaign`, no checkpoint write/import/backfill, no proof/preflight/release/viewer/frontdoor mutation, no production default change?",
            "4. Does S53 classify the future probe conservatively enough, including `mandatory_region_counting_effective`, `mandatory_region_counting_not_used`, `fallback_dominated`, `mandatory_scan_still_hot`, `visibility_missing`, `instrumentation_inconclusive`, and `safety_disqualified`?",
            "5. Are the included tests and guards enough before asking the user/project-owner to authorize that one future no-solve diagnostic probe?",
            "6. If there are no blockers, please state only that this S52/S53 readiness scope is safe to request user/project-owner authorization for the single future no-solve probe; do not treat review approval as authorization.",
            "",
...
def _manifest(
    *,
    ...
    return {
        "schema": "phase3b-signature-bucket-mandatory-region-counting-probe-external-review-package-manifest/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "run_id": str(run_id),
        "zip_path": str(zip_path),
        "zip_sha256": None,
...
def _write_staging_files(
    *,
    ...
    (staging_dir / "README.md").write_text(
        (
            "This package is for Phase3B / Endfield external review of S52/S53 "
            "mandatory region-counting probe readiness and review tooling. It needs "
            "review first; review is not authorization; if review passes request "
            "user/project-owner authorization before executing the single future "
            "enabled no-solve probe. It is not proof evidence and does not execute "
            "the probe or perform source mutation.\n"
        ),
        encoding="utf-8",
    )
...
def _required_entries() -> list[str]:
    return [
        ...
        "code_context/scripts/phase3b/checkpoint_free/signature_bucket/mandatory_region/build_mandatory_region_counting_probe_readiness.py",
        ...
        "code_context/scripts/phase3b/checkpoint_free/signature_bucket/mandatory_region/build_mandatory_region_counting_probe_external_review_package.py",
        "test_context/src/tests/phase3b/checkpoint_free/signature_bucket/mandatory_region/test_mandatory_region_counting_probe_readiness.py",
...
def _assert_package_namespace(path: Path) -> None:
    ...
    if (
        ...
        or "54_signature_bucket_mandatory_region_counting_probe_external_review_package"
        not in normalized
    ):
        raise ValueError(f"Refusing to write outside S54 external review package namespace: {path}")

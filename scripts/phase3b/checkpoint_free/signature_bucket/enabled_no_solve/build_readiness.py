...
DEFAULT_OUTPUT_DIR = ARTIFACT_ROOT / "42_signature_bucket_enabled_no_solve_probe_readiness"
DEFAULT_INPUTS = {
    ...
    "s40_review_summary": ARTIFACT_ROOT
    / "40_signature_bucket_external_review_package_v2"
    / "signature_bucket_external_review_package_002"
...
FUTURE_RUN_ID = "local_hotspot_42x32_signature_bucket_inst_no_solve_001"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("phase3b signature bucket enabled no-solve probe readiness")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build S42 readiness artifacts for a future enabled 42x32 no-solve "
            "signature-bucket instrumentation probe without executing the probe."
        )
    )
...
def build_signature_bucket_enabled_no_solve_probe_readiness(
    *,
    project_root: Path,
    output_dir: Path,
    no_write: bool = False,
    ...
    status = "completed" if readiness["classification"] == "ready_for_readiness_review" else "manual_review_required"
    paths = _paths(output_dir)
    payload = {
        "schema": "phase3b-signature-bucket-enabled-no-solve-probe-readiness/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "readiness_kind": "future_enabled_signature_bucket_no_solve_probe_plan_only",
        ...
        "next_gate": {
            "status": "hold_for_readiness_review_before_enabled_no_solve_probe",
            "s43_candidate_step": (
                "execute exactly one enabled 42x32 no-solve probe only after S42 "
                "readiness review accepts this plan"
            ),
            "blocked_actions": [
                "do_not_execute_enabled_probe_in_s42",
...
def build_future_command_template() -> dict[str, Any]:
    ...
    return {
        "schema": "phase3b-signature-bucket-enabled-no-solve-future-command/v0",
        "command": command,
...
def render_readiness_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Phase3B S42 Signature Bucket Enabled No-Solve Probe Readiness",
        "",
    ...
    for check in checks:
        ...
        lines.append(
            f"- `{check_map.get('name')}`: `{check_map.get('status')}`"
        )
    lines.extend(
        [
            "",
            "## Gate",
            "",
            "S42 does not execute the enabled probe. S43 may execute exactly one `42x32` no-solve probe only after this readiness artifact is reviewed and accepted.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def _evaluate_readiness(
    *,
    ...
    s35 = _payload(loaded_inputs, "s35_overlay_timing_probe")
    s36 = _payload(loaded_inputs, "s36_signature_bucket_strategy")
    s39 = _payload(loaded_inputs, "s39_revised_patch_spec")
    s40 = _payload(loaded_inputs, "s40_review_summary")
    s41 = _payload(loaded_inputs, "s41_implementation")
    agents_text = str(_mapping(loaded_inputs.get("agents")).get("text") or "")
    checks = [
        _check(
            "s35_overlay_probe_completed_no_solve",
            s35.get("status") == "completed"
            and s35.get("cp_solver_solve_called") is False
            and _mapping(s35.get("target")).get("candidate_key") == "42x32",
        ),
        _check(
            "s36_strategy_ready",
            s36.get("status") == "completed"
            and _mapping(s36.get("interpretation")).get("classification")
            == "signature_bucket_internal_loop_strategy_required",
        ),
        _check(
            "s39_revision_ready",
            s39.get("status") == "completed"
            and _mapping(s39.get("interpretation")).get("classification")
            == "output_path_finalization_revision_required"
            and s39.get("source_mutation_performed") is False,
        ),
        _check(
            "s40_review_passed_not_authorization",
            s40.get("review_verdict") == "safe_to_request_user_project_owner_authorization"
            and s40.get("review_approval_is_authorization") is False,
        ),
        _check(
            "s41_implemented_and_user_authorized",
            s41.get("status") == "implemented_and_verified"
            and _mapping(s41.get("review_and_authorization")).get("user_authorization_granted")
            is True,
        ),
        _check(
            "s41_no_runtime_or_checkpoint",
            _mapping(s41.get("safety_flags")).get("solver_runtime_executed") is False
            and _mapping(s41.get("safety_flags")).get("canonical_checkpoint_written") is False,
        ),
        _check(
            "agents_gate_mentions_s41",
            "Current S41 signature-bucket instrumentation state" in agents_text,
        ),
        _check(
    ...
    return {
        "classification": "ready_for_readiness_review" if ready else "manual_review_required",
        "checks": checks,
        "ready_for_future_probe_review": bool(ready),
        "future_probe_executable_now": False,
        "reason": (
            "All inputs are ready to review a single future enabled 42x32 no-solve probe plan."
            if ready
            else "One or more S35-S41 inputs or command/sensitive-path guards are not ready."
        ),
    }
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        "readiness_json": output_dir / "signature_bucket_enabled_no_solve_probe_readiness.json",
        "readiness_md": output_dir / "signature_bucket_enabled_no_solve_probe_readiness.md",
...
def _assert_readiness_namespace(path: Path) -> None:
    ...
    if (
        ...
        or "42_signature_bucket_enabled_no_solve_probe_readiness" not in normalized
    ):
        raise ValueError(f"Refusing to write outside S42 readiness namespace: {path}")

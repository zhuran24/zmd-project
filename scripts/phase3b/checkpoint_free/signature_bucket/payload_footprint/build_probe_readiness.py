...
DEFAULT_OUTPUT_DIR = ARTIFACT_ROOT / "86_signature_bucket_payload_footprint_probe_readiness"
DEFAULT_INPUTS = {
...
FUTURE_RUN_ID = "local_hotspot_42x32_signature_bucket_payload_footprint_inst_no_solve_001"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("phase3b signature bucket payload-footprint probe readiness")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build S86 readiness artifacts for one future enabled 42x32 no-solve "
            "payload-footprint stability probe without executing it."
        )
    )
...
def build_signature_bucket_payload_footprint_probe_readiness(
    *,
    project_root: Path,
    output_dir: Path,
    no_write: bool = False,
    ...
    status = (
        "completed"
        if readiness["classification"] == "ready_for_payload_footprint_probe_review"
        else "manual_review_required"
    )
    paths = _paths(output_dir)
    payload = {
        "schema": "phase3b-signature-bucket-payload-footprint-probe-readiness/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "readiness_kind": "future_enabled_signature_bucket_payload_footprint_no_solve_probe_plan_only",
        ...
        "next_gate": {
            "status": "hold_for_readiness_review_before_payload_footprint_probe",
            "candidate_step": (
                "execute exactly one enabled 42x32 no-solve payload-footprint probe "
                "only after S86 readiness review accepts this plan"
            ),
            "blocked_actions": [
                "do_not_execute_enabled_probe_in_s86",
...
def build_future_command_template() -> dict[str, Any]:
    ...
    return {
        "schema": "phase3b-signature-bucket-payload-footprint-future-command/v0",
        "command": command,
...
def validate_future_command_template(template: Mapping[str, Any]) -> dict[str, Any]:
    ...
    required_env = {
        ...
        PAYLOAD_FOOTPRINT_STABILITY_ENV_VAR: "payload_footprint_stability_env_gate_must_be_1",
    }
...
def render_readiness_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Phase3B S86 Signature Bucket Payload-Footprint Probe Readiness",
        "",
        ...
        f"- Env: `{PAYLOAD_FOOTPRINT_STABILITY_ENV_VAR}={environment.get(PAYLOAD_FOOTPRINT_STABILITY_ENV_VAR)}`",
        f"- Run id: `{future_command.get('run_id')}`",
        "",
        "## Baseline",
        "",
        f"- S82 mandatory scan seconds: `{readiness.get('baseline_mandatory_scan_seconds')}`",
        f"- S82 unstable footprint-bound fallbacks: `{readiness.get('baseline_unstable_footprint_bounds_fallbacks')}`",
        "",
        "## Readiness Checks",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "## Gate",
            "",
            "S86 does not execute the enabled probe. A later step may run exactly one `42x32` no-solve payload-footprint probe only after this readiness artifact is reviewed and accepted.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def _evaluate_readiness(
    *,
    ...
    s82 = _payload(loaded_inputs, "s82_execution")
    s80 = _payload(loaded_inputs, "s80_review")
    s83 = _payload(loaded_inputs, "s83_strategy")
    s84 = _payload(loaded_inputs, "s84_review_summary")
    s85 = _payload(loaded_inputs, "s85_implementation")
    agents_text = str(_mapping(loaded_inputs.get("agents")).get("text") or "")
    s80_interp = _mapping(s80.get("interpretation"))
    s82_safety = _mapping(s82.get("safety"))
    s83_interp = _mapping(s83.get("interpretation"))
    s85_impl = _mapping(s85.get("implementation"))
    s85_auth = _mapping(s85.get("authorization"))
    s85_safety = _mapping(s85.get("safety_flags"))
    s85_verification = _mapping(s85.get("verification"))
    checks = [
        _check(
            "s82_probe_completed_unstable_bounds",
            s82.get("status") == "completed"
            and s82.get("s80_classification") == "unstable_footprint_bounds_dominates"
            and s82.get("dominant_gap_reason") == "unstable_footprint_bounds_within_payload",
        ),
        _check(
            "s82_safety_clean",
            s82.get("execute_no_solve") is True
            and all(
                s82_safety.get(key) is False
                for key in (
            ...
            and _mapping(s82_safety.get("sensitive_path_comparison")).get("changed") is False,
        ),
        _check(
            "s80_review_completed_unstable_bounds",
            s80.get("status") == "completed"
            and s80_interp.get("classification") == "unstable_footprint_bounds_dominates",
        ),
        _check(
            "s83_strategy_completed_review_first",
            s83.get("status") == "completed"
            and s83_interp.get("classification") == "payload_footprint_stability_strategy_required"
            and s83.get("source_mutation_performed") is False
            and s83.get("review_required_before_authorization") is True,
        ),
        _check(
            "s84_review_passed_not_authorization",
            s84.get("review_verdict") == "pass"
            and s84.get("review_is_authorization") is False
            and s84.get("authorization_required_next") is True,
        ),
        _check(
            "s85_patch_implemented_and_verified",
            s85.get("status") == "implemented_and_verified"
            and (s85_impl.get("env_var") or s85_auth.get("env_var"))
            == PAYLOAD_FOOTPRINT_STABILITY_ENV_VAR
            and s85_safety.get("cp_solver_solve_called") is False
            and s85_safety.get("runtime_execution_performed") is False
            and s85_safety.get("checkpoint_written") is False
            and s85_safety.get("proof_source") is False
            and s85_safety.get("production_profile_changed") is False,
        ),
        _check(
            "s85_checkpoint_state_absent",
            s85_verification.get("canonical_checkpoint_state_exists") is False
            and s85_verification.get("canonical_checkpoint_telemetry_exists") is False,
        ),
        _check(
            "agents_gate_mentions_s85",
            "S85" in agents_text and PAYLOAD_FOOTPRINT_STABILITY_ENV_VAR in agents_text,
        ),
        _check(
    ...
    return {
        "classification": "ready_for_payload_footprint_probe_review" if ready else "manual_review_required",
        "checks": checks,
        "ready_for_future_probe_review": bool(ready),
        "future_probe_executable_now": False,
        "baseline_run_id": s82.get("run_id") or s80.get("run_id"),
        "baseline_model_build_seconds": _float(s82.get("model_build_seconds"))
        or _float(s80.get("model_build_seconds")),
        "baseline_mandatory_scan_seconds": _float(s82.get("current_mandatory_scan_seconds"))
        or _float(s80_interp.get("current_mandatory_scan_seconds")),
        "baseline_template_footprint_support_attempts": _int_or_none(
            s82.get("template_footprint_support_attempts")
        )
        or _int_or_none(s80_interp.get("template_footprint_support_attempts")),
        "baseline_template_footprint_support_used": _int_or_none(
            s82.get("template_footprint_support_used")
        )
        or _int_or_none(s80_interp.get("template_footprint_support_used")),
        "baseline_template_footprint_support_fallbacks": _int_or_none(
            s82.get("template_footprint_support_fallbacks")
        )
        or _int_or_none(s80_interp.get("template_footprint_support_fallbacks")),
        "baseline_unstable_footprint_bounds_fallbacks": _int_or_none(
            s82.get("dominant_gap_count")
        )
        or _int_or_none(s80_interp.get("dominant_gap_count")),
        "reason": (
            "S85 is verified and the single future 42x32 no-solve payload-footprint probe plan is bounded."
            if ready
            else "One or more S80-S85 inputs or command/sensitive-path guards are not ready."
        ),
    }
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        "readiness_json": output_dir / "signature_bucket_payload_footprint_probe_readiness.json",
        "readiness_md": output_dir / "signature_bucket_payload_footprint_probe_readiness.md",
...
def _assert_readiness_namespace(path: Path) -> None:
    ...
    if (
        ...
        or "86_signature_bucket_payload_footprint_probe_readiness" not in normalized
    ):
        raise ValueError(f"Refusing to write outside S86 readiness namespace: {path}")

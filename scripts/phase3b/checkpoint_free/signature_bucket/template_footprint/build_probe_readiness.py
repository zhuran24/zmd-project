...
DEFAULT_OUTPUT_DIR = ARTIFACT_ROOT / "72_signature_bucket_template_footprint_probe_readiness"
DEFAULT_INPUTS = {
...
FUTURE_RUN_ID = "local_hotspot_42x32_signature_bucket_template_footprint_inst_no_solve_001"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("phase3b signature bucket template-footprint probe readiness")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build S72 readiness artifacts for one future enabled 42x32 no-solve "
            "template-footprint support probe without executing it."
        )
    )
...
def build_signature_bucket_template_footprint_probe_readiness(
    *,
    project_root: Path,
    output_dir: Path,
    no_write: bool = False,
    ...
    status = (
        "completed"
        if readiness["classification"] == "ready_for_template_footprint_probe_review"
        else "manual_review_required"
    )
    paths = _paths(output_dir)
    payload = {
        "schema": "phase3b-signature-bucket-template-footprint-probe-readiness/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "readiness_kind": "future_enabled_signature_bucket_template_footprint_no_solve_probe_plan_only",
        ...
        "next_gate": {
            "status": "hold_for_readiness_review_before_template_footprint_probe",
            "s74_candidate_step": (
                "execute exactly one enabled 42x32 no-solve template-footprint support probe "
                "only after S72 readiness review accepts this plan"
            ),
            "blocked_actions": [
                "do_not_execute_enabled_probe_in_s72",
...
def build_future_command_template() -> dict[str, Any]:
    ...
    return {
        "schema": "phase3b-signature-bucket-template-footprint-future-command/v0",
        "command": command,
...
def render_readiness_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Phase3B S72 Signature Bucket Template-Footprint Probe Readiness",
        "",
        ...
        "## Baseline",
        "",
        f"- S68/S64 mandatory scan seconds: `{_fmt(readiness.get('baseline_mandatory_scan_seconds'))}`",
        f"- S68/S64 unsupported footprint fallbacks: `{readiness.get('baseline_unsupported_footprint_fallbacks')}`",
        "",
        "## Readiness Checks",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "## Gate",
            "",
            "S72 does not execute the enabled probe. A later step may run exactly one `42x32` no-solve template-footprint probe only after this readiness artifact is reviewed and accepted.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def _evaluate_readiness(
    *,
    ...
    s68 = _payload(loaded_inputs, "s68_execution")
    s64 = _payload(loaded_inputs, "s64_review")
    s69 = _payload(loaded_inputs, "s69_strategy")
    s70 = _payload(loaded_inputs, "s70_review_summary")
    s71 = _payload(loaded_inputs, "s71_implementation")
    agents_text = str(_mapping(loaded_inputs.get("agents")).get("text") or "")
    s64_interp = _mapping(s64.get("interpretation"))
    s64_instr = _mapping(s64.get("signature_instrumentation"))
    s64_totals = _mapping(s64_instr.get("totals"))
    s64_fallback_reasons = _mapping(s64_instr.get("fallback_reasons"))
    s68_review = _mapping(s68.get("s64_review"))
    s68_safety = _mapping(s68.get("safety"))
    s71_safety = _mapping(s71.get("safety"))
    checks = [
        _check(
            "s68_execution_completed_and_safety_clean",
            s68.get("status") == "completed"
            and s68.get("next_gate") == "prepare_template_footprint_support_strategy_or_review"
            and _s68_safety_clean(s68_safety),
        ),
        _check(
            "s64_baseline_unsupported_footprint_dominates",
            s64.get("status") == "completed"
            and s64_interp.get("classification") == "unsupported_footprint_dominates"
            and s64_interp.get("dominant_reason") == "unsupported_or_missing_template_footprint"
            and s64_instr.get("fallback_reason_visibility") == "fallback_reason_instrumentation_visible"
            and _float(s64_interp.get("mandatory_scan_seconds")) is not None
            and _int_or_none(s64_interp.get("fallback_reason_total")) is not None
            and _int_or_none(s64_totals.get("mandatory_region_counting_fallbacks")) is not None,
        ),
        _check(
            "s69_strategy_completed_and_review_first",
            s69.get("status") == "completed"
            and _mapping(s69.get("interpretation")).get("classification")
            == "unsupported_template_footprint_support_strategy_required"
            and s69.get("source_mutation_performed") is False
            and s69.get("review_required_before_authorization") is True,
        ),
        _check(
            "s70_review_passed_not_authorization",
            s70.get("review_verdict") == "pass"
            and s70.get("review_passed") is True
            and s70.get("review_is_authorization") is False
            and s70.get("authorization_required_next") is True,
        ),
        _check(
            "s71_patch_implemented_and_verified",
            s71.get("status") == "implemented_and_verified"
            and _mapping(s71.get("env_gate")).get("name") == TEMPLATE_FOOTPRINT_SUPPORT_ENV_VAR
            and s71_safety.get("source_model_mutation") is True
            and s71_safety.get("source_model_mutation_authorized") is True
            and s71_safety.get("cp_solver_solve_called") is False
            and s71_safety.get("runtime_execution_performed") is False
            and s71_safety.get("checkpoint_written") is False
            and s71_safety.get("proof_source") is False
            and s71_safety.get("production_profile_changed") is False,
        ),
        _check(
            "s71_checkpoint_state_absent",
            s71_safety.get("canonical_checkpoint_state_exists_before_after") is False
            and s71_safety.get("canonical_checkpoint_telemetry_exists_before_after") is False,
        ),
        _check(
            "agents_gate_mentions_s71",
            "Current S71 template-footprint support state" in agents_text
            or "Current S71 template-footprint support patch state" in agents_text
            or "S71 default-off template-footprint support patch" in agents_text,
        ),
        _check(
    ...
    baseline_unsupported = _int_or_none(
        s64_fallback_reasons.get("unsupported_or_missing_template_footprint")
    )
    return {
        "classification": "ready_for_template_footprint_probe_review" if ready else "manual_review_required",
        "checks": checks,
        "ready_for_future_probe_review": bool(ready),
        "future_probe_executable_now": False,
        "baseline_run_id": s68_review.get("run_id") or _mapping(s68.get("probe")).get("run_id"),
        "baseline_model_build_seconds": _float(_mapping(s68.get("probe")).get("model_build_seconds")),
        "baseline_signature_bucket_tightening_seconds": _float(
            _mapping(s68.get("probe")).get("signature_bucket_tightening_seconds")
        ),
        "baseline_mandatory_scan_seconds": _float(s64_interp.get("mandatory_scan_seconds")),
        "baseline_mandatory_region_counting_attempts": _int_or_none(
            s64_totals.get("mandatory_region_counting_attempts")
        )
        or _int_or_none(s68_review.get("mandatory_region_counting_attempts")),
        "baseline_mandatory_region_counting_used": _int_or_none(
            s64_totals.get("mandatory_region_counting_used")
        )
        or _int_or_none(s68_review.get("mandatory_region_counting_used")),
        "baseline_mandatory_region_counting_fallbacks": _int_or_none(
            s64_totals.get("mandatory_region_counting_fallbacks")
        )
        or _int_or_none(s68_review.get("mandatory_region_counting_fallbacks")),
        "baseline_unsupported_footprint_fallbacks": baseline_unsupported
        or _int_or_none(s64_interp.get("dominant_reason_count")),
        "reason": (
            "S71 is verified and the single future 42x32 no-solve template-footprint probe plan is bounded."
            if ready
            else "One or more S68-S71 inputs or command/sensitive-path guards are not ready."
        ),
    }
...
def _s68_safety_clean(safety: Mapping[str, Any]) -> bool:
    return (
        ...
        and safety.get("sensitive_path_schema") == "phase3b-sensitive-path-fingerprint-comparison/v0"
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        "readiness_json": output_dir / "signature_bucket_template_footprint_probe_readiness.json",
        "readiness_md": output_dir / "signature_bucket_template_footprint_probe_readiness.md",
...
def _assert_readiness_namespace(path: Path) -> None:
    ...
    if (
        ...
        or "72_signature_bucket_template_footprint_probe_readiness" not in normalized
    ):
        raise ValueError(f"Refusing to write outside S72 readiness namespace: {path}")

...
DEFAULT_RUN_ID = "s79_s80_support_gap_probe_review_001"
...
DEFAULT_INPUTS = {
    ...
    "s79_future_command": ARTIFACT_ROOT
    / "79_signature_bucket_template_footprint_support_gap_probe_readiness"
    ...
    / "scripts" / "phase3b" / "checkpoint_free" / "signature_bucket" / "template_footprint" / "build_support_gap_probe_readiness.py",
    "s80_builder": PROJECT_ROOT
    / "scripts" / "phase3b" / "checkpoint_free" / "signature_bucket" / "template_footprint" / "build_support_gap_probe_review.py",
    "s81_package_builder": PROJECT_ROOT
    / "scripts" / "phase3b" / "checkpoint_free" / "signature_bucket" / "template_footprint" / "build_support_gap_probe_external_review_package.py",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("phase3b signature bucket template-footprint support-gap probe external review package")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a clean-extraction-validated external review package for S79/S80 "
            "support-gap probe readiness and review tooling."
        )
    )
...
def build_signature_bucket_template_footprint_support_gap_probe_external_review_package(
    *,
    project_root: Path,
    output_dir: Path,
    ...
    payload = {
        "schema": "phase3b-signature-bucket-template-footprint-support-gap-probe-external-review-package/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "package_kind": "external_review_support_gap_probe_readiness_and_review_tooling_package",
...
def render_package_markdown(payload: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Phase3B S81 Support-Gap Probe External Review Package",
            "",
            ...
            "",
            "This package asks external review whether S79/S80 readiness and review tooling are safe to request authorization for exactly one future enabled `42x32` no-solve support-gap probe. It is not authorization.",
            "",
        ]
    )
...
def _review_request_text(
    ...
    *,
    ...
    return "\n".join(
        [
            "# Review Request: Phase3B S79/S80 Support-Gap Probe Readiness",
            "",
            ...
            "",
            "Context: S78 implemented default-off support-gap diagnostics under `EXACT_GHOST_SIGNATURE_BUCKET_TEMPLATE_FOOTPRINT_SUPPORT_GAP_INSTRUMENTATION`. S79 prepares exactly one future enabled `42x32` no-solve probe with five env gates enabled. S80 is fail-closed review tooling that will classify the dominant support-gap reason if the future probe is later authorized and executed.",
            "",
            "Please review these questions:",
            "",
            "1. Is the S79 future command sufficiently narrow: exactly one `42x32` no-solve overlay timing probe with fixed run id and the five required env gates?",
            "2. Are the S79 exact command-vector and forbidden-token guards strict enough to prevent runtime solve, checkpoint/proof/release/viewer/frontdoor drift, duplicate flags, or non-`42x32` targets?",
            "3. Is the S80 fail-closed schema conservative enough for strict sensitive-path schema, hard-boundary flags, run-id mismatch, missing timing, missing support-gap totals, and missing support-gap visibility?",
            "4. Are the S80 dominant-reason classifications appropriate: non-rectangular occupied cells, unstable footprint bounds, missing template/group metadata, missing bucket metadata, same-bucket region overlap, guard rejected, legacy-other, missing instrumentation, inconclusive, or safety-disqualified?",
            "5. If review passes, is it safe to request user/project-owner authorization for exactly one future enabled `42x32` no-solve support-gap probe, still with no runtime solve, no canonical checkpoints, no proof/preflight/release/viewer/frontdoor mutation, and no production default change?",
            "",
            "If there are no blockers, please state only that this single future enabled no-solve support-gap probe scope is safe to request user/project-owner authorization; do not treat review approval as authorization.",
            "",
...
def _manifest(
    *,
    ...
    return {
        "schema": "phase3b-signature-bucket-template-footprint-support-gap-probe-external-review-package-manifest/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "run_id": str(run_id),
        "zip_path": str(zip_path),
        "zip_sha256": None,
...
def _write_staging_files(
    *,
    ...
    (staging_dir / "README.md").write_text(
        (
            "This package is for Phase3B / Endfield external review of S79/S80 "
            "support-gap probe readiness and review tooling. It needs review first; "
...
def _validate_clean_extraction(
    *,
    ...
    s77 = _load_json(extract_dir / "evidence/s77_external_review_reply_summary.json")
    s78 = _load_json(extract_dir / "evidence/s78_signature_bucket_template_footprint_support_gap_instrumentation_implementation.json")
    s79 = _load_json(extract_dir / "evidence/s79_signature_bucket_template_footprint_support_gap_probe_readiness.json")
    request_text = (extract_dir / "review_request.md").read_text(encoding="utf-8")
    s80_builder_text = (
        extract_dir
        / "code_context/scripts/phase3b/checkpoint_free/signature_bucket/template_footprint/build_support_gap_probe_review.py"
    ...
    semantic_checks = {
        ...
        "s77_review_passed_not_authorization": s77.get("review_verdict") == "pass"
        and s77.get("review_is_authorization") is False,
        "s78_implemented_and_verified": s78.get("status") == "implemented_and_verified",
        "s79_ready": _mapping(s79.get("readiness")).get("classification")
        == "ready_for_support_gap_probe_review",
        "s79_probe_execution_disabled": s79.get("probe_execution_enabled") is False,
        "s80_fail_closed_classes_present": "support_gap_probe_inconclusive" in s80_builder_text
        and "safety_disqualified" in s80_builder_text
        and "support_gap_instrumentation_missing" in s80_builder_text,
...
def _required_entries() -> list[str]:
    return [
        ...
        "evidence/s77_external_review_reply_summary.json",
        "evidence/s77_external_review_reply_raw.md",
        ...
        "evidence/s79_future_command_template.json",
        "coordination/agents_s75_s79_gate_excerpt.md",
        ...
        "code_context/scripts/phase3b/checkpoint_free/signature_bucket/template_footprint/build_support_gap_probe_readiness.py",
        "code_context/scripts/phase3b/checkpoint_free/signature_bucket/template_footprint/build_support_gap_probe_review.py",
        "code_context/scripts/phase3b/checkpoint_free/signature_bucket/template_footprint/build_support_gap_probe_external_review_package.py",
        "test_context/src/tests/phase3b/checkpoint_free/signature_bucket/template_footprint/test_support_gap_probe_readiness.py",
        "test_context/src/tests/phase3b/checkpoint_free/signature_bucket/template_footprint/test_support_gap_probe_review.py",
        "test_context/src/tests/phase3b/checkpoint_free/signature_bucket/template_footprint/test_support_gap_probe_external_review_package.py",
    ]
...
def _package_path_for_input(key: str) -> Path:
    mapping = {
        ...
        "s77_review_summary": Path("evidence/s77_external_review_reply_summary.json"),
        "s77_review_raw": Path("evidence/s77_external_review_reply_raw.md"),
        ...
        "s79_future_command": Path("evidence/s79_future_command_template.json"),
        "agents": Path("coordination/agents_s75_s79_gate_excerpt.md"),
...
def _agents_gate_excerpt(path: Path) -> str:
    ...
    markers = [
        ...
        "Current S77 external review result and S78",
    ...
    if not lines:
        raise ValueError(f"Could not find S75-S79/review gate excerpt in {path}")
...
def _assert_package_namespace(path: Path) -> None:
    ...
    if (
        ...
        or "81_signature_bucket_template_footprint_support_gap_probe_external_review_package"
        not in normalized
    ):
        raise ValueError(f"Refusing to write outside S81 external review package namespace: {path}")

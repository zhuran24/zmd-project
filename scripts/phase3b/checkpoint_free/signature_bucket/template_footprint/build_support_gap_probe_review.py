...
DEFAULT_OUTPUT_DIR = ARTIFACT_ROOT / "80_signature_bucket_template_footprint_support_gap_probe_review"
EXPECTED_RUN_ID = "local_hotspot_42x32_signature_bucket_template_footprint_support_gap_inst_no_solve_001"
...
DOMINANT_REASON_CLASSIFICATIONS = {
    "non_rectangular_occupied_cells": "non_rectangular_occupied_cells_dominates",
    "unstable_footprint_bounds_within_payload": "unstable_footprint_bounds_dominates",
    "missing_template_or_group_metadata": "missing_template_or_group_metadata_dominates",
    "bucket_region_metadata_missing": "bucket_region_metadata_missing_dominates",
    "same_bucket_regions_overlap": "same_bucket_region_overlap_dominates",
    "region_counting_guard_rejected": "region_counting_guard_rejected_dominates",
    "legacy_scan_required_other": "legacy_scan_other_dominates",
}


def main(argv: Sequence[str] | None = None) -> int:
    args = _parse_args(argv)
    review = build_signature_bucket_template_footprint_support_gap_probe_review(
    ...
    print("phase3b signature bucket template-footprint support-gap probe review")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Review the future S79-enabled 42x32 no-solve support-gap probe and "
            "classify why template-footprint support still rejects payloads."
        )
    )
...
def build_signature_bucket_template_footprint_support_gap_probe_review(
    *,
    project_root: Path,
    readiness_path: Path,
    probe_path: Path,
    ...
    payload = {
        "schema": "phase3b-signature-bucket-template-footprint-support-gap-probe-review/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "review_kind": "enabled_signature_bucket_template_footprint_support_gap_no_solve_probe_review",
...
def render_probe_review_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Phase3B S80 Signature Bucket Template-Footprint Support-Gap Probe Review",
        "",
        f"- Status: `{payload.get('status')}`",
        f"- Classification: `{interpretation.get('classification')}`",
        f"- Dominant gap reason: `{interpretation.get('dominant_gap_reason')}`",
        f"- Dominant gap ratio: `{_fmt(interpretation.get('dominant_gap_ratio'))}`",
...
def _signature_instrumentation_summary(probe: Mapping[str, Any]) -> dict[str, Any]:
    ...
    top_entries = instrumentation.get("top_template_footprint_gap_entries")
    visible = isinstance(gap_reasons, Mapping) and isinstance(top_entries, list)
    return {
        "present": True,
        "support_gap_visibility": (
            "support_gap_instrumentation_visible"
            if visible
            else "support_gap_instrumentation_missing"
        ),
        "path": SIGNATURE_STATS_PATH,
        ...
        "top_support_gap_entries": list(top_entries or [])[:10] if isinstance(top_entries, list) else [],
    }
...
def _interpretation(
    *,
    ...
    readiness_ok = (
        ...
        and readiness_data.get("classification") == "ready_for_support_gap_probe_review"
    )
    safety_ok = all(
    ...
    if not readiness_ok:
        return {
            "classification": "support_gap_probe_inconclusive",
            "reason": "S79 readiness is missing or not accepted for this probe.",
    ...
    if (
        ...
        return {
            "classification": "support_gap_probe_inconclusive",
            "reason": "Timing, template-footprint totals, or support-gap reason counts are missing.",
    ...
    if reason_total <= 0:
        return {
            "classification": "support_gap_probe_inconclusive",
            "reason": "Support-gap instrumentation is visible but did not record any reasons.",
            "next_engineering_step": "manual_review_support_gap_empty_output",
        }
    dominant_reason, dominant_count = max(
        ((str(reason), int(count)) for reason, count in reasons.items()),
        key=lambda item: (item[1], item[0]),
    )
    dominant_ratio = dominant_count / reason_total
    classification = (
        DOMINANT_REASON_CLASSIFICATIONS.get(dominant_reason, "support_gap_probe_inconclusive")
        if dominant_ratio >= 0.5
        else "support_gap_probe_inconclusive"
    )
    return {
        "classification": classification,
        "reason": (
            f"Dominant support-gap reason is {dominant_reason} "
            f"({dominant_count}/{reason_total})."
        ),
        "dominant_gap_reason": dominant_reason,
        "dominant_gap_count": dominant_count,
        "dominant_gap_ratio": dominant_ratio,
        ...
        "mandatory_scan_reduction_ratio": _ratio_reduction(baseline_scan, current_scan),
...
def _next_step_for_classification(classification: str) -> str:
    if classification == "non_rectangular_occupied_cells_dominates":
        return "prepare_non_rectangular_footprint_strategy_or_review"
    if classification == "unstable_footprint_bounds_dominates":
        return "prepare_payload_footprint_stability_strategy_or_review"
    if classification == "missing_template_or_group_metadata_dominates":
        return "prepare_template_metadata_strategy_or_review"
    if classification == "bucket_region_metadata_missing_dominates":
        return "prepare_bucket_region_metadata_strategy_or_review"
    if classification == "same_bucket_region_overlap_dominates":
        return "prepare_region_overlap_guard_strategy_or_review"
    if classification == "region_counting_guard_rejected_dominates":
        return "prepare_region_counting_guard_strategy_or_review"
    if classification == "legacy_scan_other_dominates":
        return "manual_review_legacy_scan_other_before_more_work"
    if classification == "support_gap_instrumentation_missing":
        return "return_to_support_gap_visibility_debug"
...
def _next_gate(interpretation: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "status": (
            "hold_for_support_gap_specific_strategy"
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        "review_json": output_dir
        / "signature_bucket_template_footprint_support_gap_probe_review.json",
        "review_md": output_dir
        / "signature_bucket_template_footprint_support_gap_probe_review.md",
...
def _assert_review_namespace(path: Path) -> None:
    ...
    if (
        ...
        or "80_signature_bucket_template_footprint_support_gap_probe_review"
        not in normalized
    ):
        raise ValueError(f"Refusing to write outside S80 probe review namespace: {path}")

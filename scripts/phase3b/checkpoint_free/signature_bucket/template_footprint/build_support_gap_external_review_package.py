...
DEFAULT_OUTPUT_DIR = ARTIFACT_ROOT / "77_signature_bucket_template_footprint_support_gap_external_review_package"
DEFAULT_RUN_ID = "s76_template_footprint_support_gap_review_001"

DEFAULT_INPUTS = {
    "s74_review_summary": ARTIFACT_ROOT
    / "74_signature_bucket_template_footprint_probe_external_review_package"
    / "s72_s73_template_footprint_probe_review_001"
    / "external_review_reply_summary.json",
    "s74_review_raw": ARTIFACT_ROOT
    / "74_signature_bucket_template_footprint_probe_external_review_package"
    / "s72_s73_template_footprint_probe_review_001"
    ...
    "s71_probe_readiness_builder": PROJECT_ROOT
    / "scripts" / "phase3b" / "checkpoint_free" / "signature_bucket" / "template_footprint" / "build_probe_readiness.py",
    "s73_probe_review_builder": PROJECT_ROOT
    / "scripts" / "phase3b" / "checkpoint_free" / "signature_bucket" / "template_footprint" / "build_probe_review.py",
    "s74_package_builder": PROJECT_ROOT
    / "scripts" / "phase3b" / "checkpoint_free" / "signature_bucket" / "template_footprint" / "build_probe_external_review_package.py",
    "s76_strategy_builder": PROJECT_ROOT
    / "scripts" / "phase3b" / "checkpoint_free" / "signature_bucket" / "template_footprint" / "build_support_gap_strategy.py",
    "s77_package_builder": PROJECT_ROOT
    / "scripts" / "phase3b" / "checkpoint_free" / "signature_bucket" / "template_footprint" / "build_support_gap_external_review_package.py",
    "s71_tests": PROJECT_ROOT
    / "src"
    / "tests"
    ...
    "s74_tests": PROJECT_ROOT
    / "src"
    / "tests"
    / "test_phase3b_checkpoint_free_signature_bucket_template_footprint_probe_external_review_package.py",
    "s76_tests": PROJECT_ROOT
    / "src"
    / "tests"
    / "test_phase3b_checkpoint_free_signature_bucket_template_footprint_support_gap_strategy.py",
    "s77_tests": PROJECT_ROOT
    / "src"
    / "tests"
    / "test_phase3b_checkpoint_free_signature_bucket_template_footprint_support_gap_external_review_package.py",
}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("phase3b signature bucket template-footprint support gap external review package")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a clean-extraction-validated external review package for the S76 "
            "template-footprint support gap strategy."
        )
    )
...
def build_signature_bucket_template_footprint_support_gap_external_review_package(
    *,
    project_root: Path,
    output_dir: Path,
    ...
    payload = {
        "schema": "phase3b-signature-bucket-template-footprint-support-gap-external-review-package/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "package_kind": "external_review_template_footprint_support_gap_strategy_package",
...
def render_package_markdown(payload: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Phase3B S77 Template-Footprint Support Gap External Review Package",
            "",
            ...
            "",
            "This package asks external review whether the S76 support-gap diagnostic scope is safe to request authorization. It is not authorization.",
            "",
        ]
    )
...
def _review_request_text(
    ...
    *,
    ...
    return "\n".join(
        [
            "# Review Request: Phase3B S76/S77 Template-Footprint Support Gap Strategy",
            "",
            ...
            "",
            "Status: needs review first. This review is not authorization. If review passes request user/project-owner authorization before implementing any default-off support-gap diagnostic patch.",
            "",
            "Gate marker for automated checks: if review passes request user/project-owner authorization.",
            "",
            "Context: S75 safely ran the single reviewed 42x32 no-solve template-footprint probe. The probe was clean, but S73 classified it as `template_footprint_support_not_used`: support attempts were recorded, support used was zero, unsupported footprint fallbacks remained unchanged, and mandatory scan time did not materially improve. S76 turns that into a review-first strategy for a future default-off support-gap diagnostic, not an immediate source patch.",
            "",
            "Please review these questions:",
            "",
            "1. Is S75/S73 sufficient evidence that template-footprint support was enabled but never used on the real `42x32` overlay?",
            "2. Is it correct to stop before another probe or source patch and instead review a default-off diagnostic that explains why support was not used?",
            "3. Is the proposed env `EXACT_GHOST_SIGNATURE_BUCKET_TEMPLATE_FOOTPRINT_SUPPORT_GAP_INSTRUMENTATION` narrow enough if it only records support rejection categories under existing `signature_tightening_instrumentation`?",
            "4. Are the proposed categories useful and safe: missing occupied cells, empty occupied cells, non-rectangular occupied cells, unstable bounds within payload, missing template/group metadata, missing bucket metadata, same-bucket overlap, guard rejected, and legacy-other?",
            "5. Does the proposal preserve ModelProto, variables, constraints, candidate order, scheduler/proof/checkpoint inputs, runtime behavior, and production defaults?",
            "6. If review passes, is it safe to request user/project-owner authorization for only this future default-off support-gap diagnostic patch?",
            "",
            "If there are no blockers, please state only that this default-off support-gap diagnostic scope is safe to request user/project-owner authorization; do not treat review approval as authorization.",
            "",
...
def _manifest(
    *,
    ...
    return {
        "schema": "phase3b-signature-bucket-template-footprint-support-gap-external-review-package-manifest/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "run_id": str(run_id),
        "zip_path": str(zip_path),
        "zip_sha256": None,
...
def _write_staging_files(
    *,
    ...
    (staging_dir / "README.md").write_text(
        (
            "This package is for Phase3B / Endfield external review of the S76 "
            "template-footprint support gap strategy. It needs review first; "
            "review is not authorization; if review passes request user/project-owner "
            "authorization. It is not proof evidence and does not execute a probe or "
            "mutate source.\n"
        ),
        encoding="utf-8",
    )
...
def _validate_clean_extraction(
    *,
    ...
    s74 = _load_json(extract_dir / "evidence/s74_external_review_reply_summary.json")
    s75 = _load_json(extract_dir / "evidence/s75_signature_bucket_template_footprint_probe_execution.json")
    s73 = _load_json(extract_dir / "evidence/s73_signature_bucket_template_footprint_probe_review.json")
    s76 = _load_json(extract_dir / "evidence/s76_signature_bucket_template_footprint_support_gap_strategy.json")
    request_text = (extract_dir / "review_request.md").read_text(encoding="utf-8")
    source_text = (
        extract_dir / "source_context/src/models/exact_coordinate_master.py"
    ...
    semantic_checks = {
        ...
        "s74_review_passed_not_authorization": s74.get("review_verdict") == "pass"
        and s74.get("review_is_authorization") is False,
        "s75_completed_not_used": s75.get("status") == "completed"
        and s75.get("s73_classification") == "template_footprint_support_not_used",
        "s73_completed_not_used": s73.get("status") == "completed"
        and _mapping(s73.get("interpretation")).get("classification")
        == "template_footprint_support_not_used",
        "s76_completed": s76.get("status") == "completed"
        and _mapping(s76.get("interpretation")).get("classification")
        == "template_footprint_support_not_used_strategy_required",
        "source_context_full_enough": "EXACT_GHOST_SIGNATURE_BUCKET_TEMPLATE_FOOTPRINT_SUPPORT" in source_text
        and "mandatory_template_footprint_support_used" in source_text,
        "s76_builder_context_has_future_env": "EXACT_GHOST_SIGNATURE_BUCKET_TEMPLATE_FOOTPRINT_SUPPORT_GAP_INSTRUMENTATION"
        in s76_builder_text,
...
def _required_entries() -> list[str]:
    return [
        ...
        "review_request.md",
        "manifest.json",
        "evidence/s74_external_review_reply_summary.json",
        "evidence/s74_external_review_reply_raw.md",
        "evidence/s75_signature_bucket_template_footprint_probe_execution.json",
        "evidence/s73_signature_bucket_template_footprint_probe_review.json",
        ...
        "coordination/agents_s74_s76_gate_excerpt.md",
        ...
        "code_context/scripts/phase3b/checkpoint_free/signature_bucket/template_footprint/build_probe_readiness.py",
        "code_context/scripts/phase3b/checkpoint_free/signature_bucket/template_footprint/build_probe_review.py",
        "code_context/scripts/phase3b/checkpoint_free/signature_bucket/template_footprint/build_probe_external_review_package.py",
        "code_context/scripts/phase3b/checkpoint_free/signature_bucket/template_footprint/build_support_gap_strategy.py",
        "code_context/scripts/phase3b/checkpoint_free/signature_bucket/template_footprint/build_support_gap_external_review_package.py",
        "test_context/src/tests/phase3b/checkpoint_free/signature_bucket/template_footprint/test_probe_readiness.py",
        "test_context/src/tests/phase3b/checkpoint_free/signature_bucket/template_footprint/test_probe_review.py",
        "test_context/src/tests/phase3b/checkpoint_free/signature_bucket/template_footprint/test_probe_external_review_package.py",
        "test_context/src/tests/phase3b/checkpoint_free/signature_bucket/template_footprint/test_support_gap_strategy.py",
        "test_context/src/tests/phase3b/checkpoint_free/signature_bucket/template_footprint/test_support_gap_external_review_package.py",
    ]


def _package_path_for_input(key: str) -> Path:
    mapping = {
        "s74_review_summary": Path("evidence/s74_external_review_reply_summary.json"),
        "s74_review_raw": Path("evidence/s74_external_review_reply_raw.md"),
        "s75_execution": Path("evidence/s75_signature_bucket_template_footprint_probe_execution.json"),
        "s73_review": Path("evidence/s73_signature_bucket_template_footprint_probe_review.json"),
        "s71_implementation": Path("evidence/s71_signature_bucket_template_footprint_support_implementation.json"),
        "s76_strategy": Path("evidence/s76_signature_bucket_template_footprint_support_gap_strategy.json"),
        "agents": Path("coordination/agents_s74_s76_gate_excerpt.md"),
        ...
        "s71_probe_readiness_builder": Path(
            "code_context/scripts/phase3b/checkpoint_free/signature_bucket/template_footprint/build_probe_readiness.py"
        ),
        "s73_probe_review_builder": Path(
            "code_context/scripts/phase3b/checkpoint_free/signature_bucket/template_footprint/build_probe_review.py"
        ),
        "s74_package_builder": Path(
            "code_context/scripts/phase3b/checkpoint_free/signature_bucket/template_footprint/build_probe_external_review_package.py"
        ),
        "s76_strategy_builder": Path(
            "code_context/scripts/phase3b/checkpoint_free/signature_bucket/template_footprint/build_support_gap_strategy.py"
        ),
        "s77_package_builder": Path(
            "code_context/scripts/phase3b/checkpoint_free/signature_bucket/template_footprint/build_support_gap_external_review_package.py"
        ),
        "s71_tests": Path(
            "test_context/src/tests/phase3b/checkpoint_free/signature_bucket/template_footprint/test_probe_readiness.py"
...
def _agents_gate_excerpt(path: Path) -> str:
    ...
    if not lines:
        raise ValueError(f"Could not find S74-S76/review gate excerpt in {path}")
...
def _assert_package_namespace(path: Path) -> None:
    ...
    if (
        ...
        or "77_signature_bucket_template_footprint_support_gap_external_review_package"
        not in normalized
    ):
        raise ValueError(f"Refusing to write outside S77 external review package namespace: {path}")

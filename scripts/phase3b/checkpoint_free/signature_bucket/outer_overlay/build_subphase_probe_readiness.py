...
DEFAULT_INPUTS = {
    ...
    "s101_review_summary": ARTIFACT_ROOT
    ...
    "s102_implementation": ARTIFACT_ROOT
...
FUTURE_RUN_ID = "local_hotspot_42x32_signature_bucket_outer_overlay_subphase_inst_no_solve_001"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("phase3b signature bucket outer-overlay subphase probe readiness")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build S103 readiness artifacts for one future enabled 42x32 no-solve "
            "outer-overlay subphase instrumentation probe without executing it."
        )
    )
...
def build_signature_bucket_outer_overlay_subphase_probe_readiness(
    *,
    project_root: Path,
    output_dir: Path,
    no_write: bool = False,
    ...
    status = (
        "completed"
        if readiness["classification"] == "ready_for_outer_overlay_subphase_probe_review"
        else "manual_review_required"
    )
    paths = _paths(output_dir)
    payload = {
        "schema": "phase3b-signature-bucket-outer-overlay-subphase-probe-readiness/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "readiness_kind": "future_enabled_signature_bucket_outer_overlay_subphase_no_solve_probe_plan_only",
        ...
        "next_gate": {
            "status": "hold_for_readiness_review_before_outer_overlay_subphase_probe",
            "candidate_step": (
                "execute exactly one enabled 42x32 no-solve outer-overlay subphase "
                "probe only after S103 readiness review accepts this plan"
            ),
            "blocked_actions": [
                "do_not_execute_enabled_probe_in_s103",
...
def build_future_command_template() -> dict[str, Any]:
    ...
    return {
        "schema": "phase3b-signature-bucket-outer-overlay-subphase-future-command/v0",
        "command": command,
...
def render_readiness_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Phase3B S103 Outer Overlay Subphase Probe Readiness",
        "",
...
def _evaluate_readiness(
    *,
    ...
    checks = [
        _check("s99_probe_completed", _input_value(loaded_inputs, "s99_execution", "status") == "completed"),
        _check(
            "s97_outer_residual_hotspot",
            _nested_value(loaded_inputs, "s97_review", "interpretation", "classification")
            == "outer_exact_core_overlay_residual_hotspot",
        ),
        _check(
            "s100_subphase_strategy_ready",
            _input_value(loaded_inputs, "s100_strategy", "classification")
            == "outer_exact_core_overlay_residual_subphase_strategy_required",
        ),
        _check(
            "s101_review_passed",
            _input_value(loaded_inputs, "s101_review_summary", "review_verdict") == "pass",
        ),
        _check(
            "s102_patch_implemented_and_verified",
            _input_value(loaded_inputs, "s102_implementation", "status")
    ...
    return {
        ...
        "checks": checks,
        "s99_classification": _input_value(loaded_inputs, "s99_execution", "classification"),
        "s100_classification": _input_value(loaded_inputs, "s100_strategy", "classification"),
        "s102_enabled_fields": _input_value(loaded_inputs, "s102_implementation", "enabled_fields"),
    }
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        "readiness_json": output_dir / "signature_bucket_outer_overlay_subphase_probe_readiness.json",
        "readiness_md": output_dir / "signature_bucket_outer_overlay_subphase_probe_readiness.md",
...
def _assert_readiness_namespace(output_dir: Path) -> None:
    ...
    if "103_signature_bucket_outer_overlay_subphase_probe_readiness" not in normalized:
        raise ValueError("S103 readiness namespace required")

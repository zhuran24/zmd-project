...
DEFAULT_OUTPUT_DIR = ARTIFACT_ROOT / "145_signature_bucket_powered_support_compact_item_detail_probe_readiness"
DEFAULT_INPUTS = {
...
COMPACT_ITEM_DETAIL_INSTRUMENTATION_ENV_VAR = (
    "EXACT_GHOST_SIGNATURE_BUCKET_POWERED_SUPPORT_COMPACT_ITEM_DETAIL_INSTRUMENTATION"
)

FUTURE_RUN_ID = "local_hotspot_42x32_signature_bucket_compact_item_detail_inst_no_solve_001"
...
REQUIRED_ENVIRONMENT = {
    ...
    COMPACT_ITEM_DETAIL_INSTRUMENTATION_ENV_VAR: "1",
}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    readiness = build_signature_bucket_powered_support_compact_item_detail_probe_readiness(
        project_root=PROJECT_ROOT,
        output_dir=_resolve_path(PROJECT_ROOT, args.output_dir),
        no_write=bool(args.no_write),
    )
    print("phase3b signature bucket powered support compact-item detail probe readiness")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build S145 readiness artifacts for one future enabled 42x32 no-solve "
            "powered support compact-item detail instrumentation probe without executing it."
        )
    )
...
def build_signature_bucket_powered_support_compact_item_detail_probe_readiness(
    *,
    project_root: Path,
    output_dir: Path,
    no_write: bool = False,
    ...
    status = (
        "completed"
        if readiness["classification"] == "ready_for_powered_support_compact_item_detail_probe_review"
        else "manual_review_required"
    )
    paths = _paths(output_dir)
    payload = {
        "schema": "phase3b-signature-bucket-powered-support-compact-item-detail-probe-readiness/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "readiness_kind": "future_enabled_signature_bucket_powered_support_compact_item_detail_no_solve_probe_plan_only",
        ...
        "next_gate": {
            "status": "hold_for_readiness_review_before_powered_support_compact_item_detail_probe",
            "candidate_step": (
                "execute exactly one enabled 42x32 no-solve powered support compact-item detail "
                "probe only after S145 readiness review accepts this plan"
            ),
            "blocked_actions": [
                "do_not_execute_enabled_probe_in_S145",
...
def build_future_command_template() -> dict[str, Any]:
    ...
    return {
        "schema": "phase3b-signature-bucket-powered-support-compact-item-detail-future-command/v0",
        "command": command,
...
def render_readiness_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Phase3B S145 Powered Support Compact-Item detail Probe Readiness",
        "",
...
def _evaluate_readiness(
    *,
    ...
    checks = [
        _check(
            "s140_probe_review_completed",
            _input_value(loaded_inputs, "s140_probe_review", "status") == "completed",
        ),
        _check(
            "s140_probe_safety_clean",
            _nested_value(loaded_inputs, "s140_probe_review", "probe_safety", "sensitive_path_clean")
            is True
            and _input_value(loaded_inputs, "s140_probe_review", "checkpoint_written") is False
            and _input_value(loaded_inputs, "s140_probe_review", "proof_source") is False,
        ),
        _check(
            "s140_compact_item_still_hot",
            _nested_value(loaded_inputs, "s140_probe_review", "interpretation", "classification")
            == "compact_item_accumulation_still_hot",
        ),
        _check(
            "s143_review_passed",
            _input_value(loaded_inputs, "s143_review_summary", "review_verdict") == "pass",
        ),
        _check(
            "s143_review_not_authorization",
            _input_value(loaded_inputs, "s143_review_summary", "review_is_authorization") is False,
        ),
        _check(
            "s143_authorization_required_next",
            _input_value(loaded_inputs, "s143_review_summary", "authorization_required_next") is True,
        ),
        _check(
            "s144_patch_implemented_and_verified",
            _input_value(loaded_inputs, "s144_implementation", "status")
            == "implemented_and_verified",
        ),
        _check(
            "s144_detail_env_matches",
            _input_value(loaded_inputs, "s144_implementation", "env_var")
            == COMPACT_ITEM_DETAIL_INSTRUMENTATION_ENV_VAR,
        ),
    ...
    classification = (
        "ready_for_powered_support_compact_item_detail_probe_review"
    ...
    return {
        ...
        "checks": checks,
        "s140_classification": _nested_value(
            loaded_inputs,
            "s140_probe_review",
            "interpretation",
            "classification",
        ),
        "s143_review_verdict": _input_value(loaded_inputs, "s143_review_summary", "review_verdict"),
        "s144_env_var": _input_value(loaded_inputs, "s144_implementation", "env_var"),
        "s140_dominant_phase": _nested_value(
            loaded_inputs,
            "s140_probe_review",
            "interpretation",
            "dominant_phase",
        ),
        "s140_baseline_compact_item_seconds": _nested_value(
            loaded_inputs,
            "s140_probe_review",
            "interpretation",
            "baseline_compact_item_seconds",
        ),
        "s140_current_compact_item_seconds": _nested_value(
            loaded_inputs,
            "s140_probe_review",
            "interpretation",
            "current_compact_item_seconds",
        ),
        "s140_phase_seconds": _mapping(
            _nested_value(loaded_inputs, "s140_probe_review", "interpretation", "phase_seconds")
        ),
        "s140_totals": _mapping(
            _nested_value(loaded_inputs, "s140_probe_review", "subphase_summary", "totals")
        ),
    }
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        "readiness_json": output_dir / "signature_bucket_powered_support_compact_item_detail_probe_readiness.json",
        "readiness_md": output_dir / "signature_bucket_powered_support_compact_item_detail_probe_readiness.md",
...
def _assert_readiness_namespace(output_dir: Path) -> None:
    ...
    if "145_signature_bucket_powered_support_compact_item_detail_probe_readiness" not in normalized:
        raise ValueError("S145 readiness namespace required")

...
DEFAULT_OUTPUT_DIR = ARTIFACT_ROOT / "132_signature_bucket_powered_support_compact_item_probe_readiness"
DEFAULT_INPUTS = {
    "s128_execution": ARTIFACT_ROOT
    ...
    "s126_review": ARTIFACT_ROOT
    / "126_signature_bucket_powered_support_coverer_probe_review"
    / "signature_bucket_powered_support_coverer_probe_review.json",
    "s130_review_summary": ARTIFACT_ROOT
    / "130_signature_bucket_powered_support_coverer_compact_item_external_review_package"
    / "s128_compact_item_accumulation_review_001"
    / "external_review_reply_summary.json",
    "s131_implementation": ARTIFACT_ROOT
    / "131_signature_bucket_powered_support_compact_item_optimization_implementation"
    / "signature_bucket_powered_support_compact_item_optimization_implementation.json",
...
FUTURE_RUN_ID = "local_hotspot_42x32_signature_bucket_compact_item_opt_inst_no_solve_001"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("phase3b signature bucket powered support compact-item probe readiness")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build S132 readiness artifacts for one future enabled 42x32 no-solve "
            "powered support compact-item optimization probe without executing it."
        )
    )
...
def build_signature_bucket_powered_support_compact_item_probe_readiness(
    *,
    project_root: Path,
    output_dir: Path,
    no_write: bool = False,
    ...
    status = (
        "completed"
        if readiness["classification"] == "ready_for_powered_support_compact_item_probe_review"
        else "manual_review_required"
    )
    paths = _paths(output_dir)
    payload = {
        "schema": "phase3b-signature-bucket-powered-support-compact-item-probe-readiness/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "readiness_kind": "future_enabled_signature_bucket_powered_support_compact_item_no_solve_probe_plan_only",
        ...
        "next_gate": {
            "status": "hold_for_readiness_review_before_powered_support_compact_item_probe",
            "candidate_step": (
                "execute exactly one enabled 42x32 no-solve powered support compact-item "
                "probe only after S132 readiness review accepts this plan"
            ),
            "blocked_actions": [
                "do_not_execute_enabled_probe_in_s132",
...
def build_future_command_template() -> dict[str, Any]:
    ...
    return {
        "schema": "phase3b-signature-bucket-powered-support-compact-item-future-command/v0",
        "command": command,
...
def render_readiness_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Phase3B S132 Powered Support Compact-Item Probe Readiness",
        "",
...
def _evaluate_readiness(
    *,
    ...
    checks = [
        _check(
            "s128_probe_completed",
            str(_input_value(loaded_inputs, "s128_execution", "status")).startswith("completed"),
        ),
        _check(
            "s128_probe_clean",
            _nested_value(loaded_inputs, "s128_execution", "probe_safety", "sensitive_path_clean")
            is True,
        ),
        _check(
            "s126_compact_item_hotspot",
            _nested_value(loaded_inputs, "s126_review", "interpretation", "classification")
            == "compact_item_accumulation_hotspot",
        ),
        _check(
            "s130_review_passed",
            _input_value(loaded_inputs, "s130_review_summary", "review_verdict") == "pass",
        ),
        _check(
            "s130_review_not_authorization",
            _input_value(loaded_inputs, "s130_review_summary", "review_is_authorization") is False,
        ),
        _check(
            "s131_patch_implemented_and_verified",
            _input_value(loaded_inputs, "s131_implementation", "status")
            == "implemented_and_verified",
        ),
        _check(
            "s131_compact_item_env_matches",
            _input_value(loaded_inputs, "s131_implementation", "env_var")
            == COMPACT_ITEM_OPTIMIZATION_ENV_VAR,
        ),
    ...
    return {
        ...
        "checks": checks,
        "s126_classification": _nested_value(
            loaded_inputs,
            "s126_review",
            "interpretation",
            "classification",
        ),
        "s130_review_verdict": _input_value(loaded_inputs, "s130_review_summary", "review_verdict"),
        "s131_env_var": _input_value(loaded_inputs, "s131_implementation", "env_var"),
        "s128_baseline_dominant_phase": _input_value(loaded_inputs, "s128_execution", "dominant_phase"),
        "s128_baseline_compact_item_seconds": _input_value(
            loaded_inputs,
            "s128_execution",
            "dominant_seconds",
        ),
        "s128_baseline_phase_seconds": _mapping(
            _input_value(
                loaded_inputs,
                "s128_execution",
                "powered_support_coverer_phase_seconds",
            )
        ),
    }
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        "readiness_json": output_dir / "signature_bucket_powered_support_compact_item_probe_readiness.json",
        "readiness_md": output_dir / "signature_bucket_powered_support_compact_item_probe_readiness.md",
...
def _assert_readiness_namespace(output_dir: Path) -> None:
    ...
    if "132_signature_bucket_powered_support_compact_item_probe_readiness" not in normalized:
        raise ValueError("S132 readiness namespace required")

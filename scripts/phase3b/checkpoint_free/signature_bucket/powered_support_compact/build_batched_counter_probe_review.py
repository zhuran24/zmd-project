...
EXPECTED_RUN_ID = "local_hotspot_42x32_signature_bucket_compact_item_batched_counter_inst_no_solve_001"
DEFAULT_PROBE = (
    ARTIFACT_ROOT
...
DEFAULT_OUTPUT_DIR = ARTIFACT_ROOT / "140_signature_bucket_powered_support_compact_item_batched_counter_probe_review"
S140_NAMESPACE = "140_signature_bucket_powered_support_compact_item_batched_counter_probe_review"
...
HOTSPOT_LABELS = {
    "coverer_union_collection": "another_powered_support_subphase_hotspot",
    "disjoint_filtering": "another_powered_support_subphase_hotspot",
    "power_index_expansion": "another_powered_support_subphase_hotspot",
    "compact_item_accumulation": "compact_item_accumulation_still_hot",
    "stats_finalize": "another_powered_support_subphase_hotspot",
}


def main(argv: Sequence[str] | None = None) -> int:
    args = _parse_args(argv)
    review = build_signature_bucket_powered_support_compact_item_batched_counter_probe_review(
    ...
    print("phase3b signature bucket powered support compact-item batched-counter probe review")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Review the future S139-enabled 42x32 no-solve powered support compact-item batched-counter "
...
def build_signature_bucket_powered_support_compact_item_batched_counter_probe_review(
    *,
    project_root: Path,
    readiness_path: Path,
    probe_path: Path,
    ...
    output_dir = resolve_artifact_namespace(project_root, output_dir, S140_NAMESPACE)
    ...
    payload = {
        "schema": "phase3b-signature-bucket-powered-support-compact-item-batched-counter-probe-review/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "review_kind": "enabled_signature_bucket_powered_support_compact_item_batched_counter_no_solve_probe_review",
...
def render_probe_review_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Phase3B S140 Powered Support Compact-Item Batched-Counter Probe Review",
        "",
...
def _subphase_summary(probe: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "compact_item_batched_counter_instrumentation_present": any(
            str(key).startswith("compact_item_batched_counter")
...
def _interpretation(
    *,
    ...
    if (
        ...
        return {
            "classification": "powered_support_compact_item_batched_counter_probe_inconclusive",
            "reason": "readiness_not_ready_for_powered_support_compact_item_batched_counter_probe_review",
        }
    if not subphase_summary.get("all_required_sections_present"):
        return {
            "classification": "batched_counter_instrumentation_missing",
    ...
    baseline_compact_item_seconds = _float(
        _mapping(readiness.get("readiness")).get("s133_current_compact_item_seconds")
    )
    ...
    batched_attempts = _int(totals.get("compact_item_batched_counter_attempts"))
    batched_used = _int(totals.get("compact_item_batched_counter_used"))
    batched_fallbacks = _int(totals.get("compact_item_batched_counter_fallbacks"))
    local_update_count = _int(totals.get("compact_item_batched_counter_local_update_count"))
    merge_update_count = _int(totals.get("compact_item_batched_counter_merge_update_count"))
    if (
        not subphase_summary.get("compact_item_batched_counter_instrumentation_present")
        or batched_attempts is None
        or batched_used is None
        or batched_fallbacks is None
        or local_update_count is None
        or merge_update_count is None
    ):
        return {
            "classification": "batched_counter_instrumentation_missing",
            "reason": "compact_item_batched_counter_totals_missing",
    ...
    if (
        ...
        return {
            "classification": "compact_item_batched_counter_not_used",
            "reason": "compact_item_batched_counter_totals_zero",
            "baseline_compact_item_seconds": baseline_compact_item_seconds,
            "current_compact_item_seconds": current_compact_item_seconds,
            "compact_item_batched_counter_attempts": batched_attempts,
            "compact_item_batched_counter_used": batched_used,
            "compact_item_batched_counter_fallbacks": batched_fallbacks,
        }
    dominant_phase, dominant_seconds = max(
        phase_seconds.items(),
        key=lambda item: (float(item[1]), str(item[0])),
    )
    fallback_ratio = batched_fallbacks / batched_attempts if batched_attempts else 1.0
    if fallback_ratio > 0.5:
        classification = "compact_item_batched_counter_fallback_dominated"
    elif current_compact_item_seconds < baseline_compact_item_seconds * 0.75:
        classification = "compact_item_batched_counter_effective"
        ...
        classification = HOTSPOT_LABELS.get(
            dominant_phase,
            "another_powered_support_subphase_hotspot",
        )
    return {
        ...
        "compact_item_batched_counter_used": batched_used,
        "compact_item_batched_counter_fallbacks": batched_fallbacks,
        "compact_item_batched_counter_fallback_ratio": fallback_ratio,
        "compact_item_batched_counter_local_update_count": local_update_count,
        "compact_item_batched_counter_merge_update_count": merge_update_count,
        "phase_seconds": phase_seconds,
    }
...
def _next_gate(interpretation: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "status": "prepare_powered_support_compact_item_batched_counter_result_strategy_and_external_review",
        "classification": classification,
    }
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        "review_json": output_dir / "signature_bucket_powered_support_compact_item_batched_counter_probe_review.json",
        "review_md": output_dir / "signature_bucket_powered_support_compact_item_batched_counter_probe_review.md",
        "sensitive_path_fingerprint": output_dir / "sensitive_path_fingerprint.json",
    }


def _assert_review_namespace(output_dir: Path) -> None:
    resolve_artifact_namespace(PROJECT_ROOT, output_dir, S140_NAMESPACE)

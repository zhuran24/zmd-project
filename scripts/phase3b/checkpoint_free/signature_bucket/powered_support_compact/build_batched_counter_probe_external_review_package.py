...
S141_DIR = ARTIFACT_ROOT / "141_signature_bucket_powered_support_compact_item_batched_counter_probe_external_review_package"
S141_NAMESPACE = "141_signature_bucket_powered_support_compact_item_batched_counter_probe_external_review_package"
DEFAULT_RUN_ID = "s139_s140_compact_item_batched_counter_probe_review_001"

PACKAGE_INPUTS = {
    "s133_probe_review": ARTIFACT_ROOT
    / "133_signature_bucket_powered_support_compact_item_probe_review"
    / "signature_bucket_powered_support_compact_item_probe_review.json",
    "s137_review_summary": ARTIFACT_ROOT
    / "137_signature_bucket_powered_support_compact_item_residual_external_review_package"
    / "s135_compact_item_residual_review_001"
    / "external_review_reply_summary.json",
    "s138_implementation": ARTIFACT_ROOT
    / "138_signature_bucket_powered_support_compact_item_batched_counter_implementation"
    / "signature_bucket_powered_support_compact_item_batched_counter_implementation.json",
    "s139_readiness": ARTIFACT_ROOT
    / "139_signature_bucket_powered_support_compact_item_batched_counter_probe_readiness"
    / "signature_bucket_powered_support_compact_item_batched_counter_probe_readiness.json",
    "s139_future_command": ARTIFACT_ROOT
    / "139_signature_bucket_powered_support_compact_item_batched_counter_probe_readiness"
    ...
    "s139_readiness_builder": PROJECT_ROOT
    / "scripts" / "phase3b" / "checkpoint_free" / "signature_bucket" / "powered_support_compact" / "build_batched_counter_probe_readiness.py",
    "s140_review_builder": PROJECT_ROOT
    / "scripts" / "phase3b" / "checkpoint_free" / "signature_bucket" / "powered_support_compact" / "build_batched_counter_probe_review.py",
    "s141_package_builder": PROJECT_ROOT
    / "scripts" / "phase3b" / "checkpoint_free" / "signature_bucket" / "powered_support_compact" / "build_batched_counter_probe_external_review_package.py",
    "s139_readiness_tests": PROJECT_ROOT
    / "src"
    / "tests"
    / "test_phase3b_checkpoint_free_signature_bucket_powered_support_compact_item_batched_counter_probe_readiness.py",
    "s140_review_tests": PROJECT_ROOT
    / "src"
    / "tests"
    / "test_phase3b_checkpoint_free_signature_bucket_powered_support_compact_item_batched_counter_probe_review.py",
    "s141_package_tests": PROJECT_ROOT
    / "src"
    / "tests"
    / "test_phase3b_checkpoint_free_signature_bucket_powered_support_compact_item_batched_counter_probe_external_review_package.py",
}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("phase3b signature bucket powered support compact-item batched-counter probe review package")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build S141 external review package for S139/S140 one-shot powered "
            "support compact-item batched-counter no-solve probe readiness and review tooling."
        )
    )
    parser.add_argument("--output-dir", type=Path, default=S141_DIR)
    parser.add_argument("--run-id", default=DEFAULT_RUN_ID)
    parser.add_argument("--no-write", action="store_true")
    return parser.parse_args(argv)


def build_powered_support_compact_item_batched_counter_probe_external_review_package(
    *,
    project_root: Path,
    output_dir: Path,
    ...
    output_dir = resolve_artifact_namespace(project_root, output_dir, S141_NAMESPACE)
    run_id = validate_artifact_run_id(run_id)
    input_paths = {
    ...
    run_dir = safe_child_artifact_dir(output_dir, run_id)
    ...
    completed = (
        not no_write
        and zip_sha256 is not None
        and expanded_bundle_sha256 is not None
        and validation.get("validated") is True
        and sensitive_comparison.get("changed") is False
    )
    if completed:
        status = "completed"
    elif no_write and sensitive_comparison.get("changed") is False:
        status = "dry_run"
    else:
        status = "manual_review_required"
    payload = {
        "schema": "phase3b-signature-bucket-powered-support-compact-item-batched-counter-probe-external-review-package/v0",
        "generated_at": _now(),
        "status": status,
        "run_id": run_id,
...
def _review_request_text(package_filename: str, package_sha256: str) -> str:
    return "\n".join(
        [
            "# Review Request: Phase3B S139/S140 Powered Support Compact-Item Batched-Counter Probe Readiness",
            "",
            ...
            "",
            "Context: S138 implemented a default-off, stats-only/behavior-preserving powered support compact-item batched-counter optimization under `EXACT_GHOST_SIGNATURE_BUCKET_POWERED_SUPPORT_COMPACT_ITEM_BATCHED_COUNTER_OPTIMIZATION`. S139 is a plan-only readiness artifact for exactly one future enabled `42x32` no-solve probe, and S140 is fail-closed review tooling for the future probe output.",
            "",
            "Future command under review:",
            "",
            "`python scripts/run_phase3b_checkpoint_free_overlay_timing_probe.py --execute-no-solve --candidate-key 42x32 --run-id local_hotspot_42x32_signature_bucket_compact_item_batched_counter_inst_no_solve_001`",
            "",
            ...
            "- `EXACT_GHOST_SIGNATURE_BUCKET_POWERED_SUPPORT_COMPACT_ITEM_ACCUMULATION_OPTIMIZATION=1`",
            "- `EXACT_GHOST_SIGNATURE_BUCKET_POWERED_SUPPORT_COMPACT_ITEM_BATCHED_COUNTER_OPTIMIZATION=1`",
            "",
            "Please answer these gate questions:",
            "",
            "1. Is S139 sufficiently narrow and fail-closed for a single future `42x32 --execute-no-solve` powered support compact-item batched-counter probe?",
            "2. Does S139 correctly keep `probe_execution_enabled=false` and require review/authorization before execution?",
            "3. Is S140 strict enough on sensitive-path schema, hard-boundary flags, run id, candidate key, no-solve status, missing powered support compact-item batched-counter fields, missing timing, and missing batched-counter totals?",
            "4. Are the S140 classifications useful for deciding whether the batched counter is effective, not used, fallback dominated, still hot, or displaced to another powered support subphase?",
...
def _summary_markdown(payload: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Phase3B S141 Powered Support Compact-Item Batched-Counter Probe External Review Package",
            "",
...
def _expanded_review_bundle_text(
    *,
    ...
    parts = [
        "# Expanded Review Bundle: Phase3B S139/S140 Powered Support Compact-Item Batched-Counter Probe",
        "",
...
def _assert_package_namespace(output_dir: Path) -> None:
    resolve_artifact_namespace(PROJECT_ROOT, output_dir, S141_NAMESPACE)

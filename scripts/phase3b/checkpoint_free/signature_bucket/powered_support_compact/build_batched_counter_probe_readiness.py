...
from src.runtime.phase3b_artifact_guards import resolve_artifact_namespace  # noqa: E402
from src.search.exact_campaign import atomic_write_json  # noqa: E402

ARTIFACT_ROOT = PROJECT_ROOT / ".artifacts" / "phase3b_local_13900ks_tuning_20260430"
DEFAULT_OUTPUT_DIR = ARTIFACT_ROOT / "139_signature_bucket_powered_support_compact_item_batched_counter_probe_readiness"
S139_NAMESPACE = "139_signature_bucket_powered_support_compact_item_batched_counter_probe_readiness"
DEFAULT_INPUTS = {
...
COMPACT_ITEM_BATCHED_COUNTER_OPTIMIZATION_ENV_VAR = (
    "EXACT_GHOST_SIGNATURE_BUCKET_POWERED_SUPPORT_COMPACT_ITEM_BATCHED_COUNTER_OPTIMIZATION"
)

FUTURE_RUN_ID = "local_hotspot_42x32_signature_bucket_compact_item_batched_counter_inst_no_solve_001"
...
REQUIRED_ENVIRONMENT = {
    ...
    COMPACT_ITEM_OPTIMIZATION_ENV_VAR: "1",
    COMPACT_ITEM_BATCHED_COUNTER_OPTIMIZATION_ENV_VAR: "1",
}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    readiness = build_signature_bucket_powered_support_compact_item_batched_counter_probe_readiness(
        project_root=PROJECT_ROOT,
        output_dir=_resolve_path(PROJECT_ROOT, args.output_dir),
        no_write=bool(args.no_write),
    )
    print("phase3b signature bucket powered support compact-item batched-counter probe readiness")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build S139 readiness artifacts for one future enabled 42x32 no-solve "
            "powered support compact-item batched-counter optimization probe without executing it."
        )
    )
...
def build_signature_bucket_powered_support_compact_item_batched_counter_probe_readiness(
    *,
    project_root: Path,
    output_dir: Path,
    no_write: bool = False,
    ...
    output_dir = resolve_artifact_namespace(project_root, output_dir, S139_NAMESPACE)
    input_paths = {
    ...
    status = (
        "completed"
        if readiness["classification"] == "ready_for_powered_support_compact_item_batched_counter_probe_review"
        else "manual_review_required"
    )
    paths = _paths(output_dir)
    payload = {
        "schema": "phase3b-signature-bucket-powered-support-compact-item-batched-counter-probe-readiness/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "readiness_kind": "future_enabled_signature_bucket_powered_support_compact_item_batched_counter_no_solve_probe_plan_only",
        ...
        "next_gate": {
            "status": "hold_for_readiness_review_before_powered_support_compact_item_batched_counter_probe",
            "candidate_step": (
                "execute exactly one enabled 42x32 no-solve powered support compact-item batched-counter "
                "probe only after S139 readiness review accepts this plan"
            ),
            "blocked_actions": [
                "do_not_execute_enabled_probe_in_s139",
...
def build_future_command_template() -> dict[str, Any]:
    ...
    return {
        "schema": "phase3b-signature-bucket-powered-support-compact-item-batched-counter-future-command/v0",
        "command": command,
...
def render_readiness_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Phase3B S139 Powered Support Compact-Item Batched-Counter Probe Readiness",
        "",
...
def _evaluate_readiness(
    *,
    ...
    checks = [
        _check(
            "s133_probe_review_completed",
            _input_value(loaded_inputs, "s133_probe_review", "status") == "completed",
        ),
        _check(
            "s133_probe_safety_clean",
            _nested_value(loaded_inputs, "s133_probe_review", "probe_safety", "sensitive_path_clean")
            is True
            and _input_value(loaded_inputs, "s133_probe_review", "checkpoint_written") is False
            and _input_value(loaded_inputs, "s133_probe_review", "proof_source") is False,
        ),
        _check(
            "s133_compact_item_still_hot",
            _nested_value(loaded_inputs, "s133_probe_review", "interpretation", "classification")
            == "compact_item_accumulation_still_hot",
        ),
        _check(
            "s137_review_passed",
            _input_value(loaded_inputs, "s137_review_summary", "review_verdict") == "pass",
        ),
        _check(
            "s137_review_not_authorization",
            _input_value(loaded_inputs, "s137_review_summary", "review_is_authorization") is False,
        ),
        _check(
            "s137_authorization_required_next",
            _input_value(loaded_inputs, "s137_review_summary", "authorization_required_next") is True,
        ),
        _check(
            "s138_patch_implemented_and_verified",
            _input_value(loaded_inputs, "s138_implementation", "status")
            == "implemented_and_verified",
        ),
        _check(
            "s138_batched_counter_env_matches",
            _input_value(loaded_inputs, "s138_implementation", "env_var")
            == COMPACT_ITEM_BATCHED_COUNTER_OPTIMIZATION_ENV_VAR,
        ),
    ...
    classification = (
        "ready_for_powered_support_compact_item_batched_counter_probe_review"
    ...
    return {
        ...
        "checks": checks,
        "s133_classification": _nested_value(
            loaded_inputs,
            "s133_probe_review",
            "interpretation",
            "classification",
        ),
        "s137_review_verdict": _input_value(loaded_inputs, "s137_review_summary", "review_verdict"),
        "s138_env_var": _input_value(loaded_inputs, "s138_implementation", "env_var"),
        "s133_dominant_phase": _nested_value(
            loaded_inputs,
            "s133_probe_review",
            "interpretation",
            "dominant_phase",
        ),
        "s133_baseline_compact_item_seconds": _nested_value(
            loaded_inputs,
            "s133_probe_review",
            "interpretation",
            "baseline_compact_item_seconds",
        ),
        "s133_current_compact_item_seconds": _nested_value(
            loaded_inputs,
            "s133_probe_review",
            "interpretation",
            "current_compact_item_seconds",
        ),
        "s133_phase_seconds": _mapping(
            _nested_value(loaded_inputs, "s133_probe_review", "interpretation", "phase_seconds")
        ),
        "s133_totals": _mapping(
            _nested_value(loaded_inputs, "s133_probe_review", "subphase_summary", "totals")
        ),
    }
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        "readiness_json": output_dir / "signature_bucket_powered_support_compact_item_batched_counter_probe_readiness.json",
        "readiness_md": output_dir / "signature_bucket_powered_support_compact_item_batched_counter_probe_readiness.md",
...
def _assert_readiness_namespace(output_dir: Path) -> None:
    resolve_artifact_namespace(PROJECT_ROOT, output_dir, S139_NAMESPACE)

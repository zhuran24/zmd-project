...
S147_DIR = ARTIFACT_ROOT / "147_signature_bucket_powered_support_compact_item_detail_probe_external_review_package"
DEFAULT_RUN_ID = "s145_s146_compact_item_detail_probe_review_001"

PACKAGE_INPUTS = {
    "s140_probe_review": ARTIFACT_ROOT
    / "140_signature_bucket_powered_support_compact_item_batched_counter_probe_review"
    / "signature_bucket_powered_support_compact_item_batched_counter_probe_review.json",
    "s143_review_summary": ARTIFACT_ROOT
    / "143_signature_bucket_powered_support_compact_item_batched_counter_result_external_review_package"
    / "s140_batched_counter_result_review_001"
    / "external_review_reply_summary.json",
    "s144_implementation": ARTIFACT_ROOT
    / "144_signature_bucket_powered_support_compact_item_detail_instrumentation_implementation"
    / "signature_bucket_powered_support_compact_item_detail_instrumentation_implementation.json",
    "s145_readiness": ARTIFACT_ROOT
    / "145_signature_bucket_powered_support_compact_item_detail_probe_readiness"
    / "signature_bucket_powered_support_compact_item_detail_probe_readiness.json",
    "s145_future_command": ARTIFACT_ROOT
    / "145_signature_bucket_powered_support_compact_item_detail_probe_readiness"
    ...
    "s145_readiness_builder": PROJECT_ROOT
    / "scripts" / "phase3b" / "checkpoint_free" / "signature_bucket" / "powered_support_compact" / "build_detail_probe_readiness.py",
    "s146_review_builder": PROJECT_ROOT
    / "scripts" / "phase3b" / "checkpoint_free" / "signature_bucket" / "powered_support_compact" / "build_detail_probe_review.py",
    "s147_package_builder": PROJECT_ROOT
    / "scripts" / "phase3b" / "checkpoint_free" / "signature_bucket" / "powered_support_compact" / "build_detail_probe_external_review_package.py",
    "s145_readiness_tests": PROJECT_ROOT
    / "src"
    / "tests"
    / "test_phase3b_checkpoint_free_signature_bucket_powered_support_compact_item_detail_probe_readiness.py",
    "s146_review_tests": PROJECT_ROOT
    / "src"
    / "tests"
    / "test_phase3b_checkpoint_free_signature_bucket_powered_support_compact_item_detail_probe_review.py",
    "s147_package_tests": PROJECT_ROOT
    / "src"
    / "tests"
    / "test_phase3b_checkpoint_free_signature_bucket_powered_support_compact_item_detail_probe_external_review_package.py",
}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("phase3b signature bucket powered support compact-item detail probe review package")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build S147 external review package for S145/S146 one-shot powered "
            "support compact-item detail no-solve probe readiness and review tooling."
        )
    )
    parser.add_argument("--output-dir", type=Path, default=S147_DIR)
    parser.add_argument("--run-id", default=DEFAULT_RUN_ID)
    parser.add_argument("--no-write", action="store_true")
    return parser.parse_args(argv)


def build_powered_support_compact_item_detail_probe_external_review_package(
    *,
    project_root: Path,
    output_dir: Path,
    ...
    payload = {
        "schema": "phase3b-signature-bucket-powered-support-compact-item-detail-probe-external-review-package/v0",
        "generated_at": _now(),
        "status": status,
        "run_id": run_id,
...
def _review_request_text(package_filename: str, package_sha256: str) -> str:
    return "\n".join(
        [
            "# Review Request: Phase3B S145/S146 Powered Support Compact-Item Detail Probe Readiness",
            "",
            ...
            "",
            "Context: S144 implemented a default-off, stats-only/behavior-preserving powered support compact-item detail instrumentation patch under `EXACT_GHOST_SIGNATURE_BUCKET_POWERED_SUPPORT_COMPACT_ITEM_DETAIL_INSTRUMENTATION`. S145 is a plan-only readiness artifact for exactly one future enabled `42x32` no-solve probe, and S146 is fail-closed review tooling for the future probe output.",
            "",
            "Future command under review:",
            "",
            "`python scripts/run_phase3b_checkpoint_free_overlay_timing_probe.py --execute-no-solve --candidate-key 42x32 --run-id local_hotspot_42x32_signature_bucket_compact_item_detail_inst_no_solve_001`",
            "",
            ...
            "",
            "1. Is S145 sufficiently narrow and fail-closed for a single future `42x32 --execute-no-solve` powered support compact-item detail probe?",
            "2. Does S145 correctly keep `probe_execution_enabled=false` and require review/authorization before execution?",
            "3. Is S146 strict enough on sensitive-path schema, hard-boundary flags, run id, candidate key, no-solve status, missing powered support compact-item detail fields, missing timing, and missing detail totals?",
            "4. Are the S146 classifications useful for deciding whether compact-item detail timing points to key-build, local counter update, merge fanout, compact signature storage, absent duplicate compression, missing instrumentation, or an inconclusive/safety-failed result?",
...
def _summary_markdown(payload: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Phase3B S147 Powered Support Compact-Item Detail Probe External Review Package",
            "",
...
def _expanded_review_bundle_text(
    *,
    ...
    parts = [
        "# Expanded Review Bundle: Phase3B S145/S146 Powered Support Compact-Item Detail Probe",
        "",
...
def _assert_package_namespace(output_dir: Path) -> None:
    ...
    if "147_signature_bucket_powered_support_compact_item_detail_probe_external_review_package" not in normalized:
        raise ValueError("S147 package namespace required")

...
DEFAULT_READINESS = (
    ARTIFACT_ROOT
    / "110_signature_bucket_model_shell_subphase_probe_readiness"
    / "signature_bucket_model_shell_subphase_probe_readiness.json"
)
EXPECTED_RUN_ID = "local_hotspot_42x32_signature_bucket_model_shell_inst_no_solve_001"
DEFAULT_PROBE = (
    ARTIFACT_ROOT
...
DEFAULT_OUTPUT_DIR = ARTIFACT_ROOT / "111_signature_bucket_model_shell_subphase_probe_review"
SENSITIVE_PATH_COMPARISON_SCHEMA = "phase3b-sensitive-path-fingerprint-comparison/v0"
NON_SUCCESS_CLASSIFICATIONS = {
    "safety_disqualified",
    "model_shell_subphase_instrumentation_missing",
    "model_shell_subphase_probe_inconclusive",
}
HARD_BOUNDARY_FLAGS = (
...
HOTSPOT_LABELS = {
    "constructor_enter_to_instance_copy": "constructor_enter_to_instance_copy_hotspot",
    "dimension_and_profile_normalization": "dimension_and_profile_normalization_hotspot",
    "optional_cap_and_support_cache_initialization": (
        "optional_cap_and_support_cache_initialization_hotspot"
    ),
    "candidate_domain_or_pose_cache_initialization": (
        "candidate_domain_or_pose_cache_initialization_hotspot"
    ),
    "build_stats_initialization": "build_stats_initialization_hotspot",
    "mandatory_group_build": "mandatory_group_build_hotspot",
    "port_profile_and_boundary_cache_initialization": (
        "port_profile_and_boundary_cache_initialization_hotspot"
    ),
    "signature_bucket_seed_build": "signature_bucket_seed_build_hotspot",
    "constructor_finalize": "constructor_finalize_hotspot",
    "model_shell_unattributed_seconds": "model_shell_unattributed_hotspot",
}


def main(argv: Sequence[str] | None = None) -> int:
    args = _parse_args(argv)
    review = build_signature_bucket_model_shell_subphase_probe_review(
    ...
    print("phase3b signature bucket model-shell subphase probe review")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Review the future S110-enabled 42x32 no-solve model-shell subphase "
            "probe and classify the dominant MasterPlacementModel constructor subphase."
        )
    )
...
def build_signature_bucket_model_shell_subphase_probe_review(
    *,
    project_root: Path,
    readiness_path: Path,
    probe_path: Path,
    ...
    payload = {
        "schema": "phase3b-signature-bucket-model-shell-subphase-probe-review/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "review_kind": "enabled_signature_bucket_model_shell_subphase_no_solve_probe_review",
...
def render_probe_review_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Phase3B S111 Signature Bucket Model-Shell Subphase Probe Review",
        "",
...
def _subphase_summary(probe: Mapping[str, Any]) -> dict[str, Any]:
    ...
    raw_phase_seconds = _mapping(residual.get("model_shell_subphase_seconds"))
    phase_seconds = {
        str(key): float(value)
        for key, value in raw_phase_seconds.items()
        if isinstance(value, (int, float))
    }
    unattributed = residual.get("model_shell_unattributed_seconds")
    if isinstance(unattributed, (int, float)):
        phase_seconds["model_shell_unattributed_seconds"] = float(unattributed)
    required_numeric = {
        "model_shell_total_seconds": _float(residual.get("model_shell_total_seconds")),
        "model_shell_subphase_total_seconds": _float(
            residual.get("model_shell_subphase_total_seconds")
        ),
        "model_shell_unattributed_seconds": _float(residual.get("model_shell_unattributed_seconds")),
    }
    return {
        "exact_core_reuse_residual_present": bool(residual),
        "model_shell_instrumentation_enabled": residual.get("model_shell_instrumentation_enabled") is True,
        "has_model_shell_subphase_seconds": bool(raw_phase_seconds),
        "all_required_sections_present": bool(
            residual
            and residual.get("model_shell_instrumentation_enabled") is True
...
def _interpretation(
    *,
    ...
    if (
        ...
        != "ready_for_model_shell_subphase_probe_review"
    ):
        return {
            "classification": "model_shell_subphase_probe_inconclusive",
            "reason": "readiness_not_ready_for_model_shell_subphase_probe_review",
        }
    if not subphase_summary.get("all_required_sections_present"):
        return {
            "classification": "model_shell_subphase_instrumentation_missing",
            "reason": "model_shell_subphase_instrumentation_missing",
        }
    ...
    if not all(isinstance(value, (int, float)) for value in required_numeric.values()):
        return {
            "classification": "model_shell_subphase_probe_inconclusive",
            "reason": "required_numeric_model_shell_fields_missing",
        }
    phase_seconds = {
        str(key): float(value)
    ...
    if not phase_seconds:
        return {
            "classification": "model_shell_subphase_probe_inconclusive",
            "reason": "no_numeric_model_shell_subphase_seconds",
        }
    ...
    classification = HOTSPOT_LABELS.get(
        dominant_phase,
        "model_shell_subphase_instrumentation_visible",
    )
    return {
...
def _next_gate(interpretation: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "status": "prepare_model_shell_hotspot_specific_strategy_and_external_review",
        "classification": classification,
    }
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        "review_json": output_dir / "signature_bucket_model_shell_subphase_probe_review.json",
        "review_md": output_dir / "signature_bucket_model_shell_subphase_probe_review.md",
...
def _assert_review_namespace(output_dir: Path) -> None:
    ...
    if "111_signature_bucket_model_shell_subphase_probe_review" not in normalized:
        raise ValueError("S111 probe review namespace required")

...
from src.search.phase3b.mandatory_core.mandatory_core_encoding import (
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build Phase 3B mandatory-core encoding inventory."
    )
    ...
    parser.add_argument(
        "--anchor-indices",
        default=None,
        help="Optional comma-separated anchor indices to inspect instead of campaign samples.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(".artifacts/phase3b_mandatory_core_encoding"),
    )
...
def main() -> int:
    ...
    if not args.no_write:
        ...
        prefix = args.output_prefix or f"mandatory_core_encoding_{str(args.candidate)}"
        json_path = output_dir / f"{prefix}.json"
        md_path = output_dir / f"{prefix}.md"
        txt_path = output_dir / f"{prefix}.txt"
        atomic_write_json(json_path, report)
        _atomic_write_text(md_path, render_phase3b_mandatory_core_encoding_markdown(report))
        _atomic_write_text(txt_path, render_phase3b_mandatory_core_encoding_text(report))
        print(f"mandatory_core_encoding_json={_display_path(project_root, json_path)}")
        print(f"mandatory_core_encoding_md={_display_path(project_root, md_path)}")
        print(f"mandatory_core_encoding_txt={_display_path(project_root, txt_path)}")
    return 0
...
def _print_summary(report: Mapping[str, Any]) -> None:
    ...
    print("phase3b mandatory-core encoding inventory")
    print(f"- candidate: {candidate.get('key')}")
    print(f"- evaluated: {bool(status.get('evaluated', False))}")
    print(f"- outcome: {status.get('outcome')}")
    print(f"- mandatory groups: {summary.get('group_count')}")
    print(f"- mandatory slots: {summary.get('slot_count')}")
    print(f"- diagnostic proto variables: {proto.get('variable_count')}")
    print(f"- diagnostic proto constraints: {proto.get('constraint_count')}")
    print(f"- disabled residual actives: {residual.get('active_var_count')}")

...
from src.search.phase3b.anchor_inventory.constraint_inventory import (
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build Phase 3B anchor constraint inventory for selected forced anchors."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(".artifacts/phase3b_anchor_constraint_inventory"),
    )
...
def main() -> int:
    ...
    if not args.no_write:
        ...
        prefix = args.output_prefix or f"anchor_constraint_inventory_{str(args.candidate)}"
        json_path = output_dir / f"{prefix}.json"
        md_path = output_dir / f"{prefix}.md"
        txt_path = output_dir / f"{prefix}.txt"
        atomic_write_json(json_path, report)
        _atomic_write_text(md_path, render_phase3b_anchor_constraint_inventory_markdown(report))
        _atomic_write_text(txt_path, render_phase3b_anchor_constraint_inventory_text(report))
        print(f"anchor_constraint_inventory_json={_display_path(project_root, json_path)}")
        print(f"anchor_constraint_inventory_md={_display_path(project_root, md_path)}")
        print(f"anchor_constraint_inventory_txt={_display_path(project_root, txt_path)}")
    return 0
...
def _print_summary(report: Mapping[str, Any]) -> None:
    ...
    for entry in list(report.get("anchors", [])):
        ...
        print(
            ...
            f"refs={entry.get('direct_u_reference_total')} "
            f"counts={entry.get('direct_u_reference_counts')}"
        )

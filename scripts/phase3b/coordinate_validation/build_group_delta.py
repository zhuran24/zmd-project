...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a default-off Phase 3B coordinate-validation group/family delta diagnostic."
    )
    ...
    parser.add_argument(
        "--variants",
        default=",".join(DEFAULT_GROUP_DELTA_VARIANTS),
        help="Comma-separated variants. Known: " + ",".join(KNOWN_GROUP_DELTA_VARIANTS),
    )
    parser.add_argument(
        "--max-groups",
        type=int,
        default=None,
        help="Limit group-expanded variants to the first N greedy-ordered groups.",
    )
    parser.add_argument(
        "--max-families",
        type=int,
        default=None,
        help="Limit family-expanded variants to the first N greedy-ordered families.",
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(".artifacts/phase3b_coordinate_validation_group_delta"),
    )
...
def main() -> int:
    ...
    if not args.no_write:
        ...
        prefix = args.output_prefix or "coordinate_validation_group_delta"
        ...
        _atomic_write_text(
            md_path,
            render_phase3b_coordinate_validation_group_delta_markdown(report),
        )
        _atomic_write_text(
            txt_path,
            render_phase3b_coordinate_validation_group_delta_text(report),
        )
        print(f"coordinate_validation_group_delta_json={_display_path(project_root, json_path)}")
        print(f"coordinate_validation_group_delta_md={_display_path(project_root, md_path)}")
        print(f"coordinate_validation_group_delta_txt={_display_path(project_root, txt_path)}")
    return 0
...
def _print_summary(report: Mapping[str, Any]) -> None:
    ...
    print("phase3b coordinate validation group delta")
    ...
    if first_narrower:
        print(
            "- first non-full infeasible: "
            f"{first_narrower.get('case_id')} "
            f"variant={first_narrower.get('variant')}"
        )
    elif first_infeasible:
        print(
            "- first infeasible: "
            f"{first_infeasible.get('case_id')} "
            f"variant={first_infeasible.get('variant')}"
        )

...
from src.search.phase3b.coordinate_validation.no_overlap_subset_delta import (
    DEFAULT_NO_OVERLAP_SUBSET_GROUP_ID,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a Phase 3B coordinate-validation NoOverlap2D interval subset diagnostic."
    )
    parser.add_argument("--project-root", type=Path, default=PROJECT_ROOT)
    parser.add_argument("--candidate", default="67x13")
    parser.add_argument("--anchor-index", type=int, default=119)
    parser.add_argument("--group-id", default=DEFAULT_NO_OVERLAP_SUBSET_GROUP_ID)
    ...
    parser.add_argument(
        "--variants",
        default=None,
        help="Comma-separated variant list. Defaults to the required NoOverlap2D subset variants.",
    )
    parser.add_argument(
        "--solver-profile-json",
        default=None,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(".artifacts/phase3b_coordinate_validation_no_overlap_subset_delta"),
    )
...
def main() -> int:
    ...
    if not args.no_write:
        ...
        prefix = args.output_prefix or "coordinate_validation_no_overlap_subset_delta"
        ...
        _atomic_write_text(
            md_path,
            render_phase3b_coordinate_validation_no_overlap_subset_delta_markdown(report),
        )
        _atomic_write_text(
            txt_path,
            render_phase3b_coordinate_validation_no_overlap_subset_delta_text(report),
        )
        print(f"coordinate_validation_no_overlap_subset_delta_json={_display_path(project_root, json_path)}")
        print(f"coordinate_validation_no_overlap_subset_delta_md={_display_path(project_root, md_path)}")
        print(f"coordinate_validation_no_overlap_subset_delta_txt={_display_path(project_root, txt_path)}")
    return 0
...
def _print_summary(report: Mapping[str, Any]) -> None:
    ...
    print("phase3b coordinate validation no-overlap subset delta")
    ...
    print(f"- no-overlap constraints: {inventory.get('no_overlap_constraint_count')}")
    print(f"- first status change: {first_change.get('variant')}")
    print(f"- implicated variants: {delta.get('implicated_variants')}")

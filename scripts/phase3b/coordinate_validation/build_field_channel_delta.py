...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a Phase 3B coordinate-validation field-channel diagnostic."
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Group id to evaluate. Repeatable and comma-separated. Defaults to "
            + ",".join(DEFAULT_FIELD_CHANNEL_GROUP_IDS)
        ),
    )
    parser.add_argument(
        "--fields",
        default=",".join(FIELD_CHANNEL_VARIANTS),
        help="Comma-separated field variants. Known: " + ",".join(FIELD_CHANNEL_VARIANTS),
    )
    parser.add_argument(
        "--include-ghost",
        action="store_true",
        help="Validate each field-channel case with the requested ghost anchor blocked.",
    )
    parser.add_argument(
        "--collect-force-equality-labels",
        action="store_true",
        help="Store forced x/y/mode equality labels for each field-channel case.",
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(".artifacts/phase3b_coordinate_validation_field_channel_delta"),
    )
...
def main() -> int:
    ...
    report = build_phase3b_coordinate_validation_field_channel_delta(
        ...
        group_ids=_parse_csv_values(args.group_id),
        field_variants=_parse_csv_values([args.fields]),
        include_ghost=bool(args.include_ghost),
        collect_force_equality_labels=bool(args.collect_force_equality_labels),
    ...
    if not args.no_write:
        ...
        prefix = args.output_prefix or "coordinate_validation_field_channel_delta"
        ...
        _atomic_write_text(
            md_path,
            render_phase3b_coordinate_validation_field_channel_delta_markdown(report),
        )
        _atomic_write_text(
            txt_path,
            render_phase3b_coordinate_validation_field_channel_delta_text(report),
        )
        print(f"coordinate_validation_field_channel_delta_json={_display_path(project_root, json_path)}")
        print(f"coordinate_validation_field_channel_delta_md={_display_path(project_root, md_path)}")
        print(f"coordinate_validation_field_channel_delta_txt={_display_path(project_root, txt_path)}")
    return 0
...
def _print_summary(report: Mapping[str, Any]) -> None:
    ...
    first_infeasible = _mapping(delta.get("first_infeasible_entry"))
    print("phase3b coordinate validation field-channel delta")
    ...
    if first_infeasible:
        print(
            "- first infeasible: "
            f"{first_infeasible.get('group_id')} "
            f"field={first_infeasible.get('field_variant')}"
        )

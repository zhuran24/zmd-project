...
DEFAULT_OUTPUT_DIR = ARTIFACT_ROOT / "117_signature_bucket_port_profile_cache_probe_readiness"
DEFAULT_INPUTS = {
    "s113_execution": ARTIFACT_ROOT
    / "113_signature_bucket_model_shell_subphase_probe_execution"
    / "signature_bucket_model_shell_subphase_probe_execution.json",
    "s111_review": ARTIFACT_ROOT
...
FUTURE_RUN_ID = "local_hotspot_42x32_signature_bucket_port_profile_cache_inst_no_solve_001"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("phase3b signature bucket port/profile cache probe readiness")
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build S117 readiness artifacts for one future enabled 42x32 no-solve "
            "port/profile cache instrumentation probe without executing it."
        )
    )
...
def build_signature_bucket_port_profile_cache_probe_readiness(
    *,
    project_root: Path,
    output_dir: Path,
    no_write: bool = False,
    ...
    status = (
        "completed"
        if readiness["classification"] == "ready_for_port_profile_cache_probe_review"
        else "manual_review_required"
    )
    paths = _paths(output_dir)
    payload = {
        "schema": "phase3b-signature-bucket-port-profile-cache-probe-readiness/v0",
        "generated_at": datetime.now(timezone.utc).replace(microsecond=0).isoformat(),
        "status": status,
        "readiness_kind": "future_enabled_signature_bucket_port_profile_cache_no_solve_probe_plan_only",
        ...
        "next_gate": {
            "status": "hold_for_readiness_review_before_port_profile_cache_probe",
            "candidate_step": (
                "execute exactly one enabled 42x32 no-solve port/profile cache "
                "probe only after S117 readiness review accepts this plan"
            ),
            "blocked_actions": [
...
def build_future_command_template() -> dict[str, Any]:
    ...
    return {
        "schema": "phase3b-signature-bucket-port-profile-cache-future-command/v0",
        "command": command,
...
def render_readiness_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Phase3B S117 Port/Profile Cache Probe Readiness",
        "",
...
def _evaluate_readiness(
    *,
    ...
    checks = [
        _check("s113_probe_completed", _input_value(loaded_inputs, "s113_execution", "status") == "completed"),
        _check(
            "s113_port_profile_hotspot",
            _input_value(loaded_inputs, "s113_execution", "classification")
            == "port_profile_and_boundary_cache_initialization_hotspot",
        ),
        _check(
            "s111_port_profile_hotspot",
            _nested_value(loaded_inputs, "s111_review", "interpretation", "classification")
            == "port_profile_and_boundary_cache_initialization_hotspot",
        ),
        _check(
            "s114_strategy_ready",
            _input_value(loaded_inputs, "s114_strategy", "classification")
            == "port_profile_boundary_cache_subphase_strategy_required",
        ),
        _check(
            "s115_review_passed",
            _input_value(loaded_inputs, "s115_review_summary", "review_verdict") == "pass",
        ),
        _check(
            "s116_patch_implemented_and_verified",
            _input_value(loaded_inputs, "s116_implementation", "status")
            == "implemented_and_verified",
        ),
        _check(
            "s116_port_profile_env_matches",
            _input_value(loaded_inputs, "s116_implementation", "env_var")
            == PORT_PROFILE_CACHE_INSTRUMENTATION_ENV_VAR,
        ),
    ...
    return {
        ...
        "checks": checks,
        "s113_classification": _input_value(loaded_inputs, "s113_execution", "classification"),
        "s114_classification": _input_value(loaded_inputs, "s114_strategy", "classification"),
        "s116_env_var": _input_value(loaded_inputs, "s116_implementation", "env_var"),
    }
...
def _paths(output_dir: Path) -> dict[str, Path]:
    return {
        "readiness_json": output_dir / "signature_bucket_port_profile_cache_probe_readiness.json",
        "readiness_md": output_dir / "signature_bucket_port_profile_cache_probe_readiness.md",
...
def _assert_readiness_namespace(output_dir: Path) -> None:
    ...
    if "117_signature_bucket_port_profile_cache_probe_readiness" not in normalized:
        raise ValueError("S117 readiness namespace required")

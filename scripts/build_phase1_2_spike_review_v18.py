#!/usr/bin/env python3
"""Build phase1_2_spike_review_v18.zip — GPT v17 四审 MINOR-全-fix snapshot.

v17 → v18 change: GPT pro v17 四审 (两 reviewer 共识 GO_WITH_MINOR) 给 5 项
MINOR finding. v18 全 fix 后 rebuild 求 clean GO. 5 项:
...
OUT_DIR = Path("/tmp/_phase1_2_pkg_v18")
PROJECT_DIR = OUT_DIR / "project"
SEVENZ_PATH = OUT_DIR / "project.7z"
OUT_ZIP = Path("/home/zhuran24/linwin_share/phase1_2_spike_review_v18.zip")
...
```bash
sha256sum -c code_context/SHA256SUMS.spike_code.txt
```
...
+ v18 新加 `code_context/spike/` review-only source snapshot.

Build: master commit `9d5d7fa` (详 `COMMIT_LOG.md`) + spike overlay (6 file +
1 SPIKE_COMMIT_LOG, 详 `SPIKE_COMMIT_LOG.md`, spike branch HEAD `13a7079`;
data-producing Phase B rerun commit `6e6db10`).
...
`f7b88b6`), 13 commit (v18 加 doclag), Phase A (A1/A2/A3) + Phase B (B1-B6) +
verdict-fix. 实施 code 不入 canonical scripts/ (PR #1 verdict-only style); v18
加 review-only `code_context/spike/` 镜像; 实测数据 6 file overlay 进 zip:

- `docs/research/prod_scale_spike_design_20260525/spike_run_20260526/verdict.md`
  — 13 hard G PASS / 1 G SOFT-FAIL (G6a) / 0 hard N + Finding 5 cover + Layer 2 risk register
- `docs/research/prod_scale_spike_design_20260525/spike_run_20260526/phase_a_report.md`
...
| G3 build 10K cut | ≤ 30s | 2.00s | PASS |
| G4 build 50K cut | ≤ 300s | 2.01s | PASS |
| G4b build 100K cut | ≤ 600s | 2.02s | PASS |
...
```bash
unzip -q phase1_2_spike_review_v18.zip
cd _phase1_2_pkg_v18
...
head -20 data/cuts/spike/telemetry_77754.jsonl
grep '"rss_sample_after_solve"' data/cuts/spike/telemetry_*.jsonl

# v18 review-only source snapshot (per MINOR #2):
...
- `phase_b_results.json` — Phase B aggregate
- `telemetry_77754.jsonl` — Phase B raw telemetry (204 rss_sample + 14
  proto_sample + 5 rss_sample_after_solve + 1 dark_matter_emit)
...
| (v18 build) | review-pkg v18 build script — GPT v17 四审 MINOR 全 fix |
"""


def fetch_spike_commit_log() -> str:
    """Dump spike branch commit log (12 commit on top of master)."""
...
+ B6 rerun + v17 verdict doclag + v18 doclag (per GPT v17 四审).
...
(per GPT v17 四审 Reviewer B patch 0002 spec):
...
- `data/cuts/spike/telemetry_77754.jsonl` (v17: 204 rss + 14 proto +
  1 dark_matter + 5 rss_sample_after_solve)
...
def main() -> int:
    ...
    (PROJECT_DIR / "README.md").write_text(README_V18, encoding="utf-8")
    ...
    (OUT_DIR / "README.md").write_text(README_V18, encoding="utf-8")

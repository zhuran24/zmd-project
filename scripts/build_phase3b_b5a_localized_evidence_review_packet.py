...
from src.search.phase3b_b5a_localized_evidence_review_packet import (
    ...
    write_phase3b_b5a_localized_evidence_review_packet,
)


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build Phase 3B B5A localized evidence review packet artifact."
    )
    ...
    parser.add_argument("--review-record-payload", type=Path, default=None)
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(".artifacts/phase3b_b5a_localized_evidence_review_packet_20260425"),
    )
    parser.add_argument("--output-prefix", default="b5a_localized_evidence_review_packet")
...
def main() -> int:
    ...
    report = build_phase3b_b5a_localized_evidence_review_packet(
        ...
        review_record_payload_path=args.review_record_payload,
    )
    _print_summary(report)
    if not bool(args.no_write):
        output_dir = _resolve_output_dir(project_root, args.output_dir)
        paths = write_phase3b_b5a_localized_evidence_review_packet(
            report,
            output_dir,
        ...
        for key, value in paths.items():
            print(
                "b5a_localized_evidence_review_packet_"
...
def _print_summary(report: Mapping[str, Any]) -> None:
    ...
    print("phase3b B5A localized evidence review packet")
    print(f"- review packet ready: {bool(status.get('review_packet_ready', False))}")
    print(
        "- review record validator ready: "
        f"{bool(status.get('review_record_validator_ready', False))}"
    )
    print(
        "- review record payload provided: "
        f"{bool(status.get('review_record_payload_provided', False))}"
    )
    print(
    ...
    print(
        "- review record payload validation status: "
        f"{status.get('review_record_payload_validation_status')}"
    )

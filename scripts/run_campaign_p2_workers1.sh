...
# Workers=1 search 弱会产生更多 UNKNOWN, but certified campaigns must
# preserve strict UNKNOWN terminal semantics by default.  Explicitly setting this
# env to 1 now fail-closes in run_outer_search instead of producing best-effort
# certified evidence.
export EXACT_OUTER_SKIP_UNKNOWN="${EXACT_OUTER_SKIP_UNKNOWN:-0}"

# Community blueprint hint (D step 2): user-tuned IP v2 blueprint -> Dict[instance_id, pose_idx]
# Generated by scripts/blueprint_to_master_hint.py. 224 instances override greedy hint.
# Empty value disables (greedy hint only).
...
echo "[run_campaign_p2_workers1] EXACT_COMMUNITY_BLUEPRINT_HINT_PATH=${EXACT_COMMUNITY_BLUEPRINT_HINT_PATH:-<unset, greedy only>}"

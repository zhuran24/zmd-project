#!/usr/bin/env python3
"""Build phase1_1_gpt_pro_review_v7.zip for GPT pro Phase 1.1 Step A-H audit.
...
OUT_DIR = Path("/tmp/_phase1_1_pkg_v7")
OUT_ZIP = Path("/home/zhuran24/linwin_share/phase1_1_gpt_pro_review_v7.zip")
...
FILES = [
    ...
    "docs/research/p3_b_design_v2_20260521/external_review/gpt_pro_phase1_1_v3_audit_NOT_GO.md",
    "docs/research/p3_b_design_v2_20260521/external_review/gpt_pro_phase1_1_v4_audit_round1_NOT_GO.md",
    "docs/research/p3_b_design_v2_20260521/external_review/gpt_pro_phase1_1_v4_audit_round2_NOT_GO.md",
    "docs/research/p3_b_design_v2_20260521/external_review/gpt_pro_phase1_1_v5_audit_round1_NOT_GO.md",
...
CHANGELOG = """# Commit log (Phase 1.1 Step A-O)

18 commit (含 5 build-script infra commit). 每 commit message 在 git log (commit hash 列).
...
| 3c2360d | scripts/build_phase1_1_gpt_pro_review_v3.py + 2 GPT v2 audit archive | review-pkg v3 build script |
| a38620c | families/region_capacity.py + test_family_region_capacity.py + tests F401 cleanup | F1 contributing_groups 去重 + tuple demand check + gap consistency; ruff F401 12 个清 (notes: ruff.toml ignore 让 fix silent skip — Step M force fix) |
| (v4-related infra) | scripts/build_phase1_1_gpt_pro_review_v4.py + GPT v3 audit archive | review-pkg v4 build script |
| 273fbff | replay.py + lifecycle.py (BState fields) + families/{cutset, component_reach}.py + 3 test file + ruff真清 | replay canonical_rules=None HOLD fail-closed; F2 commodity_demands registry require; F4 commodity_routes registry require; ruff F401 真清 (force fix override per-file-ignores) |
| (v5-related infra) | scripts/build_phase1_1_gpt_pro_review_v5.py + GPT v4 r1/r2 audit archive | review-pkg v5 build script |
| afef8f1 | families/cutset.py + store.py + 3 test file | F2 加 contributing 去重 + commodity_routes require + cross-partition check; CutStore.add_cut default initial_state="held" (test fixture 显式 initial_state="active" bypass) |
| (v6-related infra) | scripts/build_phase1_1_gpt_pro_review_v6.py + GPT v5 r1/r2 audit archive | review-pkg v6 build script |
| c8fb7ef | families/{region_capacity, cutset, component_reach}.py + store.py + 2 test file | F1 GHOST_AGNOSTIC 时验 ghost∩R=∅; F2/F4 reject GHOST_AGNOSTIC (Phase 1.1 fail-closed); on_ghost_rect_changed lazy import replay_cut full validator gate; add_cut initial_state validate 前置 |
"""

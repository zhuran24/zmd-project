...
from src.search.phase3b_family_lookup_medium_repro import (
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a medium standalone Element plus family lookup diagnostic repro."
    )
    ...
    parser.add_argument(
        "--variants",
        default=",".join(DEFAULT_MEDIUM_REPRO_VARIANTS),
        help="Comma-separated medium repro variants.",
    )
    parser.add_argument("--slot-limit", type=int, default=763)
    parser.add_argument("--powered-slot-limit", type=int, default=763)
    parser.add_argument(
        "--powered-template-filter",
        default=None,
        help="Optional powered-slot template filter, e.g. protocol_storage_box.",
    )
    parser.add_argument("--family-limit-per-slot", type=int, default=9999)
    parser.add_argument(
        "--solver-profile-json",
        default=None,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(".artifacts/phase3b_family_lookup_medium_repro"),
    )
...
def main() -> int:
    ...
    report = build_phase3b_family_lookup_medium_repro(
        ...
        slot_limit=int(args.slot_limit),
        powered_slot_limit=int(args.powered_slot_limit),
        powered_template_filter=_parse_optional(args.powered_template_filter),
        family_limit_per_slot=int(args.family_limit_per_slot),
        solver_parameter_profile=_parse_solver_profile(args.solver_profile_json),
    )
    _print_summary(report)
    if not args.no_write:
        output_dir = _resolve_output_dir(project_root, args.output_dir)
        json_path = output_dir / "family_lookup_medium_repro.json"
        md_path = output_dir / "family_lookup_medium_repro.md"
        txt_path = output_dir / "family_lookup_medium_repro.txt"
        atomic_write_json(json_path, report)
        _atomic_write_text(
            md_path,
            render_phase3b_family_lookup_medium_repro_markdown(report),
        )
        _atomic_write_text(
            txt_path,
            render_phase3b_family_lookup_medium_repro_text(report),
        )
        print(f"family_lookup_medium_repro_json={_display_path(project_root, json_path)}")
        print(f"family_lookup_medium_repro_md={_display_path(project_root, md_path)}")
        print(f"family_lookup_medium_repro_txt={_display_path(project_root, txt_path)}")
    return 0
...
def _print_summary(report: Mapping[str, Any]) -> None:
    ...
    print("phase3b family lookup medium repro")
    ...
    print(f"- selected pole slots: {extraction.get('selected_slot_count', 0)}")
    print(f"- selected powered slots: {extraction.get('selected_powered_slot_count', 0)}")
...
def _parse_optional(raw_value: str | None) -> str | None:

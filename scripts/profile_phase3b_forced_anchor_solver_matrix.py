...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Profile forced-anchor exact master status across solver branching modes."
    )
    ...
    parser.add_argument(
        "--anchor-indices",
        default=None,
        help="Optional comma-separated anchor indices to evaluate instead of the first sampled anchors.",
    )
    parser.add_argument("--time-limit-seconds", type=float, default=20.0)
    parser.add_argument("--worker-count", type=int, default=4)
    parser.add_argument(
        "--search-branchings",
        default=",".join(DEFAULT_SEARCH_BRANCHINGS),
        help="Comma-separated solver branchings: fixed,automatic,portfolio.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(".artifacts/phase3b_forced_anchor_solver_matrix"),
    )
...
def main() -> int:
    ...
    report = build_phase3b_forced_anchor_solver_matrix(
        ...
        search_branchings=_parse_csv(args.search_branchings),
    )
    _print_summary(report)
    if not args.no_write:
        output_dir = _resolve_output_dir(project_root, args.output_dir)
        prefix = args.output_prefix or f"forced_anchor_solver_matrix_{str(args.candidate)}"
        json_path = output_dir / f"{prefix}.json"
        md_path = output_dir / f"{prefix}.md"
        txt_path = output_dir / f"{prefix}.txt"
        atomic_write_json(json_path, report)
        _atomic_write_text(md_path, render_phase3b_forced_anchor_solver_matrix_markdown(report))
        _atomic_write_text(txt_path, render_phase3b_forced_anchor_solver_matrix_text(report))
        print(f"forced_anchor_solver_matrix_json={_display_path(project_root, json_path)}")
        print(f"forced_anchor_solver_matrix_md={_display_path(project_root, md_path)}")
        print(f"forced_anchor_solver_matrix_txt={_display_path(project_root, txt_path)}")
    return 0
...
def _print_summary(report: Mapping[str, Any]) -> None:
    ...
    unknown_diagnostics = _mapping(matrix.get("unknown_diagnostics"))
    print("phase3b forced-anchor solver matrix")
    ...
    print(
        ...
        f"{unknown_diagnostics.get('zero_branch_unknown_count', 0)}"
    )
    print(f"- by branching: {matrix.get('status_counts_by_branching', {})}")

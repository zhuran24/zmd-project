...
from src.search.phase3b_coordinate_validation_anchor119_row_domain_reviewer_record_prep import (
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a Phase 3B anchor119 row-domain reviewer record prep artifact."
    )
...
def main() -> int:
    ...
    if args.no_write:
        print("phase3b anchor119 row-domain reviewer record prep")
        print(
            "reviewer_record_prep_ready="
            + str(bool(report.get("status", {}).get("reviewer_record_prep_ready", False)))
        )
        print(
    ...
    json_path = output_dir / "anchor119_row_domain_reviewer_record_prep.json"
    md_path = output_dir / "anchor119_row_domain_reviewer_record_prep.md"
    txt_path = output_dir / "anchor119_row_domain_reviewer_record_prep.txt"
    ...
    print(f"anchor119_row_domain_reviewer_record_prep_json={json_path.resolve()}")
    print(f"anchor119_row_domain_reviewer_record_prep_md={md_path.resolve()}")
    print(f"anchor119_row_domain_reviewer_record_prep_txt={txt_path.resolve()}")
    return 0

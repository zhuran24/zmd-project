#!/usr/bin/env python3
"""Build phase1_2_spike_review_v20.zip — GPT v19 六审 semantics + doclag fix.
...
OUT_DIR = Path("/tmp/_phase1_2_pkg_v20")
PROJECT_DIR = OUT_DIR / "project"
SEVENZ_PATH = OUT_DIR / "project.7z"
OUT_ZIP = Path("/home/zhuran24/linwin_share/phase1_2_spike_review_v20.zip")
...
(v20 含 toy_translator F3 真 2-literal parse).

Build: master commit `9d5d7fa` (详 `COMMIT_LOG.md`) + spike overlay (7 data
file + 1 SPIKE_COMMIT_LOG, 详 `SPIKE_COMMIT_LOG.md`, spike branch HEAD
`ae650f2`; data-producing Phase B rerun commit `ed48a05` post-F3-real-parse fix).
...
```bash
unzip -q phase1_2_spike_review_v20.zip
cd _phase1_2_pkg_v20
...
| (v20 build) | review-pkg v20 build script — GPT v19 六审 semantics + doclag fix |
"""
...
(per GPT v17 四审 Reviewer B patch 0002 spec; v20 含 toy_translator F3 真
2-literal parse + Python 3.14 portability fix on feasible_smoke + failfast_probe):

- `scripts/spike_prod_scale_runner.py` (v20: write_verdict_md 5 处 doclag)
- `scripts/spike_prod_scale_lib/__init__.py`
- `scripts/spike_prod_scale_lib/toy_translator.py` (v19: + port_exposure
  to nogood_families + Python 3.14 portability fix; v20: + F3 real 2-literal
  parse in _cert_literal_pairs)
- `scripts/spike_prod_scale_lib/scale_ramp.py` (v19: doclag 50 cert)
- `scripts/spike_prod_scale_lib/filter_mock.py`
- `scripts/spike_prod_scale_lib/feasible_smoke.py` (v20: Python 3.14 portability)
- `scripts/spike_prod_scale_lib/oracle_emit_fixture.py` (v19: doclag F3 real-emit)
- `scripts/spike_prod_scale_lib/off_limits_check.py`
- `scripts/spike_prod_scale_lib/telemetry.py`
- `scripts/spike_prod_scale_lib/failfast_probe.py` (v20: Python 3.14 portability)

Verdict data overlay 入包 (覆盖到 project 相同路径, v20 8 files):

- `docs/research/prod_scale_spike_design_20260525/spike_run_20260526/verdict.md`
  (v20: refresh G/raw artifact + recommended-next-step + 六审 finding close)
- `docs/research/prod_scale_spike_design_20260525/spike_run_20260526/phase_a_report.md`
...
def main() -> int:
    ...
    (PROJECT_DIR / "README.md").write_text(README_V20, encoding="utf-8")
    ...
    (OUT_DIR / "README.md").write_text(README_V20, encoding="utf-8")

#!/usr/bin/env python3
"""Build phase1_2_spike_review_v22.zip — GPT v21 八审 fix (求 clean GO 九审).

v21 → v22 change: GPT pro v21 八审 (独立 reviewer) verdict GO_WITH_MINOR /
PATCH_REQUIRED — catch 3 finding:

- V21-8F1 MED: v21 F3 malformed fail-closed incomplete. fix 放在
  `if payload is not None:` 块**内**, payload decode fail (bad b64 / 非
  dict root) 时进不到 F3 分支, fallback synthetic 3-literal hide schema
  drift; root list/string 调 .get() raise AttributeError.
  v22 fix: spike branch `66cf16e` apply patch —
  - `_decode_cert_b64`: 加 isinstance(payload, dict) guard, 非 dict
    return None (防 caller .get raise)
  - `_cert_literal_pairs`: F3 family branch 移**出** `if payload is
    not None:` 块到 fallback 之前. F3 + payload None / invalid type
    → return [] (不 fallback synthesize).
  - 加 spike-lib `test_toy_translator_f3_malformed.py` micro-probe 9 case
    (6 F3 + 3 non-F3) self-test, 实测 9/9 PASS — fail-closed contract
    锁住跟着 review-pkg 入包.

- V21-8F2 NIT: spike snapshot 4 处 source comment 措辞过 strong
  ("Python 3.14 stdlib regression where json.loads ... Verified on
  Python 3.14.5") — 没 upstream issue/CPython bug ref, observation 应
  降级为 spike-local portability workaround.
  v22 fix: spike branch `66cf16e` 改 4 处措辞 → "Observed in spike
  runner on Python 3.14.x; use read_bytes().decode('utf-8') as a
  spike-local portability workaround. No master src impact claimed."

- V21-8F3 NIT: verdict.md line 145 "GO_WITH_MINOR to v20 package ...
  Recommend v20 package build" stale ref — v21/v22 包应该有 self-ref
  而不是只指 v20.
  v22 fix: spike branch `66cf16e` 改 verdict.md → "Historical v20
  rerun verdict: ... v21/v22 packages apply post-review fixes on top of
  same data set; see README v20→v21→v22 sections for current package
  status."

Master side 另一 reviewer (readiness review) catch hardcoded `.venv` +
3 旧测试文件名 (F7/F8/F9 readiness gate) issue, 前 turn 已 land commit
`5d214f4` fix(gate-script): exit_criteria use sys.executable + F7/F8/F9
real test names. v22 包自然 carry 此 master fix.

B2 数据 **不重跑**: per GPT 八审 reviewer 明确, F3 fail-closed fix 只
影响 malformed path (root_null / root_list / root_string / bad_b64 +
missing fields). 真实 6 条 F3 fixture 全 valid, 2-literal parse 不变,
patch 只是 defensive guard.

V21 historical change (preserved in v22):

v19 → v20 change: GPT pro v19 六审 verdict GO_WITH_MINOR — catch 2
finding (1 MED semantics + 1 LOW doclag). v20 fix 全部并 rebuild 求 clean GO:

- MED semantics overclaim: v19 加 `port_exposure` 到 `nogood_families` 修了
  100K skip bug, 但 `_cert_literal_pairs()` 对 F3 走 deterministic 3-literal
  synthetic fallback (因 F3 payload 没有 `oracle_assignment_witness` key —
  F3 用 `facility_group` / `facility_pose_id` / `blocking_facility` 三 key).
  v19 commit claim "AddBoolOr(NOT facility, NOT blocker) two-literal
  no-good" 是 overclaim — F3 cert 100K oversample 11,961 条全走 3-literal
  synthetic, 不是真 F3 payload parse.
  v20 fix: spike branch `b59f19d` apply patch 0001 toy_translator —
  `_cert_literal_pairs()` 加 F3 优先解析 (family == "port_exposure" 直接
  从 payload 取 (facility_group, facility_pose_id) + (blocking_facility[0],
  blocking_facility[2]) 作 2-literal). fallback to
  oracle_assignment_witness / synthetic 仅当不是 F3.
  v20 rerun: spike branch `ed48a05` Phase B rerun. 5/5 tier 仍 PASS,
  100K xlate 从 v19 1.66s 减 to 1.27s — F3 真 2-literal (vs 3-literal
  synthetic) 轻 1/3 literal ref. 全 G threshold PASS.

- LOW doclag (4 文件): verdict.md + spike_prod_scale_runner.py
  write_verdict_md + README.md 多处旧 B2 数字残留:
  - Finding 5 #3 "build 2.04–3.39s, RSS 0.61–0.83 GB, solve 0.73–0.87s"
  - Layer 2 risk #1 "solve_wall_s 0.7-0.9s"
  - Unexpected #3 "0.83 GB"
  - wall 解释 "3.4s for 100K"
  - "v14 package" stale ref → "v20 package"
  - README G8 行措辞 "raw rss_sample max 1.03GB" 不准 (实际 raw
    rss_sample max=0.979 GB; 1.03 GB 来自 rss_sample_after_solve event).
  v20 fix: spike branch `b59f19d` runner write_verdict_md 5 处 +
  verdict.md 5 处 + master README G8 行措辞拆 sample type.

v20 also re-uses the v18-v19 framework verbatim (all v17→v19 changes still
hold): code_context/spike/ overlay (MINOR #2 v18), Gemini F3 archive
(MINOR #3 v18), active_port_witness P1.5+ risk (MINOR #4 v18), F3
self-blocker guard (MINOR #5 v18), v18 doclag (MINOR #1 v18),
toy_translator port_exposure nogood_families fix (v19), code_context
manifest cmd (v19).

v19 六审 patches/0001 union of 4 hunks:
- scripts/spike_prod_scale_lib/toy_translator.py: F3 payload real-parse
- scripts/spike_prod_scale_runner.py write_verdict_md: 5 处 doclag
- docs/research/.../verdict.md: 5 处 doclag
- README.md (build script output): G8 行措辞

Historical v17→v18 change set (still in v20):
...
OUT_DIR = Path("/tmp/_phase1_2_pkg_v22")
PROJECT_DIR = OUT_DIR / "project"
SEVENZ_PATH = OUT_DIR / "project.7z"
OUT_ZIP = Path("/home/zhuran24/linwin_share/phase1_2_spike_review_v22.zip")
...
SPIKE_OVERLAY_FILES = [
    ...
    # v19 rerun telemetry (post nogood_families port_exposure fix) — kept for history
    "data/cuts/spike/telemetry_128896.jsonl",
    # v20 rerun telemetry (post F3 real-literal-parse) — primary
    "data/cuts/spike/telemetry_278858.jsonl",
]
...
SPIKE_CODE_SNAPSHOT_FILES = [
    ...
    # v22: F3 malformed micro-probe (GPT V21-8F1 lock) — 9 case self-test
    # verifying _cert_literal_pairs fail-closed contract for malformed F3
    # cert payload (bad b64, non-dict root, missing fields).
    "scripts/spike_prod_scale_lib/test_toy_translator_f3_malformed.py",
]
...
v21 之后 GPT pro v21 八审 (独立 reviewer) verdict GO_WITH_MINOR /
PATCH_REQUIRED — catch 3 finding (1 MED F3 malformed fail-closed incomplete +
2 NIT). 同期另一 reviewer 跑 readiness review catch 1 master gate script issue
(hardcoded `.venv` + 3 旧测试文件名). v22 全 fix:

| Commit | Branch | Subject |
|---|---|---|
| `5d214f4` | master | fix(gate-script): exit_criteria use sys.executable + F7/F8/F9 real test names |
| `66cf16e` | spike | [SPIKE-V22-PATCH] F3 malformed fail-closed completion + Python 3.14 wording + verdict v20→v21 stale ref |
| (v22 build) | master | review-pkg v22 build script — GPT 八审 V21-8F1 MED + 2 NIT fix |

八审 finding fix 状态:

- **V21-8F1 (MED) F3 malformed fail-closed incomplete — CLOSED**:
  v21 fix 把 F3 fail-closed 放在 `if payload is not None:` 块**内**,
  payload decode fail 时 (bad b64 / 非 dict root 例如 list / string)
  进不到 F3 分支 → fallback synthetic 3-literal hide schema drift; 加上
  payload root 非 dict 时 `payload.get()` raise AttributeError.
  v22 fix (spike `66cf16e`):
  - `_decode_cert_b64`: 加 `isinstance(payload, dict)` guard, 非 dict
    return None (防 caller .get raise)
  - `_cert_literal_pairs`: F3 family branch 移**出** `if payload is not
  ...
  patch 是 defensive guard.

- **V21-8F2 (NIT) spike snapshot Python 3.14 wording 过 strong — CLOSED**:
  v21 spike snapshot 4 处 source comment 写 "Python 3.14 stdlib regression
  where json.loads ... Verified on Python 3.14.5" — 没 upstream CPython
  issue / bug report ref, observation 应降级为 spike-local portability
  workaround 而非 "已证实 stdlib regression".
  v22 fix (spike `66cf16e`): 4 处措辞 (toy_translator.py 1 + failfast_probe.py
  1 + feasible_smoke.py 2) → "Observed in spike runner on Python 3.14.x;
  use read_bytes().decode('utf-8') as a spike-local portability workaround.
  No master src impact claimed."

- **V21-8F3 (NIT) verdict.md `GO_WITH_MINOR to v20 package` stale ref — CLOSED**:
  v21 verdict.md line 145 仍说 "Recommend v20 package build" — v21/v22 包
  应有 self-ref.
  v22 fix (spike `66cf16e`): 改 "Historical v20 rerun verdict:
  GO_WITH_MINOR ... v21/v22 packages apply post-review fixes on top of
  same data set; see README v20→v21→v22 sections for current package
  status (this verdict block is preserved as the underlying spike run
  conclusion, not a re-issued recommendation)."

另一 reviewer (readiness review, 非 GPT 八审同人) catch fix:

- **Readiness gate script hardcoded `.venv` + 3 旧测试文件名 — CLOSED**:
  `scripts/production_readiness_gate.py` exit_criteria 段 hardcode
  `.venv/bin/python` (在 conda / 系统 Python 环境 OOB) 加 reference 3 旧测
  试文件名 (F7/F8/F9 readiness gate test 已 rename). v22 master commit
  `5d214f4` fix: 改用 `sys.executable` + 改 reference 真名.

## v20 → v21 状态变化

v20 之后 GPT pro v20 七审两 reviewer (A + B) 共识 GO_WITH_MINOR + patch 后
CLEAN GO. 两 reviewer finding overlap 不完全, v21 取 union 全 fix:

| Commit | Branch | Subject |
|---|---|---|
| `d342784` | master | refactor(F9): split generate_density_envelope_cuts to fix radon D(27) → B(10) |
| `a2338e4` | spike | [SPIKE-V21-PATCH] F3 fail-closed + verdict raw artifacts + G rows label + lint |
| (v21 build) | master | review-pkg v21 build script — v20 七审 union fix |

七审 finding union fix 状态:

- **Reviewer A V20-7F1 (LOW) verdict.md Raw artifacts telemetry count stale — CLOSED**:
  `verdict.md` Raw artifacts 漏写 `5 rss_sample_after_solve` (`phase_b_results.json`
  + README 都有). v21 spike `a2338e4` 补 verdict.md + spike_prod_scale_runner
  generator (regen 不复发).

- **Reviewer A V20-7F2 (LOW/NIT) F3 malformed payload synthetic fallback — CLOSED**:
  当前 6 条有效 F3 fixture 都 2-literal parse, 但 malformed/schema-drift F3
  仍 fall through 到 generic synthetic 3-literal fallback (隐藏 schema drift).
  v21 spike `a2338e4` toy_translator F3 valid → return [2-literal]; F3 invalid
  → return [] (caller skip), 非 F3 family 继续 witness/synthetic fallback.

- **Reviewer A V20-7F3 (NIT) verdict.md G rows label 误读 — CLOSED**:
  G2/G3/G4/G4b 数字 = `build_wall_s + translation_wall_s` 但 label 写 `build`;
  G8 写 `1.03GB` 没区分 sample type. v21 spike `a2338e4` label → `build+translate`;
  G8 → `after-solve max 1.0316GB`.

- **Reviewer B F9 density_envelope_oracle radon D(27) — CLOSED**:
  `generate_density_envelope_cuts` 函数 radon D(27), README claim "no D" 不符.
...
- **Reviewer B README radon stale — CLOSED**:
  ...
  v21 update → "average B (5.16), max C(20), no D" (F9 refactor 后实测).

- **Reviewer B spike code lint (no-placeholder f-string) — CLOSED**:
  v21 spike `a2338e4` 清理 `code_context/spike/spike_prod_scale_lib/feasible_smoke.py`
  / `toy_translator.py` / `spike_prod_scale_runner.py` 几处 `f"..."` 无 `{var}`
  → 改 `"..."` (runtime 行为不变).

注: Python 3.14 portability tweak 仅是 spike runner 本地 observation 上的
workaround (read_bytes().decode("utf-8") 替 read_text)，spike-only，无 master src
影响. 不作为 "已证实 stdlib regression" 主张.

## v19 → v20 状态变化

v19 (post-port_exposure-nogood-fix + doclag) 之后 GPT pro v19 六审
verdict GO_WITH_MINOR — catch 2 个 finding (1 MED semantics + 1 LOW doclag).
v20 全 fix:

| Commit | Branch | Subject |
|---|---|---|
| `b59f19d` | spike | [SPIKE-V20-PATCH-0001] toy_translator F3 real literal parse + verdict/runner doclag |
| `ed48a05` | spike | [SPIKE-V20-RERUN-B2] B2 + Phase B rerun with F3 real literal parse |
| `ae650f2` | spike | [SPIKE-V20-VERDICT-UPDATE] G rows refresh per actual v20 rerun 数字 |
| (v20 build) | master | review-pkg v20 build script — GPT v19 六审 semantics + doclag fix |

六审 finding fix 状态:

- **MED semantics overclaim (F3 走 synthetic 3-literal 不是真 2-literal) — CLOSED**:
  Root cause = v19 加 `port_exposure` 到 `nogood_families` 修了 skip bug,
  但 `_cert_literal_pairs()` 对 F3 走 deterministic 3-literal synthetic
  fallback (因 F3 payload 没有 `oracle_assignment_witness` key — F3 用
  `facility_group` / `facility_pose_id` / `blocking_facility` 三 key 表达
  cert 主体). v19 commit claim "AddBoolOr(NOT facility, NOT blocker)
  two-literal no-good" 是 overclaim — 100K F3 oversample 11,961 条全走
  synthetic. v20 fix (spike `b59f19d`): `_cert_literal_pairs()` 加
  family == "port_exposure" 优先解析分支, 直接从 payload 取
  (facility_group, facility_pose_id) + (blocking_facility[0],
  blocking_facility[2]) 作 2-literal. fallback to
  oracle_assignment_witness / synthetic 仅当不是 F3.

  v20 rerun (spike `ed48a05`) Phase B verify:

  | Tier | Target | Applied | Match | xlate (s) |
  |---|---|---|---|---|
  | 0 | 0 | 0 | YES | 0.00 |
  | 1K | 1,000 | 1,000 | YES | 0.01 |
  | 10K | 10,000 | 10,000 | YES | 0.14 |
  | 50K | 50,000 | 50,000 | YES | 0.61 |
  | 100K | 100,000 | 100,000 | YES | 1.27 |

  100K xlate 从 v19 1.66s 降到 v20 1.27s (-23%) — F3 真 2-literal
  vs synthetic 3-literal 轻 1/3 literal ref. 5/5 tier 仍 match. 全 G
  threshold PASS.

- **LOW doclag (verdict.md + runner + README 多处旧数字残留) — CLOSED**:
  v19 verdict.md / spike_prod_scale_runner.write_verdict_md 多处旧 B2
  数字 (Finding 5 #3 "build 2.04-3.39s / RSS 0.61-0.83 GB / solve
  0.73-0.87s", Layer 2 risk "0.7-0.9s", Unexpected #3 "0.83 GB", wall
  解释 "3.4s for 100K", "v14 package" stale ref) → v20 全 refresh per
  v20 actual rerun 数字. README G8 行措辞 "raw rss_sample max 1.03 GB"
  不准 (实际 raw rss_sample max=1.010 GB; 1.03 GB 来自
  rss_sample_after_solve event) → v20 拆 sample type 明确两 source.

verdict.md auto-refresh (spike `ed48a05` 含 step 2 runner regen + `ae650f2`
含 step 3 数字微调) 含 v20 patch + B2 rerun commit SHA + recommended-
next-step "v20 package + 六审 finding closed statement" + v20 actual
rerun 数字 (build 1.94–2.09s, xlate 0.00–1.27s, solve 0.72–0.97s).

## v18 → v19 状态变化 (历史, 全部仍 in v20)

v18 五审 catch B2 100K cut_count_applied 88,039 not 100,000 — root cause
toy_translator `nogood_families` set 缺 `port_exposure`. v19 spike
`b0b8bef` fix nogood_families + 3 file doclag, `4e80405` Phase B rerun
verify 5/5 tier applied == target, `2b11147` verdict refresh + raw
artifact telemetry. v20 进一步发现 v19 commit claim "two-literal" 是
overclaim (走 synthetic 3-literal) — 见上面 v19→v20 段.

## v17 → v18 状态变化 (历史, 全部仍 in v20)
...
`f7b88b6`), 19 commit (v18 加 doclag + v19 加 nogood_families fix + B2 rerun
+ verdict-update + v20 加 F3 真 2-literal parse + B2 rerun + verdict-update),
Phase A (A1/A2/A3) + Phase B (B1-B6) + verdict-fix. 实施 code 不入 canonical
scripts/ (PR #1 verdict-only style); v18 起 review-only `code_context/spike/`
镜像; v20 实测数据 8 file overlay 进 zip:
...
- `data/cuts/spike/scale_ramp_results.jsonl` — B2 5-tier ramp 0/1K/10K/50K/100K
  实测 (v20 rerun, 5/5 tier `cut_count_applied` == `cut_count_target`)
- `data/cuts/spike/phase_b_results.json` — Phase B aggregate (B3 feasible_smoke
  10K cut + B2 ramp + B4 filter_mock 10 iter + G/N criteria pass map; v20 rerun)
...
- `data/cuts/spike/telemetry_128896.jsonl` — v19 rerun raw telemetry (历史保留 —
  post-nogood_families fix 但 F3 仍走 synthetic, 208 rss_sample + 14 proto_sample
  + 5 rss_sample_after_solve + 1 dark_matter_emit)
- `data/cuts/spike/telemetry_278858.jsonl` — v20 rerun raw telemetry (post-F3
  real-literal-parse, 205 rss_sample + 14 proto_sample + 5 rss_sample_after_solve
  + 1 dark_matter_emit; pid 278858, primary verdict data source)

### Spike verdict — G6a SOFT-FAIL only

| Criterion | Threshold | Actual (v20 rerun) | Status |
|---|---|---|---|
| G1 build 0 cut | ≤ 10s | 1.94s | PASS |
| G2 build 1K cut | ≤ 20s | 1.99s | PASS |
| G3 build 10K cut | ≤ 30s | 2.07s | PASS |
| G4 build 50K cut | ≤ 300s | 2.00s | PASS |
| G4b build 100K cut | ≤ 600s | 2.09s | PASS |
| G5 0 cut feasibility solve | ≤ 30s | 0.72s (OPTIMAL) | PASS |
| G7 100K solve wall (measure) | — | 0.97s (OPTIMAL) | n/a |
| G8 RSS peak | ≤ 20 GB | rss_peak_gb_after_solve max 1.0316 GB @ 100K; raw telemetry max including rss_sample_after_solve event = 1.0316 GB; rss_sample-only event max = 1.010 GB | PASS |
| G9 proto @ 50K | ≤ 500 MB | 17.9 MB | PASS |
| G9 proto @ 100K | ≤ 1 GB | 19.6 MB | PASS |
...
| 3 | build wall / proto / RSS / solve wall 实测 | B2 ramp (v20 rerun, F3 real 2-literal): build 1.94–2.09s + translation 0.00–1.27s, proto 16.3–19.6 MB, build RSS 0.84–0.90 GB; after-solve RSS max 1.0316 GB; solve 0.72–0.97s across 0–100K (5/5 tier `cut_count_applied` == `cut_count_target` post toy_translator F3 real-literal-parse fix) | YES |
...
radon cc src/cuts/ -s -a: average B (5.16), max C(20), no D
...
head -20 data/cuts/spike/telemetry_278858.jsonl   # v20 primary rerun (F3 real 2-literal)
head -20 data/cuts/spike/telemetry_128896.jsonl   # v19 historical (nogood_families fix, F3 still synthetic)
head -20 data/cuts/spike/telemetry_77754.jsonl    # pre-v19 historical (pre-port_exposure-fix)
...
- `scale_ramp_results.jsonl` — B2 5-tier ramp 0/1K/10K/50K/100K (v20 rerun
  `cut_count_applied` == `cut_count_target` 全 5/5)
- `phase_b_results.json` — Phase B aggregate (v20 rerun)
- `telemetry_278858.jsonl` — v20 rerun raw telemetry (205 rss_sample + 14
  proto_sample + 5 rss_sample_after_solve + 1 dark_matter_emit; primary,
  post-F3-real-literal-parse fix)
- `telemetry_128896.jsonl` — v19 rerun raw telemetry (历史保留 — 208
  rss_sample + 14 proto_sample + 5 rss_sample_after_solve + 1 dark_matter_emit;
  post-nogood_families fix 但 F3 仍走 synthetic)
...
def fetch_spike_commit_log() -> str:
    """Dump spike branch commit log (19 commit on top of master)."""
...
v18 五审) + v20 F3 real 2-literal parse + B2 rerun + verdict-update (per GPT
...
(per GPT v17 四审 Reviewer B patch 0002 spec; v22 含 v21 F3 fail-closed +
F9 generator refactor + lint + Python 3.14 wording 降级 + F3 malformed
micro-probe self-test 11 files):

- `scripts/spike_prod_scale_runner.py` (v21: write_verdict_md G2-G4b label
  `build` → `build+translate`, G8 `1.03GB` → `after-solve max 1.0316GB`,
  Raw artifacts 加 `rss_sample_after_solve` count, lint cleanup)
- `scripts/spike_prod_scale_lib/__init__.py`
- `scripts/spike_prod_scale_lib/toy_translator.py` (v19: + port_exposure
  to nogood_families + Python 3.14 portability tweak; v20: + F3 real 2-literal
  parse in _cert_literal_pairs; v21: F3 valid → return [2-literal], F3 invalid
  → return [] when payload-is-not-None; v22: _decode_cert_b64 加 isinstance
  dict guard + F3 family branch 移出 `if payload is not None:` 块 → F3 +
  bad b64 / non-dict root → [] (不 raise / 不 synthesize); 4 处 Python 3.14
  wording 降级 → spike-local portability workaround)
- `scripts/spike_prod_scale_lib/scale_ramp.py` (v19: doclag 50 cert)
- `scripts/spike_prod_scale_lib/filter_mock.py`
- `scripts/spike_prod_scale_lib/feasible_smoke.py` (v20: Python 3.14
  portability tweak; v21: lint cleanup; v22: 2 处 Python 3.14 wording 降级)
...
src 影响. v22 已把 4 处 source comment 措辞降级为 "Observed locally on Python
3.14.x; spike-local portability workaround; no master src impact claimed"
(per GPT V21-8F2 NIT) — 不作为 "已证实 stdlib regression" 主张.

Verdict data overlay 入包 (覆盖到 project 相同路径, v22 8 files — v20 数据 +
v21 verdict.md 行级 fix + v22 verdict.md `v20 package` stale ref 改):

- `docs/research/prod_scale_spike_design_20260525/spike_run_20260526/verdict.md`
  (v21: G2-G4b label `build` → `build+translate`, G8 `1.03GB` → `after-solve
  max 1.0316GB`, Raw artifacts 加 `5 rss_sample_after_solve`)
- `docs/research/prod_scale_spike_design_20260525/spike_run_20260526/phase_a_report.md`
- `data/cuts/spike/oracle_emit_fixture_45cert.jsonl` (50 cert × 9 family)
- `data/cuts/spike/scale_ramp_results.jsonl` (v20 rerun, 5/5 tier match)
- `data/cuts/spike/phase_b_results.json` (v20 rerun)
- `data/cuts/spike/telemetry_77754.jsonl` (pre-v19 historical: 204 rss + 14
  proto + 1 dark_matter + 5 rss_sample_after_solve)
- `data/cuts/spike/telemetry_128896.jsonl` (v19 historical: 208 rss + 14
  proto + 1 dark_matter + 5 rss_sample_after_solve; nogood_families fix but
  F3 still synthetic)
- `data/cuts/spike/telemetry_278858.jsonl` (v20 rerun primary: 205 rss + 14
  proto + 1 dark_matter + 5 rss_sample_after_solve; post-F3-real-literal-parse)
...
def main() -> int:
    ...
    (PROJECT_DIR / "README.md").write_text(README_V22, encoding="utf-8")
    ...
    for rel_str in SPIKE_CODE_SNAPSHOT_FILES:
        ...
        if not check_path.exists():
            ...
            return 1
    print(f"code_context/spike/ snapshot present check: {len(SPIKE_CODE_SNAPSHOT_FILES)}/{len(SPIKE_CODE_SNAPSHOT_FILES)} files OK")
    ...
    (OUT_DIR / "README.md").write_text(README_V22, encoding="utf-8")

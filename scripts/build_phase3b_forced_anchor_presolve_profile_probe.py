...
from src.search.phase3b_forced_anchor_presolve_profile_probe import (
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Probe forced-anchor base across CP-SAT presolve/search profiles."
    )
    ...
    parser.add_argument(
        "--profiles-json",
        default=None,
        help="Optional JSON list of solver parameter profiles.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(".artifacts/phase3b_forced_anchor_presolve_profile_probe"),
    )
...
def main() -> int:
    ...
    if not args.no_write:
        ...
        json_path = output_dir / "forced_anchor_presolve_profile_probe.json"
        md_path = output_dir / "forced_anchor_presolve_profile_probe.md"
        txt_path = output_dir / "forced_anchor_presolve_profile_probe.txt"
        atomic_write_json(json_path, report)
        _atomic_write_text(
            md_path,
            render_phase3b_forced_anchor_presolve_profile_probe_markdown(report),
        )
        _atomic_write_text(
            txt_path,
            render_phase3b_forced_anchor_presolve_profile_probe_text(report),
        )
        print(f"forced_anchor_presolve_profile_probe_json={_display_path(project_root, json_path)}")
        print(f"forced_anchor_presolve_profile_probe_md={_display_path(project_root, md_path)}")
        print(f"forced_anchor_presolve_profile_probe_txt={_display_path(project_root, txt_path)}")
    return 0
...
def _print_summary(report: Mapping[str, Any]) -> None:
    ...
    print("phase3b forced-anchor presolve profile probe")
...
def _parse_profiles(raw_value: str | None) -> list[dict[str, Any]]:
    if raw_value is None or not str(raw_value).strip():
        return [dict(profile) for profile in DEFAULT_PRESOLVE_PROFILE_PROFILES]

...
This program is designed to be supplied to a fixed isolated Python interpreter
on standard input.  Its privileged surface is intentionally limited to
same-file-descriptor reads of procfs and the running manager executable.  It
does not write files, send signals, start subprocesses, or access the network.
Every process outcome is one strict JSON object on stdout.
"""
...
SCHEMA = "privileged-systemd-manager-exe-attestation-v1"
...
class AttestorError(RuntimeError):
    """A fail-closed request, procfs, or executable identity failure."""
...
def _hash_regular_fd(
    ...
    if before.st_size <= 0:
        raise AttestorError(f"{label}: empty executable")
    if before.st_size > MAX_EXECUTABLE_BYTES:
        raise AttestorError(f"{label}: executable exceeds the fixed size cap")
    try:
    ...
    while True:
        ...
        if total > MAX_EXECUTABLE_BYTES:
            raise AttestorError(f"{label}: executable exceeds the read cap")
        digest.update(block)
    after = os.fstat(descriptor)
...
def _manager_executable(pid: int) -> dict[str, object]:
    ...
    try:
        ...
        if not os.path.isabs(target_raw):
            raise AttestorError("manager executable: target is not an absolute path")
        if target_raw.endswith(b" (deleted)"):
            raise AttestorError("manager executable: target is deleted")
        try:
            target = target_raw.decode("utf-8", "strict")
        except UnicodeDecodeError as exc:
            raise AttestorError("manager executable: target is not strict UTF-8") from exc

        target_flags = os.O_RDONLY | os.O_CLOEXEC | os.O_NOFOLLOW
        try:
            target_fd = os.open(target_raw, target_flags)
        except OSError as exc:
            raise AttestorError(f"manager executable: cannot O_NOFOLLOW-open target {target!r}: {exc}") from exc
        try:
            actual_before = os.fstat(actual_fd)
            target_before = os.fstat(target_fd)
            if not stat.S_ISREG(actual_before.st_mode):
                raise AttestorError("manager executable: proc executable is not regular")
            if not stat.S_ISREG(target_before.st_mode):
                raise AttestorError("manager executable: target is not regular")
            if (
                actual_before.st_dev,
                actual_before.st_ino,
            ) != (
                target_before.st_dev,
                target_before.st_ino,
            ):
                raise AttestorError("manager executable: actual and target dev/inode differ")
        ...
        if _stable_regular_identity(actual_after) != _stable_regular_identity(target_after):
            raise AttestorError("manager executable: actual and target identities differ")
        if actual_sha != target_sha or actual_size != target_size:
            raise AttestorError("manager executable: actual and target bytes differ")
        try:
            final_target_raw = os.readlink(os.fsencode(proc_exe))
            final_proc_stat = os.stat(proc_exe, follow_symlinks=True)
        except OSError as exc:
            raise AttestorError(f"manager executable: final proc recheck failed: {exc}") from exc
        if final_target_raw != target_raw:
            raise AttestorError("manager executable: target changed during attestation")
        if (
            final_proc_stat.st_dev,
            final_proc_stat.st_ino,
        ) != (
            actual_after.st_dev,
            actual_after.st_ino,
        ):
            raise AttestorError("manager executable: proc dev/inode changed during attestation")
        return {
            "path": target,
            "size_bytes": target_size,
            "mode": stat.S_IMODE(target_after.st_mode),
            "mode_octal": f"{stat.S_IMODE(target_after.st_mode):04o}",
            "sha256": target_sha,
...
def _parse_arguments(argv: list[str]) -> tuple[int, int, str, str]:
    if len(argv) != 8:
    ...
    if tuple(argv[0::2]) != expected_names:
...
def main() -> int:
    try:
        pid, expected_starttime, expected_boot_id, owner = _parse_arguments(sys.argv[1:])
        starttime_before = _pid_starttime(pid)
        boot_id_before = _boot_id()
        if starttime_before != expected_starttime:
            raise AttestorError("manager starttime differs from the unprivileged request")
        if boot_id_before != expected_boot_id:
            raise AttestorError("boot_id differs from the unprivileged request")
        ...
        if starttime_after != starttime_before:
            raise AttestorError("manager starttime changed during privileged attestation")
        if boot_id_after != boot_id_before:
            raise AttestorError("boot_id changed during privileged attestation")
        _emit(
            {
                "schema": SCHEMA,
                "status": "PASS",

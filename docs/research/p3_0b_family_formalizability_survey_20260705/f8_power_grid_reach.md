> **【终态注 2026-07-08，F8 已退役（前提为假）】** 本文是 2026-07-05 对 pre-M2 代码的调研快照。M2 批 B 由 owner 以游戏规则关闭：「电线杆不需要连电网……协议核心会自动连上它们」——**F8 的数学前提（电杆须经 pole-jump 链连通 protocol_core 才供电）为假**，整个 family 定性 retired-false-premise，任何 F8 cut 都是对合法布局的潜在 false-INFEASIBLE。certified 主链无恙（六谓词从未要求电杆连通）。**形式化 F8 取消**（非解锁）。物理删除搭 M3 step_8 reseal 批；当前代码为惰性墓碑（generator 默认关）。批 A 曾把 CoverSet 半统一到 12×12 stencil（`power_grid_reach_v2_coverset_stencil`），该改动对 F7 仍有效。本文全部内容自此为史料，供后人理解 F8 当年的设计与它为何而死。裁定登记：记忆卡 `p1-3-m2-coverage-stencil-ruling`。

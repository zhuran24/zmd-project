...
- **模型修正，非命题变更（owner 07-18 原则性纠正，点火时采纳）**：要求
  的最优解从未变——始终是游戏 1.1 四号谷地的真最优；变的是模型对游戏
  的忠实度。可行域三向变化（+2,064 补域、箱口重生成、中枢 14 进 sink）
  是**修正旧模型误差**，此前一切"池内最优/UNSAT/上界"数字是误差品
  而非旧命题的合法结论（批 4 在修正后的模型上重跑才有终值）；

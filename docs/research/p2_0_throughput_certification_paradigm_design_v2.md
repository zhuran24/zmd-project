...
## v3.1 pending 修订记录(2026-07-11 凌晨,owner 亲自提出,待 P2.0b 实现时正式吸收)

**动因(owner 原话大意)**:「流量没问题」的真语义=布局从基地开始运转,经有限瞬态进入循环态(周期稳态),且该循环态永续。「布局没问题」与「布局的流量没问题」等价(其余六谓词均静态)。

**修订一:A12 升级——启动可达性从可选转必要**。TP7-D 周期日历只证「循环态存在+永续」(起点=周期初态);CE3 已实证「稳态存在但空启动死锁」的布局真实存在。发布级证书必须包含从真实初态的可达性证明,A12 的「另给 finite startup trace」不再是可选。

**修订二:真实初态=玩家可达初态集合(非全空网)**。游戏事实(owner 核证):玩家不能把物料放到传送带中段,但**可以把物料放进单位的输入缓存格**(实例:植物循环器启动需手动放种子/作物进种植机/采种机缓存;该机制是通用初始化通道,不限于自循环对)。据此发布级证书扩为三件套:
1. **bootstrap manifest**:预置清单(machine_id → {commodity: count}),verifier 查合法性(缓存容量/物品合法域/可获得性);它同时是给玩家的启动说明书。
2. **startup trace**:从 manifest 初态(传送带空+缓存按 manifest)逐 tick 仿真至系统全状态与周期日历某 tick 吻合(进入轨道),verifier 重放。
3. **周期日历**(原 TP7-D):进轨后由周期闭合+确定性给出永续。
红测:CE3 布局+空 manifest → 拒;CE3+正确种子 manifest+trace → 过。

**不变式(重要)**:TP7-S Farkas 不可行证书不受影响——预置是一次性注入,循环态周期闭合要求物料净守恒(缓存存量含在状态闭合内),预置只选择吸引域、不改变循环态存在性。静态不可行的布局预置救不了。分工:预置管可达性,静态层管存在性。

**新语义缺口 D9:缓存预置合法域**——哪些单位有输入缓存/容量/可放物品域(仅配方输入?),经 `throughput_semantics` 机器化消解(同 D1-D8)。**附带公理候选(可获得性)**:manifest 物品限「外部可得类」,封 bootstrap 悖论(布局要求预置只有它自己能造的中间品);争议场景留裁定。

**数学注记**:状态有限+演化确定 ⇒ 任意轨迹=有限瞬态+循环(鸽巢),「进入某循环」结构免费;全部证明重量在「进入的循环良性(达标)+瞬态可给出可重放」——manifest+trace 恰好承担,无需新数学。

**v3.1 补充(同日,owner 追问「有限时间过长怎么办/『可接受时间』如何定义」)**:
1. **`startup_ticks` 显式字段**:trace 长度 L 即启动时长的精确后验值,入证书封面字段——「存在有限时间」在证书形态下升级为「时长=具体数字+可重放」,不存在未定义量。
2. **verifier 格式上限**:L ≤ 工程上限(数值待定,10⁶ tick 量级候选),超限拒收(资源保护,非质量判断)。
3. **「可接受阈值」明确外置**:谓词层只承诺 L 有限且精确报告;阈值是发布策略参数(owner 可后定可修改,不碰谓词)/玩家消费口(说明书直接给启动时长)。理论注记:一般带仲裁系统瞬态无实用先验上界(marked graph 类才有多项式界),后验实测是正确工程选择;缩短 L 的手段在 manifest 侧(预置越接近轨道瞬态越短;传送带在途不可预置故 L>0 常态),启动时长天然成为布局质量指标(未来目标函数低优先级尾巴候选,后话)。

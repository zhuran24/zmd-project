...
| G10 oracle real-emit 45 cert (A3) | ≥45 + 0 unsound | 44 cert / 0 unsound | PASS (A3 phase_a_report) |
...
| 2 | 真 cut body 分布 (replacing toy 1-3-5 literal) | A3 jsonl 44 cert × 9 family with real `pose_count` / `cell_count` / `literal_count` per cert | YES |
...
- A3 oracle fixture: `data/cuts/spike/oracle_emit_fixture_45cert.jsonl` (44 cert × 9 family)

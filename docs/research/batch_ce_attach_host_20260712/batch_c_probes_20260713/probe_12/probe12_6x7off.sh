#!/bin/bash
# probe_12 = cap 收敛验证臂:6×6 attach-on + EXACT_B1_BINDING_ALT_CAP=1500
...
.venv/bin/python docs/research/batch_ce_attach_host_20260712/attach_host_runner.py \
  --ghost-w 6 --ghost-h 7 --attach off --workers 6 --master-seconds 1800 \
  --run-tag "batch_c_probe_12_6x7" --out "$D/cell.json" > "$D/run.log" 2>&1 &
PYPID=$!
echo "$(date +%T) probe12 solve pid=$PYPID (cap=1500)" >> "$D/probe12.log"
t0=$SECONDS
...
if kill -0 $PYPID 2>/dev/null; then
  ...
  echo "$(date +%T) probe12 TIMEOUT@5400s" >> "$D/probe12.log"
else
  wait $PYPID; RC=$?
  echo "$(date +%T) probe12 EXITED rc=$RC cell=$([ -f $D/cell.json ] && echo yes || echo no) wall=$((SECONDS-t0))s" >> "$D/probe12.log"
fi

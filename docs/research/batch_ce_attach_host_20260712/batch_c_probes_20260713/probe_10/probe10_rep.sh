#!/bin/bash
# probe_10 = cap 收敛验证臂:6×6 attach-on + EXACT_B1_BINDING_ALT_CAP=1500
...
D=/tmp/claude-1000/-home-zhuran24-zmd-pj/3e9c4e4c-c0ae-4a71-98f5-05f8b3a5a644/scratchpad/batch_c_probe_10
...
.venv/bin/python docs/research/batch_ce_attach_host_20260712/attach_host_runner.py \
  ...
  --run-tag "batch_c_probe_10_rep" --out "$D/cell.json" > "$D/run.log" 2>&1 &
PYPID=$!
echo "$(date +%T) probe10 solve pid=$PYPID (cap=1500)" >> "$D/probe10.log"
t0=$SECONDS
...
if kill -0 $PYPID 2>/dev/null; then
  ...
  echo "$(date +%T) probe10 TIMEOUT@5400s" >> "$D/probe10.log"
else
  wait $PYPID; RC=$?
  echo "$(date +%T) probe10 EXITED rc=$RC cell=$([ -f $D/cell.json ] && echo yes || echo no) wall=$((SECONDS-t0))s" >> "$D/probe10.log"
fi

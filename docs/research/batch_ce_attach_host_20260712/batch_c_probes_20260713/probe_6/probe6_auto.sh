#!/bin/bash
# probe_6 = F-5 修复验证臂:binding AUTOMATIC search + 6 worker(修复后 env 注入应真正生效)
...
EXACT_SUBPROBLEM_PARAMS="search_branching=0" \
.venv/bin/python docs/research/batch_ce_attach_host_20260712/attach_host_runner.py \
  ...
  --run-tag "batch_c_probe_6_auto" --out "$D/cell.json" > "$D/run.log" 2>&1 &
PYPID=$!
echo "$(date +%T) probe6 solve pid=$PYPID (binding AUTOMATIC+w6)" >> "$D/probe6.log"
t0=$SECONDS
...
if kill -0 $PYPID 2>/dev/null; then
  ...
  echo "$(date +%T) probe6 TIMEOUT@7200s" >> "$D/probe6.log"
else
  wait $PYPID; RC=$?
  echo "$(date +%T) probe6 EXITED rc=$RC cell=$([ -f $D/cell.json ] && echo yes || echo no) wall=$((SECONDS-t0))s" >> "$D/probe6.log"
fi

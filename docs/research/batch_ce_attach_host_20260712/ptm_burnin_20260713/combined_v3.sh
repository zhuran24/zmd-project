#!/bin/bash
# v3:cycle 2-5 [taskset 隔离: solve@cpu0-6 (w6,master1800s) + stress@cpu7-23 → 冷却 ≤43°C+驻留10min]
...
COOL_TARGET=43; COOL_MAX=1500; COOL_HOLD=600; SOLVE_WALL_MAX=3600
get_temp() { sensors 2>/dev/null | grep 'Package id 0' | sed 's/Package id 0:[^+]*+\([0-9]*\).*/\1/' | head -1; }
cd "$REPO" || exit 1
sleep 180  # 补足 cycle 1 冷段驻留
echo "$(date +%T) CYCLE 1/5 COOL end temp=$(get_temp)C (v3 接管,43C 判据)" >> "$D/cycle.log"
for i in 2 3 4 5; do
  peak=0
  echo "$(date +%T) CYCLE $i/5 HOT begin temp=$(get_temp)C (v3: taskset solve@0-6 stress@7-23)" >> "$D/cycle.log"
  ...
  sleep $COOL_HOLD
  echo "$(date +%T) CYCLE $i/5 COOL end temp=$(get_temp)C total_cool=$((SECONDS-t0))s" >> "$D/cycle.log"
done
echo "$(date +%T) COMBINED v3 DONE" >> "$D/cycle.log"

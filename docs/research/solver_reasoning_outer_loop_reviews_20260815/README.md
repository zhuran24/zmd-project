...
[`experiment_two_w0_unary_lowering_canary_20260816/`](experiment_two_w0_unary_lowering_canary_20260816/README.md) 保存 owner 已窄解冻的第二层真实消费实验：只把已证 W0 Judgment 编译成研究面的一元 binding-domain 约束，运行 baseline／observer-noop／treatment 三臂，并把局部 family 塌缩、热点迁移、资源轨迹与终点中性分账。该实验不解冻通用 D3/D4，不修改 `src/`，不产生认证或发布效力。

...
> **冻结纪律：** 本文件首次进入 Git 的提交即为协议身份。implementation 与 run 不得原地修改判据；任何偏差只能记 `PROTOCOL_VIOLATION` 或新建 v2。

## 1. 唯一实验问题

本实验只回答：

> 一个已经由独立 checker 证明的 W0 单原子 Judgment，能否被精确编译成 PortBindingModel 中的一元约束，在真实 binding→routing 路径前消灭其授权拒绝的全部观测循环，同时不扩大拒绝集合，并把后续热点与端到端代价完整记账？

它不检验跨布局普遍性，不检验通用 theorem compiler，也不要求 certified-exact 上下界移动。
...
每个 arm 前必须先以 `--coverage off` 执行 W0 独立 checker；不是 `PASS` 则该 arm 不启动。
...
即对应 `__unused__` BoolVar 等于 1。禁止添加其他约束、改变 search strategy、改变 required counts、预删其他 slot、复用 routing 观测结果或注入 selection hash 黑名单。
...
本对象要求更强的等值：lowering 只拒绝且拒绝全部 `Active_041` selection。implementation 必须比较 baseline 与 treatment CpModel proto，证明变量、原有约束、search strategy 与 objective 等值，treatment 只追加一个目标 unary linear constraint。
...
- unsafe／exploratory env override：必须为空；
- full journals 写入 `.artifacts/solver_reasoning_outer_loop_w0_unary_canary_20260816/`；tracked 只保 compact summary、hash 与判读。
...
- `j_trigger_true_count`／`j_trigger_false_count`；
...
这些是 `MODEL_SIZE`／`BOX_DOMAIN`，不冒充联合可行 binding 数。
...
热点迁移本身不判失败。未被观测的迁移、或终点语义没有更强收益时超过冻结容差的净成本回归才判负。

### 6.4 终点行

概念与公式由 [`02_ENDPOINT_METRICS_PROTOCOL_V1.md`](02_ENDPOINT_METRICS_PROTOCOL_V1.md) 定义；机器身份、当前账本 hashes 与合成扰动向量由 [`03_ENDPOINT_METRICS_PROTOCOL_V1.json`](03_ENDPOINT_METRICS_PROTOCOL_V1.json) 冻结。

W0 Judgment 不产生完整 witness，也不产生 rectangle-level 全称排除；预注册预期为：
...
必须同时核验 tracked exact-status、research ledger source 与认证／发布表面在运行前后哈希不变。该终点中性只能写“编译与测量基建”，不得写成全局 exact 进展。
...
lowering sound，但 C 仍出现任一 `Active_041` selection，或未消灭目标 port spec。
...
- 把点状 fallback 写成 family compiler 普遍成立；

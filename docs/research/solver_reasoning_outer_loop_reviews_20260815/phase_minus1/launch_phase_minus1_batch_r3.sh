...
HARNESS="${SCRIPT_DIR}/phase_minus1_harness_r3.py"
ARTIFACT_ROOT="${ROOT}/.artifacts/solver_reasoning_outer_loop_phase_minus1_20260815"
...
if [[ "${1:-}" == "--worker" ]]; then
  ...
  echo "harness_revision=r3_compact_append_only_journals_v1"
...
run_id="${1:-phase-minus1-r3-$(date -u +%Y%m%dT%H%M%SZ)}"

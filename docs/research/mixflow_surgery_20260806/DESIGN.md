...
#### 11.5.2 箱作为汇流区终点的安全论证（最终形态，三层）

**裁决状态：已在手。** owner 2026-08-07 裁定箱是汇流区合法终点（台账 `75d8132`），
canonical `mixed_commodity_flow.terminal_clause` 的措辞改判挂**下一批 freeze-ritual**。
本节记的是那次裁决的完整论证，三层独立，任一层单独成立即足以支撑「箱不是可中毒终端」，
三层叠加使结论极稳。

**第 (a) 层：堵塞判据在物理入量下根本不可达（owner 的账，本批新增，也是最关键的一层）**

箱的 3 个物理输入口，每口 1 件 / 2s ⇒ 一个 10s 冲刷周期内**至多进货 15 件**。

- **件数维度**：缓存 6 槽 × 50 件/槽 = **300 件**，且每 10s 清零 ⇒ 15 ≪ 300，件数从来
  不是约束；
- **槽数维度**：纯流喂养下 3 个口至多喂**3 种**商品 ⇒ 至多占 **3 槽 < 6** ⇒
  **canonical 写的那个堵塞判据（6 槽全占）物理上到不了。**

所以箱不但不可毒死，**在纯流喂养下连"暂时堵门"都不存在**。canonical 把箱标成
class (2)「BOUNDED mixed absorber」时，那个 bound 是真的；但**这个 bound 在本实例的物理
入量下是不可达的**——标签为真，危险条件为空。

**第 (b) 层：即便入量账被绕过，后果也只是吞吐**

理论上要让 6 槽全占，得靠混流带在 10s 内送来 **7 种以上**商品。本实例做不到：仓储系
候选商品只有 2 个终品（§11.12 前置 5 的绑定域构造事实），**凑不出 7 种**。而即使凑出
来了，后果也只是货在门口**等待 ≤10s**（下一个冲刷周期必然清空）——这属吞吐 / 堵塞层，
`PROJECT_LOCK.md` §1A B 明列 **OUT-OF-SCOPE**。且等待的后果要么是上游原地待着（A1），
要么是内容盲分流器把货拨到另一条支路——**而汇流区闭包不变量保证每条支路都仍在闭包内**
（§11.3），拨到哪都到不了可中毒的收货方。

**第 (c) 层：永不中毒（本席原论证，独立于上面两层）**

让机器口不安全的根本机制是**永久占用**：A9 的 intake recipe-blind + A1 的 no return
⇒ 非配方件落进缓存槽后没有消费通道，**永远**占死那个槽。箱没有这个机制——10s flush
无条件清空全部槽，而供电由**认证谓词 6** 保证（A8：不通电就不 flush，谓词 6 要求被
供电设施落在电杆覆盖里）。**所以箱结构上不可能被毒死**，与入量多少无关。

**这次是怎么错过 (a) 层的（值得记一笔）**：本席、主线与设计席**全线都在
「class (2) = 有界」这个标签上推理**——争论的是「那个界怎么算、要不要编码进模型」，
**没有人把 canonical 里同时在案的三个参数（口数 × 带速 × 冲刷周期）拼一次账**。拼完
只要十秒，结论是那个界根本够不着。owner 对此的定性是**规则形态与推理流程两病并发**
（台账 `75d8132`，记忆卡 `classification-labels-hide-parameters`）：**从参数推，不从标签
推**；**限制的危险条件要像 cut 触发器一样做可达性审计**；**owner-only 闸位应当是"账算齐
了再上桌"，不是"拿不准就上交"**。§11.13 是这条教训在本文件里的制度化残余。
...
#### 11.5.3b 裁决已在手，但触发条件没有提前

**本批仍不做箱口**，这不是保守，是触发条件没到：§11.5.3 的红利空集没变（箱零实例），
§11.5.5 听诊协议的诊断臂**还没有任何布局可送**。owner 的裁决把**闸**打开了，没有把
**触发条件**提前——二期该在什么时候动手，仍由听诊协议决定。
...
**前置 4b：跑一遍 §11.13 的可达性尺子。** 接入批新引入的每条限制，逐条问「危险条件在
物理入量下可达吗」，并按 §11.13.2 的判别法区分「建模型的限制」与「防上游的守卫」——
前者不可达就删，后者不可达正是它的职责。本批自查表在 §11.13.1，其中**箱口那行已判为
多余、二期删**，**无环化那行已订正为「为审查经济保留、非 soundness 义务」**。
...
### 11.13 制度化残余：每条限制的危险条件，在物理入量下可达吗

**本节是箱口事故（§11.5.2 末尾）的制度化残余。** 那次全线三个席位都在
「class (2) = 有界」这个**标签**上推理，没有人把 canonical 里同时在案的三个**参数**
（口数 × 带速 × 冲刷周期）拼一次账——拼完只要十秒，结论是那个界根本够不着。

**常设条目（加进本文件与后续批次的攻击面 / 前提清单）**：

> **本批引入或保留的每一条限制，其危险条件在物理入量下是否可达？**

这和仓库已有的那条教训同源：**结构上不可达的触发器永不有机触发**（cut 框架 7,187 轮
零激活）。当时是"触发器到不了所以机制白装"，这次是"危险条件到不了所以限制白加"——
**同一个可达性审计，一个用在机制上，一个用在限制上。**

#### 11.13.1 本批自查（把这条尺子先量自己）

| 限制 | 危险条件 | 物理入量下可达？ | 判定 |
|---|---|---|---|
| 机器口 front 纯流排除（保留） | 非配方件落进缓存槽、**永久**占死 | **可达**——单个错件即可，无 flush 清（A9+A1）| 限制成立 |
| 箱口按机器口规则（一期保留）| 6 槽全占 | **不可达**——每 10s 至多进 15 件、至多占 3 槽（§11.5.2 (a)）| **限制多余**，二期删 |
| 混合 owner 格回机器口规则（新增）| 货经该格进入机器口 | **可达**——两台不同设施的口可朝同一格 | 限制成立 |
| 无环化禁环（新增）| drain 区成环、货循环 | 危险本身**出域**——循环 = 货到不了 = 吞吐 / 堵塞，§1A B out-of-scope | **不为 soundness，为审查经济**（见下）|
| drain 出口要求邻格在域外（新增）| 端口机身格可路由，终端边被别家声明 | 生产输入下**不可达**——真实 placement 机身必占格 | **仍保留**，属另一类（见下）|

#### 11.13.2 自查逼出的两条订正

**一、无环化是为审查经济保留的，不是为 soundness。** §11.4 本来就写了「闭环取真不
unsound」（货循环连收货方都到不了）。用本节这把尺子量：环的危险性**与箱的堵塞同属吞吐
层，同样 out-of-scope**——既然我们用「阻塞属吞吐」把箱口限制判为多余，就不能同时用
「环不好看」把禁环判为必要。**结论：禁环不是 soundness 义务。** 它现在保留的理由只有
两条，都要写明：(i) 它让审查者少一个必须被说服的形态（team-lead 08-07 「若禁环代价小
宁可禁掉」）；(ii) §11.4/§11.7 实测它**零成本**。**所以一旦它显出成本，它是第一个该被
摘掉的东西，且摘掉不需要重做任何 soundness 论证**——只需注意 §11.8 记的那条耦合（已
否决的静态剪枝依赖它）。这条订正是本节这把尺子量出来的，不是原设计里的。

**二、这把尺子不能用来删 fail-closed 守卫，两者是不同类。** 表里最后一行的危险条件
「生产输入下不可达」，但它**恰恰因此才该留**：

- **建模型的限制**：它在**表达一条游戏机制的危险**。危险条件不可达 ⇒ 限制在描述一个
  不存在的东西 ⇒ **应删**（箱口案）。
- **防上游的守卫**：它在**防御畸形输入或上游 bug**。「不可达」正是它要**保证**的性质，
  不是它多余的证据 ⇒ **不能删**（drain 出口案；它挡下的正是 §11.11b 那个不忠实探针）。

判别法一句话：**问这条限制的危险条件由谁保证不可达**。由**物理 / 冻结数据**保证的，
限制多余；由**这条限制自己**保证的，它就是那个保证，删了就没了。

#### 11.13.3 owner 给的两条流程口径（一并制度化）

- **从参数推，不从标签推。** canonical 里的分类标签（class (1)/(2)/(3)、
  「BOUNDED」「structurally non-rejecting」）是**结论的名字**，不是结论本身；同一份
  canonical 里往往同时躺着推出该结论的参数。**引用标签前先把参数拼一次账。**
- **owner-only 闸位 = 账算齐了再上桌，不是拿不准就上交。** 本席把箱口 promotion 判为
  「owner 级动作」是对的，但**上交时那笔入量账没算**——算了就会发现根本不需要动裁决面，
  或者至少能把裁决问题问得准得多。

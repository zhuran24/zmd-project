...
- **条件 1（certified 默认关本手术 / 门控开关）**：§9.8 的结论是「③ 抽掉了开关的
  两条动机，接入批应先决定还要不要开关」。U-01 不改变这一点——本批既没有恢复
  可行域放大（红利为零，§10.4），也没有引入新的求解代价。若将来落 §10.4 的
  `wh_drain` 草案，开关问题要重开：那才是真正放大可行域的改动。
...
- **条件 1（certified 默认关本手术 / 门控开关）**：§10.6 说「若将来落 `wh_drain`
  草案，开关问题要重开」——**它落了，所以开关问题现在就是开着的**。汇流区是本手术
  第一个真正放大可行域的改动（§11.6 几何 A 由 INFEASIBLE 翻 FEASIBLE），也是第一个
  在某些口数下有真实求解代价的改动（§11.9）。接入批必须显式判：整套手术要不要
  env 门控、门控粒度是「整手术」还是「汇流区单独」。**本批的建议是汇流区单独可关**
  ——它依赖分叉的分类、反之不依赖，回退面干净（Q10b 那张表）。
...
- **条件 6（witness adapter 子图样兼容）**：§10.6 那条欠账在汇流区下**变重**——
  反建 routing 的消费者拿不到 `operation_type` 时不只是「比生产者严」，而是整个汇流

"""Path 17 D2 Phase 2 — 8 anchor max_iter=10 multi-anchor campaign.

复用 Path 14/15 同 anchor matrix 同 budget. paradigm verdict 标准:

GO 信号 (任一):
- ≥1 non-corner anchor 进入 CERTIFIED
- ≥4/8 anchor 进入 "D2 通过 → routing 真 verify FEASIBLE" — paradigm strong
  evidence even if not certified

NO-GO (同 PCR-CUT Phase 5 / RAB-SEP Phase 5 同 pattern):
- 0/8 CERTIFIED, 全 anchor UNPROVEN at max_iter=10
- master 加 10 cut 仍系统性给 routing-infeasible layouts
- 说明 D2 cut 是 necessary 不 sufficient (跟之前 paradigm 同质)
"""
...
def main() -> int:
    print("=== Path 17 D2 Phase 2 — 8-anchor LBBD campaign ===")
    ...
    if cert_count >= 1:
        print(">>> ✅ Phase 2 BREAKTHROUGH ({} anchor certified)".format(cert_count))
        return 0
    print(">>> 🟡 Phase 2: 0/8 CERTIFIED — paradigm not breakthrough at this scale")
    return 1

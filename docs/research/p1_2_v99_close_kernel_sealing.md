...
The local repository seal includes the follow-up F-CAM-R8-02 durable resume-sanitization fix and the 2026-06-18 no-close-kernel review follow-ups: public `ExactCampaign.mark_candidate_result()` cannot self-authorize proof-bearing strong-status freshness, the raw freshness sealer fails closed, verified strong producers are caller-bound to the `run_outer_search` controller path, root entrypoint `main.py` is covered by the certified exact source digest, and locked projects must pass the V99 manifest/checker before `compute_exact_artifact_hashes()` can mint a fresh campaign source digest. The latest interim authority hardening also pins producer/writer authority, stores proof-bearing freshness on live record stamps instead of a mutable registry, blocks stale strong statuses from frontier pruning, and keeps test helpers from patching the production grant path. The original V99 package was generated before these local fixes; this repository keeps them and rebinds the close-kernel source hashes for `src/search/exact_campaign.py`, `src/search/outer_search.py`, `src/search/certified_artifact_contract.py`, and the checker manifest entry. Known blocker: same-process closure-cell mutation of the verified writer can still bypass this interim hardening; checker pass is not a clean P1.2 close claim until that residual is resolved or explicitly scoped out. The root close packet records the local merge in `P1_2_TECHNICAL_CLOSE_PACKET/10_local_merge_reseal.md`.

...
PROMPT = """你是 hitting set / power network / soundness 审查员. Endfield 工业规划求解器 Phase 1.2 P1.2B-F7 **Round 2** cross-check (commit `9f21901`).

# Round 1 finding 摘要 + fix

R1 NOT_GO 4 finding:
- R1#1 CRITICAL fail-open: validator `_validate_coverset_empty` 漏 exclude facility_cells → replay 时 facility 不在 cell_owner → free_cells 含 facility 自身 9 cell → pole 2×2 可 anchor 内部 → cover_full 非空 → 100% reject 合法 cut.
  **Fix landed**: validator phase 6/7 内联 mask = ghost ∪ exterior ∪ cell_owner ∪ facility_cells (full) / ghost ∪ exterior ∪ facility_cells (ghost-only). oracle 同样 inline + 新 helper `_full_free_cells_minus_facility` / `_ghost_only_free_cells_minus_facility`. compute_cover_set_ghost_only helper 已 retire (callers build mask directly).
- R1#2 MEDIUM O(N) pose lookup: Phase 1.5+ defer
- R1#3 LOW float sqrt: Phase 1.5+ defer
- R1#4 INFO watcher omit ghost_blocked_pole_cells: sound optimization, 不修

Gap A (1×1 vs 2×2 spec drift) CONFIRMED + 实施 follow real data sound.
Gap B (Euclidean metric) CONFIRMED + Phase 1.5+ active_assumption.
Gap C (ghost ∩ cell_owner) REJECTED — monotone subset.

# Round 2 任务

按 v3 loop: verify R1 fix + push 找 round 2 new finding (任何 severity).

1. **Verify R1#1 fail-closed fix** (facility_cells exclude):
   - 真没漏路径? validator + oracle 两端都 exclude.
   - 边界: facility cells 已经在 ghost 内 (覆盖完全)? exclude 跟现 mask 含 ghost 是 set 操作 union, 不影响 sound.
   - cell_owner 含 facility cells 时 union 等价 — 不引入 over-rejection.

2. **Verify oracle ghost-only mask 一致**:
   - oracle `_ghost_only_free_cells_minus_facility` ↔ validator phase 7 inline mask — 都是 ghost ∪ exterior ∪ facility_cells. byte-equal 吗?
   - 跨 worker reproducibility?

3. **Round 2 new finding**: ≥2 (不接 0). 任何 severity OK.

4. **F7 整体 maturity 评估**: Phase 1.2 close 还是仍 paradigm 风险?
...
## R1 Fix Verify
- R1#1 facility_cells exclude (validator + oracle): CORRECT / PARTIAL / WRONG / NEW_GAP — 详释
- Oracle/validator mask 一致性: CORRECT / DRIFT — 详释

...
class UnitReferenceHandle(Protocol):
    """One persistent ordinary-user sd-bus reference."""

    @property
    def acquired_unit(self) -> str | None:
        """Return the currently held unit name."""

    def acquire(self, *, unit_name: str, expected_manager_owner: str) -> dict[str, str]:
        """Call RefUnit once on the persistent connection."""

    def verify(self, *, expected_manager_owner: str) -> dict[str, str]:
        """Replay the same client and manager owner while held."""

    def release(self, *, unit_name: str, expected_manager_owner: str) -> dict[str, str]:
        """Call UnrefUnit once on the same connection."""

    def close(self) -> None:
        """Close after a successful explicit release."""

    def abort_close(self) -> bool:
        """Drop any held reference and close after failure."""
...
class LifecycleAdapter(Protocol):
    ...
    def open_unit_reference(self) -> UnitReferenceHandle:
        """Open the package-pinned persistent user-bus reference."""
    ...
    def capture_reference_state(
        ...
        *,
        ...
        """Capture the loaded unit immediately after RefUnit."""
    ...
    def wait_reference_stability(self, hold_ns: int) -> None:
        """Wait the preregistered interval while the same reference is held."""
...
def build_pinned_epoch_observer(
    ...
    if (
        set(pre_run.get("epoch_observation_paths", {})) != EPOCH_PHASES
        or set(pre_run.get("epoch_transcript_paths", {})) != EPOCH_PHASES
    ):
        raise OrchestratorError("pre-run manager epoch phase set drifted")
    pinned_environment = _load_pinned_environment(pre_run)

    def observe(phase: str) -> EpochCapture:
        if phase not in EPOCH_PHASES:
...
class SubprocessLifecycleAdapter:
    ...
    def open_unit_reference(self) -> UnitReferenceHandle:
        tools = self.pre_run["tool_identities"]
        module = _load_pinned_module(
            tools["systemd_unit_reference"],
            module_name=(f"_ab16_systemd_unit_reference_{tools['systemd_unit_reference']['sha256'][:12]}"),
        )
        try:
            reference = module.PersistentUnitReference(
                library_path=self.pre_run["launch"]["libsystemd_path"],
                expected_library_identity=tools["libsystemd"],
            )
        except Exception as exc:
            raise OrchestratorError("persistent unit reference initialization failed") from exc
        return reference
    ...
    def capture_reference_state(
        ...
        *,
        ...
        raw = self._show(unit_name, SYSTEMD_REFERENCE_FIELDS)
        if (
            raw["LoadState"] != "loaded"
            or raw["CollectMode"] != "inactive-or-failed"
            or raw["InvocationID"] != invocation_id
        ):
            raise OrchestratorError("RefUnit did not preserve the exact loaded unit")
    ...
    def capture_terminal(
        ...
        *,
        ...
        deadline = self._monotonic() + int(self.pre_run["reference_contract"]["terminal_transition_deadline_seconds"])
        while self._monotonic() <= deadline:
            ...
            if raw["LoadState"] == "not-found" or not raw["InvocationID"]:
                raise OrchestratorError("unit was collected before exact terminal InvocationID capture")
            if raw["InvocationID"] != invocation_id:
                raise OrchestratorError("terminal InvocationID differs from launch")
            if raw["CollectMode"] != "inactive-or-failed":
                raise OrchestratorError("terminal CollectMode drifted")
    ...
    def wait_reference_stability(self, hold_ns: int) -> None:
        if type(hold_ns) is not int or hold_ns <= 0:
            raise OrchestratorError("reference stability interval is invalid")
        self.sleep(hold_ns / 1_000_000_000)
    ...
    def _cleanup_selected_unit(
        ...
        *,
        ...
        deadline = self._monotonic() + int(self.pre_run["reference_contract"]["post_unref_cleanup_deadline_seconds"])
        present, observed_control_group = self._selected_unit_state(
            unit_name=unit_name,
            expected_invocation_id=launch.invocation_id,
            timeout=min(5.0, max(0.001, deadline - self._monotonic())),
        )
        ...
        if present:
            remaining = deadline - self._monotonic()
            if remaining <= 0:
                raise OrchestratorError("selected unit cleanup deadline expired")
            self._run(
...
def _orchestrate_with_adapter_unprotected(
    *,
    ...
    reference = adapter.open_unit_reference()
    cleanup_state["reference"] = reference
    reference_observation, _reference_epoch_identity = _write_epoch(
        lifecycle=lifecycle,
        pre_run=pre_run,
        adapter=adapter,
    ...
    acquisition_call = reference.acquire(
        unit_name=pre_run["unit_name"],
        expected_manager_owner=expected_manager_owner,
    )
    observed_reference = adapter.capture_reference_state(
        unit_name=pre_run["unit_name"],
        invocation_id=launch.invocation_id,
    )
    reference_acquisition = lifecycle.build_reference_acquisition_record(
        pre_run,
        pre_run_snapshot.identity,
        selection,
        selection_snapshot.identity,
        invocation_id=launch.invocation_id,
        acquisition_observation=reference_observation,
        acquired_at_monotonic_ns=observed_reference.captured_at_monotonic_ns,
        call_evidence=acquisition_call,
        systemd_raw=observed_reference.systemd_raw,
    )
    ...
    release = lifecycle.build_release_record(
        ...
        reference_acquisition_identity=reference_acquisition_identity,
        keeper_pid=launch.supervisor_pid,
        keeper_starttime=launch.supervisor_starttime,
    ...
    first_terminal_observation, _first_terminal_epoch_identity = _write_epoch(
        lifecycle=lifecycle,
        pre_run=pre_run,
        adapter=adapter,
    ...
    adapter.wait_reference_stability(int(pre_run["reference_contract"]["stability_hold_ns"]))
    stable_terminal_observation, _stable_terminal_epoch_identity = _write_epoch(
        lifecycle=lifecycle,
        pre_run=pre_run,
        adapter=adapter,
    ...
    terminal = lifecycle.build_terminal_record(
        ...
        invocation_id=launch.invocation_id,
        first_observation=first_terminal_observation,
        stable_observation=stable_terminal_observation,
        first_captured_at_monotonic_ns=first_terminal.captured_at_monotonic_ns,
        stable_captured_at_monotonic_ns=stable_terminal.captured_at_monotonic_ns,
        first_systemd_raw=first_terminal.systemd_raw,
        stable_systemd_raw=stable_terminal.systemd_raw,
    )
    ...
    reference_release = lifecycle.build_reference_release_record(
        ...
        terminal_identity,
        invocation_id=launch.invocation_id,
        release_observation=reference_release_observation,
        released_at_monotonic_ns=adapter.monotonic_ns(),
        call_evidence=reference_release_call,
    )
    ...
    detached = verifier.verify_detached_paths(
        ...
        reference_acquisition_path=pre_run["output_paths"]["reference_acquisition"],
        release_path=pre_run["output_paths"]["release"],
        terminal_path=pre_run["output_paths"]["terminal"],
        reference_release_path=pre_run["output_paths"]["reference_release"],
...
def _publish_abort_reference_release(
    *,
    cleanup_state: Mapping[str, Any],
    held_unit: str | None,
    explicit_unref_succeeded: bool,
    original_failure: Exception,
) -> None:
    pre_run = cleanup_state["pre_run"]
    output = Path(pre_run["output_paths"]["abort_reference_release"])
    if os.path.lexists(output):
        raise OrchestratorError("abort reference-release output already exists")
    lifecycle = cleanup_state["lifecycle"]
    launch = cleanup_state["launch"]
    record = {
        "authorizations": {
        ...
        "explicit_unref_succeeded": explicit_unref_succeeded,
        "failure_class": type(original_failure).__name__,
        "held_unit_before_abort": held_unit,
        "invocation_id": launch.invocation_id,
        "pre_run_authority_identity": cleanup_state["pre_run_identity"],
    ...
    *,
    ...
    try:
        ...
        if not isinstance(launch, LaunchEvidence):
            raise
        try:
            reference = cleanup_state.get("reference")
            if reference is not None:
                held_unit = reference.acquired_unit
                explicit_unref_succeeded = reference.abort_close()
                _publish_abort_reference_release(
                    cleanup_state=cleanup_state,
                    held_unit=held_unit,
                    explicit_unref_succeeded=explicit_unref_succeeded,

...
ARM_GATE_SCHEMA = "noncert-cuts-ab16-arm-credibility-gate-v2"
...
RESOURCE_SCHEMA = "noncert-cuts-ab16-detached-resource-terminal-v2"
...
def _identity(value: object, label: str) -> Mapping[str, Any]:
    if type(value) is not dict or set(value) not in (
        ...
        raise GateError(f"{label} must have a detached identity key set")
    record = value
    if (
...
def _detached_identity(value: object, label: str) -> dict[str, object]:
    ...
    return {field: record[field] for field in ("path", "sha256", "size_bytes")}
...
def build_arm_gate(
    *,
    ...
    if (
        ...
        or _detached_identity(
            preterminal_resource["runner_selection_identity"],
            "preterminal runner selection identity",
        )
        != _detached_identity(selected_identity, "selected identity")
        or _detached_identity(
            preterminal_resource["payload_result_identity"],
            "preterminal payload result identity",
        )
        != _detached_identity(result_identity, "result identity")
        or _detached_identity(
            preterminal_resource["verifier_tool_identity"],
            "preterminal verifier tool identity",
        )
        != _detached_identity(
            expected_resource_tool,
            "expected verifier tool identity",
        )
        ...
        or _detached_identity(
            detached_resource["runner_selection_identity"],
            "detached runner selection identity",
        )
        != _detached_identity(selected_identity, "selected identity")
        or dict(
            _identity(
                detached_resource["resource_verification_identity"],
                "detached resource verification identity",
            )
        )
        != resource_preterminal_id

...
def _resolve_project_root(project_root: Path) -> Path:
    ...
    try:
        ...
        raise ShelfPowerLaunchError("PROJECT_ROOT_INVALID", f"Git probe failed for {root}: {exc}") from exc

    fields = completed.stdout.splitlines()
    if len(fields) != 3 or fields[0] != "true":
        raise ShelfPowerLaunchError("PROJECT_ROOT_INVALID", f"invalid Git probe for {root}")
    try:
        reported_root = Path(fields[1]).resolve(strict=True)
        reported_git_dir = Path(fields[2]).resolve(strict=True)
    except OSError as exc:
        raise ShelfPowerLaunchError("PROJECT_ROOT_INVALID", f"invalid Git paths for {root}: {exc}") from exc
    if reported_root != root or not reported_git_dir.is_dir():
        raise ShelfPowerLaunchError("PROJECT_ROOT_INVALID", f"Git top-level mismatch for {root}")
    ...
    try:
        ...
        raise ShelfPowerLaunchError("PROJECT_ROOT_INVALID", f"invalid .git marker for {root}: {exc}") from exc
    if marker_git_dir != reported_git_dir:
        raise ShelfPowerLaunchError("PROJECT_ROOT_INVALID", f"Git directory mismatch for {root}")
    return root

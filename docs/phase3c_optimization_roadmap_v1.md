...
| 4 | **Domain-level prechecks (R3-revised → blueprint stalled on paper access)** — R12 audits `ab23c3975f6040dc8` + `a6cefca1489dff952`. Net status: #8 partial landed env-gated (commit 16970ff); #10 跳过 (data-model mismatch); #11 待 data-model verify; **#1 DFF blueprint received but BLOCKED on paper-level formula verification** — Carlier-Clautiaux-Moukrim 2007 EJOR §3.2 formula needs precise reading; paper is paywalled; GitHub code search requires auth; agent's pseudocode contains ambiguous "原归一系数" that prevents soundness-by-construction. Implementing without verified formula risks breaking certified_exact (LB over-tight could cut feasible solutions). 9 others demoted to P1. | R3 `a37cf00dce550f097` + R12 `ab23c3975f6040dc8` + `a6cefca1489dff952` | currently stalled at #8 env-gated only; #1 DFF needs paper access to unblock | requires user decision (paper access? math reviewer? skip?) | stalled |
...
| 28 | **改造 5 (sentinel-row for power pole shell guard) — DEMOTED FROM P0 #6** | R12 `a7676847d82e04abb` | original blueprint missed inactive d_lo/d_hi pinning to (0,0) — direct `(max_shell, max_shell)` sentinel would make model INFEASIBLE. Correct patch needs decoupling inactive path + OnlyEnforceIf on distance constraints + explicit sentinel forcing (~15 lines across 3 functions) + 2 unit tests. | ~1-1.5 days (was estimated 0.5d in original blueprint) | needs verified patch + regression test |

...
| # | Item | Source | Save | Cost | ROI |
|---|------|--------|------|------|-----|
| 14 | **AlphaEvolve cut-evolution PoC** — **R13 refresh 2026-05-10 by `a8a448561dbacf07c` + audit `a062ff6396a691d74`**: OpenEvolve v0.2.27 (2026-03-18) late beta + Claude/DeepSeek 已支持 ✅; LLM-LNS ICML'25 spotlight 打过 Gurobi/SCIP/GNN&GBDT/Light-MILPopt ✅; AlphaEvolve arXiv 2506.13131 (2025-06) Google DeepMind ✅。**工时 1 周→2-3 day**, pip install + 改 evaluator + 复用 TSP example。**API 成本修订**: DeepSeek V3 已被 V4 取代，**V4-flash $0.14/$0.28 per M tok ≈ $2-3 / 500 iter**（比原 V3 估的 $5 更便宜）/ Claude Opus 4.7 $5/$25 ≈ $30-50 with prompt cache / Gemini 2.5 Flash $0.30/$2.50 ≈ $10-15。Go/no-go gate: spike 出的 hint 在 24h shadow run 里能否把 binding subproblem 平均求解时间降 ≥ 5%——降不到 5% KILL。**立即可做**。 | R10 `a08591c75ba861e6e` + R13 `a8a448561dbacf07c` audit `a062ff6396a691d74` | DeepSeek V4-flash 主 + Claude Sonnet 4.6 副 ensemble / 1 周 PoC framework reuse | 2-3 day (不是 1 周) | gated by go/no-go (spike 5% threshold) |

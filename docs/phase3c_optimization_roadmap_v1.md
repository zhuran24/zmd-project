...
| # | Item | Source | Save | Cost | ROI |
|---|------|--------|------|------|-----|
| 14 | **AlphaEvolve cut-evolution PoC** — **R13 refresh 2026-05-10 by `a8a448561dbacf07c` + audit `a062ff6396a691d74` + verify `a7f6f2df056acf347`**: OpenEvolve v0.2.27 (2026-03-18) late beta + Claude/DeepSeek 已支持 ✅; LLM-LNS ICML'25 spotlight 打过 Gurobi/SCIP/GNN&GBDT/Light-MILPopt ✅; AlphaEvolve arXiv 2506.13131 ✅。**工时 1 周→2-3 day**, pip install + 改 evaluator + 复用 TSP example。**LLM backend 决策（用户 Max 20x quota 角度修订）**: 原 audit 推荐 V4-flash 主, 个人 budget 视角下 Sonnet 4.6 单跑就好; 但用户是 Anthropic Max 订阅 ($200/月, 5x/20x quota), API credits cover 充足角度下 **Opus 4.7 单跑更优**。理由: PoC 关键是**遵守 AI safety contract / exactness contract** (PROJECT_LOCK 红线), Opus 指令遵循最稳, 翻车成本最低; 500 iter 成本 Opus ~$110 / Sonnet ~$22 / V4-flash $0.6, Max quota 视角差距不痛; 性能 SWE-bench Opus 比 Sonnet 高 2-5 pt, marginal 但 PoC 阶段花得起。代价: Opus 单次 latency 2-3x 慢, 500 iter 跑 2-4 小时 (Sonnet 1-2 小时)。**默认 Opus 4.7 单跑**, 备选 Sonnet 4.6 (quota 紧 / 想快迭代时)。Go/no-go gate: spike 出的 hint 在 24h shadow run 里能否把 binding subproblem 平均求解时间降 ≥ 5% — 降不到 5% KILL。**立即可做**。 | R10 `a08591c75ba861e6e` + R13 `a8a448561dbacf07c` audit `a062ff6396a691d74` + verify `a7f6f2df056acf347` | Claude Opus 4.7 单 backend (备选 Sonnet 4.6) / 1 周 PoC framework reuse | 2-3 day (不是 1 周) | gated by go/no-go (spike 5% threshold) |
...
| 18 | **MUS via CPMpy QuickXplain** — **R13 refresh 2026-05-10 by `ab56e030d7ec24cad` + audit `ae3860a1dc6cbabb8` (CONFIRMED 8/9)**: CPMpy 0.10.0 (2026-01-19) MUS API 稳定 ✅ + `from cpmpy.tools.explain import quickxplain` 一行可用 ✅ + OR-Tools 9.15 已对齐 ✅。SMT-COMP 2025 QF_LIA: OpenSMT 1, cvc5 2, Yices2 3, Z3-alpha 4。**工时 4d→1-2d**, 立即可做。**注意**: cvc5 pin 应改 1.3.4（不是 R13 transcript 写的 1.3.3，差 1 天，调研当天已升 patch）。 | R5 `a3bef849bbe8777ab` + R13 `ab56e030d7ec24cad` audit `ae3860a1dc6cbabb8` | conflict-extraction quality up; CPMpy 0.10.0 已对齐 ortools 9.15 | 1-2 day (不是 4 day) | ~3× |

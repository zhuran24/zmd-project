...
| 4 | **Domain-level prechecks (R3-revised: 4 P0a, 8 P1)** — R12 audit `ab23c3975f6040dc8` revised: R3 #8 already mostly implemented (probing_level=3, symmetry_level=3 done); R3 #3 already in CP-SAT internal (`use_energetic_reasoning_in_no_overlap_2d`); R3 #7 already covered by existing `compute_exact_static_area_lower_bound`. **P0a (4 conditions, 4-6 days, ROI +8-16%)**: #1 DFF + area lower bound, #10 Power Budget, #11 Storage I/O, #8 remaining 2 params (`presolve_substitution_level=2` + `use_strong_propagation_in_disjunctive=True`). 8 others (#2/#4/#5/#6/#7/#9/#12) demoted to P1 in this row. | R3 `a37cf00dce550f097` + R12 `ab23c3975f6040dc8` | +8-16% (revised down from R3's "+11-25%") | 4-6 days | ~5× |

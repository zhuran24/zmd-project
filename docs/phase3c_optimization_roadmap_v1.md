...
| 19 | **Linux migration + THP/cgroups (Fedora Workstation, target version 43)** — selected 2026-05-08 by user preference ("红客 + 新技术 + 稳定" → Fedora 系). **Version selection (verified 2026-05-08 via endoflife.date/fedora)**: Fedora 44 GA'd 2026-04-28 but COPR ecosystem may lag 1-2 months; Fedora 43 (released 2025-10-28, EOL 2026-12) is mature + COPR-complete + sufficient for 168h campaign window. Fedora 42 EOLs 2026-05-27 (do NOT install). Fedora 41 already EOL. **Project compatibility on 43**: Python 3.13 default ✓, glibc 2.40+ ✓ (ortools 9.15 manylinux wheel direct install), kernel 6.11+ (THP / cgroups v2 / io_uring all available), dnf has mimalloc / jemalloc / tcmalloc out of the box. Recommended ramp: dual-boot first (preserve Windows fallback), benchmark vs current Windows baseline (4-worker run), full-migrate only if ≥15% improvement confirmed. **Optional kernel upgrade (post-baseline)**: install `bieszczaders/kernel-cachyos` COPR (verified 2026-05-08 via github.com/CachyOS/linux-cachyos) for BORE/EEVDF scheduler — 4 variants available (linux-cachyos-bore / -rt-bore / -bore-lto / -lts). Expected +5-10% on CPU-bound long-runs (community-maintained, not CachyOS official). Lower-risk than switching to CachyOS distro itself: keeps Fedora ecosystem, only swaps kernel. CachyOS distro itself REJECTED — rolling release breaks 168h-run stability + OR-Tools wheel is manylinux baseline (CachyOS x86-64-v3 packages don't accelerate the hot path without source rebuild = same as P1 #13). | R2 `a8a152668dd067210` + manual version + COPR verification 2026-05-08 | base Fedora +15-35%; +cachyos-kernel +5-10% on top | ~1 day dual-boot + base benchmark; +1 hr COPR install | ready to start whenever user willing |

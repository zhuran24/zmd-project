...
| 29 | **CP-SAT `symmetry_level=0` 实验**（profile 新发现 2026-05-10）— P1 #20 part 1 短跑 profile 显示 `DetectAndExploitSymmetriesInPresolve` + `FindCpModelSymmetries` + `GraphSymmetryFinder::FindSymmetries` 累计 **~3% CPU 时间**。项目已经做过手动对称性破坏（grouped encoding + signature monotonic + R2 round audit `a96266f5aed302928` 证 99% 已处理），CP-SAT 内部再 detect 是冗余先验。1 行 `solver.parameters.symmetry_level = 0`（默认 2）禁止内部 detection 应该正向 ~3%（前提：手动 breaking 真已覆盖；如果有遗漏，关掉会让 search 慢）。微基准：A/B 跑 5min × 2 看耗时 + cert hash 一致。 | profile artifacts in `docs/research/profiles/p1_20_short_profile_20260510/` | ~3% (gated by 微基准 cert hash 一致) | ~30min A/B 实验 | ~6-10× (3% 收益 / 30min 工时, 1 行参数改) |

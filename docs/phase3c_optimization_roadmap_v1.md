...
| # | Item | Source | Save | Cost | ROI |
|---|------|--------|------|------|-----|
| 14 | **AlphaEvolve cut-evolution PoC** — **R13 refresh 2026-05-10 by `a8a448561dbacf07c` + audit `a062ff6396a691d74` + verify `a7f6f2df056acf347`**: OpenEvolve v0.2.27 (2026-03-18) late beta + Claude/DeepSeek 已支持 ✅; LLM-LNS ICML'25 spotlight 打过 Gurobi/SCIP/GNN&GBDT/Light-MILPopt ✅; AlphaEvolve arXiv 2506.13131 ✅。**工时 1 周→2-3 day**, pip install + 改 evaluator + 复用 TSP example。**LLM backend 决策修订**: 原 audit 推荐 "DeepSeek V4-flash 主 + Sonnet 副 ensemble", 但 quick verify (Anthropic vs DeepSeek 性能 + 额度) 反转: SWE-bench/HumanEval 三家 ±1 分钟级 (79-80.6%); DeepSeek V4 在 LiveCodeBench 高 19-21 pt 但那是算法竞赛 brilliance, 我们 PoC 关键是**遵守 AI safety contract / exactness contract**, Sonnet 指令遵循更稳。**额度完全独立** (Anthropic / DeepSeek 不同公司不同账号不同 quota)。500 iter 成本: Sonnet $22 / V4-flash $0.6 / V4-pro $7 — 绝对值 Sonnet $22 不痛, 混搭双 SDK + prompt 适配代价 > $20 节约, ROI 负。**改用 Sonnet 4.6 单跑**, 不混搭。Go/no-go gate: spike 出的 hint 在 24h shadow run 里能否把 binding subproblem 平均求解时间降 ≥ 5% — 降不到 5% KILL。**立即可做**。 | R10 `a08591c75ba861e6e` + R13 `a8a448561dbacf07c` audit `a062ff6396a691d74` + verify `a7f6f2df056acf347` | Claude Sonnet 4.6 单 backend / 1 周 PoC framework reuse | 2-3 day (不是 1 周) | gated by go/no-go (spike 5% threshold) |

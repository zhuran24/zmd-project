...
| # | Item | Source | Save | Cost | ROI |
|---|------|--------|------|------|-----|
| 14 | **AlphaEvolve cut-evolution PoC** — **R13 refresh 2026-05-10 by `a8a448561dbacf07c`**: OpenEvolve v0.2.27 (2026-03-18) late beta + Claude/DeepSeek 已支持; LLM-LNS ICML'25 spotlight 打过 Gurobi 是离 LBBD 最近 SOTA。**工时 1 周→2-3 day**, pip install + 改 evaluator + 复用 TSP example。**API 成本 $25→$5 (DeepSeek V3 主) / $30-50 (Claude prompt cache)**。Go/no-go gate: spike 出的 hint 在 24h shadow run 里能否把 binding subproblem 平均求解时间降 ≥ 5%——降不到 5% KILL。**升级到"立即可做"**。 | R10 `a08591c75ba861e6e` + R13 `a8a448561dbacf07c` | DeepSeek V3 ensemble Claude 副 / 1 周 PoC framework reuse | 2-3 day (不是 1 周) | gated by go/no-go (spike 5% threshold) |
...
| 18 | **MUS via CPMpy QuickXplain** — **R13 refresh 2026-05-10 by `ab56e030d7ec24cad`**: CPMpy 0.10.0 (2026-01-19) MUS API 稳定 + `from cpmpy.tools.explain import quickxplain` 一行可用 + OR-Tools 9.15 已对齐。**工时 4d→1-2d**, 立即可做。 | R5 `a3bef849bbe8777ab` + R13 `ab56e030d7ec24cad` | conflict-extraction quality up; CPMpy 0.10.0 已对齐 ortools 9.15 | 1-2 day (不是 4 day) | ~3× |
| 30 | **plateau-based 动态阶段切换** （R13 新增 2026-05-10 by `a3356c51d9d10daab`）— Lübke&Berg AAAI'25 anytime MaxSAT 思路: 不强行固定 25h/50h/85h 时间切分; Stage-2 提前进 Stage-3 if dual_growth_rate < threshold for 4h。补 P1 #7 主流程的硬切分 fallback。CP'25 #21 Koops VeriPB PB-OPT 工业可用 (RoundingSat/Sat4j 全套 logging) 但中间 ε-gap 没标准 schema → R11 #6 VeriPB exporter 工时 90min→4-6h, 自定义 transcoder 不可避免 (建议先做 Stage-3 final OPT cert, 中间 ε-gap cert 暂缓)。 | R13 `a3356c51d9d10daab` + Lübke&Berg AAAI'25 + Koops CP'25 #21 | 防 Stage-2 dual 增长慢时浪费时间 | 2h (动态切换) + 4-6h (R11 #6 修订估时) | gated by P1 #7 主流程 land |
| 31 | **Pumpkin solver D''' binding-subproblem audit PoC** （R13 P3→P2 升级 2026-05-10 by `adee29cf670b5c3dc`）— Pumpkin v0.3.0 PyPI (2026-02-11) Python binding 完整可用; element + 线性 + cumulative 够覆盖 binding 子问题。跟 OR-Tools CP-SAT 跑同一 binding 实例对照, 独立 audit 通道 (不替换 main solver)。**circuit/no_overlap_2d 仍缺**所以 master/routing 不行, 仅 binding。 | R6 `a6341e4ac38a35db5` + R13 `adee29cf670b5c3dc` | binding 子问题独立 audit 通道, 防止 OR-Tools regression 没被察觉 | ~3-5 day (PyPI install + binding instance 转化 + 双 solver 对照 harness) | gated by 真长跑 baseline |
| 32 | **Glasgow + VeriPB 3.0 sidecar audit PoC** （R13 P3→P2 升级 2026-05-10 by `adee29cf670b5c3dc`）— Glasgow Constraint Solver gcspy Python binding (2026-05-09 仍每日 commit) + 招牌 VeriPB 3.0 proof logging。binding/routing 子问题作 audit-only 通道, 不替换 main solver。作者自称"no stable API" + C++23 编译门槛高 (GCC 13+ / Clang 21+) 是风险。 | R6 `a97ae4416eb4bd8cf` + R13 `adee29cf670b5c3dc` | proof-logging 独立验证通道 (P1 #7 ε-Certified 主流程的"硬证据" sidecar) | ~5-7 day (gcspy 学习 + 编译 + cert harness) | gated by P1 #7 完成 |

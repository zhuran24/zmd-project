...
| 4 | **Domain-level prechecks (R3-revised: 4 P0a → 2 + 1 partial, after data-model check)** — R12 audit `ab23c3975f6040dc8` plus 2026-05-08 data-model verification: R3 #8 partial landed env-gated (commit 16970ff); R3 #10 (Power Budget) **跳过** — facility_templates uses `needs_power: bool` not numeric power supply/demand, so "sum(supply) ≥ sum(demand)" doesn't map; real precheck would be 80m coverage geometry, much larger than 0.5d. R3 #11 (Storage I/O) needs similar data-model check before accepting estimate. **Remaining P0a: #1 DFF + area lower bound** (only one data-model verified). 9 others demoted to P1. | R3 `a37cf00dce550f097` + R12 `ab23c3975f6040dc8` | revised down: only #1 DFF reliably scoped (~1-2d, +5-10%); #8 env-gated (A/B-pending); #10 跳过 (data mismatch); #11 待 verify | ~1-2d for #1 DFF | revised |

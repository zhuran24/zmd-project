...
| 07-30 | AB16 强制链的 A031–A033 均已冻结。A033 通过 fresh Gate A、Gate-B qualification 与 package/campaign 创建，但 formal 阶段只发布 admission，未发布 guardian-ready、attempt consumption、selection、unit、terminal 或 arm。冻结证据不能恢复被旧 orchestrator 遮蔽的 selected-supervisor stderr；241-byte canonical guardian socket path 同时暴露 Linux pathname AF_UNIX 的必现后续阻断。当前修复保留 canonical absolute artifact identity，以 retained-dirfd alias、O_PATH leaf pin、parent/leaf join 与 SO_PEERCRED 完成内部 transport，并保留 exact selected-loader import controls；A031–A033 不可修补或重试，下一 fresh root 必须重新绑定提交与完整门禁 | [AB16 current status](../research/noncert_cuts_ab16_20260724/README.md) |

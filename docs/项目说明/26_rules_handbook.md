...
- **certified 面供电判死不留可复用结构信息（在案观察）**：certified 路径的供电覆盖约束全部留在 master（`ExactCoordinateMaster._add_geometric_power_coverage_constraints`），供电不可行表现为 **master 直接 INFEASIBLE**（`src/search/benders_loop.py` 的 `RUN_STATUS_INFEASIBLE` 返回点），既不做子问题也不升 UNKNOWN，**不产生 cut、core 或 conflict_set**——下一个 ghost rect 从零重推。相关的 witness-complete cut 缺口（`PROJECT_LOCK.md:703` 第三条子项）只存在于被禁 flag `EXACT_POWER_PLACEMENT_SUBPROBLEM` 内部，certified 实跑不可达，因此那条缺口**不是** certified 路径的缺口。出处同上。

...
| 物品准入口（限制口） | 游戏里存在，现役 main **刻意不建模**：几何上等价于一条直带（能放它的格就能放直带）。该省略的 authority 是条件式：仅在没有候选池或认证谓词消费物品准入口时成立；任何“禁止只有筛选口才能实现的事”的谓词都构成消费，接入时 authority 立即失效，豁免与 statement 必须一并重裁（canonical `item_admission_port_exclusion.authority`）。该省略列入 `model_stricter_faces` **第 (6) 项**，属于显式认证 scope 限制，不是无条件 safe-exclusion 定理，也不能支撑 de-mix 禁令。**「建模必要性 = 零」的三腿论证目前不足**：速率腿缺占空前件（见下行速率引理两前件纪律）、分拣终点腿仅在「两种货可去同一终端」时成立（模拟器判例：异终端分拣不可替代，且分拣零吞吐税），仅存仓库口混吃一腿。未接入的 de-mix 禁令是消费该豁免的谓词；若接入，必须先重裁该 authority。owner 处置为：先放着即维持现状；随系统性规则梳理／墙审计首轮回桌，物品准入口作为种子案例。 | `semantics.item_admission_port_exclusion`；判例 `.artifacts/gpt_pro_review_batch_20260807/verdict/fen2/SIM_JUDGE_D1.md`；决策页同目录 `ADJUDICATION_fen2.md` B1 |

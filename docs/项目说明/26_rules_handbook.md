...
| 物品准入口（限制口） | 游戏里存在，**刻意不建模**：几何上等价于一条直带（能放它的格就能放直带），过滤 / 限速功能属吞吐层 OUT-OF-SCOPE。**建模必要性 = 零**——不是"分拣没用"，而是"有不拒收的终点可用时，分拣从不是结构性必需" | `semantics.item_admission_port_exclusion` 及其 `rationale_restated` |

...
| A12 | 2026-08-13 三面防污染审计三笔挂账：①三门（obligations checker / strong-status allowlist checker / preflight）PASS 文案不自带「不证什么」限界——**挂下一次触及任一文件的 Chain B/C 批顺走，不单开**；②`EXACT_MASTER_FRONT_CLEAR_LIFT` 定理复证与 full-pool golden 接门禁——**挂 redesign 批 5/6**；③零税措辞止血批待做（`semantic_validator.py`「违反冻结真理」七处文案簇、`package_review_snapshot.py` proof_checker 命名、`b_design_v2_exit_criteria.py` 判据不符仍 PASS 一处真 bug）。明细/触发器/勿动清单 | `docs/research/plane_mixing_audit_20260813/FINDINGS.md`；owner 拍板 `00_master_roadmap.md` 文末（`5191abe`/`b3500cc`） |
| A13 | 2026-08-13 文档补丁链（GPT Pro 外包整理，phase1→phase4 batch2 共 13 包，位于 `~/下载/zmd_do*.zip`+`zmd_document_*.zip`）落地挂账：评审已完成、**补丁均未应用**、落地适配批待 GPT Pro 制作（phase4 batch3/4 亦未做）。**触发器＝任何会话应用该补丁链或其适配批时**：①按仓外交接文档验收——8 条 blocker 适配 + §8 账本权威接口改造（owner 已裁定：`decisions.jsonl` 显式非权威化＋指针校验、claims `authority_basis` 准入判据、R3/R4/SMM4 三条 machine→research_authority 降级、`representation_class` 四类标签），流程＝分支→`git apply --exclude=CLAUDE.md`→**立即 commit**→适配批→checker 全绿；②同批义务：把交接文档 §8 裁定内容带进 tracked 位置（现仅存仓外）；③`CLAUDE.md` 两步调和（先补全 `AGENT_OPERATIONS.md` 颗粒度知识、再换轻量版，「git 历史空白重建」「untracked 会被并发清」两条防呆留本体）单独提交。**禁**：直接跑包内 `apply_*.sh`（其回滚含 `git clean -fdq`，共享工作区致命） | 交接文档 `~/下载/zmd_文档补丁链落地评审交接_20260813.md`（**仓外**）；owner 裁定 `00_master_roadmap.md` 文末（`b3500cc`）+ `OWNER_DECISION_SUMMARY.md` 头部 08-13 追记（`db9fd78`） |

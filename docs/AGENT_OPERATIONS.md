...
对账不能只扫 `claims.jsonl`。至少三查：claim/CURRENT；dossier 登记的 summary 与效力声明（`inventory-only` 成果可能本来就没有 claim）；同名 ERRATA、冻结工件和具名 proof root。对计划中的每个待测指标还要反问：已有定理或工件是否已经决定它的取值，避免把必然结果误读成新实验信号。

开工搜索应覆盖坐标、正式名、代码键、数值特征和同义命题；收官时写明新结果与既有 claim 的关系（同一、加强、矛盾或无关），附 claim ID 或可复现搜索式。引用旧结论时必须携带它的前提、作用域和最新勘误，不能只读原包 judgment。
...
派活必须显式指定席位或模型，不依赖默认路由。能力更强的同源复核可提高第二次推理质量，但不能进入异源证据桶；承重结论的异源确认仍由不同来源承担。先画来源矩阵再分工，合并席或裁断席若与被合并的核查链同源，仍属于自己审自己。
...
需要独立发现结构时，优先给 owner 的前理论原话或去术语化案例，让复核方自己命名、自己选取证据并形成问题分解；只给最小仓库坐标，不给策展过的事实清单。不要把候选答案、内部术语和预期结论埋进问题框架。

采用盲问-对撞两段式：A 段不展示我方草案，B 段才亮出原框架比较差异。A 段不得用“不要采用某思路”之类负向提示，因为回避声明本身会种下该框架；优先使用未污染的新窗口或新线程，已经浸泡在项目语境中的线程只作 B 段对撞席。视角不可能归零，只能靠最小化框架与多份独立成形答案降低相关性。

<a id="search-negatives"></a>
“搜索不到”不能直接升级为“不存在”。承重全称否定必须先声明搜索域，列出同义措辞、正式名/代码键、数值特征和可能的生成路径，再使用覆盖该域的机器枚举、注册表或穷尽检索；关键词零命中只是一条弱证据。

能写成有界正面陈述时，优先写“我在 X/Y/Z 入口找到这些结果”，不要写“只有、零、无一处”。仍需全称否定时，由不同来源复核搜索域与同义表，而不是让原作者重复同一组关键词。

...
activation:
  layer_hint: L1
  must_know: false
  reason: 判某项属 P1.2 还是 P1.3 时该读(P1.2 收口已完成,本卡的边界辨析在 P1.3 期仍是判据):"生产 seal 跑通"=算力墙 P1.3、"canonical→geometry 语义半"=F1-F9 前置 P1.3、P1.2=soundness 不是吞吐定理。不读易把算力墙项误当认证链缺口、或把 P1.3 主体误记成 P1.2 遗留。
provenance:
  op: record
...
---
**【终态注 2026-07-07】本卡两件外审前项均已完成(甲 `28d9d2c` 收窄、乙冻结仪式 `c9b41b3` 包已产出并送审),三轮收口外审 0 真·上-TCB 洞,P1.2 已由 owner_manual_decision 关闭、P1.3 开启(见 [[p1-2-closed-p1-3-open-20260707]])。仍有效部分=下方 P1.2/P1.3 边界辨析与两个更正。**

2026-07-06 owner 追问"计划中 P1.2 外审前所有闭合条件都满足了吗",两路 codex 审计后厘清。**答案(当时):不是全满足。** 记结果 + P1.2/P1.3 边界,并更正我此前把两个 P1.3 项列进外审前清单的糙处。

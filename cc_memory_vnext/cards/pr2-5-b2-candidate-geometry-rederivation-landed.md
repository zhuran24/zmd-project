...
== 订正 2026-08-08（跨层对账 G-8 字节身份换代）==

本卡池身份(size 45774305/sha a914ba63…)是 07-06 落地当代值,已 superseded——现行冻结池=54,467,709 字节/sha f05b1291…(front 错位修正+池重生成后),a914 在 hash-incompatible 拒收链。B2 机制描述仍有效,gate 断言常量已随 LOCKED_EXACT_ARTIFACT_SHA256 换代;字节身份一律以 preflight FROZEN_ARTIFACTS/certified_artifact_contract 现值为准,别抄本卡数值。

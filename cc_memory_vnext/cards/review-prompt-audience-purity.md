...
== ⑤ 不写角色扮演开场(owner 2026-07-05 点破)==
"你是一名对抗性设计审查专家,同时精通 XX/YY/ZZ"——这种身份声明对模型能力**零提升**(能力由任务内容和材料激发,不由角色标签解锁),占 token,还可能诱导"扮演专家腔调"而非直接干活。owner 的检验问句:"你觉得这种提示词对你自己有用吗?"——答案是没用,所以别写给别的模型。直接从任务和材料开始("附件 zip 是一份设计稿…你的任务是攻击它")。**真正有用的部分**:具体攻击面清单、输出格式约束(BLOCK/CONCERN 分级+必附修复补丁)、校准指令(宁误报勿漏报/攻击失败要明说)、材料指引(先读 CONTEXT.md)——这些是行为规范,不是身份声明。实例:2026-07-05 sidecar 设计稿审查提示词开场白被 owner 点名删除。

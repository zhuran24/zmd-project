...
- **(b)/① 已达成**(commit `9d224d8`,2026-07-05):`_discover_project_snapshot_modules()` 从 rglob 全树(672 模块含三禁+367 scripts)改为**硬编码 24 模块显式白名单**+逐模块存在性校验;child `_SnapshotFinder` 只服务白名单、`sys.path` 清成仅 stdlib → child 误 import 三禁**真 fail-closed**(源码机制验证,`pr2_l0_micro_verifier_core.py:1409-1431`);red-line 两条转硬断言、11 passed。完整 reseal(per-fn TCB sha + V99 floor + obligations + allowlist,LF index-blob 三处对 pin=CI 安全)、双 checker(14/64、65/83)+ --full 3819 + --slow 30 全绿。**注意仍是 ①**:三禁/scripts 的物理 `@source:` 文件仍在快照供孙进程 replay(硬地板),②(物理排除)明确 defer——① 缩暴露面+供应链、**不缩语义 TCB**。
...
- **但不从测试/seal/checker 自动推导 release closure**:07-05 时点直接闸=owner 手动门 `blocked_manual_review_count` / `p1_3b_entry_allowed=false`(`phase_1_2_spike_close.json`);07-07 gate 已由 owner_manual_decision 转 `closed_manual_owner_decision`,`p1_3b_entry_allowed=true`,`next_phase_entry.allowed=true`。所以"做 (b)"本身仍不是自动开门机制。
...
== 决策指引 ==
- (b)/① **已于 `9d224d8` 落地**(见现状,选 ① 硬编码白名单、不重构 benders)。诚实状态:runtime forbidden-import closure(a)+ snapshot module-manifest 最小化(b/①)均 done;**别叫"PR2-1/TCB 全 done"**——② 物理排除 defer、语义 TCB 硬地板仍在。TCB 线按策略到此停(不造新硬化活);P1.2 close 里程碑已于 2026-07-07 owner-gated/manual decision 完成,后续里程碑=P1.3/发布时点项;(b) 满足其技术必要条件之一这件事不从测试/seal/checker 自动推导开门。

...
== 决策指引 ==
- (b)/① **已于 `9d224d8` 落地**(见现状,选 ① 硬编码白名单、不重构 benders)。诚实状态:runtime forbidden-import closure(a)+ snapshot module-manifest 最小化(b/①)均 done;**别叫"PR2-1/TCB 全 done"**——② 物理排除 defer、语义 TCB 硬地板仍在。TCB 线按策略到此停(不造新硬化活);下一里程碑 P1.2 close 是 owner-gated(手动 clean-review 计数),(b) 满足其技术必要条件之一但不自动开门。

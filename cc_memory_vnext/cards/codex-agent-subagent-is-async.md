...
**正确用法**: 把"codex 跑完之后才做的事"放到**完成通知驱动的那个回合**里去做(收到通知 = 那一刻才 triage / 应用 / 注入后续), 而不是预排在 spawn 之后的串行/定时位置。需要"超时兜底"时, 用显式的超时**提示词**(非直接执行危险动作)+ 防误伤校验, 别用裸的长延迟定时器。相关: 跨模型审在敏感时序机制上不可省(我自己分析漏了异步本质, codex 一眼看穿)。

...
== 追加(2026-07-08,M2 批 C 亲历):close-kernel 保护是三层嵌套,docstring 措辞也在 fail-closed 范围 ==

①sink 名单(close_kernel_contract.sink_files,64 个)⊂ ②hash 钉面(obligations 另钉的 8 个 src/cuts 文件)⊂ ③**内容扫描面(checker 对全 src 树 rglob \*.py 做大小写敏感子串扫描,任何文件含 proof-bearing token 如大写 INFEASIBLE 就必须已注册,否则 unregistered sink fail-closed)**。批 B 给 F8 写墓碑 docstring 用了「false-INFEASIBLE」字样→两个退役文件被扫进应注册面→checker 红(c4326f1 换词修复)。操作纪律:**任何 src/scripts 改动(哪怕纯注释)提交前跑双 checker**;写含 INFEASIBLE/CERTIFIED/proof-bearing 等 token 的注释前想想扫描面。

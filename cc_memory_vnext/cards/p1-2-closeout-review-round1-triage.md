...
- **round-2 提示词**:`pr2_pkg\p1_2_closeout_20260706\entry_8_semantics.md`——审"判定者的数学"(通过全部检查的东西是否真满足命题 P),非"谁能绕过判定者"。八靶区 T1-T8(谓词几何/端口计数/routing 图等价/供电度量/lex 穷尽/false-INFEASIBLE 侧/精确算术与解释层/退化输入)+双向对照法(代码实际检查的命题 vs 规则要求的命题)+历史真洞校准+**覆盖矩阵硬性要求**(零发现也须交"审了什么、凭什么判闭合",治 round-1"没找到=没信息")。不给守卫地图;排除 round-1 全部 11 簇同族噪声。同一送审包(冻结树未变)。已按 relay 规程 staged 剪贴板(顶=包路径、底=entry_8 全文,回读核验过),剩 owner 手动上传。

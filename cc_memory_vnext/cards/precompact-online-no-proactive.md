...
scope:
  ...
  paths: ["C:/Users/22957/.claude/skills/precompact", "C:/Users/22957/cc_watchdog/owner_sleep.flag"]
  symbols: []
status: active
priority: P0
severity: high
triggers:
  ...
  keywords: [precompact, /compact, /precompact, 记忆更新回合, owner_sleep.flag, 在线, 压缩上下文, 主动压缩, 陈述计划, 预告压缩]
...
provenance:
  ...
  evidence: ["python cc_memory/mem.py read precompact-skill-owner-skill --body", "python cc_memory/mem.py read offline-mode-autonomy-criterion --body"]
...
只有 owner 明确让 CC 执行压缩，例如敲 `/precompact`，或说"你来压一下""跑 precompact"，才进入在线手动触发分支。即便进入该分支，当前回合也不能顺手更新记忆并直接 `/compact`；必须先发送"记忆更新回合"提示词，由那个专门回合完成记忆更新后再注入 `/compact`。

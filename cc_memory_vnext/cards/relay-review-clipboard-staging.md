...
2. **每条提示词在剪贴板历史里必须可区分、可归属**（08-07 纠正二+三，常设；本质要求不是"带数字"）：标签格式 `【提示词 N · 主题 · vX(MMDD-HHMM)】`+空行，**直接写进 PROMPT 文件顶部**（对评审无害）。两层归属：编号+主题=归属到份；**版次戳=归属到版本——同一提示词每次重上剪贴板前必须更新版次戳**，否则新旧两版同标签在历史里无法区分（08-07 实撞：份 5 修订重上后新旧同标签）。组包时 PROMPT 出生即带标签，重上前先改戳再 wl-copy。

...
**机制详见 `Pre-compact` SKILL(权威,`.claude/skills/Pre-compact/`)**:记忆更新回合结束时起 `-RunAfterTurn` worker → 脚本(`precompact_judge_run.py`)后台起 codex 起草(**不再有单独的 spawn/空查漏回合**)→ 跑完注入「草稿在…」= 我唯一的 **triage 回合**(triage+应用,必须一个连续回合做完);**`/compact` 由脚本用 seq 排在草稿之后自动注入(我不手动 -Send /compact)**——不是「绝不预排」,而是靠 triage 一个连续回合做完来避开 idle 空窗;fail-open 两处兜底,查漏≠压缩前置门。owner 要求扩窗/自定义时改手动派 codex(cc_memory `precompact-judge-failopen-expands-next-window`)。**只有调用 Pre-compact skill 才级联三阶段;单独注入「记忆更新回合」=只记一遍、不级联。** 查漏草稿 inbox-first(默认记 cc_memory 条目,够格才晋升卡)。系统化遥测预筛/小模型评估器仍属 V2。

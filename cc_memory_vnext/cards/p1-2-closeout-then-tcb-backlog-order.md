...
- **正确顺序**:阶段1→批2a→批2b(已 DONE)→#5-F spike→#1(阶段3 枢纽,带 #5-F-B + fused 一批)→阶段4(#9b/#9c)→ 代码重新冻结+fresh reseal → 收口外审(本地 codex 多镜头 + GPT Pro relay 按需 + owner 判定合适 + owner 手动门)。**不起外审 round-21**(close-kernel 画线仍站着)。(时点注:此为 07-06 早间口径;07-06 晚收窄后「先 backlog 后收口」被推翻——收口先行,收口动作已于 07-07 完成关门,四阶段转为发布时点执行序。)
...
== 一句话版(07-06 版)==
阶段1(#8+#9a尾+#6确认)→ 批2a(#2/#3 带两条纯核心抽取,先抽核心后改语义)→ 批2b(B2 带 outer_search 接缝,批后收 306s 巨无霸;已 DONE)→ #5-F spike(带 fused child 固定税实验并拍板)→ #1 最小闭包(吞 l0-snapshot/lazy import/若成立的 fused child)→ 阶段4(#9b/#9c)→ **收口 P1.2**(冻结+fresh reseal→外审到 owner 判定合适→owner 关手动门)。提速线整体溶解进主线,一次 reseal 都不多付。(时点注:「收口 P1.2」这一步已于 2026-07-07 提前完成——07-06 晚收窄令把收口从 backlog 之后提到即刻,四阶段剩余项转发布时点。)

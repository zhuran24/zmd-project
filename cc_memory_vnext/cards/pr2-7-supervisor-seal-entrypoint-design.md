...
== 四条安全边界(硬守)==
1. **只 mint campaign 级 CERTIFIED**,不 publish、不碰 owner 门、不推导 P1.2 closed。公开交付面(final_solution/blueprint/manifest)仍走 `resolve_p1_2_publish_open_gate()` owner gate 逐次校验;gate 已于 2026-07-07 转 `closed_manual_owner_decision`(非 seal 入口自动推导)。通电只补"入口存在"这个机器条件。**别给入口加 publish 调用**(测试用 spy 钉死:seal 期间 `publish_verified_certified_delivery_surface` 零调用)。

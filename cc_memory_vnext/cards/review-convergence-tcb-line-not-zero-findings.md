...
provenance:
  ...
  evidence:
    ...
    - "2026-07-06:P1.2 方案完整性评估指出两处悬空(收口前提集合未闭合、'三连 clean'与画线哲学两套逻辑并存),owner 两拍板关闭:#2/#3/#8/#9b 均算收口前提(收口前提=全 backlog 编码项);'三连'是图方便说法非硬判据,实际=外审到 owner 判定合适为止。据此补'三澄清'段。落盘批 main 03ea2be。"
    - "2026-07-06 晚:首条('全 backlog=前提')同日被 owner 收窄——厘清手滑/故意二分后,防蓄意内鬼类全部延期到发布时点、非 P1.2 前提,编码前提实质清空(fb6169b);三澄清段首条按此更新。'三连 clean'澄清不受影响、仍现行。"
...
2026-07-04 晚 owner 质疑"为什么 #1~#9 没做完就打审包收口",澄清一个此前被会话推广过头的点:**那条 TCB 线是 2026-07-03 关于 close-kernel(round-20)画的,只管"close-kernel 外审循环停不停",声明的受信假设是基础设施层(解释器/stdlib/OR-Tools/OS 隔离/冻结几何字节);它【不】授权"P1.2 整体收口不等所有 TCB 深化"。** 上面那句"收口不等这些空格填满"是会话替 owner 记的,被 owner 本人收窄:**P1.2 收口的前提【至少包含 #1 最小 TCB 闭包(child 不再 import 项目域、快照不扫全 src)】——TCB 没真正做到最小(隔离验证 child 还 import certified_frontier/exact_campaign 等一大堆项目域)时,不能算 P1.2 收口。** 正确顺序修正为:**先做深化(至少 #1,以 #1 为枢纽、带 #5-F-B + fused 一批)→ 做完再走收口外审**,不是"先收口后深化"。本条推翻 [[p1-2-closeout-then-tcb-backlog-order]] 的"先收口后深化"前置板——那张卡已同步修正。

== 三澄清(owner 2026-07-06,P1.2 方案完整性评估后的两拍板;首条同日晚间再被收窄)==
- **收口前提范围:同日两变,以晚间为准**:早间 owner 对"#2/#3/#8/#9b 是否也是收口前提"亲答**"算"**(收口前提=全 backlog 编码项)——**同日晚间被 owner 自己收窄**:厘清"内鬼=故意而非手滑"后拍板,所有"仅防蓄意内鬼"硬化(#8 深化/#2/#3/#5-F/#9b/#9c/Option B)**延期到发布时点、非 P1.2 闭合前提** → **编码前提实质清空**(#1 剩余工程全在桶内或 out-of-scope),行使 go_criteria:30 close-scope 活口;冻结仪式随即走完(冻结树 c9b41b3、送审包已备)。详见 [[deliberate-insider-hardening-deferred-to-release]](P0)。#9a 维持部署时点不变。
- **"三连 clean"语义归位**:owner 明说「三连」只是当时图方便的说法、**不是硬判据**——真实判据与本卡哲学完全同构:**收口外审进行到 owner 判定合适为止**(轮数可多可少),结束靠 owner 拍板、不靠数字自然达成。gate JSON 的 `required_consecutive_clean_full_reviews=3` 与关门确认字段的 "three clean reviews" 字样保留为机器兼容值(checker pin 死,同 `p1_3b_*` 模式);唯一权威关门动作=`owner_manual_decision`。至此"数字判据"在本项目里彻底退位:close-kernel 外审停(07-03 画线)、收口外审停(07-06 澄清),两处终止条件统一为 owner 拍板。

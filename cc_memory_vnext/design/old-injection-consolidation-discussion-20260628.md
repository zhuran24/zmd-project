...
- **关联未决**:整库迁移/冻结(本议题 C)、③ 的真地基「可观测提交点记忆闸」(见 `observable-commitment-gate-20260628.md`,已取代 ③ 原"每回合看守"framing)。

## 7. 后记(2026-06-28,owner 指出 + 判官补)

- **B/C 不是本会话新发明**:冻结(C)+ 迁移(B)早在 `design/MASTER_PLAN.md`(§2「旧 cc_memory 上线即冻只读绝不双活」+ §7「种子卡迁移清单·提炼非倾倒」)。owner 当场指出、我核对坐实。
- **原计划比本会话更激进**:它本意 = 精华迁完(那 ~21 卡就是)+ **剩余直接冻成只读存档**,没有"覆盖域图"、没打算全迁那 115 条。本会话的「覆盖域图=自萎缩债务仪表 + 把 115 条慢慢全迁」其实是**更保守的 legacy-bridge 变体**(原计划眼里等于在"倾倒")。
- **真正的分叉**(待 owner 拍,连同冻结裁决 C):走**原计划(激进**:剩余冻存档、覆盖域图退役)还是 **本会话(保守**:留债务仪表、慢慢全迁)——取决于那 115 条里多少是"还该被主动召回的活红线" vs "翻翻就行的历史存档"。
